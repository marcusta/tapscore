...
- **Result render vocabulary is closed + presentation-only.** The result-render-contract vNext lives in `server/domain/strategies/result-vocabulary.ts` (closed tones/markers/grid-component ids + smart constructors) and `result-sections-vnext.ts` (vNext section descriptors). It is **defined but not yet wired** (Phase 0) — nothing emits it until the contract is wired in later phases. Once wired, formats produce visual hints via the smart constructors (`marker.diamond(...)`, `cell.score(...)`), never a golf word (`birdie`, `win5`) in a token name. The types are the source of truth; do not restate the token list in docs.
