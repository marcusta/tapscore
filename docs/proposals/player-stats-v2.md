...
Three additions. Each follows the shipped pattern end to end: a key in
`ENUM_VALUES`, a `StatPrompt` in the twinned `stat-prompts` modules with
...
never offered again). Capture cost is bounded: today's worst case is 7 prompts
on a par-4 trouble/miss hole; everything below adds at most 2 optional
segments to that worst case, and both are one-tap segments, not steppers.
...
`tee | approach | short_or_green`, visible when `penalties` ≥ 1. Unlocks
penalty attribution in insights ("both penalties off the tee") and lets the
waterfall's penalty bar break down by cause without changing its total.
Lowest priority of the four: the stroke count is already correct, only the
...
4. **Capture v2** (§3.1–3.3 together — one vocabulary migration, one prompts
   change, one views change): directions + bunker, then the compass and fan
   visuals. §3.4 rides along or follows.
...
- **Strokes gained / GPS / dispersion overlays** (Tangent, Shot Pattern
  ...
  boundary stands.
- **Bunker stroke counts, chips-per-hole counts** — shot-by-shot entry in
  disguise. Hole-level lie classes give ~90% of the signal at one tap.
