...
// create flow (2.6e M2/M3). `courses.api` / `tees.api` / `formats.api` are
// auth-gated (admin / signed-in surfaces); the no-login wizard can't pick a
// course/tee or enumerate formats through them. This `setup` API exposes only
// the SELECT-side a guest needs to build a `RoundSetupDraft` — course catalog,
// a course's tees with gender ratings, and the serializable format catalog —
