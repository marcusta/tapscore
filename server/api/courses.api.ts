...
export function createCoursesApi(svc: CourseService, authz: CourseManagementAuthz) {
    ...
    return {
        ...
        update:     { method: 'POST'   as const, path: '/courses/update',  fn: async (input: Static<typeof UpdateCourseInput>, c: Context) => { await gate(c); return svc.update(input.id, { name: input.name, holeCount: input.holeCount, holes: input.holes }); }, schema: UpdateCourseInput, middleware: mw },
