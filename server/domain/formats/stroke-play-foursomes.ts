...
// (`ParticipantInput.playingHandicap`) is the allowance-adjusted team PH —
...
export const strokePlayFoursomes: FormatStrategy = {
    ...
    compute(input: SlotInput, slot: FormatSlot): SlotResult {
        const participantResults = input.participants.map((p) => {
            ...
            if (links.length !== 2) {
                throw new Error(
                    `stroke-play foursomes slot #${slot.slotIndex}: participant ${p.participantId} needs exactly 2 player links (got ${links.length})`,
                );
            }
