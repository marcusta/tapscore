...
| M6 — Dogfood + plugin deletion/extension proof | AWAITING VISUAL VERIFY | `14b7c33`+`137b38c`+`d3714db` | All automatable items done (legacy deletion, 40 client tests, deletion proof w/ one 2.7a-scoped exception, viewport verify): see checked items below. Remaining: user real-round dogfood + final 2.6e gate confirmation |
...
- [ ] **Dogfood:** play a couple of real on-course rounds end to end (create → players → formats → share → score → results) with no login. Capture any engine gap surfaced (handicap edge cases, format behaviour, route handling) as notes against the relevant phase — this is the whole point of going FriendlyRound-first. _(User's part — remaining item.)_
