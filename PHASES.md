...
**Planned slice — composition/scoring decoupling (dogfood-surfaced 2026-06-17, design accepted, not built):** score a team-composition's balls (scramble/greensomes/foursomes) with stroke/match/stableford — e.g. two scramble teams play match play. Engine bones exist (`ballSelector` resolves any pool; team balls carry a by-rank PH; stroke/stableford score per-ball). Blockers: scoring formats are own-ball-only (`ballRequirement`), match assumes one producer, no draft/builder/UI wiring. Design + per-layer plan + locked decisions in **`docs/adr/0002-composition-scoring-decoupling.md`** (team PH reused; per-hole strokes off course SI; `scoresAnyBall` opt-in; allowance inherited; MVP = scramble/greensomes/foursomes × {match,stableford,stroke}). Sequence around M6 dogfood.
