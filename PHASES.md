...
**Resume here:** **Phase 2.6c is COMPLETE** — new ball-creation & format coverage + kitchen-sink multi-slot seed landed on branch `phase-2.6c-new-formats` (off `slice-6-static-deletion`), commit `7650f24` (+ this ledger commit). Added 3 ball-creation strategies (`greensomes_pair`, `scramble_team`, `modified_alt_shot_pair`) in the separate ball-creation registry, 2 formats (`greensomes`, `scramble`) wrapped as plugins (ids added to the architecture renderer-guard list), 6 seeds incl. the 6-ball/7-slot `multi-format-extreme-round` kitchen-sink and the cross-tee `mixed-tee-round`, and a `scripts/seed-authoring.ts` helper for authoring `RoundDefinition`s directly. Gate green: 400 pass, all typechecks, 19 fixtures (13 existing numerically identical + 6 new); visual gate `tmp/formats/slice-2.6c-verify.html` user-approved 2026-06-14. See the full record in the "### 2.6c" section below.
