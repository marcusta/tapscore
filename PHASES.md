...
**Resume here:** Slice 3b is `COMPLETE` (commit `6ce0945`; automated gates + user-approved visual gate). The Round now owns an **explicit ordered itinerary** of play-hole occurrences and **playing groups**:

- **Schema:** migration `022` (`round_play_holes` — id = `hash(round_id, play_hole_def_id)`, `UNIQUE(round_id, play_hole_def_id|ordinal|id)`; `round_play_tee_holes` — durable `tee_ref` snapshot key + nullable live `tee_id`), `023` (`playing_groups` with composite same-round FK `(round_id, start_play_hole_id)→round_play_holes(round_id,id)`; `playing_group_balls` with `UNIQUE(ball_id)` = one group per ball), `024` (backfill itinerary from frozen `round_course_holes` + `round_type`; `tee_times`→`playing_groups`; single-group rounds get ball membership, multi-group leave it empty + diagnostic; historical `definition_json` NOT rewritten).
- **Authoring vs persisted split:** `RoundDefinitionInput` (loose) → `normalize` (runs ONCE at the top of `compile`) → `ResolvedRoundDefinition` (`schemaVersion: 'resolved-v1'`, persisted as `definition_json`). Conventional routes get defaults; non-standard routes (explicit itinerary / repeated holes / non-`official` SI) must declare `routeHandicapPolicy` (`missing_route_handicap_policy`). Posting eligibility is gated on full-rated coverage, never automatic. `normalize` owns route-structural validation (`si_out_of_cycle`, `duplicate_si_rank`, `duplicate_play_hole_def_id`, group start refs).
- **Compiler/persist:** `compile.ts` builds the itinerary + per-tee occurrence rows and derives **exhaustive + exclusive** producer/ball group membership (`producer_not_in_any_group`, `producer_in_multiple_groups`, `team_ball_crosses_playing_groups`, `ball_not_assigned_to_group`). `persist.ts` diff-upserts the new tables by content-addressed id — a **reorder preserves play-hole ids** (two-phase ordinal write avoids `UNIQUE(round_id, ordinal)` collisions); add inserts, remove deletes.
- **Read model:** `Round` now carries `playHoles` (ordered occurrences + per-tee snapshots), `routeSi`, `routeHandicapPolicy` (+ posting), `routeSections`, and `playingGroups` (rotated `playedOrder`, `groupRelativeOrder`, derived start/end). Legacy (pre-3b) defs normalize-on-read; next recompile upgrades them to `resolved-v1`.
- **tee_times live path RETIRED:** `TeeTimeService`, `tee-times.api.ts`, `tee-times.gen.ts` deleted and unwired from `main.ts`/`services/index.ts`/`src/api.ts`. The `tee_times` TABLE + migration 011 survive ONLY as a backfill source — physical drop deferred to the later legacy-schema slice (same posture as `round_format_slots`).
- **3a deferrals still stand:** `round_format_slots` table + `createLegacy` + migration 019 + `synthesize-legacy.ts` + `backfill/round-definitions.ts` remain the deprecated legacy bridge.
- **Explicitly deferred to Slice 3c:** per-producer route-CH freezing before ball derivation; `orphaned_events_after_correction` event surfacing + rescoring consumers; explicit setup-correction target-path reservation (2.6d). Score events do NOT yet key on `play_hole_id`.

Start **Slice 3c — Event, scoring, and scorecard itinerary migration** (migrate score/metadata events + scorecard materialisation key onto stable `play_hole_id`; strategies iterate occurrences; centralise handicap-stroke allocation over `(playingHandicap, occurrenceStrokeIndex, allocationCycleSize)`; retire `round_type` as scoring authority).
