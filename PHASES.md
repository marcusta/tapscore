...
**Resume here:** Slice 3c is `COMPLETE` (commit `0fd1e92`; automated gates + user-approved visual gate). Scoring now runs end-to-end on stable play-hole **occurrence** ids; `round_type` is no longer a scoring/segmentation authority.
...
**Next:** start **Slice 4 — Compiler validation** (branch off `main` once 3c merges, or off `slice-3c-itinerary-scoring`). See the Slice 4 task list below.
