...
**Next is phase 2.6d-final — Core engine integrity and ADR closure** (ledger below). Branch off `phase-2.6d-bis-allowance` or the merged engine branch. Do not start 2.6e until every 2.6d-final gate is green. The canonical static fixtures (`seed:formats` + `render:formats` + `check:format-fixtures`, now 20 rounds) remain the expected-result oracle.
...
- **Deferred to 2.6d-final:** expose one generic format-action endpoint with structured diagnostics. Generic score entry remains unaware of individual action types.
...
- [ ] Keep ball creation server-owned: the client submits format id, producers/teams, format config, and allowance only. It never chooses or reconstructs ball-strategy ids/configs; the registered format plugin's `planSetup` supplies those details.

- [ ] **Gate:** create and reopen individual, own-ball team, team-ball, multi-slot, repeated-hole, custom-SI, and shotgun rounds. Explicitly create Greensomes, 2-player Scramble, and 4-player Scramble through `POST /rounds/from-draft`; verify the persisted definitions use the server-planned `greensomes_pair`/weighted and `scramble_team`/by-rank strategies and that displayed ball CH arithmetic matches the approved engine fixtures. Compare all persisted definitions against approved static fixtures/server output.
