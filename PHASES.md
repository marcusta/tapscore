...
Backend-only skeleton mirroring `apps/starter`. The framework was initially consumed from the sibling checkout and is now committed under `vendor/basics-core` via `file:./vendor/basics-core` for self-contained installs. Pinned `hono@4.11.9`, `kysely@0.27.6`, `kysely-bun-sqlite@0.3.2`, `@sinclair/typebox@0.34.48` stay aligned with the vendored snapshot.
...
- Add the `generate` package command (now backed by the committed `vendor/basics-core/generate-api.ts` adapter).
...
**Current resume (2026-07-13):** Phase 4 remains **AWAITING VISUAL VERIFY**. Before requesting visual approval, land the review's security-boundary, architecture, and verification/framework-tooling remediation, rerun the canonical gates, and regenerate the focused Phase 4 artifact. Then close the Phase 4 visual gate explicitly; do not start Phase 5 early. Historical 2.6b-final execution details remain below for audit only.
...
| M6 — Dogfood + plugin deletion/extension proof | COMPLETE | `14b7c33`+`137b38c`+`d3714db` | Legacy deletion, 40 client tests, deletion proof (one 2.7a-scoped exception: synthesize-legacy bridge), viewport verify; real-round dogfood user-confirmed 2026-07-03. **2.6e CLOSED** |
...
- [x] **Dogfood:** play a couple of real on-course rounds end to end (create → players → formats → share → score → results) with no login. → Real rounds played (visible in dev data: Linköping kitchen-sink Stableford/Stroke/Taliban/Umbrella, better-ball match rounds); _user-confirmed working 2026-07-03_. No engine gaps reported.
- [x] Delete legacy client catalogs, labels, decomposition logic, and format switches made obsolete by descriptors/adapters. → `14b7c33`: deleted dead auth stubs `src/score/*` + `src/results/*` (+ routes/imports); `/rounds` row made inert (admin `Round` has no share token — no live drill-in in 2.6e scope); sweep confirmed zero format-id switches/maps left in `src/` (remaining ids are generated type unions, the M2 default-slot seed, the generic FORMATIONS enum, and prose comments).
- [x] Add client tests for catalog-driven setup, team validation, mixed topology, repeated routes, generic sections, pair-only results, format actions, and missing-adapter fallback. → `137b38c`: 4 files / 40 pure-logic tests under `tests/` (`create/format-catalog`, `create/setup-service` via captured drafts, `round/result-render-grouping` incl. XSS + unknown-kind diagnostics, `round/route-occurrences`); `bun test tests/` 60 pass. Format actions skipped: no built-in declares action types and no client adapter seam exists yet.
- [x] Perform the full plugin deletion test across server + static + mobile: deleting the module and central registrations removes every production trace without editing generic infrastructure. → Proven on `taliban_better_ball` (strategy module + co-located test + registration entry removed; shared match-play presenter correctly retained): checks green, architecture tests green, boot clean, catalog served 8 descriptors with zero traces, other formats still plan/compile; restore byte-identical, 555 server + 60 client tests pass, fixture oracle green. **One documented trace:** `synthesize-legacy.ts:82` (ratchet-exempted legacy scoring_mode→format_id bridge) — deleted by Phase 2.7a. Note: the ledger's historical "20 fixtures" figure is stale; the oracle is 14 rounds / 13 expected signatures.
- [x] Verify narrow and wide viewport flows, but give the user a focused review brief rather than every permutation. → 375px + desktop verified in browser: landing list, live-round score tab (carousel, keypad rows, share box, orange hole bar), leaderboard (ranked points + OUT/IN scorecard with birdie markers), full `/create` wizard (9-format catalog, teams step). Zero console errors. Client artifact rebuilt (`d3714db`).

**Final 2.6e gate:**

- [x] All checks/tests and canonical fixture workflows are green. → 3 typechecks, 555 server + 60 client tests, fixture oracle (14 rounds / 13 signatures) green.
- [x] Static verification pages remain the approved arithmetic/result oracle. → unchanged throughout M6.
- [x] No-login flow works end to end: create → players → formats → share link → trust-based scoring → per-format results, with login never on the critical path. → real-round dogfood, user-confirmed 2026-07-03.
- [x] One canonical server registry; generic formats require no client code; special formats require at most one colocated adapter + one client registration. → deletion proof on `taliban_better_ball`: zero client edits, zero production traces (single ratchet-exempted legacy-bridge entry, deleted by 2.7a).
- [x] Agent starts the local app and gives one clickable browser link plus 2–5 required scenarios with exact expected outcomes. → done 2026-07-03 (localhost:5183, 4-item dogfood brief).
- [x] User confirms the focused scenarios; commit `phase 2.6e complete: no-login friendly-round mobile client`. → confirmed 2026-07-03.
...
- **Friends list (added 2026-07-03, user request):** one-directional contact list, no approval flow (`friendships`: player_id, friend_player_id, created_at). Authenticated player search (username/display-name substring, excludes self + soft-deleted) → add/remove friends. `players` gains nullable `gender` (registration/profile field) so a friend dropped onto a roster carries the tee-rating gender; missing gender stays editable on the roster row. Create-flow Players step offers **From friends** (adds `player`-ref producers with server-resolved name + current index) alongside guest rows — the spec §2 "relations to other players (friends)" made concrete.
- **Logged-in nav rework:** HOME / FRIENDS / PROFILE. Retire the legacy `/rounds` (inert admin list) and `/players` ("friends you keep score for" roster) screens — Phase 3's my-rounds + friends replace their purpose; the logged-in app becomes the no-login app, enriched.
...
**Status: COMPLETE (2026-07-04).** Gate passed: user tested the live flows (register, profile handicap chain, gender, friends search/add, From-friends roster, guest+friend mixed rounds) 2026-07-03/04 + verify page. Post-gate additions during testing: friend-pool dev seed `44db45b`, gender UI `a4e2a3a`, round-mode tabbar hidden `d059676`, load-once guards `ae94e18` (a one-off 9k-request storm was root-caused to a stale vite-HMR dev session running a superseded URL-mirror effect — not reproducible on clean HEAD, structurally prevented by untracked `$swap` + the new guards; dev-only, artifact builds unaffected). Implemented: server `889fbcc` (registration w/ session issuance, manual handicap via `handicap.service.record`, `GuestClaimService` w/ tombstone columns mig 032, optional-session `creator_player_id` + `recorded_by_player_id`, dashboard my-rounds), client `67a8050` (register mode, `/profile` w/ history, claim-me card, add-me player-ref producers, landing my-rounds), `7d1382b` (shareToken on produced entries), `95a20ca` (seeds `player-manual-handicap`/`guest-claim`/`attributed-scoring` + `scripts/render-phase3-verify.ts` → `tmp/formats/phase3-verify.html`, regen `bun scripts/render-phase3-verify.ts`). **Friends addition:** server `3ef14c8` (mig 033 `friendships` + `players.gender`, `friend.service` idempotent add / 404 unknown / 409 self, `GET /players/search` w/ `isFriend` + LIKE-escape, `POST /players/me/profile`), client `72c0e79` (`/friends` screen w/ debounced sequence-guarded search, `addFriend`≡`addMe` player-ref path w/ gender lock, "+ From friends" picker, nav → HOME/FRIENDS/PROFILE, deleted legacy `/rounds` + `/players` screens, fixed a round-view URL-mirror bug that clobbered other screens' query params). Gates: **583 server + 125 client tests**, fixture oracle 14 rounds identical. Both loops browser-verified (3-account friends flow). Awaiting user approval (verify page + live friends flow), then commit the completion marker.
...
- `bf50aca` live leaderboards rank by **pace**, not absolute totals (metric-declared: stableford 2 pts/hole, stroke play to-par via `pace:'par'`); ± pace chips render next to totals; one stableford fixture's order was corrected (it encoded the old wrong sort on a thru-9 entry).
- `ff4f42e` self-join group picker ("Group 1 · 09:00 — 2 of 4" / "Start a new group") + groups born with `capacity max(4, members)` (closes the flagged capacity behavior).
- `f9a7f9c` **ADR-0004 — sides as subjects**: engine-level `best_net` aggregation synthesizes a virtual score stream per multi-ball team at materialization; ANY ball-ranking format consumes team subjects unchanged (the north star: any format × any composition, as data). Equivalence proven: stableford-over-sides ≡ stableford_better_ball numerically. Formats untouched; ratchet forbids aggregation in format modules; new fixture `kopenhamnare-sides-round` (oracle now 15 rounds).
- `761a793`+`1766bec` **edit round after creation**: versioned `round_setup_drafts` (mig 034) beside the definition chain; token-scoped `GET/POST /friendly-rounds/setup` recompiling via the correction path; self-join keeps the stored draft canonical. Locks: course/route frozen once scored, scored producers unremovable (`producer_has_scores`), complete rounds sealed. Retroactive-format proven (match play added on hole 3 arrives scored thru 3). Client: Edit round card → create flow prefilled via pure `draft-to-forms` (guest ids + producer def-ids preserved — round-trip identity test), Save changes/Cancel, humanized refusals.
...
Tables:
- `competitions` — id, name, lifecycle (`draft → setup → active → finalized`), `default_config_json` (default slots + category-tee map + start-list mode), `aggregation` (strategy id + config JSON), `point_template_id?` (plain nullable TEXT already reserved by migration 037 for Phase 5), `cut_rules_json?`, `is_results_final`, `results_finalized_at`, `owner_player_id`. Tour/series FKs arrive with their phases under the FK-target rule.
...
**Status: AWAITING VISUAL VERIFY (2026-07-13).** All six slices landed, one commit each: schema+lifecycle+authz `93e5d3c`, materialisation `178eb76`, aggregation registry `28f1490`, cut+finalize+audit `01d2352`, client `d778f49`, seeds+verify `8232299`. Gates: 3 typechecks clean, **824 server + 268 client tests, 0 fail**, fixture oracle 15 rounds identical, aggregation ratchet green (negative controls in-test). Notable implementation decisions: competition rounds also get a `friendly_rounds` wrapper row (token front door — every existing token-scoped surface works unchanged; public landing list filters them out); `aggregation_json` single column `{strategyId, config}`; finalize is the only door to `finalized` (generic transition refuses `finalize_reserved`; schema-level consistency CHECK); cut reads the STORED rule, tie-at-line advances all (golf convention), `within_strokes` references the leader; `competition_audit_events` append-only log (mig 038) records `cut_applied`/`finalized` with actor + payload; stroke finalization writes gross AND net row sets; client aggregation picker is descriptor-driven via open `GET /setup/aggregations` (`configFields` on descriptors — new strategies need zero client changes); nav gained a 4th "Comps" tab. Verify page: `bun scripts/render-phase4-verify.ts` → `tmp/formats/phase4-verify.html` (5 required checks incl. R1+R2 arithmetic w/ shared-position tie, tie-at-the-line cut, per-round allowance override 100%→85%, official-vs-live distinction, gross/net reorder). Deferred: round count on the competitions list payload; `custom` cut type rejected (no semantics yet).

**Review remediation complete (2026-07-13):** anonymous global FriendlyRound and guest-player reads are no longer published (token-scoped play/setup remains intact); the competition detail screen is now a shallow shell over setup, roster, rounds, and results modules with declarative board rendering; Phase 4 persistence operations are held behind explicit query inventories; project/framework verification uses the committed vendored adapter; and stale domain/migration guidance is corrected. Closure gates: all three typechecks clean, **1078 project + 205 framework tests, 0 fail**, fixture oracle 15 rounds identical, production build clean, query-inventory and source-hygiene ratchets green. `tmp/formats/phase4-verify.html` was regenerated. Phase 4 remains **AWAITING VISUAL VERIFY**; the only next action is the mandatory focused visual review and explicit approval, not Phase 5 implementation.
...
**Migration constraint:** migration 037 already created
`competitions.point_template_id` as nullable TEXT, without an FK because the
parent table did not yet exist. Phase 5 must **not** add a column with that name.
Create `point_templates` first, then either rebuild `competitions` so the
existing column gains a real FK while preserving all Phase 4 checks, indexes,
and data, or install an explicit equivalent integrity mechanism covering both
invalid references and parent deletion. Record and test the chosen mechanism;
service-layer validation alone is not schema integrity.
...
**Slice 9a implementation plan (2026-07-27).** Four steps, deployable server-first (clients keep polling until the client swap lands):

1. **`server/services/round-events-hub.ts`** — in-process pub/sub: `subscribe(roundId, cb) → dispose`, `notify(roundId, eventId)`. Single Bun process owns the SQLite DB, so in-process suffices — no external broker, no per-connection DB polling. Wired into the two cursor movers (`RoundService.bumpResultCursor` + `recordLatestEvent`); a ~200 ms per-round trailing-edge debounce inside the hub closes the emit-before-commit race (`bumpResultCursor` can run inside a caller's transaction) and pre-buys 9b's coalescing.
2. **`GET /api/friendly-rounds/events?token=…&since=<eventId>`** as `text/event-stream` via `hono/streaming`'s `streamSSE`, mounted as a raw Hono route (streaming doesn't fit the descriptor → `*.gen.ts` generator; promotion into `@basics/core` deferred to 9b — the framework is a versioned tarball and 9a shouldn't wait on a release). Token-scoped, **no session required** — identical before/after N2's bearer auth, so iOS can use it pre-N2. On connect, emit the current cursor immediately (covers the stale-`since` gap fill and gives a fresh client its first cursor). Messages carry `id: <latestEventId>` (browser `Last-Event-ID` auto-reconnect for free) + single-line JSON `{"latestEventId":"…","status":"…"}` — one event type, no SSE exotica, so the Swift parser stays ~30 lines of bytes-stream splitting. `status` rides along because the stream ends on a completed round and `EventSource` auto-reconnects on stream end — without status in the payload a remotely-finished round reconnect-loops forever; with it the client closes its own gate. Finish/reopen transitions notify the hub too (status changes are otherwise invisible to an open stream). Heartbeat comment ~25 s against proxy idle timeouts; stream ends after emitting on a completed round.
3. **Durable cursor** (the native-note prerequisite, done web-side now): `resultCursor` moves from the private in-memory field on `src/round/round.service.ts` to localStorage keyed by token. Prerequisite refactor from the web-client-debt list rides along: one `deviceStore<T>(key, codec, cap?)` replacing the three copy-pasted wrappers (`seen-rounds.ts`, `device-rounds.ts`, `friend-sort-pref.ts`) rather than adding a fourth copy. Guard: never send a persisted cursor on a request when no result is cached in memory — an `unchanged` answer with nothing to render would blank the board (`loadResult` already omits the cursor; keep it that way).
4. **Client swap at `src/round/round.component.ts:481`** — the gate effect starts/stops a small `EventSource` wrapper (`src/round/live-result.ts`) instead of the `setInterval`; each message triggers the existing `pollResult()` refetch, leaving the presenter pipeline untouched. `shouldPoll` stays the sole gate (backgrounding closes the stream — the battery-honest requirement); each message also applies the payload's `status` to the round signal, so a round finished on another phone closes the gate instead of reconnect-looping. Repeated SSE failure (~3 consecutive errors without a successful open) degrades to the existing 20 s interval, so the fallback is zero new code. Initial connect passes the persisted cursor as `since`.
...
**Status: BUILT (2026-07-27), commits `37b6ee8` (foundation) → `2dee017` (features) → `b5e74d2` (screens); GATE PARTIALLY VERIFIED.** Four opus implement/review waves, every finding arbitrated and applied. Headless-proven: deep link (`-tapscoreDeepLink`, DEBUG launch arg) into the live dev round renders real data end to end (course, 4 players, hole scores with par tinting) — byToken → balls → result → ResultLayout fold on device; iOS bundle 261 tests / 0 failures; generator byte-proof 21/21. Awaiting interactive verification (simulator input access not granted this session): score entry from the app → web board, SSE push into the app's leaderboard tab, then the two user-only legs — SIWA on a real device and the cold share-link <30 s timing. Known follow-ups: `FormatsEndpoints.list` answers 401 anonymously (stats step silently disabled for anonymous rounds — server question), JoKS-outage backoff, the three AdvancePolicy QUIRKs pending a joint policy+tests decision.

**N5 — identity linking (2026-07-28, commit `996d001`) + server-URL policy (`c2094cc`).** Link-first, never fork: `POST /auth/native/login` (bearer sibling of the framework's cookie login; no enumeration oracle; own rate-limit keyspace), `created` flag on `/auth/apple`, iOS password door + Connect-Apple affordance with fork guards (an expired bearer on the link path is a FAILED link — revoke, don't adopt). ADR-0005 §"Linking in practice": email may only ever suggest, never merge; row-merge stays deferred. URL policy: `APIConfiguration.default` is production everywhere; dev runs pass `-apiBaseURL` explicitly; server-config UI is `super_admin`-only via `/me/roles`, failing closed, absent for everyone else. Also fixed: iOS `me()` now reads `/players/me` (the `/auth/me` AuthUser shape broke native sign-in against real servers). Follow-ups: widen native login to full `Player`; web password reset (more load-bearing now); `FormatsEndpoints.list` anonymous 401.

**Design parity (2026-07-27 evening, commit `b68db81`).** The iOS app now carries the web's clubhouse-scorecard identity: `scripts/generate-theme-swift.ts` emits `ThemeTokens.swift` from `src/theme.ts` (committed output, regenerate via `bun run generate:theme` on any web theme change — the raw palettes are exported from theme.ts for exactly this), Fraunces/Archivo bundled, `DesignSystem/` primitives spec'd from the web CSS, all screens restyled view-layer-only and verified by side-by-side screenshots against the web at mobile viewport (both appearances). The `-tapscoreGallery` DEBUG screen is the primitive review surface. Residual polish: leaderboard player-name truncation vs the web's column widths; format chips show raw ids until the anonymous formats-endpoint question is settled.
...
- **Create-flow convergence (2026-07-28).** `docs/proposals/create-flow-behavior.md` is the owner-approved behavior contract for round creation, written by scouting the web's actual code + live behavior and now implemented on iOS. Its §13 records nine web gaps the owner considers defects — headline items: the unsearchable course selector (W1), silent first-course auto-select shipping an unchosen `courseId` (W2), alphabetical tee order with no Swedish canon or length sort (W3), no gender tee defaults (W4), the silent gender/tee rating mismatch whose only feedback is a raw uuid-leaking server message (W5), the searchless friends picker (W6), and the undisclosed `postingEligible:false` on rotated start holes (W8). Converging the web onto the spec (including the owner-mandated Course → Players → Format order, W9) is a standing follow-up; the spec's numbered behaviors are the implementation checklist.
