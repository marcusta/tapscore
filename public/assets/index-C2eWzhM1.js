...
</tr>`;const G=gs(n.plan.slotDefId,L.ballIds),E=n.isOpen(G),K=Fc(G),te=Zi(N,n.routeSections,e,{mode:n.mode??"product"}),se=["lb-rank__row--expandable"];return g.lead&&se.push("lb-rank__lead"),E&&se.push("lb-rank__row--open"),`<tr class="${se.join(" ")}" data-expand-key="${M(G)}">
  <td class="lb-rank__pos">${g.position}</td>
...
</div>`}const Dc={ranked:Bc,match_summary:(s,e)=>jc(s,e)},Gc={"default-score-grid":Hc,"compact-match-grid":Ac,"category-matrix-grid":Lc};function qc(s){return`<div class="lb-diag">Unrenderable result section <code>${M(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Vc(s){return`<div class="lb-diag">Unsupported score-grid component <code>${M(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Uc(s,e,t,n=null){const i=Dc[s.kind];return i?i(s,e,t,n):qc(s.kind)}function Zi(s,e,t,n){const i=Qi(s),r=Gc[i];return r?r(s,e,t,n):Vc(i)}function er(s,e,t=Ji,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${M(s.formatLabel)}.</div>`:s.leaderboard.map(r=>Uc(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${M(s.formatLabel)}.</div>`}function tr(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>Zi(r,e,t,{mode:i})).join(`
`)}const Kc=b(`
