...
</div>`}function Lt(o,e){const t=o.matches.map(s=>{const n=v(s.sideA.ballIds.map(e).join(" & ")),r=v(s.sideB.ballIds.map(e).join(" & ")),a=s.magnitude===0?"AS":`${s.magnitude} UP`,c=s.finished?"Final":`thru ${s.thru}`,d=s.leader==="a"?" lb-mp__team--lead":"",u=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
    <div class="lb-mp__team lb-mp__team--a${d}">${n}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${v(a)}</span><span class="lb-mp__status">${v(c)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${u}">${r}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${v(o.title)}</h4>${t}
</div>`}function Mt(o){return`<div class="lb-diag">Unrenderable result section <code>${v(o)}</code> — no generic view yet. Results are not hidden.</div>`}function Bt(o,e){return o.leaderboard.length===0&&o.cards.length===0?`<div class="lb-empty">No scores entered yet for ${v(o.formatLabel)}.</div>`:o.leaderboard.map(s=>s.kind==="ranked"?Rt(s,e):s.kind==="match_summary"?Lt(s,e):Mt(s.kind)).join("")||`<div class="lb-empty">No leaderboard metric for ${v(o.formatLabel)}.</div>`}function Ft(o,e,t){return o.cards.length===0?"":o.cards.map(s=>Ht(s,e,t)).join(`
`)}const At=b(`
...
`),Dt=b('<button bind="pill" class="lb__pill" type="button"></button>');class qt extends E{static styles=`
        .lb {
            ...
            & .lb-grid .lb-sum { width: 2.4em; font-weight: 700; background: ${i("surface-sunken")}; }
    ...
    `;svc=this.inject(re);selected=new h(0);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots();return e[this.selected.get()]??e[0]??null};render(){this.track(y(()=>{const s=this.slots().length;s>0&&this.selected.get()>=s&&this.selected.set(0)}));const e=this.wire(At,{status:{className:()=>{const s=this.svc.resultLoading.get(),n=this.svc.result.get()===null;return s||n?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}}),t=this.ref(e,"selector");return this.track(y(()=>{t.className=this.slots().length>1?"lb__selector":"lb__selector hidden"})),this.$each(t,new $(()=>this.slots().length>1?this.slots():[]),(s,n,r)=>this.slotPill(s,n,r),s=>s.slotDefId),e}slotPill(e,t,s){return this.wireEl(Dt,{pill:{textContent:e.formatLabel,className:()=>this.selected.get()===t?"lb__pill active":"lb__pill",onclick:()=>this.selected.set(t)}},s)}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=c=>this.svc.nameOf(c),n=Bt(t,s),r=Ft(t,e.routeSections,s),a=r?`<h3 class="lb-cards__head">Scorecard</h3>${r}`:"";return n+a}}const Gt=b(`
...
`);class Kt extends E{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);function cs(o){return o.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class ds extends E{static styles=`
        .players {
            padding: ${l("xl")} ${l("lg")} ${l("2xl")};
...
`),ws=b('<div bind="row" class="results-slot"></div>');class $s extends E{static styles=`
        .results {
            padding: ${l("lg")} ${l("lg")} ${l("2xl")};
...
`);class Ss extends E{static styles=`
    ...
    `;router=this.inject(z);render(){const e=this.wire(ks,{});return this.spawn(et,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":pe,"/round":Kt,"/create":ns,"/login":os,"/rounds":ms,"/players":ds,"/score":_s,"/results":$s},pe),e}}O.get(Be);const ye=O.get(z),_e=O.get(se);await Ae(Ss,"#app",{hot:void 0,onInit:async()=>{await _e.load(),_e.currentUser.get()&&ye.route.get()==="/login"&&ye.navigate("/",!0)}});export{E as C,z as R,h as S,Be as T,f as a,K as b,$ as c,y as e,w as r,b as t};
