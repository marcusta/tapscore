(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const Lt="modulepreload",Dt=function(i){return"/tapscore/"+i},De={},Ht=function(e,t,s){let n=Promise.resolve();if(t&&t.length>0){let d=function(u){return Promise.all(u.map(p=>Promise.resolve(p).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const l=document.querySelector("meta[property=csp-nonce]"),c=l?.nonce||l?.getAttribute("nonce");n=d(t.map(u=>{if(u=Dt(u),u in De)return;De[u]=!0;const p=u.endsWith(".css"),m=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${m}`))return;const g=document.createElement("link");if(g.rel=p?"stylesheet":Lt,p||(g.as="script"),g.crossOrigin="",g.href=u,c&&g.setAttribute("nonce",c),document.head.appendChild(g),p)return new Promise((S,v)=>{g.addEventListener("load",S),g.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${u}`)))})}))}function r(l){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=l,window.dispatchEvent(c),!c.defaultPrevented)throw l}return n.then(l=>{for(const c of l||[])c.status==="rejected"&&r(c.reason);return e().catch(r)})};class Mt{constructor(){this.tracking=null,this.batching=!1,this.pending=new Set}subscribe(e){this.tracking&&(e.add(this.tracking),this.tracking.deps.add(e))}notify(e){for(const t of[...e])this.batching?this.pending.add(t):t.run()}runTracked(e,t){ot(e);const s=this.tracking;this.tracking=e;try{t()}finally{this.tracking=s}}untrack(e){const t=this.tracking;this.tracking=null;try{return e()}finally{this.tracking=t}}batch(e){this.batching=!0;try{e()}finally{this.batching=!1;const t=[...this.pending];this.pending.clear();for(const s of t)s.run()}}}const V=new Mt;function ot(i){for(const e of i.deps)e.delete(i);i.deps.clear()}class h{constructor(e){this.subs=new Set,this.val=e}get(){return V.subscribe(this.subs),this.val}peek(){return this.val}set(e){Object.is(this.val,e)||(this.val=e,V.notify(this.subs))}update(e){this.set(e(this.val))}}class w{constructor(e){this.subs=new Set,this.val=void 0;const t=this,s={run(){V.runTracked(s,()=>{const n=e();Object.is(t.val,n)||(t.val=n,V.notify(t.subs))})},deps:new Set};s.run()}get(){return V.subscribe(this.subs),this.val}peek(){return this.val}}function k(i){const e={run(){V.runTracked(e,i)},deps:new Set};return e.run(),()=>ot(e)}function re(i){V.batch(i)}function K(i){return V.untrack(i)}class Ft{constructor(){this.instances=new Map}get(e){let t=this.instances.get(e);return t||(t=new e,this.instances.set(e,t)),t}set(e,t){this.instances.set(e,t)}reset(){this.instances.clear()}}const A=new Ft,se="/tapscore/".replace(/\/+$/,"");function $e(i){return se?i===se?"/":i.startsWith(se+"/")?i.slice(se.length):i:i}function At(i){return se+i}class N{constructor(){this.route=new h($e(location.pathname??"/")),this.search=new h(location.search??""),window.addEventListener("popstate",()=>re(()=>{this.route.set($e(location.pathname)),this.search.set(location.search)}))}navigate(e,t){const s=typeof t=="boolean"?{replace:t}:t??{},n=e.indexOf("#"),r=n>=0?e.slice(n):"",l=n>=0?e.slice(0,n):e,c=l.indexOf("?"),d=c>=0?l.slice(0,c):l,u=c>=0?l.slice(c+1):"",p=s.query!==void 0?Bt(s.query):u?"?"+u:"",m=At(d)+p+r;(s.replace?history.replaceState:history.pushState).call(history,null,"",m),re(()=>{this.route.set(d),this.search.set(p)})}back(){history.back()}link(e,t="active"){const s=e.split("#")[0].split("?")[0];return{onclick:n=>{n.preventDefault(),this.navigate(e)},className:()=>{const n=this.route.get();return n===s||n.startsWith(s+"/")?t:""}}}params(e){const t=e.split("/");return new w(()=>{const s=this.route.get().split("/"),n={};for(const[r,l]of t.entries())l.startsWith(":")&&(n[l.slice(1)]=s[r]??"");return n})}query(e){return new w(()=>new URLSearchParams(this.search.get()).get(e)??void 0)}queries(){return new w(()=>{const e={};for(const[t,s]of new URLSearchParams(this.search.get()))e[t]=s;return e})}}function Bt(i){const e=new URLSearchParams;for(const[s,n]of Object.entries(i))n==null||n===""||e.set(s,String(n));const t=e.toString();return t?"?"+t:""}function Gt(i){return e=>i[e]}function qt(i,e){const t=(n,r,l)=>{const c=Object.entries(n).map(([d,u])=>`--${d}:${u}`).join(";");return`${r}{color-scheme:${l};${c}}`},s=document.createElement("style");return s.textContent=t(i,'[data-theme="light"]',"light")+t(e,'[data-theme="dark"]',"dark"),document.head.appendChild(s),n=>`var(--${n})`}const He="basics-js-theme";class Kt{constructor(){this.dark=new h(!1);const e=localStorage.getItem(He),t=matchMedia("(prefers-color-scheme: dark)").matches;this.dark.set(e?e==="dark":t),k(()=>{const s=this.dark.get();document.documentElement.setAttribute("data-theme",s?"dark":"light"),localStorage.setItem(He,s?"dark":"light")})}toggle(){this.dark.update(e=>!e)}}function _(i){const e=document.createElement("template");return e.innerHTML=i,e}function Vt(i,e){let t;for(const s of Object.keys(e))i.startsWith(s+"/")&&(!t||s.length>t.length)&&(t=s);return t?e[t]:void 0}const Me=new Set;class T{constructor(e={}){this.props=e,this.disposers=[],this.children=[];const t=this.constructor;if(t.styles&&!Me.has(t)){Me.add(t);const s=document.createElement("style");s.textContent=t.styles,document.head.appendChild(s)}}onMount(){}onDestroy(){}inject(e){return A.get(e)}track(e){this.disposers.push(e)}ref(e,t){return e.querySelector(`[bind="${t}"]`)}spawn(e,t,...s){const n=K(()=>{const r=new e(s[0]);return r.mount(t),r});return this.children.push(n),n}mount(e){e.appendChild(this.render()),this.onMount()}destroy(){this.onDestroy();for(const e of this.children)e.destroy();this.children.length=0;for(const e of this.disposers)e();this.disposers.length=0}wire(e,t,s){const n=s??(l=>this.track(l)),r=e.content.cloneNode(!0);for(const l of r.querySelectorAll("[bind]")){const c=t[l.getAttribute("bind")];if(c)if(typeof c=="function")n(k(()=>{const d=c();l instanceof HTMLInputElement||l instanceof HTMLTextAreaElement?l.value=String(d):l.textContent=String(d)}));else for(const[d,u]of Object.entries(c)){const p=d.includes("-");d.startsWith("on")&&typeof u=="function"?l.addEventListener(d.slice(2),u):typeof u=="function"?n(k(()=>{const m=u();p?l.setAttribute(d,String(m)):l[d]=m})):p?l.setAttribute(d,String(u)):l[d]=u}}return r}wireEl(e,t,s){return this.wire(e,t,s).firstElementChild}slot(e,t){const s=this.props[e];if(s==null)return!1;const n=this.ref(t,e);return n?(typeof s=="string"?n.textContent=s:typeof s=="function"&&s.prototype instanceof T?this.spawn(s,n):typeof s=="function"&&s(n,{spawn:(r,l,...c)=>this.spawn(r,l,...c),track:r=>this.track(r)}),!0):!1}$each(e,t,s,n=(r,l)=>l){const r=typeof t=="function"?t:()=>t.get(),l=new Map,c=new Map;this.track(()=>{for(const d of c.values())d.forEach(u=>u());c.clear()}),this.track(k(()=>{const d=r(),u=new Map;for(const[m,g]of d.entries()){const S=n(g,m);if(l.has(S))u.set(S,l.get(S));else{const v=[];u.set(S,K(()=>s(g,m,$=>v.push($)))),c.set(S,v)}}for(const[m,g]of l)u.has(m)||(g.remove(),c.get(m)?.forEach(S=>S()),c.delete(m));let p=e.firstChild;for(const m of u.values())m===p?p=p.nextSibling:e.insertBefore(m,p);l.clear();for(const[m,g]of u)l.set(m,g)}))}$condition(e,t,s,n){let r=null;this.track(k(()=>{r&&(r.remove(),r=null);const l=t.get();r=K(()=>l?s():n?.()??null),r&&e.appendChild(r)}))}$swap(e,t,s,n){let r=null;this.track(k(()=>{r&&(r.destroy(),r=null),e.textContent="";const l=t.get(),c=s[l]??Vt(l,s)??n;c&&(r=K(()=>{const d=new c;return d.mount(e),d}))})),this.track(()=>r?.destroy())}}async function Wt(i,e,t){const s=document.querySelector(e);s.textContent="";const n=A.get(N);let r=null,l=!1,c=null,d=!!t?.hot?.data.hmr;const u=async p=>{r&&(r.destroy(),r=null,s.textContent=""),p?(c||(c=(await Ht(()=>import("./obs-shell.component-CXngNVky.js"),[])).ObsShellComponent),r=K(()=>new c)):(!d&&t?.onInit&&(await t.onInit(),d=!0),r=K(()=>new i)),K(()=>r.mount(s)),l=p};await u($e(location.pathname).startsWith("/_obs")),k(()=>{const p=n.route.get().startsWith("/_obs");p!==l&&u(p)}),t?.hot&&(t.hot.data.hmr=!0,t.hot.dispose(()=>r?.destroy()),t.hot.accept())}class M extends Error{constructor(e,t,s,n){super(t),this.status=e,this.details=s,this.traceId=n,this.name="ApiError"}}const Ut=10,ue=[];let he=[],ne=null;function Yt(i){ue.push(i),ue.length>Ut&&ue.shift()}function Qt(i,e,t){const s={code:i,message:e,url:typeof location<"u"?location.href:"",context:[...ue],timestamp:new Date().toISOString()};t!==void 0&&(s.traceId=t),he.push(s),Xt()}function Xt(){ne||(ne=setTimeout(at,5e3))}function at(){if(ne&&(clearTimeout(ne),ne=null),he.length===0)return;const i=he;he=[];for(const e of i){const t=JSON.stringify(e);typeof navigator<"u"&&navigator.sendBeacon?navigator.sendBeacon("/api/_obs/errors",new Blob([t],{type:"application/json"})):typeof fetch<"u"&&fetch("/api/_obs/errors",{method:"POST",headers:{"Content-Type":"application/json"},body:t}).catch(()=>{})}}typeof document<"u"&&document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&at()});const Jt=3e4,Zt=2,le=new Map,lt=new WeakMap;function es(i){if(i instanceof M)return i.traceId;if(i!=null&&typeof i=="object")return lt.get(i)}async function f(i){if(i.method==="GET"){const e=le.get(i.url);if(e)return e;const t=Fe(i,Zt);return le.set(i.url,t),t.then(()=>le.delete(i.url),()=>le.delete(i.url)),t}return Fe(i,0)}async function Fe(i,e){const t=i.timeout??Jt;let s;for(let n=0;n<=e;n++){const r=crypto.randomUUID();try{return await ss(ts(i,r),t)}catch(l){if(s=l,!(l instanceof M)&&l!=null&&typeof l=="object"&&lt.set(l,r),l instanceof M||n===e)break;await new Promise(c=>setTimeout(c,1e3*2**n))}}throw s}async function ts(i,e){const t={"X-Trace-Id":e},s={method:i.method,headers:t};i.body!==void 0&&(t["Content-Type"]="application/json",s.body=JSON.stringify(i.body));const n=await fetch(i.url,s),r=n.headers.get("x-trace-id")??e;if(Yt({type:"api",detail:`${i.method} ${i.url}`,timestamp:new Date().toISOString()}),!n.ok){const l=await n.json().catch(()=>({error:n.statusText}));throw new M(n.status,l.error??n.statusText,l.details,r)}return n.json()}function ss(i,e){let t;const s=new Promise((n,r)=>{t=setTimeout(()=>r(new Error("Request timeout")),e)});return Promise.race([i,s]).finally(()=>clearTimeout(t))}async function I(i,e,t){re(()=>{i.set(!0),e.set(null)});try{const s=await t();return i.set(!1),s}catch(s){const n=ns(s);re(()=>{i.set(!1),e.set(n)}),Qt(n.code,n.message,es(s));return}}function ns(i){return i instanceof M?i.status===401?{code:"auth",message:"Unauthorized"}:i.status===409?{code:"conflict",message:"Data has changed — please try again"}:i.status===400?{code:"validation",message:i.message}:{code:"server",message:"Server error"}:i instanceof Error?i.message==="Request timeout"?{code:"timeout",message:"Request timeout"}:{code:"network",message:"Network error"}:{code:"unknown",message:"Unknown error"}}function is(i){return{me:()=>f({method:"GET",url:`${i}/auth/me`}),login:e=>f({method:"POST",url:`${i}/auth/login`,body:e}),logout:()=>f({method:"POST",url:`${i}/auth/logout`,body:{}})}}class L{constructor(){this.api=is("/api"),this.currentUser=new h(null),this.loading=new h(!1),this.error=new h(null)}async load(){const e=await I(this.loading,this.error,()=>this.api.me());e&&this.currentUser.set(e),this.error.get()?.code==="auth"&&this.error.set(null)}async login(e,t){const s=await I(this.loading,this.error,()=>this.api.login({username:e,password:t}));return s?(this.currentUser.set(s),!0):!1}async logout(){await I(this.loading,this.error,()=>this.api.logout());const e=this.error.get();(!e||e.code==="auth")&&this.currentUser.set(null)}}const Ae={radius:"12px","radius-pill":"999px","radius-sm":"6px","font-display":"'Fraunces', Georgia, serif",shadow:"0 1px 2px rgba(30, 53, 38, 0.08)","shadow-elevated":"0 4px 16px rgba(30, 53, 38, 0.14)"},o=qt({...Ae,bg:"#f2eee2",surface:"#fbf9f1","surface-sunken":"#e9e4d4",primary:"#2c5e3f","primary-text":"#f7f4ea","btn-bg":"#fbf9f1","btn-hover":"#efeada",text:"#1e3526","text-muted":"#6b7a6e",border:"#d8d2bf","topbar-bg":"#1e3526","active-bg":"#1e3526","active-text":"#f7f4ea","hover-bg":"#ece7d7","input-bg":"#ffffff",accent:"#b08d3e","accent-soft":"#f0e6cd",error:"#a0463c","under-par":"#a0463c","over-par":"#345b8a","hole-bar":"#e6a23f","hole-bar-text":"#3a2a0d"},{...Ae,bg:"#15231a",surface:"#1d2f22","surface-sunken":"#101b14",primary:"#5d9b75","primary-text":"#0f1a13","btn-bg":"#24392b","btn-hover":"#2e4836",text:"#e6e1d2","text-muted":"#8da093",border:"#33493a","topbar-bg":"#0f1a13","active-bg":"#5d9b75","active-text":"#0f1a13","hover-bg":"#273c2e","input-bg":"#101b14",accent:"#cfa84f","accent-soft":"#3a3320",error:"#d48a82","under-par":"#d48a82","over-par":"#8db2e0","hole-bar":"#c08a35","hole-bar-text":"#160f04",shadow:"0 1px 2px rgba(0, 0, 0, 0.3)","shadow-elevated":"0 4px 16px rgba(0, 0, 0, 0.4)"}),j=i=>`var(--${i})`,a=Gt({xs:"0.25rem",sm:"0.5rem",md:"0.75rem",lg:"1rem",xl:"1.5rem","2xl":"2rem"}),x=(i=j("radius"))=>`
...
`);class Ts extends T{static styles=`
        .tabbar {
            display: flex;
    ...
    `;router=this.inject(N);auth=this.inject(L);landing=this.inject(Te);newCount=new w(()=>this.auth.currentUser.get()?this.landing.newRounds.get().length:0);render(){return this.wire(Es,{root:{className:()=>{const e=this.router.route.get();return!this.auth.currentUser.get()||e==="/login"||e==="/round"?"tabbar hidden":"tabbar"}},homeLink:this.router.link("/"),badge:{textContent:()=>{const e=this.newCount.get();return e===0?"":String(e)},className:()=>this.newCount.get()===0?"tabbar__badge":"tabbar__badge show"},friendsLink:this.router.link("/friends"),compsLink:this.router.link("/competitions"),profileLink:this.router.link("/profile")})}}const ze=class ze extends T{render(){return this.el=document.createElement("div"),this.el.className="ui-overlay",this.el.style.background=this.props.bg??"rgba(0,0,0,0.4)",this.el.style.zIndex=String(this.props.zIndex??50),this.el.addEventListener("click",()=>{this.props.onclose?this.props.onclose():this.props.open.set(!1)}),this.track(k(()=>{const e=this.props.open.get();this.el.classList.toggle("open",e),this.props.scrollLock&&(document.body.style.overflow=e?"hidden":"")})),this.el}onDestroy(){this.props.scrollLock&&(document.body.style.overflow="")}};ze.styles=`
        .ui-overlay {
            position: fixed;
            inset: 0;
            opacity: 0;
    ...
    `;let xe=ze;const R=i=>`var(--${i})`,Re=class Re extends T{render(){const e=document.createElement("div");this.spawn(xe,e,{open:this.props.open,bg:"rgba(0,0,0,0.4)",zIndex:199,scrollLock:!0,onclose:()=>this.handleCancel()}),this.dialogEl=document.createElement("div"),this.dialogEl.className="ui-confirm",this.dialogEl.style.zIndex="200";const t=document.createElement("h2");t.className="ui-confirm__title",t.textContent=this.props.title??"Confirm",this.dialogEl.appendChild(t);const s=document.createElement("p");if(s.className="ui-confirm__message",typeof this.props.message=="function"){const c=this.props.message;this.track(k(()=>{s.textContent=c()}))}else s.textContent=this.props.message;this.dialogEl.appendChild(s);const n=document.createElement("div");n.className="ui-confirm__actions";const r=document.createElement("button");r.className="ui-confirm__btn ui-confirm__btn--cancel",r.textContent=this.props.cancelLabel??"Cancel",r.addEventListener("click",c=>{c.stopPropagation(),this.handleCancel()}),n.appendChild(r);const l=document.createElement("button");return l.className=this.props.danger?"ui-confirm__btn ui-confirm__btn--danger":"ui-confirm__btn ui-confirm__btn--confirm",l.textContent=this.props.confirmLabel??"Confirm",l.addEventListener("click",c=>{c.stopPropagation(),this.props.open.set(!1),this.props.onconfirm()}),n.appendChild(l),this.dialogEl.appendChild(n),this.dialogEl.addEventListener("click",c=>c.stopPropagation()),e.appendChild(this.dialogEl),this.track(k(()=>{this.dialogEl.classList.toggle("open",this.props.open.get())})),e}handleCancel(){this.props.open.set(!1),this.props.oncancel&&this.props.oncancel()}};Re.styles=`
        .ui-confirm {
            position: fixed;
            top: 50%;
            left: 50%;
    ...
    `;let B=Re;function Cs(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function ft(){return Cs()}class oe{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await I(this.loading,this.error,()=>b.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=ft()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function gt(i){const e=A.get(oe);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}function Ns(i){return i.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:vs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(gt).join(" · ")}))}function Ps(i){return i.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const ie={fromMyRounds:Ns,fromDeviceRounds:Ps},zs=14,Rs=1440*60*1e3;function ee(i,e){return e(i)}function js(i,e,t,s=zs){const n=e-s*Rs,r=[],l=[];for(const c of i){const d=ee(c,t);if(d.status==="complete"){const u=d.completedAt?Date.parse(d.completedAt):NaN;(Number.isNaN(u)||u>=n)&&l.push(c)}else r.push(c)}return r.sort((c,d)=>qe(ee(c,t).lastActivityAt,ee(d,t).lastActivityAt)),l.sort((c,d)=>qe(ee(c,t).completedAt,ee(d,t).completedAt)),{ongoing:r,finished:l}}function qe(i,e){const t=i?Date.parse(i):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,r=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===r?0:r-n}const Os=_(`
    <div class="landing">
...
`),Ls='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',Ds=_(`
    <div class="round-row">
        ...
        <button bind="del" type="button" class="round-row__del" aria-label="Delete round">${Ls}</button>
    </div>
`),bt={not_started:"Not started",active:"Live",complete:"Finished"};class Ke extends T{static styles=`
        .landing {
    ...
    `;svc=this.inject(Te);auth=this.inject(L);router=this.inject(N);loggedIn=new w(()=>this.auth.currentUser.get()!==null);rows=new w(()=>this.loggedIn.get()?ie.fromMyRounds(this.svc.myRounds.get()):ie.fromDeviceRounds(this.svc.deviceRounds.get()));parts=new w(()=>js(this.rows.get(),Date.now(),e=>e));ongoing=new w(()=>this.parts.get().ongoing);finished=new w(()=>this.parts.get().finished);newRows=new w(()=>this.loggedIn.get()?ie.fromMyRounds(this.svc.newRounds.get()):[]);deleteOpen=new h(!1);deleteTarget=new h(null);askDelete(e,t,s){this.deleteTarget.set({token:e,roundId:t,name:s}),this.deleteOpen.set(!0)}render(){this.loggedIn.get()?this.svc.loadMine():this.svc.loadDevice();const e=()=>this.rows.get().length>0,t=this.wire(Os,{createBtn:{onclick:()=>this.router.navigate("/create")},signin:{className:()=>this.loggedIn.get()?"landing__signin hidden":"landing__signin",onclick:()=>this.router.navigate("/login")},history:{className:()=>e()?"landing__history":"landing__history hidden",onclick:()=>this.router.navigate("/history")},newSection:{className:()=>this.newRows.get().length>0?"landing__section-block landing__new":"landing__section-block landing__new hidden"},newCount:()=>{const n=this.newRows.get().length;return n===0?"":String(n)},ongoingSection:{className:()=>this.ongoing.get().length>0?"landing__section-block":"landing__section-block hidden"},ongoingCount:()=>{const n=this.ongoing.get().length;return n===0?"":String(n)},finishedSection:{className:()=>this.finished.get().length>0?"landing__section-block":"landing__section-block hidden"},finishedCount:()=>{const n=this.finished.get().length;return n===0?"":String(n)},empty:{className:()=>e()?"landing__empty hidden":"landing__empty"}});this.$each(this.ref(t,"newList"),this.newRows,(n,r,l)=>this.roundRow(n,l),n=>n.key),this.$each(this.ref(t,"ongoingList"),this.ongoing,(n,r,l)=>this.roundRow(n,l),n=>n.key),this.$each(this.ref(t,"finishedList"),this.finished,(n,r,l)=>this.roundRow(n,l),n=>n.key),this.spawn(B,this.ref(t,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:()=>{const n=this.deleteTarget.get();return`Delete ${n?`“${n.name}”`:"this round"}? This permanently removes it and all its scores for everyone. This can't be undone.`},confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const n=this.deleteTarget.get();n&&this.svc.remove(n.token,n.roundId)}});const s=n=>{n.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1)};return window.addEventListener("keydown",s),this.track(()=>window.removeEventListener("keydown",s)),t}roundRow(e,t){return this.wireEl(Ds,{row:{disabled:()=>e.token===null,onclick:()=>{e.token!==null&&this.router.navigate("/round",{query:{token:e.token}})}},course:()=>e.courseName||"Round",role:{textContent:()=>e.roleLabel??"",className:()=>e.roleLabel?"round-row__role":"round-row__role hidden"},status:{textContent:()=>bt[e.status]??e.status,className:()=>`round-row__status s-${e.status}`},date:()=>e.date??"",formats:()=>e.formats??"",del:{className:()=>e.token===null?"round-row__del hidden":"round-row__del",onclick:()=>{e.token!==null&&this.askDelete(e.token,e.roundId??"",e.courseName||"this round")}}},t)}}function Hs(i){return[...i].sort((e,t)=>{const s=Ve(e),n=Ve(t);return n!==s?n-s:e.key.localeCompare(t.key)})}function Ve(i){const e=i.completedAt??i.lastActivityAt,t=e?Date.parse(e):NaN;return Number.isNaN(t)?Number.NEGATIVE_INFINITY:t}const Ms=_(`
    <div class="history">
...
`),Fs='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',As=_(`
    <div class="round-row">
        <button bind="row" type="button" class="round-row__main">
            <div class="round-row__top">
                <span bind="course" class="round-row__course"></span>
...
`);class Bs extends T{static styles=`
        .history {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(Te);auth=this.inject(L);router=this.inject(N);loggedIn=new w(()=>this.auth.currentUser.get()!==null);rows=new w(()=>Hs(this.loggedIn.get()?ie.fromMyRounds(this.svc.myRounds.get()):ie.fromDeviceRounds(this.svc.deviceRounds.get())));deleteOpen=new h(!1);deleteTarget=new h(null);askDelete(e,t,s){this.deleteTarget.set({token:e,roundId:t,name:s}),this.deleteOpen.set(!0)}render(){this.loggedIn.get()?this.svc.loadMine():this.svc.loadDevice();const e=this.wire(Ms,{back:{onclick:()=>this.router.navigate("/")},empty:{className:()=>this.rows.get().length===0?"history__empty":"history__empty hidden"}});this.$each(this.ref(e,"list"),this.rows,(s,n,r)=>this.roundRow(s,r),s=>s.key),this.spawn(B,this.ref(e,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:()=>{const s=this.deleteTarget.get();return`Delete ${s?`“${s.name}”`:"this round"}? This permanently removes it and all its scores for everyone. This can't be undone.`},confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const s=this.deleteTarget.get();s&&this.svc.remove(s.token,s.roundId)}});const t=s=>{s.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1)};return window.addEventListener("keydown",t),this.track(()=>window.removeEventListener("keydown",t)),e}roundRow(e,t){return this.wireEl(As,{row:{disabled:()=>e.token===null,onclick:()=>{e.token!==null&&this.router.navigate("/round",{query:{token:e.token}})}},course:()=>e.courseName||"Round",role:{textContent:()=>e.roleLabel??"",className:()=>e.roleLabel?"round-row__role":"round-row__role hidden"},status:{textContent:()=>bt[e.status]??e.status,className:()=>`round-row__status s-${e.status}`},date:()=>e.date??"",formats:()=>e.formats??"",del:{className:()=>e.token===null?"round-row__del hidden":"round-row__del",onclick:()=>{e.token!==null&&this.askDelete(e.token,e.roundId??"",e.courseName||"this round")}}},t)}}const Gs=180,We=4,qs=12;function J(i,e){return e<=0?0:Math.max(0,Math.min(e-1,i))}function Ks(i){const{dragDistance:e,velocity:t,itemWidth:s}=i;if(Math.abs(e)<qs)return 0;const n=e+t*Gs,r=Math.round(-n/s);return Math.max(-We,Math.min(We,r))}const Ue="tapscore:pending-scores:v1",Vs=336*60*60*1e3,Ye=200;function Ws(){try{return globalThis.localStorage??null}catch{return null}}function Us(i){if(typeof i!="object"||i===null)return!1;const e=i;return typeof e.token=="string"&&typeof e.ballId=="string"&&typeof e.playHoleId=="string"&&(typeof e.strokes=="number"||e.strokes===null)&&(e.eventType==="score_entered"||e.eventType==="score_cleared")&&typeof e.clientEventId=="string"&&typeof e.queuedAt=="number"}class Ys{entries=[];storage;constructor(e=Ws(),t=Date.now()){this.storage=e,this.entries=this.load();const s=this.applyHygiene(t);s.length!==this.entries.length&&(this.entries=s,this.persist())}enqueue(e){const t=this.entries.findIndex(s=>s.token===e.token&&s.ballId===e.ballId&&s.playHoleId===e.playHoleId);t>=0?this.entries[t]=e:this.entries.push(e),this.entries=this.applyHygiene(e.queuedAt),this.persist()}remove(e){const t=this.entries.filter(s=>s.clientEventId!==e);t.length!==this.entries.length&&(this.entries=t,this.persist())}entriesFor(e){return this.entries.filter(t=>t.token===e)}size(){return this.entries.length}applyHygiene(e){const t=this.entries.filter(s=>e-s.queuedAt<=Vs);return t.length>Ye?t.slice(t.length-Ye):t}load(){if(!this.storage)return[];try{const e=this.storage.getItem(Ue);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t.filter(Us):[]}catch{return[]}}persist(){if(this.storage)try{this.storage.setItem(Ue,JSON.stringify(this.entries))}catch{}}}const Qs=["1st","2nd","3rd","4th","5th","6th","7th","8th"],U=(i,e)=>`${i}|${e}`;function _t(i){return i.players.map(e=>e.displayName).join(" & ")||i.label||"Ball"}function Xs(i,e,t){return i?!(i.minPar!==void 0&&e<i.minPar||i.maxPar!==void 0&&e>i.maxPar||i.pars&&!i.pars.includes(e)||i.holes&&!i.holes.includes(t)):!0}class Z{constructor(e=new Ys){this.queue=e}queue;loading=new h(!1);error=new h(null);friendlyRound=new h(null);round=new h(null);balls=new h([]);scorecards=new h([]);cells=new h(new Map);result=new h(null);resultLoading=new h(!1);resultError=new h(null);resultCursor=null;holeIdx=new h(0);groupIdx=new h(0);selectedSlot=new h(null);token=null;loadSeq=0;resultSeq=0;flushing=!1;pendingSlotIndex=null;async loadByToken(e,t){const s=e!==this.token;this.token=e;const n=++this.loadSeq;s&&this.resetForNewToken(t),A.get(oe).load();const r=await I(this.loading,this.error,()=>b.friendlyRounds.byToken({token:e}));if(!r||n!==this.loadSeq||e!==this.token)return;if(this.friendlyRound.set(r.friendlyRound),this.round.set(r.round),pe({token:e,courseName:r.round.courseNameSnapshot??"",status:r.round.status,completedAt:r.round.completedAt,lastSeenAt:new Date().toISOString()}),A.get(L).currentUser.get()&&ks(r.round.id),this.pendingSlotIndex!==null){const u=r.round.formatSlots[this.pendingSlotIndex]?.slotDefId??null;this.pendingSlotIndex=null,u!==null&&this.selectedSlot.set(u)}const[l,c]=await Promise.all([b.friendlyRounds.balls({token:e}).catch(()=>[]),b.friendlyRounds.scorecard({token:e}).catch(()=>[])]);n!==this.loadSeq||e!==this.token||(this.cells.set(new Map),this.scorecards.set(c),this.balls.set(l),await this.flushPending())}deleting=new h(!1);async deleteRound(){const e=this.token;if(!e||this.deleting.get())return!1;this.deleting.set(!0);try{await b.friendlyRounds.remove({token:e}),mt(e);const t=this.round.get()?.id;return t&&ut(t),!0}catch{return!1}finally{this.deleting.set(!1)}}finishing=new h(!1);async finishRound(){const e=this.token;if(!e||this.finishing.get())return null;this.finishing.set(!0);try{const t=await b.friendlyRounds.finish({token:e}),s=this.round.get();return e===this.token&&s&&(this.round.set({...s,status:t.status,completedAt:t.completedAt}),pe({token:e,courseName:s.courseNameSnapshot??"",status:t.status,completedAt:t.completedAt,lastSeenAt:new Date().toISOString()})),{status:t.status}}catch{return null}finally{this.finishing.set(!1)}}async reopenRound(){const e=this.token;if(!e||this.finishing.get())return null;this.finishing.set(!0);try{const t=await b.friendlyRounds.reopen({token:e}),s=this.round.get();return e===this.token&&s&&(this.round.set({...s,status:t.status,completedAt:null}),pe({token:e,courseName:s.courseNameSnapshot??"",status:t.status,completedAt:null,lastSeenAt:new Date().toISOString()})),{status:t.status}}catch{return null}finally{this.finishing.set(!1)}}async loadResult(){const e=this.token;if(!e)return;const t=++this.resultSeq,s=await I(this.resultLoading,this.resultError,()=>b.friendlyRounds.result({token:e}));t!==this.resultSeq||e!==this.token||s&&(this.resultCursor=s.cursor,s.unchanged||this.result.set(s.result))}async pollResult(){const e=this.token;if(!e)return;const t=++this.resultSeq;let s;try{s=await b.friendlyRounds.result({token:e,...this.resultCursor!==null?{cursor:this.resultCursor}:{}})}catch{return}t!==this.resultSeq||e!==this.token||(this.resultCursor=s.cursor,s.unchanged||this.result.set(s.result))}ballNameById=new w(()=>{const e=new Map;for(const t of this.balls.get())e.set(t.id,_t(t));for(const t of this.result.get()?.slots??[])for(const s of t.subjectLabels??[])e.set(s.ballId,s.label);return e});nameOf(e){return this.ballNameById.get().get(e)??e}groupLabelByBallId=new w(()=>{const e=new Map,t=this.groups();return t.length<2||t.forEach((s,n)=>{for(const r of s.ballIds)e.set(r,`Group ${n+1}`)}),e});groupLabelOf(e){return this.groupLabelByBallId.get().get(e)??null}selectedSlotDefId(){const e=this.round.get()?.formatSlots??[];if(e.length===0)return null;const t=this.selectedSlot.get();return t!==null&&e.some(s=>s.slotDefId===t)?t:e[0]?.slotDefId??null}selectSlot(e){this.selectedSlot.set(e)}groups(){return this.round.get()?.playingGroups??[]}group(){const e=this.groups();return e[this.groupIdx.get()]??e[0]??null}playedOrder(){return this.group()?.playedOrder??[]}holeIndex(){return J(this.holeIdx.get(),this.playedOrder().length)}currentPlayedHole(){return this.playedOrder()[this.holeIndex()]??null}playHoleById(e){return this.round.get()?.playHoles.find(t=>t.id===e)??null}currentPlayHole(){const e=this.currentPlayedHole();return e?this.playHoleById(e.playHoleId):null}parFor(e){return(e?this.playHoleById(e)?.par:null)??4}occLabel(e){const t=this.round.get(),s=t?.playHoles.find(l=>l.id===e);if(!t||!s)return"";const n=t.playHoles.filter(l=>l.courseHoleNumber===s.courseHoleNumber).sort((l,c)=>l.ordinal-c.ordinal);if(n.length===1)return`${s.courseHoleNumber}`;const r=n.findIndex(l=>l.id===e);return`${s.courseHoleNumber} (${Qs[r]??`${r+1}th`})`}canPrevHole(){return this.holeIndex()>0}canNextHole(){return this.holeIndex()<this.playedOrder().length-1}prevHole(){this.holeIdx.set(J(this.holeIndex()-1,this.playedOrder().length))}nextHole(){this.holeIdx.set(J(this.holeIndex()+1,this.playedOrder().length))}strokesFor(e,t){const s=this.cells.get().get(U(e,t));return s?s.strokes:this.scorecards.get().find(l=>l.ballId===e)?.holes.find(l=>l.playHoleId===t)?.strokes??null}statusFor(e,t){return this.cells.get().get(U(e,t))?.status??null}metadataFor(e,t,s){const n=this.cells.get().get(U(e,t));return n&&n.metadata!==void 0?n.metadata?.[s]:this.scorecards.get().find(c=>c.ballId===e)?.holes.find(c=>c.playHoleId===t)?.metadata?.[s]}metadataInputs(){const e=A.get(oe),t=this.round.get()?.formatSlots??[],s=[],n=new Set;for(const r of t){const l=e.byId(r.formatId)?.requirements.scoreEntry?.metadata??[];for(const c of l)n.has(c.key)||(n.add(c.key),s.push(c))}return s}metadataInputsForHole(e){return e?this.metadataInputs().filter(t=>Xs(t.appliesWhen,e.par,e.courseHoleNumber)):[]}async setScore(e,t,s,n){const r=U(e,t),l=crypto.randomUUID();this.patchCell(r,{strokes:s,metadata:n,status:"saving",clientEventId:l});const c=this.token;c&&(this.enqueue(c,e,t,s,n,l),await this.post(c,e,t,s,n,l))}async retry(e,t){const s=U(e,t),n=this.cells.get().get(s);if(!n)return;this.patchCell(s,{...n,status:"saving"});const r=this.token;r&&(this.enqueue(r,e,t,n.strokes,n.metadata,n.clientEventId),await this.post(r,e,t,n.strokes,n.metadata,n.clientEventId))}async flushPending(){const e=this.token;if(!(!e||this.flushing)){this.flushing=!0;try{for(const t of this.queue.entriesFor(e)){if(e!==this.token)return;this.patchCell(U(t.ballId,t.playHoleId),{strokes:t.strokes,metadata:t.metadata,status:"saving",clientEventId:t.clientEventId}),await this.post(e,t.ballId,t.playHoleId,t.strokes,t.metadata,t.clientEventId)}}finally{this.flushing=!1}}}enqueue(e,t,s,n,r,l){this.queue.enqueue({token:e,ballId:t,playHoleId:s,strokes:n,eventType:n===null?"score_cleared":"score_entered",clientEventId:l,...r!==void 0?{metadata:r}:{},queuedAt:Date.now()})}async post(e,t,s,n,r,l){const c=U(t,s);try{await b.friendlyRounds.score({token:e,ballId:t,playHoleId:s,strokes:n,eventType:n===null?"score_cleared":"score_entered",clientEventId:l,...r!=null?{metadata:r}:{}}),this.queue.remove(l);const d=this.cells.get().get(c);d&&d.clientEventId===l&&this.patchCell(c,{...d,status:"saved"});const u=this.round.get();e===this.token&&u&&u.status==="not_started"&&this.round.set({...u,status:"active"})}catch{const d=this.cells.get().get(c);d&&d.clientEventId===l&&this.patchCell(c,{...d,status:"error"})}}patchCell(e,t){const s=new Map(this.cells.get());s.set(e,t),this.cells.set(s)}resetForNewToken(e){this.resultSeq++,this.resultCursor=null,this.friendlyRound.set(null),this.round.set(null),this.balls.set([]),this.scorecards.set([]),this.cells.set(new Map),this.result.set(null),this.resultError.set(null),this.holeIdx.set(e?.holeIdx??0),this.groupIdx.set(e?.groupIdx??0);const t=e?.selectedSlot;this.pendingSlotIndex=null,typeof t=="string"?this.selectedSlot.set(t):typeof t=="number"?(this.pendingSlotIndex=t,this.selectedSlot.set(null)):this.selectedSlot.set(null)}}const Y=60,Qe=8,ke=4,Js=Array.from({length:ke*2+1},(i,e)=>e-ke),Zs="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",en=_(`
...
`);class on extends T{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
        ...
        .se-row {
            ...
            & .se-row__scores { display: flex; align-items: center; padding-right: ${Qe}px; flex-shrink: 0; }
            & .se-row__slot { width: ${Y}px; display: flex; align-items: center; justify-content: center; }
            & .se-row__prev {
    ...
    `;svc=this.inject(Z);holeIdx=this.svc.holeIdx;modalOpen=new h(!1);currentBallIdx=new h(0);extendedOpen=new h(!1);extendedScore=new h(10);statsOpen=new h(!1);pendingMeta=new h({});lastMetaKey=null;toastMsg=new h(null);dragOffset=new h(0);transitioning=new h(!1);ptr=null;pendingSteps=null;settleTimer=null;advanceTimer=null;flashTimer=null;hasScoring=new w(()=>this.svc.balls.get().length>0);group=()=>this.svc.group();playedOrder=()=>this.svc.playedOrder();holeIndex=()=>this.svc.holeIndex();currentHole=()=>this.svc.currentPlayedHole();occAtOffset=e=>{const t=this.playedOrder();return t[J(this.holeIndex()+e,t.length)]??null};ballsInGroup=()=>{const e=this.group();if(!e)return[];const t=new Map(this.svc.balls.get().map(s=>[s.id,s]));return e.ballIds.map(s=>t.get(s)).filter(s=>!!s)};parFor=e=>this.svc.parFor(e);occLabel=e=>this.svc.occLabel(e);ballName=e=>_t(e);metaInputs=()=>this.svc.metadataInputsForHole(this.svc.currentPlayHole()).filter(e=>e.kind==="boolean");displayScore=e=>e===null?"–":String(e);toParValue=e=>{let t=0,s=0,n=!1;for(const r of this.playedOrder()){const l=this.svc.strokesFor(e.id,r.playHoleId);l!==null&&l>0&&(t+=l,s+=this.parFor(r.playHoleId),n=!0)}return n?t-s:null};toParText=e=>{const t=this.toParValue(e);return t===null?"–":t===0?"E":t>0?`+${t}`:`${t}`};toParClass=e=>{const t=this.toParValue(e);return`se-row__topar ${t===null||t===0?"even":t<0?"under":"over"}`};scoreLabel=(e,t)=>{if(e===1)return"HIO";const s=e-t;return s<=-4||s>=5?"OTHER":{"-3":"ALBA","-2":"EAGLE","-1":"BIRDIE",0:"PAR",1:"BOGEY",2:"DOUBLE",3:"TRIPLE",4:"QUAD"}[String(s)]??""};render(){this.track(()=>{this.advanceTimer&&clearTimeout(this.advanceTimer),this.flashTimer&&clearTimeout(this.flashTimer),this.settleTimer&&clearTimeout(this.settleTimer)}),this.track(k(()=>{const r=this.ballsInGroup().length;r>0&&this.currentBallIdx.get()>=r&&this.currentBallIdx.set(0)}));const e=this.wire(en,{root:{className:()=>this.hasScoring.get()?"se":"se hidden"},close:{onclick:()=>{this.statsOpen.set(!1),this.modalOpen.set(!1)}},modal:{className:()=>this.modalOpen.get()?"se-modal":"se-modal hidden"},modalTitle:()=>{const r=this.currentHole();return r?`Hole ${this.occLabel(r.playHoleId)} · Par ${this.parFor(r.playHoleId)}`:""},extended:{className:()=>this.extendedOpen.get()?"se-pad__ext":"se-pad__ext hidden"},extVal:()=>String(this.extendedScore.get()),extMinus:{onclick:()=>this.extendedScore.set(Math.max(10,this.extendedScore.get()-1))},extPlus:{onclick:()=>this.extendedScore.set(this.extendedScore.get()+1)},extCancel:{onclick:()=>this.extendedOpen.set(!1)},extOk:{onclick:()=>{this.extendedOpen.set(!1),this.commit(this.extendedScore.get())}},toast:{className:()=>this.toastMsg.get()?"se-toast":"se-toast hidden",textContent:()=>this.toastMsg.get()??""},stats:{className:()=>this.statsOpen.get()?"se-stats":"se-stats hidden"},statsBack:{onclick:()=>this.statsOpen.set(!1)},statsHole:()=>{const r=this.currentHole();return r?`Hole ${this.occLabel(r.playHoleId)} · Par ${this.parFor(r.playHoleId)}`:""},statsTitle:()=>{const r=this.ballsInGroup()[this.currentBallIdx.get()];return r?this.ballName(r):""},statsScore:()=>{const r=this.ballsInGroup()[this.currentBallIdx.get()],l=this.currentHole();return!r||!l?"":this.displayScore(this.svc.strokesFor(r.id,l.playHoleId))},statsNext:{textContent:()=>this.hasMoreUnscored()?"Next ›":"Done ›",onclick:()=>{this.statsOpen.set(!1),this.advance()}}}),t=this.ref(e,"viewport"),s=this.ref(e,"track");this.bindCarouselPointer(t,s),this.track(k(()=>{s.style.transition=this.transitioning.get()?Zs:"none",s.style.transform=`translateX(${this.dragOffset.get()}px)`})),this.$each(s,new w(()=>Js),(r,l,c)=>this.holeItem(r,c),r=>r),this.$each(this.ref(e,"rows"),new w(()=>{const r=this.playedOrder(),l=this.holeIndex(),c=r[l];if(!c)return[];const d=l>0?r[l-1].playHoleId:null;return this.ballsInGroup().map(u=>({ball:u,ph:c.playHoleId,prevPh:d}))}),(r,l,c)=>this.playerRow(r.ball,r.ph,r.prevPh,c),r=>`${r.ball.id}|${r.ph}`),this.$each(this.ref(e,"modalList"),new w(()=>this.ballsInGroup()),(r,l,c)=>this.modalRow(r,l,c),r=>r.id);const n=this.ref(e,"keys");for(const r of[1,2,3,4,5,6,7,8,9])n.appendChild(this.numberKey(r));return n.appendChild(this.specialKey("10+","","se-key",()=>this.openExtended())),n.appendChild(this.specialKey("✕","clear","se-key clear",()=>this.commit(null))),n.appendChild(this.specialKey("0","pick up","se-key muted",()=>this.commit(0))),this.$each(this.ref(e,"statsBody"),new w(()=>this.metaInputs()),(r,l,c)=>this.metaChip(r,c),r=>r.key),this.track(k(()=>{if(!this.modalOpen.get()){this.lastMetaKey=null;return}const r=this.ballsInGroup()[this.currentBallIdx.get()],l=this.currentHole();if(!r||!l)return;const c=`${r.id}|${l.playHoleId}`;if(c===this.lastMetaKey)return;this.lastMetaKey=c;const d={};for(const u of this.metaInputs())d[u.key]=this.svc.metadataFor(r.id,l.playHoleId,u.key)===!0;this.pendingMeta.set(d)})),e}holeItem(e,t){return this.wireEl(tn,{item:{className:()=>{const s=e===-1&&this.holeIndex()<=0;return`se-hole${e===0?" active":""}${s?" gone":""}`}},hnum:{textContent:()=>{const s=this.occAtOffset(e);return s?this.occLabel(s.playHoleId):""}},hpar:{textContent:()=>{const s=this.occAtOffset(e);return s?`Par ${this.parFor(s.playHoleId)}`:""}}},t)}playerRow(e,t,s,n){return this.wireEl(sn,{name:{textContent:this.ballName(e)},topar:{textContent:()=>this.toParText(e),className:()=>this.toParClass(e)},prev:{textContent:()=>s?this.displayScore(this.svc.strokesFor(e.id,s)):""},cval:{textContent:()=>this.displayScore(this.svc.strokesFor(e.id,t))},circle:{className:()=>this.svc.strokesFor(e.id,t)===null?"se-row__circle empty":"se-row__circle",onclick:()=>this.openModalForBall(e.id)}},n)}modalRow(e,t,s){const n=e.players.length>1?`Team · CH ${e.courseHandicap}`:`CH ${e.players[0]?.courseHandicap??e.courseHandicap}`;return this.wireEl(nn,{mrow:{className:()=>this.currentBallIdx.get()===t?"se-mrow sel":"se-mrow",onclick:()=>this.currentBallIdx.set(t)},mname:{textContent:this.ballName(e)},mhcp:{textContent:n},mval:{textContent:()=>{const r=this.currentHole();return r?this.displayScore(this.svc.strokesFor(e.id,r.playHoleId)):"–"}}},s)}numberKey(e){return this.wireEl(Xe,{key:{className:()=>{const t=this.currentHole();return(t?e===this.parFor(t.playHoleId):!1)?"se-key par":"se-key"},onclick:()=>this.commit(e)},num:{textContent:String(e)},lbl:{textContent:()=>{const t=this.currentHole();return t?this.scoreLabel(e,this.parFor(t.playHoleId)):""}}})}specialKey(e,t,s,n){return this.wireEl(Xe,{key:{className:s,onclick:n},num:{textContent:e},lbl:{textContent:t}})}openModalForBall(e){const t=this.ballsInGroup().findIndex(s=>s.id===e);this.currentBallIdx.set(t<0?0:t),this.extendedOpen.set(!1),this.statsOpen.set(!1),this.modalOpen.set(!0)}openExtended(){this.extendedScore.set(10),this.extendedOpen.set(!0)}commit(e){const t=this.ballsInGroup(),s=this.currentHole(),n=t[this.currentBallIdx.get()];if(!s||!n)return;const r=e===null?void 0:this.metaSnapshot();this.svc.setScore(n.id,s.playHoleId,e,r),e!==null&&e>0&&this.metaInputs().length>0?this.statsOpen.set(!0):this.advance()}hasMoreUnscored=()=>{const e=this.ballsInGroup(),t=this.currentHole();if(!t)return!1;const s=this.currentBallIdx.get();return e.some((n,r)=>r!==s&&this.svc.strokesFor(n.id,t.playHoleId)===null)};metaSnapshot(){const e=this.metaInputs();if(e.length===0)return;const t=this.pendingMeta.get(),s={};for(const n of e)s[n.key]=t[n.key]===!0;return s}setMeta(e,t){const s=this.pendingMeta.get();this.pendingMeta.set({...s,[e]:t});const n=this.ballsInGroup()[this.currentBallIdx.get()],r=this.currentHole();if(!n||!r)return;const l=this.svc.strokesFor(n.id,r.playHoleId);l!==null&&this.svc.setScore(n.id,r.playHoleId,l,this.metaSnapshot())}metaChip(e,t){return this.wireEl(rn,{glabel:{textContent:e.label},miss:{className:()=>this.pendingMeta.get()[e.key]?"se-seg":"se-seg on-miss",onclick:()=>this.setMeta(e.key,!1)},hit:{className:()=>this.pendingMeta.get()[e.key]?"se-seg on-hit":"se-seg",onclick:()=>this.setMeta(e.key,!0)}},t)}advance(){const e=this.ballsInGroup(),t=this.currentHole();if(!t)return;const s=d=>this.svc.strokesFor(e[d].id,t.playHoleId)!==null,n=this.currentBallIdx.get();for(let d=n+1;d<e.length;d++)if(!s(d))return this.currentBallIdx.set(d);for(let d=0;d<n;d++)if(!s(d))return this.currentBallIdx.set(d);const r=this.playedOrder();if(this.holeIndex()>=r.length-1){this.flash("Round complete"),this.modalOpen.set(!1);return}this.flash(`Hole ${this.occLabel(t.playHoleId)} done`);const c=t.playHoleId;this.advanceTimer&&clearTimeout(this.advanceTimer),this.advanceTimer=setTimeout(()=>{this.advanceTimer=null,this.currentHole()?.playHoleId===c&&(this.holeIdx.set(J(this.holeIndex()+1,this.playedOrder().length)),this.currentBallIdx.set(0))},700)}flash(e){this.toastMsg.set(e),this.flashTimer&&clearTimeout(this.flashTimer),this.flashTimer=setTimeout(()=>{this.flashTimer=null,this.toastMsg.get()===e&&this.toastMsg.set(null)},1100)}snap(e){this.pendingSteps=e,this.transitioning.set(!0),this.dragOffset.set(-e*Y),this.settleTimer&&clearTimeout(this.settleTimer),this.settleTimer=setTimeout(()=>this.finishSettle(),420)}finishSettle(){if(this.pendingSteps===null)return;const e=this.pendingSteps;this.pendingSteps=null,this.settleTimer&&(clearTimeout(this.settleTimer),this.settleTimer=null),this.transitioning.set(!1),e!==0&&this.holeIdx.set(J(this.holeIndex()+e,this.playedOrder().length)),this.dragOffset.set(0)}bindCarouselPointer(e,t){t.addEventListener("transitionend",n=>{n.propertyName==="transform"&&this.finishSettle()}),e.addEventListener("pointerdown",n=>{this.ptr||this.transitioning.get()||this.playedOrder().length<=1||(this.ptr={id:n.pointerId,startX:n.clientX,startY:n.clientY,lastX:n.clientX,lastTime:Date.now(),velocity:0,horiz:!1},this.dragOffset.set(0),e.setPointerCapture?.(n.pointerId))}),e.addEventListener("pointermove",n=>{const r=this.ptr;if(!r||r.id!==n.pointerId)return;const l=n.clientX-r.startX,c=n.clientY-r.startY;if(!r.horiz){if(Math.abs(c)>Math.abs(l)&&Math.abs(c)>8||Math.abs(l)<=8)return;r.horiz=!0}const d=Date.now(),u=Math.max(1,d-r.lastTime);r.velocity=(n.clientX-r.lastX)/u,r.lastX=n.clientX,r.lastTime=d,this.dragOffset.set(l)});const s=n=>{const r=this.ptr;if(!r||r.id!==n.pointerId)return;const l=n.clientX-r.startX,c=r.horiz;if(this.ptr=null,e.releasePointerCapture?.(n.pointerId),!c){this.dragOffset.set(0);return}this.snap(Ks({dragDistance:l,velocity:r.velocity,itemWidth:Y}))};e.addEventListener("pointerup",s),e.addEventListener("pointercancel",n=>{!this.ptr||this.ptr.id!==n.pointerId||(this.ptr=null,e.releasePointerCapture?.(n.pointerId),this.snap(0))})}}const yt=()=>null;function y(i){return String(i).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function an(i,e){const t=[...i].sort((r,l)=>r.canonicalOrdinal-l.canonicalOrdinal);if(e.length===0)return[{label:"TOT",holes:t,playHoleIds:new Set(t.map(r=>r.playHoleId))}];const s=[...e].sort((r,l)=>r.fromCanonicalOrdinal-l.fromCanonicalOrdinal),n=[];for(const r of s){const l=t.filter(c=>c.canonicalOrdinal>=r.fromCanonicalOrdinal&&c.canonicalOrdinal<=r.toCanonicalOrdinal);l.length!==0&&n.push({label:r.label,holes:l,playHoleIds:new Set(l.map(c=>c.playHoleId))})}return n}function ln(i){return i.kind==="si"?"lb-c-si":i.kind==="given"?"lb-c-given":i.kind==="status"?"lb-c-status":i.kind==="category"?"lb-c-cat":""}function dn(i){const e=[i.kind==="category"?"lb-r-cat":`lb-r-${i.kind}`];return(i.kind==="si"||i.kind==="given")&&e.push("lb-r-dim"),i.team&&e.push(`lb-team-${i.team}`),e.join(" ")}function cn(i){return i&&i.marker?i.marker.template:null}function un(i){const e=i?.marker?.tone;return e==="success"||e==="warning"||e==="danger"?` lb-mark-tone--${e}`:""}function hn(i,e){const t=i.cells.filter(s=>e.has(s.playHoleId));if(i.aggregate==="sum"){const s=t.map(n=>n.value).filter(n=>n!==null);return s.length===0?"—":String(s.reduce((n,r)=>n+r,0))}if(i.aggregate==="last"){for(let s=t.length-1;s>=0;s--){const n=t[s].value;if(n!==null)return Number.isInteger(n)?String(n):n.toFixed(1)}return"—"}return"—"}function pn(i){return i.filter(e=>!(e.startsWith("slot #")||/^CH -?\d/.test(e)||/^PH -?\d/.test(e)))}function Ce(i,e,t,s){const n=an(i.holes,e),r=$=>{const D=`<tr><th class="lb-rowlabel">Hole</th>${$.holes.map(C=>`<th>${y(C.occurrenceLabel)}</th>`).join("")}<th class="lb-sum">${y($.label)}</th></tr>`,F=i.rows.map(C=>{const O=new Map(C.cells.map(W=>[W.playHoleId,W])),G=W=>C.emphasis?`<strong>${W}</strong>`:W,Nt=$.holes.map(W=>{const q=O.get(W.playHoleId),Rt=q?.title?` title="${y(q.title)}"`:"",ge=G(y(q?.display??"")),Oe=cn(q),jt=un(q),be=q?.marker?.label,Ot=be?` title="${y(be)}" aria-label="${y(be)}"`:"";let Le=Oe?`<span class="lb-mark lb-mark--${Oe}${jt}"${Ot}>${ge}</span>`:ge;return q?.team&&(Le=`<span class="lb-pill lb-pill--${q.team}">${ge}</span>`),`<td class="${ln(C)}"${Rt}>${Le}</td>`}).join(""),Pt=`<td class="lb-sum">${G(hn(C,$.playHoleIds))}</td>`,zt=C.subjectBallId?y(t(C.subjectBallId))+(C.label?" "+y(C.label):""):y(C.label);return`<tr class="${dn(C)}"><th class="lb-rowlabel">${zt}</th>${Nt}${Pt}</tr>`}).join("");return`<div class="lb-card__scroll"><table class="lb-grid"><thead>${D}</thead><tbody>${F}</tbody></table></div>`},l=n.map($=>r($)).join(""),c=i.title.groups.map($=>$.map(D=>y(t(D))).join(" & ")).filter(Boolean).join(i.title.joiner),d=s.mode==="verification"?i.subtitleFacts:pn(i.subtitleFacts),u=d.length?`<div class="lb-card__sub">${d.map(y).join(" · ")}</div>`:"",p=s.mode==="verification"&&i.footnotes.length?`<div class="lb-card__notes"><span class="lb-card__notes-label">Points breakdown</span>${i.footnotes.map($=>`<span class="lb-card__note">${y($)}</span>`).join("")}</div>`:"",m=s.mode==="verification"&&i.caption?`<p class="lb-card__caption">${y(i.caption)}</p>`:"",g=i.totals.length?`<ul class="lb-card__totals">${i.totals.map($=>`<li>${y($.label)} = <strong>${$.value??"—"}</strong></li>`).join("")}</ul>`:"",S=c?`<header class="lb-card__head"><h4>${c}</h4>${u}</header>`:u;return`<article class="${s.cardModifier?`lb-card ${s.cardModifier}`:"lb-card"}">
  ${S}
  ${l}
  ${p}${m}${g}
</article>`}function mn(i,e,t,s){return Ce(i,e,t,s)}function fn(i,e,t,s){return Ce(i,e,t,{...s,cardModifier:"lb-card--compact-match"})}function gn(i,e,t,s){return Ce(i,e,t,{...s,cardModifier:"lb-card--category-matrix"})}function bn(i,e){const t=new Set(i.map(e));return t.size!==1?null:[...t][0]??null}function _n(i){if(i===void 0)return"";const e=i===0?"E":i>0?`+${i}`:`−${Math.abs(i)}`;return` <span class="lb-rank__pace lb-rank__pace--${i===0?"even":i>0?"over":"under"}">${y(e)}</span>`}function yn(i,e,t=yt){const s=i.entries.map(n=>{const r=bn(n.ballIds,t),l=r?` <span class="lb-rank__group">${y(r)}</span>`:"";return`<tr class="${n.position===1?"lb-rank__lead":""}">
  <td class="lb-rank__pos">${n.position}</td>
  <td class="lb-rank__who"><span class="lb-rank__name">${y(n.ballIds.map(e).join(" & "))}</span>${l}</td>
  <td class="lb-rank__total">${n.total??"—"}${_n(n.paceDelta)}</td>
...
</div>`}function vn(i,e){const t=i.matches.map(s=>{const n=y(s.sideA.ballIds.map(e).join(" & ")),r=y(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,c=s.finished?"Final":`thru ${s.thru}`,d=s.leader==="a"?" lb-mp__team--lead":"",u=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
    <div class="lb-mp__team lb-mp__team--a${d}">${n}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${y(l)}</span><span class="lb-mp__status">${y(c)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${u}">${r}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${y(i.title)}</h4>${t}
</div>`}const wn={ranked:yn,match_summary:(i,e)=>vn(i,e)},$n={"default-score-grid":mn,"compact-match-grid":fn,"category-matrix-grid":gn};function xn(i){return i.componentId??"default-score-grid"}function kn(i){return`<div class="lb-diag">Unrenderable result section <code>${y(i)}</code> — no generic view yet. Results are not hidden.</div>`}function Sn(i){return`<div class="lb-diag">Unsupported score-grid component <code>${y(i)}</code> — no generic view yet. Results are not hidden.</div>`}function In(i,e,t){const s=wn[i.kind];return s?s(i,e,t):kn(i.kind)}function En(i,e,t,s){const n=xn(i),r=$n[n];return r?r(i,e,t,s):Sn(n)}function Tn(i,e,t=yt){return i.leaderboard.length===0&&i.cards.length===0?`<div class="lb-empty">No scores entered yet for ${y(i.formatLabel)}.</div>`:i.leaderboard.map(n=>In(n,e,t)).join("")||`<div class="lb-empty">No leaderboard metric for ${y(i.formatLabel)}.</div>`}function Cn(i,e,t,s={}){if(i.cards.length===0)return"";const n=s.mode??"product";return i.cards.map(r=>En(r,e,t,{mode:n})).join(`
`)}const Nn=_(`
...
`);class Pn extends T{static styles=`
    ...
    `;svc=this.inject(Z);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(Nn,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=d=>this.svc.nameOf(d),r=Tn(t,s,d=>this.svc.groupLabelOf(d)),l=Cn(t,e.routeSections,s),c=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return r+c}}function zn(i,e){if(!e)return[];const t=[],s=new Set;for(const n of i)for(const r of n.players){if(r.playerId===e)return[];r.guestPlayerId===null||s.has(r.guestPlayerId)||(s.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const Rn=_(`
...
`);class On extends T{static styles=`
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");claiming=new h(!1);error=new h("");claimable(){return zn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async claim(e){const t=this.tokenQ.get();if(!(!t||this.claiming.get())){this.error.set(""),this.claiming.set(!0);try{await b.friendlyRounds.claimGuest({token:t,guestPlayerId:e}),await this.svc.loadByToken(t)}catch(s){this.error.set(s instanceof M&&s.status===409?"Already claimed — or you already play in this round under your account.":s instanceof M&&s.status===404?"That guest is no longer claimable on this round.":"Could not claim right now. Try again.")}finally{this.claiming.set(!1)}}}render(){const e=this.wire(Rn,{root:{className:()=>this.claimable().length>0?"claim-card":"claim-card hidden"},err:{textContent:()=>this.error.get()}});return this.$each(this.ref(e,"rows"),()=>this.claimable(),(t,s,n)=>this.wireEl(jn,{name:()=>t.displayName,claim:{disabled:()=>this.claiming.get(),onclick:()=>{this.claim(t.guestPlayerId)}}},n),t=>t.guestPlayerId),e}}function Q(i){return typeof i=="object"&&i!==null&&typeof i.get=="function"}const P=i=>`var(--${i})`,je=class je extends T{constructor(){super(...arguments),this.open=new h(!1),this.highlightIndex=new h(-1),this.optionEls=[],this.onOutsidePointer=e=>{this.wrapperEl.contains(e.target)||this.open.set(!1)}}render(){const e=document.createElement("div");e.className="ui-select",this.wrapperEl=e;const t=this.props.zIndex??50;this.triggerEl=document.createElement("button"),this.triggerEl.className="ui-select__trigger",this.triggerEl.setAttribute("type","button"),this.triggerEl.setAttribute("role","combobox"),this.triggerEl.setAttribute("aria-haspopup","listbox");const s=document.createElement("span");s.className="ui-select__trigger-label",this.triggerEl.appendChild(s);const n=document.createElement("span");n.className="ui-select__chevron",n.textContent="▾",n.setAttribute("aria-hidden","true"),this.triggerEl.appendChild(n),this.triggerEl.addEventListener("click",l=>{l.stopPropagation(),this.toggle()}),this.triggerEl.addEventListener("keydown",l=>{this.handleTriggerKeydown(l)}),e.appendChild(this.triggerEl),this.dropdownEl=document.createElement("div"),this.dropdownEl.className="ui-select__dropdown",this.dropdownEl.setAttribute("role","listbox"),this.dropdownEl.style.zIndex=String(t),this.dropdownEl.addEventListener("keydown",l=>{this.handleDropdownKeydown(l)}),e.appendChild(this.dropdownEl);const r=l=>{this.optionEls=[],this.dropdownEl.textContent="";for(let c=0;c<l.length;c++){const d=l[c],u=document.createElement("button");if(u.className="ui-select__option",u.setAttribute("type","button"),u.id=`ui-select-opt-${c}`,d.disabled){u.classList.add("ui-select__option--header"),u.disabled=!0,u.setAttribute("role","presentation"),u.setAttribute("aria-disabled","true");const g=document.createElement("span");g.className="ui-select__option-label",g.textContent=d.label,u.appendChild(g),this.dropdownEl.appendChild(u),this.optionEls.push(u);continue}if(u.setAttribute("role","option"),d.icon){const g=document.createElement("span");g.className="ui-select__option-icon",g.textContent=d.icon,u.appendChild(g)}const p=document.createElement("span");p.className="ui-select__option-label",p.textContent=d.label,u.appendChild(p);const m=document.createElement("span");m.className="ui-select__check",m.setAttribute("aria-hidden","true"),u.appendChild(m),u.addEventListener("click",g=>{g.stopPropagation(),this.selectOption(d.value)}),u.addEventListener("mouseenter",()=>{this.highlightIndex.set(c)}),this.dropdownEl.appendChild(u),this.optionEls.push(u)}};return Q(this.props.options)?this.track(k(()=>{const l=Q(this.props.options)?this.props.options.get():this.props.options;r(l)})):r(this.props.options),this.track(k(()=>{const l=this.props.value.get(),c=Q(this.props.options)?this.props.options.get():this.props.options,d=c.find(u=>u.value===l);d?(s.textContent=d.icon?`${d.icon} ${d.label}`:d.label,this.triggerEl.classList.remove("ui-select__trigger--placeholder")):(s.textContent=this.props.placeholder??"",this.triggerEl.classList.toggle("ui-select__trigger--placeholder",!!this.props.placeholder));for(let u=0;u<c.length;u++){const p=this.optionEls[u];if(!p)continue;const m=c[u].value===l;p.setAttribute("aria-selected",String(m)),p.classList.toggle("ui-select__option--selected",m);const g=p.querySelector(".ui-select__check");g&&(g.textContent=m?"✓":"")}})),this.track(k(()=>{const l=this.open.get();if(this.dropdownEl.classList.toggle("open",l),n.classList.toggle("ui-select__chevron--open",l),this.triggerEl.setAttribute("aria-expanded",String(l)),l?document.addEventListener("pointerdown",this.onOutsidePointer,!0):document.removeEventListener("pointerdown",this.onOutsidePointer,!0),l){const c=Q(this.props.options)?this.props.options.get():this.props.options,d=this.props.value.get(),u=c.findIndex(m=>m.value===d),p=c.findIndex(m=>!m.disabled);this.highlightIndex.set(u>=0?u:p)}})),this.track(k(()=>{const l=this.highlightIndex.get();for(let c=0;c<this.optionEls.length;c++)this.optionEls[c].classList.toggle("ui-select__option--highlighted",c===l);l>=0&&this.optionEls[l]&&(this.triggerEl.setAttribute("aria-activedescendant",`ui-select-opt-${l}`),this.optionEls[l].scrollIntoView({block:"nearest"}))})),this.props.disabled!=null&&(Q(this.props.disabled)?this.track(k(()=>{const l=this.props.disabled.get();this.triggerEl.classList.toggle("ui-select__trigger--disabled",l),this.triggerEl.disabled=l})):this.props.disabled&&(this.triggerEl.classList.add("ui-select__trigger--disabled"),this.triggerEl.disabled=!0)),e}toggle(){this.open.update(e=>!e)}selectOption(e){re(()=>{this.props.value.set(e),this.open.set(!1)}),this.triggerEl.focus()}handleTriggerKeydown(e){switch(e.key){case"Enter":case" ":e.preventDefault(),this.toggle();break;case"ArrowDown":e.preventDefault(),this.open.get()?this.moveHighlight(1):this.open.set(!0);break;case"ArrowUp":e.preventDefault(),this.open.get()?this.moveHighlight(-1):this.open.set(!0);break;case"Escape":this.open.get()&&(e.preventDefault(),this.open.set(!1));break}}handleDropdownKeydown(e){switch(e.key){case"ArrowDown":e.preventDefault(),this.moveHighlight(1);break;case"ArrowUp":e.preventDefault(),this.moveHighlight(-1);break;case"Enter":case" ":{e.preventDefault();const t=this.highlightIndex.get(),s=Q(this.props.options)?this.props.options.get():this.props.options;t>=0&&t<s.length&&!s[t].disabled&&this.selectOption(s[t].value);break}case"Escape":e.preventDefault(),this.open.set(!1),this.triggerEl.focus();break;case"Tab":this.open.set(!1);break}}moveHighlight(e){const t=Q(this.props.options)?this.props.options.get():this.props.options;if(t.length===0||!t.some(n=>!n.disabled))return;let s=this.highlightIndex.get();do s+=e,s<0&&(s=t.length-1),s>=t.length&&(s=0);while(t[s].disabled);this.highlightIndex.set(s)}onDestroy(){document.removeEventListener("pointerdown",this.onOutsidePointer,!0)}};je.styles=`
        .ui-select {
            position: relative;
    ...
    `;let ae=je;function Ln(i,e,t){if(!e||t!=="not_started")return!1;for(const s of i)for(const n of s.players)if(n.playerId===e)return!1;return!0}const Je="new";function Dn(i){const e=i.map((s,n)=>{const r=s.ballIds.length,l=[`Group ${n+1}`];return s.startTime.includes(":")&&l.push(s.startTime),{value:s.id,label:`${l.join(" · ")} — ${r} of ${s.capacity}`,disabled:r>=s.capacity}}),t=e.find(s=>!s.disabled);return e.push({value:Je,label:"Start a new group",disabled:!1}),{options:e,defaultValue:t?.value??Je}}const Hn=_(`
...
`);class Mn extends T{static styles=`
        .join-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${E()}
            ...
            & .join-card__diag {
                ...
                color: ${o("text-muted")};
                &:empty { display: none; }
                & a { color: ${o("accent")}; font-weight: 600; }
            }
            & .join-card__err {
                margin: ${a("sm")} 0 0;
                font-size: 0.85rem;
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");joining=new h(!1);error=new h("");diagnostics=new h([]);teeId=new h("");tees=new h([]);loadedForCourseId=null;groupChoice=new h("");eligible(){return Ln(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,b.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}diagnosticMessage(e){return e.code==="missing_gender"||e.code==="missing_handicap_index"?`${e.message} — <a data-nav="/profile">update your profile</a>.`:e.message}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const s=this.groupChoice.get(),n=await b.friendlyRounds.join({token:e,teeId:t,...s?{groupChoice:s}:{}});n.ok?await this.svc.loadByToken(e):this.diagnostics.set(n.diagnostics)}catch(s){this.error.set(s instanceof M&&s.status===409?s.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(k(()=>this.ensureTeesLoaded()));const e=new w(()=>Dn(this.svc.groups()));this.track(k(()=>{const r=e.get(),l=this.groupChoice.get();(!l||!r.options.some(c=>c.value===l&&!c.disabled))&&this.groupChoice.set(r.defaultValue)}));const t=this.wire(Hn,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},groupRow:{className:()=>this.svc.groups().length>0?"join-card__group":"join-card__group hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{innerHTML:()=>this.diagnostics.get().map(r=>this.diagnosticMessage(r)).join(" · "),onclick:r=>{const c=r.target.closest("[data-nav]")?.getAttribute("data-nav");c&&(r.preventDefault(),this.router.navigate(c))}},err:{textContent:()=>this.error.get()}}),s=new ae({value:this.teeId,options:{get:()=>this.tees.get().map(r=>({value:r.id,label:r.name}))},placeholder:"Tee"});s.mount(this.ref(t,"teeHost")),this.track(()=>s.destroy());const n=new ae({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return n.mount(this.ref(t,"groupHost")),this.track(()=>n.destroy()),t}}const Fn=_(`
...
`);class An extends T{static styles=`
        .edit-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${E()}
    ...
    `;router=this.inject(N);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&b.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(Fn,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function Bn(i,e){if(!e)return!1;for(const t of i)for(const s of t.players)if(s.playerId===e)return!0;return!1}const Gn=_(`
...
`);class qn extends T{static styles=`
        .leave-card {
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return Bn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await b.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(Gn,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(B,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function Kn(i){return!(i.tab!=="leaderboard"||!i.pageVisible||i.status==="complete")}const Vn=2e4;function Wn(i){if(!(i===null||i===""))return/^\d+$/.test(i)?Number(i):i}const Un=_(`
...
`),Yn=_('<button bind="pill" class="round-view__fmt" type="button"></button>'),Qn=_('<button bind="pill" class="round-view__grp" type="button"></button>');class Xn extends T{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
    ...
    `;svc=this.inject(Z);router=this.inject(N);tokenQ=this.router.query("token");initPos=this.readUrlPosition();tab=new h(this.initPos.tab);pageVisible=new h(!document.hidden);hasRound=new w(()=>this.svc.round.get()!==null);hasScoring=new w(()=>this.svc.balls.get().length>0);deleteOpen=new h(!1);finishOpen=new h(!1);isComplete=new w(()=>this.svc.round.get()?.status==="complete");shareUrl=new w(()=>{const e=this.tokenQ.get();return e?`${location.origin}/round?token=${e}`:""});render(){this.track(k(()=>{const c=this.tokenQ.get();c&&this.svc.loadByToken(c,this.initPos).then(()=>{this.tab.get()==="leaderboard"&&this.svc.loadResult()})}));const e=()=>{this.svc.flushPending()};window.addEventListener("online",e),this.track(()=>window.removeEventListener("online",e));const t=()=>this.pageVisible.set(!document.hidden);document.addEventListener("visibilitychange",t),this.track(()=>document.removeEventListener("visibilitychange",t));let s=null;this.track(k(()=>{const c=Kn({tab:this.tab.get(),pageVisible:this.pageVisible.get(),status:this.svc.round.get()?.status??null});c&&s===null?s=setInterval(()=>{this.svc.pollResult()},Vn):!c&&s!==null&&(clearInterval(s),s=null)})),this.track(()=>{s!==null&&clearInterval(s)}),this.track(k(()=>{const c=this.tab.get(),d=this.svc.selectedSlotDefId(),u=this.svc.holeIdx.get();if(this.router.route.get()!=="/round"||!this.hasRound.get())return;const p={token:this.tokenQ.get()};c==="leaderboard"&&(p.tab="board");const m=this.svc.round.get()?.formatSlots[0]?.slotDefId??null;d&&d!==m&&(p.slot=d),u>0&&(p.hole=u+1),this.router.navigate(this.router.route.get(),{replace:!0,query:p})}));const n={not_started:"Not started",active:"Live",complete:"Finished"},r=this.wire(Un,{back:{onclick:()=>this.router.navigate("/")},notfound:{className:()=>!this.hasRound.get()&&!this.svc.loading.get()?"round-view__notfound":"round-view__notfound hidden"},body:{className:()=>this.hasRound.get()?"round-view__body":"round-view__body hidden"},course:()=>this.svc.round.get()?.courseNameSnapshot??"Round",status:()=>{const c=this.svc.round.get()?.status??"not_started";return n[c]??c},date:()=>this.svc.round.get()?.date??"",route:()=>{const c=this.svc.round.get();return c?`${c.playHoles.length} holes`:""},scorePanel:{className:()=>this.tab.get()==="score"?"round-view__panel":"round-view__panel hidden"},groupTabs:{className:()=>this.svc.groups().length>1?"round-view__groups":"round-view__groups hidden"},lbPanel:{className:()=>this.tab.get()==="leaderboard"?"round-view__panel":"round-view__panel hidden"},shareUrl:{value:()=>this.shareUrl.get()},copy:{onclick:()=>{navigator.clipboard?.writeText(this.shareUrl.get())}},finishBtn:{textContent:()=>this.isComplete.get()?"Reopen round":"Finish round",onclick:()=>this.finishOpen.set(!0),disabled:()=>this.svc.finishing.get()},deleteBtn:{onclick:()=>this.deleteOpen.set(!0),disabled:()=>this.svc.deleting.get()},dock:{className:()=>this.hasRound.get()?"round-view__dock":"round-view__dock hidden"},holebar:{className:()=>this.tab.get()==="score"&&this.hasScoring.get()?"round-hole":"round-hole hidden"},holePar:()=>String(this.svc.parFor(this.svc.currentPlayedHole()?.playHoleId??null)),holeNum:()=>{const c=this.svc.currentPlayedHole();return c?this.svc.occLabel(c.playHoleId):""},holeSi:()=>{const c=this.svc.currentPlayHole()?.baseStrokeIndex;return c!=null?String(c):"–"},holePrev:{onclick:()=>this.svc.prevHole(),disabled:()=>!this.svc.canPrevHole()},holeNext:{onclick:()=>this.svc.nextHole(),disabled:()=>!this.svc.canNextHole()},tabScore:{className:()=>this.tab.get()==="score"?"round-tabs__tab active":"round-tabs__tab",onclick:()=>this.tab.set("score")},tabBoard:{className:()=>this.tab.get()==="leaderboard"?"round-tabs__tab active":"round-tabs__tab",onclick:()=>{this.tab.set("leaderboard"),this.svc.loadResult()}}});this.$each(this.ref(r,"groupTabs"),new w(()=>this.svc.groups()),(c,d,u)=>this.groupPill(d,u),c=>c.id),this.$each(this.ref(r,"formats"),new w(()=>this.svc.round.get()?.formatSlots??[]),(c,d,u)=>this.slotPill(c,d,u),c=>c.slotDefId),this.spawn(on,this.ref(r,"scoring")),this.spawn(Pn,this.ref(r,"leaderboard")),this.spawn(An,this.ref(r,"edit")),this.spawn(On,this.ref(r,"claim")),this.spawn(Mn,this.ref(r,"join")),this.spawn(qn,this.ref(r,"leave")),this.spawn(B,this.ref(r,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:"This permanently removes the round and all its scores for everyone. This can't be undone.",confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.svc.deleteRound().then(c=>{c&&this.router.navigate("/")})}}),this.spawn(B,this.ref(r,"finishConfirmHost"),{open:this.finishOpen,title:"Finish or reopen round",message:()=>this.isComplete.get()?"Reopen this round? It'll move back to your ongoing rounds.":"Finish this round? It'll move to your finished rounds. You can still edit or reopen it any time.",cancelLabel:"Cancel",onconfirm:()=>{this.isComplete.get()?this.svc.reopenRound():this.svc.finishRound()}});const l=c=>{c.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1),c.key==="Escape"&&this.finishOpen.get()&&this.finishOpen.set(!1)};return window.addEventListener("keydown",l),this.track(()=>window.removeEventListener("keydown",l)),r}readUrlPosition(){const e=new URLSearchParams(location.search),t=e.get("slot"),s=Number(e.get("hole"));return{tab:e.get("tab")==="board"?"leaderboard":"score",selectedSlot:Wn(t),holeIdx:Number.isFinite(s)&&s>0?s-1:0}}groupPill(e,t){return this.wireEl(Qn,{pill:{textContent:()=>{const s=this.svc.groups()[e];if(!s)return`Group ${e+1}`;const n=[`Group ${e+1}`];s.startTime.includes(":")&&n.push(s.startTime);const r=this.svc.playHoleById(s.startPlayHoleId)?.courseHoleNumber;return r!==void 0&&s.startOrdinal!==1&&n.push(`H${r}`),n.join(" · ")},className:()=>this.svc.groupIdx.get()===e?"round-view__grp active":"round-view__grp",onclick:()=>this.svc.groupIdx.set(e)}},t)}slotPill(e,t,s){return this.wireEl(Yn,{pill:{textContent:()=>gt(e),className:()=>this.tab.get()==="leaderboard"&&this.svc.selectedSlotDefId()===e.slotDefId?"round-view__fmt active":"round-view__fmt",onclick:()=>{this.svc.selectSlot(e.slotDefId),this.tab.get()!=="leaderboard"&&(this.tab.set("leaderboard"),this.svc.loadResult())}}},s)}}function vt(i){return i.handicapIndex*(i.slope/113)+(i.courseRating-i.par)}function Jn(i){return Math.round(vt(i))}function Zn(i){if(!i)return null;const e=/^slots\[slot-(\d+)\]/.exec(i);return e?Number(e[1]):null}function ei(i){if(!i)return null;const e=/^formats\[(\d+)\]/.exec(i);return e?Number(e[1]):null}function wt(i){return ei(i.path)??Zn(i.path)}function ti(i,e){return i.filter(t=>wt(t)===e)}function si(i){return i.filter(e=>!e.path?.startsWith("producers")&&!e.path?.startsWith("playingGroups")&&e.path!=="route"&&wt(e)===null)}function te(i){return`${i} ${i===1?"player":"players"}`}function de(i,e){const t=i.formatId?e(i.formatId)??i.formatId:null,s=i.teamLabel;switch(i.code){case"team_size_above_max":if(t&&s&&i.actual!==void 0&&i.allowedMax!==void 0)return`${s} has ${te(i.actual)} — ${t} allows at most ${i.allowedMax} per team.`;break;case"team_size_below_min":if(t&&s&&i.actual!==void 0&&i.allowedMin!==void 0)return`${s} has ${te(i.actual)} — ${t} needs at least ${i.allowedMin} per team.`;break;case"empty_team_grouping":if(t&&s)return`${s} has no players — add at least one, or remove the team.`;break;case"team_count_above_max":if(t&&i.actual!==void 0&&i.allowedMax!==void 0)return`${i.actual} teams — ${t} allows at most ${i.allowedMax}.`;break;case"team_count_below_min":if(t&&i.actual!==void 0&&i.allowedMin!==void 0)return`${i.actual} teams — ${t} needs at least ${i.allowedMin}.`;break;case"slot_ball_count_above_max":if(t&&i.actual!==void 0&&i.allowedMax!==void 0)return`${te(i.actual)} in ${t} — it scores at most ${i.allowedMax}.`;break;case"slot_ball_count_below_min":if(t&&i.actual!==void 0&&i.allowedMin!==void 0)return`${te(i.actual)} in ${t} — it needs at least ${i.allowedMin}.`;break;case"slot_ball_count_not_multiple":if(t&&i.actual!==void 0)return`${t} pairs its balls, so it needs an even number — ${te(i.actual)} won't pair up.`;break;case"missing_team_grouping":if(t)return`${t} needs its players grouped into teams — tick the teams it scores.`;break;case"producer_has_scores":return i.message;case"scored_ball_orphaned":return i.message;case"edit_locked_course_route":return"Scores have already been recorded — the course and route are locked for this round.";case"round_complete":return"This round is complete — its setup can no longer be edited.";case"not_editable":return"This round can no longer be edited."}return i.message}function ni(i){return i?i.type==="flat"?String(i.pct):i.bands.length>0?String(i.bands[0].pct):"100":"100"}function ii(i){const e=i.roundType;if(e==="full_18"||e==="front_9"||e==="back_9")return{preset:e,startHole:ri(i)};const t=(i.route?.playHoles??[]).map(l=>l.courseHoleNumber),s=t[0]??1,n=new Set(t);return{preset:t.length<=9&&[...n].every(l=>l<=9)?"front_9":t.length<=9&&[...n].every(l=>l>=10)?"back_9":"full_18",startHole:s}}function ri(i){return i.roundType==="back_9"?10:1}function oi(i,e=()=>""){let t=1,s=1,n=1,r=1;const l=new Map,c=i.producers.map(v=>{const $=t++;l.set(v.producerDefId,$);const D=v.playerRef.kind==="guest";return{key:$,name:e(v.producerDefId),handicapIndex:String(v.handicapIndex),gender:v.gender??"M",teeId:v.teeId,producerDefId:v.producerDefId,...D?{guestPlayerId:v.playerRef.id}:{playerId:v.playerRef.id,genderKnown:v.gender!=null}}}),d=new Map;(i.teams??[]).forEach(v=>{d.set(v.id,s++)});const u=(i.teams??[]).map(v=>{const $=d.get(v.id),D={},F={};for(const C of v.members)if("producerDefId"in C){const O=l.get(C.producerDefId);O!==void 0&&(D[O]=String(C.allowancePct))}else{const O=d.get(C.teamId);O!==void 0&&(F[O]=!0)}return{key:$,kind:v.kind??"single_ball",formation:v.formation??"scramble",pctByPlayer:D,memberTeams:F}}),p=(i.playingGroups??[]).map(v=>{const $={};for(const D of v.members){const F=l.get(D);F!==void 0&&($[F]=!0)}return{key:n++,startTime:v.startTime??"",startHole:v.startHole??null,members:$}}),m=i.formats.map(v=>{const $={},D={},F=v.subjects;if(F){const C=new Set;for(const O of F)if(O.kind==="player"){const G=l.get(O.producerDefId);G!==void 0&&C.add(G)}else{const G=d.get(O.teamId);G!==void 0&&(D[G]=!0)}for(const O of c)$[O.key]=C.has(O.key)}return{key:r++,formatId:v.formatId,allowancePct:ni(v.allowanceConfig),subjectPlayers:$,subjectTeams:D}}),{preset:g,startHole:S}=ii(i);return{courseId:i.courseId,preset:g,startHole:S,players:c,teams:u,groups:p,formatSlots:m,nextKey:t,nextTeamKey:s,nextGroupKey:n,nextSlotKey:r}}const ai=["scramble","greensomes","foursomes","custom"],ce=2,_e=10,li="ABCDEFGH",di={full_18:"Full 18",front_9:"Front 9",back_9:"Back 9"};class ci{loading=new h(!1);error=new h(null);courses=new h([]);tees=new h([]);courseId=new h("");preset=new h("full_18");startHole=new h(1);players=new h([]);teams=new h([]);groups=new h([]);formatSlots=new h([]);submitting=new h(!1);diagnostics=new h([]);submitError=new h(null);editToken=new h(null);hasScores=new h(!1);editStatus=new h(null);editBlockedReason=new h(null);editPlayedAt=null;catalog=A.get(oe);nextKey=1;nextSlotKey=1;nextTeamKey=1;nextGroupKey=1;reset(){this.courses.set([]),this.tees.set([]),this.courseId.set(""),this.preset.set("full_18"),this.startHole.set(1),this.players.set([]),this.teams.set([]),this.groups.set([]),this.formatSlots.set([]),this.diagnostics.set([]),this.submitError.set(null),this.submitting.set(!1),this.error.set(null),this.editToken.set(null),this.hasScores.set(!1),this.editStatus.set(null),this.editBlockedReason.set(null),this.editPlayedAt=null,this.nextKey=1,this.nextSlotKey=1,this.nextTeamKey=1,this.nextGroupKey=1}async load(){this.catalog.load().then(()=>this.ensureDefaultSlot());const e=await I(this.loading,this.error,()=>b.setup.courses());e&&(this.courses.set(e),!this.courseId.get()&&e.length>0&&await this.selectCourse(e[0].id))}async loadForEdit(e){this.reset(),this.editToken.set(e),await this.catalog.load();const t=await I(this.loading,this.error,()=>b.friendlyRounds.setup({token:e}));if(!t)return;if(this.editStatus.set(t.status),!t.editable){this.editBlockedReason.set(t.reason);return}this.hasScores.set(t.hasScores),this.editPlayedAt=t.draft.playedAt;const s=await I(this.loading,this.error,()=>b.setup.courses());s&&this.courses.set(s);const n=await I(this.loading,this.error,()=>b.setup.teesByCourse({courseId:t.draft.courseId}));this.tees.set(n??[]);const r=await I(this.loading,this.error,()=>b.friendlyRounds.balls({token:e})),l=new Map;for(const d of r??[])for(const u of d.players)l.set(u.producerDefId,u.displayName);const c=oi(t.draft,d=>l.get(d)??"");this.courseId.set(c.courseId),this.preset.set(c.preset),this.startHole.set(c.startHole),this.players.set(c.players),this.teams.set(c.teams),this.groups.set(c.groups),this.formatSlots.set(c.formatSlots),this.nextKey=c.nextKey,this.nextTeamKey=c.nextTeamKey,this.nextGroupKey=c.nextGroupKey,this.nextSlotKey=c.nextSlotKey}async selectCourse(e){this.courseId.set(e),this.preset.set("full_18"),this.startHole.set(1);const s=await I(this.loading,this.error,()=>b.setup.teesByCourse({courseId:e}))??[];this.tees.set(s);const n=new Set(s.map(l=>l.id)),r=s[0]?.id??"";this.players.set(this.players.get().map(l=>({...l,teeId:n.has(l.teeId)?l.teeId:r}))),this.players.get().length===0&&this.addPlayer()}addPlayer(){const e=this.tees.get()[0]?.id??"";this.players.set([...this.players.get(),{key:this.nextKey++,name:"",handicapIndex:"",gender:"M",teeId:e}])}addMe(e){this.addFriend(e)}addFriend(e){if(this.hasPlayer(e.id))return;const t=this.tees.get()[0]?.id??"";this.players.set([...this.players.get(),{key:this.nextKey++,name:e.displayName,handicapIndex:e.handicapIndex===null?"":String(e.handicapIndex),gender:e.gender??"M",genderKnown:e.gender!=null,teeId:t,playerId:e.id}])}hasPlayer(e){return this.players.get().some(t=>t.playerId===e)}removePlayer(e){this.players.set(this.players.get().filter(t=>t.key!==e)),this.groups.set(this.groups.get().map(t=>{if(t.members[e]===void 0)return t;const s={...t.members};return delete s[e],{...t,members:s}}))}patchPlayer(e,t){this.players.set(this.players.get().map(s=>s.key===e?{...s,...t}:s))}ensureDefaultSlot(){if(this.formatSlots.get().length>0)return;const e=this.catalog.byId("stableford_individual")??this.catalog.descriptors.get()[0];e&&this.addFormatSlot(e.id)}addFormatSlot(e){const t=e??this.catalog.byId("stableford_individual")?.id??this.catalog.descriptors.get()[0]?.id??"",s={key:this.nextSlotKey++,formatId:t,allowancePct:"100",subjectPlayers:{},subjectTeams:{}};this.formatSlots.set([...this.formatSlots.get(),s])}setSlotAllowance(e,t){this.patchFormatSlot(e,{allowancePct:t})}removeFormatSlot(e){this.formatSlots.set(this.formatSlots.get().filter(t=>t.key!==e))}patchFormatSlot(e,t){this.formatSlots.set(this.formatSlots.get().map(s=>s.key===e?{...s,...t}:s))}setSlotFormat(e,t){this.patchFormatSlot(e,{formatId:t})}slotByKey(e){return this.formatSlots.get().find(t=>t.key===e)??null}teamLetter(e){return li[e]??`T${e+1}`}formations=ai;addTeam(){this.teams.set([...this.teams.get(),{key:this.nextTeamKey++,kind:"single_ball",formation:"scramble",pctByPlayer:{},memberTeams:{}}])}teamKindOf(e){return this.teamByKey(e)?.kind??"single_ball"}setTeamKind(e,t){this.teams.set(this.teams.get().map(s=>s.key===e?{...s,kind:t,memberTeams:t==="single_ball"?{}:s.memberTeams}:s)),this.pruneStaleTeamSubjects()}eligibleNestedTeams(e){return this.teams.get().filter(t=>t.key!==e&&t.kind==="single_ball")}teamHasTeamMember(e,t){return this.teamByKey(e)?.memberTeams[t]===!0}setTeamMemberTeam(e,t,s){const n=this.teamByKey(e);if(!n||n.kind!=="multi_ball"||t===e)return;const r={...n.memberTeams};if(s){if(this.teamMemberCount(e)>=_e)return;r[t]=!0}else delete r[t];this.teams.set(this.teams.get().map(l=>l.key===e?{...l,memberTeams:r}:l))}teamMemberCount(e){const t=this.teamByKey(e);return t?Object.keys(t.pctByPlayer).length+Object.keys(t.memberTeams).filter(s=>t.memberTeams[Number(s)]).length:0}pruneStaleTeamSubjects(){this.formatSlots.set(this.formatSlots.get().map(e=>{let t=!1;const s={...e.subjectTeams};for(const n of this.teams.get())s[n.key]===!0&&!this.teamKindFitsFormat(e.formatId,n.kind)&&(delete s[n.key],t=!0);return t?{...e,subjectTeams:s}:e}))}isSideFormat(e){return this.catalog.isSideFormat(e)}teamKindFitsFormat(e,t){return this.isSideFormat(e)?t==="multi_ball":t==="single_ball"||this.catalog.acceptsSideSubjects(e)}removeTeam(e){this.teams.set(this.teams.get().filter(t=>t.key!==e).map(t=>{if(t.memberTeams[e]===void 0)return t;const s={...t.memberTeams};return delete s[e],{...t,memberTeams:s}})),this.formatSlots.set(this.formatSlots.get().map(t=>{if(t.subjectTeams[e]===void 0)return t;const s={...t.subjectTeams};return delete s[e],{...t,subjectTeams:s}}))}teamByKey(e){return this.teams.get().find(t=>t.key===e)??null}teamLabel(e){const t=this.teams.get().findIndex(s=>s.key===e.key);return`Team ${this.teamLetter(Math.max(0,t))}`}setTeamFormation(e,t){this.teams.set(this.teams.get().map(s=>s.key===e?{...s,formation:t}:s))}teamMemberIn(e,t){return this.teamByKey(e)?.pctByPlayer[t]!==void 0}setTeamMember(e,t,s){const n=this.teamByKey(e);if(!n)return;const r={...n.pctByPlayer};if(s){if(r[t]!==void 0||this.teamMemberCount(e)>=_e)return;r[t]=r[t]??"100"}else delete r[t];this.teams.set(this.teams.get().map(l=>l.key===e?{...l,pctByPlayer:r}:l))}teamSize(e){return this.teamMemberCount(e)}teamAtMaxSize(e){return this.teamSize(e)>=_e}teamBallCh(e){const t=this.teamByKey(e);if(!t)return null;let s=0;for(const n of this.players.get()){const r=t.pctByPlayer[n.key];if(r===void 0)continue;const l=this.derivedCH(n);if(!l)return null;s+=this.parsePct(r)*l.ch/100}return Math.round(s)}teamsBelowMin(){return this.teams.get().filter(e=>this.teamMemberCount(e.key)>0&&this.teamMemberCount(e.key)<ce)}isTeamLive(e){const t=Object.keys(e.pctByPlayer).length;if(e.kind==="single_ball")return t>=ce;let s=t;for(const n of this.teams.get())e.memberTeams[n.key]===!0&&n.kind==="single_ball"&&Object.keys(n.pctByPlayer).length>=ce&&s++;return s>=ce}liveTeamKeySet(){return new Set(this.teams.get().filter(e=>this.isTeamLive(e)).map(e=>e.key))}setTeamPct(e,t,s){const n=this.teamByKey(e);!n||n.pctByPlayer[t]===void 0||this.teams.set(this.teams.get().map(r=>r.key===e?{...r,pctByPlayer:{...r.pctByPlayer,[t]:s}}:r))}groupsEnabled(){return this.groups.get().length>0}splitIntoGroups(){if(this.groupsEnabled())return;const e={};for(const t of this.players.get())e[t.key]=!0;this.groups.set([{key:this.nextGroupKey++,startTime:"",startHole:null,members:e},{key:this.nextGroupKey++,startTime:"",startHole:null,members:{}}])}clearGroups(){this.groups.set([])}addGroup(){this.groupsEnabled()&&this.groups.set([...this.groups.get(),{key:this.nextGroupKey++,startTime:"",startHole:null,members:{}}])}removeGroup(e){const t=this.groups.get().filter(s=>s.key!==e);this.groups.set(t.length>1?t:[])}groupByKey(e){return this.groups.get().find(t=>t.key===e)??null}groupLabel(e){const t=this.groups.get().findIndex(s=>s.key===e.key);return`Group ${Math.max(0,t)+1}`}groupMemberIn(e,t){return this.groupByKey(e)?.members[t]===!0}setGroupMember(e,t,s){this.groups.set(this.groups.get().map(n=>{const r=n.key===e,l=n.members[t]===!0;if(r&&s&&!l)return{...n,members:{...n.members,[t]:!0}};if(l&&(!r||!s)){const c={...n.members};return delete c[t],{...n,members:c}}return n}))}setGroupStartTime(e,t){this.groups.set(this.groups.get().map(s=>s.key===e?{...s,startTime:t}:s))}setGroupStartHole(e,t){this.groups.set(this.groups.get().map(s=>s.key===e?{...s,startHole:t}:s))}groupSize(e){const t=this.groupByKey(e);return t?this.players.get().filter(s=>t.members[s.key]===!0).length:0}ungroupedPlayers(){if(!this.groupsEnabled())return[];const e=new Set;for(const t of this.groups.get())for(const s of Object.keys(t.members))t.members[Number(s)]&&e.add(Number(s));return this.players.get().filter(t=>!e.has(t.key))}crossGroupTeamWarnings(){if(!this.groupsEnabled())return[];const e=new Map;this.groups.get().forEach((s,n)=>{for(const r of Object.keys(s.members))s.members[Number(r)]&&e.set(Number(r),n)});const t=[];for(const s of this.teams.get()){if(s.kind!=="single_ball"||!this.isTeamLive(s))continue;const n=new Set;for(const r of Object.keys(s.pctByPlayer)){const l=e.get(Number(r));l!==void 0&&n.add(l)}n.size>1&&t.push(`${this.teamLabel(s)} plays one combined ball, but its players are in different groups — keep them in the same group.`)}return t}buildGroups(e,t){return this.groups.get().map(s=>({members:e.filter(n=>s.members[n.key]===!0).map(n=>t.get(n.key)),...s.startTime.trim()!==""?{startTime:s.startTime.trim()}:{},...s.startHole!==null?{startHole:s.startHole}:{}})).filter(s=>s.members.length>0)}diagnosticsForGroups(){return this.diagnostics.get().filter(e=>e.path?.startsWith("playingGroups"))}subjectPlayerIn(e,t){return this.slotByKey(e)?.subjectPlayers[t]!==!1}setSubjectPlayer(e,t,s){const n=this.slotByKey(e);n&&this.patchFormatSlot(e,{subjectPlayers:{...n.subjectPlayers,[t]:s}})}subjectTeamIn(e,t){return this.slotByKey(e)?.subjectTeams[t]===!0}setSubjectTeam(e,t,s){const n=this.slotByKey(e);n&&this.patchFormatSlot(e,{subjectTeams:{...n.subjectTeams,[t]:s}})}selectedCourse(){return this.courses.get().find(e=>e.id===this.courseId.get())??null}teeById(e){return this.tees.get().find(t=>t.id===e)??null}presetLabel(e){return di[e]}presetHoles(){const e=(this.selectedCourse()?.holes??[]).map(t=>t.holeNumber).sort((t,s)=>t-s);switch(this.preset.get()){case"front_9":return e.filter(t=>t<=9);case"back_9":return e.filter(t=>t>=10);default:return e}}startHoleOptions(){return this.presetHoles()}setPreset(e){this.preset.set(e);const t=this.presetHoles();t.includes(this.startHole.get())||this.startHole.set(t[0]??1),this.groups.set(this.groups.get().map(s=>s.startHole!==null&&!t.includes(s.startHole)?{...s,startHole:null}:s))}derivedCH(e){const t=Number.parseFloat(e.handicapIndex);if(!Number.isFinite(t))return null;const s=this.teeById(e.teeId);if(!s)return null;const n=s.ratings.find(l=>l.gender===e.gender);if(!n)return null;const r={handicapIndex:t,slope:n.slope,courseRating:n.courseRating,par:n.par};return{ch:Jn(r),raw:vt(r),rating:n,teeName:s.name}}diagnosticsForPlayer(e){return this.diagnostics.get().filter(t=>t.path?.startsWith(`producers[${e}]`))}humanizedRoster(){return this.diagnostics.get().filter(e=>e.path==="producers").map(e=>de(e,t=>this.catalog.labelOf(t)))}humanizedRoute(){return this.diagnostics.get().filter(e=>e.path==="route").map(e=>de(e,t=>this.catalog.labelOf(t)))}playersInNoFormat(){const e=this.players.get(),t=new Set;for(const s of this.formatSlots.get()){for(const n of e)s.subjectPlayers[n.key]!==!1&&t.add(n.key);for(const n of this.teams.get())if(s.subjectTeams[n.key]===!0)for(const r of e)n.pctByPlayer[r.key]!==void 0&&t.add(r.key)}return e.filter(s=>!t.has(s.key))}diagnosticsForFormat(e){return ti(this.diagnostics.get(),e)}humanizedForFormat(e){return this.diagnosticsForFormat(e).map(t=>de(t,s=>this.catalog.labelOf(s)))}generalDiagnostics(){return si(this.diagnostics.get())}humanizedGeneral(){return this.generalDiagnostics().map(e=>de(e,t=>this.catalog.labelOf(t)))}parsePct(e){const t=Number.parseInt(e,10);return Number.isFinite(t)?t:100}buildTeams(e,t){const s=this.liveTeamKeySet(),n=[];for(const r of this.teams.get()){if(!s.has(r.key))continue;const l=e.filter(c=>r.pctByPlayer[c.key]!==void 0).map(c=>({producerDefId:t.get(c.key),allowancePct:this.parsePct(r.pctByPlayer[c.key])}));if(r.kind==="multi_ball")for(const c of this.teams.get())r.memberTeams[c.key]===!0&&c.key!==r.key&&c.kind==="single_ball"&&s.has(c.key)&&l.push({teamId:String(c.key)});n.push({id:String(r.key),label:this.teamLabel(r),formation:r.formation,kind:r.kind,members:l})}return n}buildFormats(e,t){const s=this.liveTeamKeySet();return this.formatSlots.get().map(n=>{const r=this.isSideFormat(n.formatId),l=[];if(!r)for(const c of e)n.subjectPlayers[c.key]!==!1&&l.push({kind:"player",producerDefId:t.get(c.key)});for(const c of this.teams.get())n.subjectTeams[c.key]===!0&&s.has(c.key)&&this.teamKindFitsFormat(n.formatId,c.kind)&&l.push({kind:"team",teamId:String(c.key)});return{formatId:n.formatId,allowanceConfig:{type:"flat",pct:this.parsePct(n.allowancePct)},subjects:l}})}buildRoute(){const e=this.presetHoles(),t=this.startHole.get(),s=e.indexOf(t);return s<=0?{roundType:this.preset.get()}:{roundType:"custom_holes",route:{playHoles:[...e.slice(s),...e.slice(0,s)].map(r=>({courseHoleNumber:r})),routeHandicapPolicy:{type:"explicit",postingEligible:!1}}}}async submit(){this.diagnostics.set([]),this.submitError.set(null);const e=this.players.get();if(!this.courseId.get())return this.submitError.set("Pick a course first."),{ok:!1};if(e.length===0)return this.submitError.set("Add at least one player."),{ok:!1};if(this.formatSlots.get().length===0)return this.submitError.set("Add at least one format."),{ok:!1};const t=[];if(e.forEach((n,r)=>{n.name.trim()||t.push({code:"missing_name",message:"Name required",path:`producers[${r}].name`}),Number.isFinite(Number.parseFloat(n.handicapIndex))||t.push({code:"missing_index",message:"Handicap index required",path:`producers[${r}].handicapIndex`}),n.teeId||t.push({code:"missing_tee",message:"Pick a tee",path:`producers[${r}].teeId`})}),t.length>0)return this.diagnostics.set(t),{ok:!1};const s=this.editToken.get();this.submitting.set(!0);try{const n=new Map;e.forEach((g,S)=>{n.set(g.key,g.producerDefId??(s?`p-${g.key}`:`p${S+1}`))});const r=[];for(const g of e){const S=Number.parseFloat(g.handicapIndex),v=g.playerId?{kind:"player",id:g.playerId}:g.guestPlayerId?{kind:"guest",id:g.guestPlayerId}:{kind:"guest",id:(await b.guestPlayers.create({displayName:g.name.trim(),gender:g.gender,handicapIndex:S})).id};r.push({producerDefId:n.get(g.key),playerRef:v,handicapIndex:S,gender:g.gender,teeId:g.teeId})}const{roundType:l,route:c}=this.buildRoute(),d=this.buildTeams(e,n),u=this.buildGroups(e,n),p={courseId:this.courseId.get(),playedAt:this.editPlayedAt??new Date().toISOString().slice(0,10),roundType:l,...c?{route:c}:{},producers:r,...d.length>0?{teams:d}:{},formats:this.buildFormats(e,n),...u.length>0?{playingGroups:u}:{}};if(s){const g=await b.friendlyRounds.editSetup({token:s,draft:p});return g.ok?{ok:!0,token:s}:(this.diagnostics.set(g.diagnostics),{ok:!1})}const m=await b.friendlyRounds.create({draft:p});return m.ok?(pe({token:m.friendlyRound.shareToken,courseName:m.round.courseNameSnapshot??"",status:m.round.status,completedAt:m.round.completedAt,lastSeenAt:new Date().toISOString()}),{ok:!0,token:m.friendlyRound.shareToken}):(this.diagnostics.set(m.diagnostics),{ok:!1})}catch(n){return this.submitError.set(n instanceof M?n.message:s?"Could not save the round. Try again.":"Could not create the round. Try again."),{ok:!1}}finally{this.submitting.set(!1)}}}class Ne{loading=new h(!1);error=new h(null);player=new h(null);history=new h([]);saving=new h(!1);saveError=new h(null);async load(e=!1){if(!e&&(this.player.get()!==null||this.loading.get()))return;const t=await I(this.loading,this.error,()=>Promise.all([b.players.me(),b.players.myHandicapHistory()]));if(!t)return;const[s,n]=t;this.player.set(s),this.history.set(n)}clear(){this.player.set(null),this.history.set([]),this.error.set(null),this.saveError.set(null)}async saveIndex(e){return await I(this.saving,this.saveError,()=>b.players.updateHandicap({handicapIndex:e}))?(await this.load(!0),!0):!1}async saveGender(e){const t=await I(this.saving,this.saveError,()=>b.players.updateProfile({gender:e}));return t?(this.player.set(t),!0):!1}}function ye(i,e){return i.displayName.localeCompare(e.displayName,"sv",{sensitivity:"base"})}function Pe(i,e="frecency"){return e==="alpha"?[...i].sort(ye):[...i].sort((t,s)=>{const n=t.frecency,r=s.frecency,l=n>0,c=r>0;if(l!==c)return l?-1:1;if(!l)return ye(t,s);if(r!==n)return r-n;const d=t.lastPlayedAt?Date.parse(t.lastPlayedAt):NaN,u=s.lastPlayedAt?Date.parse(s.lastPlayedAt):NaN,p=Number.isNaN(d)?Number.NEGATIVE_INFINITY:d,m=Number.isNaN(u)?Number.NEGATIVE_INFINITY:u;return m!==p?m-p:ye(t,s)})}const ui=1440*60*1e3;function hi(i,e){if(!i)return null;const t=Date.parse(i),s=Date.parse(e);if(Number.isNaN(t)||Number.isNaN(s))return null;const n=Math.floor((s-t)/ui);if(n<=0)return"today";if(n===1)return"yesterday";if(n<7)return`${n} days ago`;if(n<14)return"last week";if(n<30)return`${Math.floor(n/7)} weeks ago`;if(n<60)return"last month";if(n<365)return`${Math.floor(n/30)} months ago`;const r=Math.floor(n/365);return r===1?"last year":`${r} years ago`}function pi(i,e){if(i.sharedRoundCount<=0)return"never played";const t=hi(i.lastPlayedAt,e),s=`played ${i.sharedRoundCount}×`;return t?`${s}, ${t}`:s}const $t=2;function Ze(i){return i.trim().length>=$t}function xt(i){return Pe(i,"frecency")}function mi(i,e){return xt([...i.filter(t=>t.id!==e.id),e])}function fi(i,e){return i.filter(t=>t.id!==e)}function et(i,e,t){return i.map(s=>s.id===e?{...s,isFriend:t}:s)}function gi(i,e,t=()=>{},s=300){let n=0,r;return l=>{const c=l.trim(),d=++n;if(r!==void 0&&clearTimeout(r),r=void 0,c.length<$t){e(c,[]);return}r=setTimeout(()=>{i(c).then(u=>{d===n&&e(c,u)},u=>{d===n&&t(c,u)})},s)}}const kt="tapscore.friends.sort.v1";function St(){try{return typeof localStorage<"u"?localStorage:null}catch{return null}}function bi(i=St()){if(!i)return"frecency";let e;try{e=i.getItem(kt)}catch{return"frecency"}return e==="alpha"?"alpha":"frecency"}function _i(i,e=St()){if(e)try{e.setItem(kt,i)}catch{}}class fe{loading=new h(!1);error=new h(null);friends=new h([]);loaded=new h(!1);sortMode=new h(bi());query=new h("");searching=new h(!1);searchError=new h(null);results=new h([]);resultsFor=new h("");mutating=new h(!1);mutateError=new h(null);runSearch=gi(e=>b.players.search({q:e}),(e,t)=>{this.searching.set(!1),this.results.set(t),this.resultsFor.set(e)},(e,t)=>{this.searching.set(!1),this.results.set([]),this.resultsFor.set(e),this.searchError.set({code:"network",message:t instanceof Error?t.message:"Search failed. Try again."})});async load(e=!1){if(!e&&(this.loaded.get()||this.loading.get()))return;const t=await I(this.loading,this.error,()=>b.friends.list());t&&(this.friends.set(xt(t)),this.loaded.set(!0))}setQuery(e){this.query.set(e),this.searchError.set(null),this.searching.set(e.trim().length>=2),this.runSearch(e)}async add(e){await I(this.mutating,this.mutateError,()=>b.friends.add({friendId:e.id}))&&(this.friends.set(mi(this.friends.get(),{id:e.id,username:e.username,displayName:e.displayName,gender:e.gender,handicapIndex:e.handicapIndex,sharedRoundCount:0,lastPlayedAt:null,frecency:0})),this.results.set(et(this.results.get(),e.id,!0)))}setSortMode(e){this.sortMode.set(e),_i(e)}async remove(e){await I(this.mutating,this.mutateError,()=>b.friends.remove({friendId:e}))&&(this.friends.set(fi(this.friends.get(),e)),this.results.set(et(this.results.get(),e,!1)))}clear(){this.friends.set([]),this.loaded.set(!1),this.query.set(""),this.results.set([]),this.resultsFor.set(""),this.error.set(null),this.searchError.set(null),this.mutateError.set(null),this.searching.set(!1)}}const yi=["full_18","front_9","back_9"],vi=_(`
...
`);class Ii extends T{static styles=`
        .setup {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
            ...
            & .setup__add {
                width: 100%; margin-top: ${a("md")}; padding: ${a("md")}; ${x()}
            ...
            & .fslot {
                ...
                & .brow {
                    ...
                    & .brow__del { margin-left: auto; width: 30px; height: 30px; ${x()} font-size: 0.8rem; color: ${o("text-muted")}; }
                }
                & .fslot__addband {
                    align-self: flex-start; padding: ${a("xs")} ${a("sm")}; ${x()}
...
`);class Ti extends T{static styles=`
        .login {
            max-width: 340px;
            margin: 0 auto;
            padding: 14vh ${a("xl")} 0;
...
`);function nt(i){return i.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class zi extends T{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(fe);auth=this.inject(L);router=this.inject(N);render(){const e=()=>this.auth.currentUser.get()!==null;e()&&this.svc.load();const t=this.wire(Ci,{anon:{className:()=>e()?"friends__anon hidden":"friends__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/friends"}})},body:{className:()=>e()?"friends__body":"friends__body hidden"},search:{value:()=>this.svc.query.get(),oninput:n=>this.svc.setQuery(n.target.value)},searchHint:{textContent:()=>{const n=this.svc.query.get().trim();return n.length>0&&!Ze(n)?"Type at least 2 characters.":this.svc.searching.get()?"Searching…":""}},searchErr:{textContent:()=>this.svc.searchError.get()?.message??""},resultsEmpty:{className:()=>{const n=this.svc.query.get().trim();return Ze(n)&&!this.svc.searching.get()&&this.svc.searchError.get()===null&&this.svc.resultsFor.get()===n&&this.svc.results.get().length===0?"friends__empty":"friends__empty hidden"}},friendsEmpty:{className:()=>this.svc.loaded.get()&&this.svc.friends.get().length===0?"friends__empty":"friends__empty hidden"},sortToggle:{className:()=>this.svc.friends.get().length>0?"friends__sort":"friends__sort hidden"},sortFrecency:{"aria-pressed":()=>String(this.svc.sortMode.get()==="frecency"),onclick:()=>this.svc.setSortMode("frecency")},sortAlpha:{"aria-pressed":()=>String(this.svc.sortMode.get()==="alpha"),onclick:()=>this.svc.setSortMode("alpha")}});this.$each(this.ref(t,"results"),this.svc.results,(n,r,l)=>this.wireEl(Ni,{initials:()=>nt(n.displayName),name:()=>n.displayName,username:()=>`@${n.username}`,hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),add:{className:()=>this.isFriendNow(n.id)?"friend-row__add hidden":"friend-row__add",disabled:()=>this.svc.mutating.get(),onclick:()=>{const c=this.svc.results.get().find(d=>d.id===n.id);c&&!c.isFriend&&this.svc.add(c)}},added:{className:()=>this.isFriendNow(n.id)?"friend-row__added":"friend-row__added hidden"}},l),n=>n.id);const s=new Date().toISOString();return this.$each(this.ref(t,"friends"),()=>Pe(this.svc.friends.get(),this.svc.sortMode.get()),(n,r,l)=>this.wireEl(Pi,{initials:()=>nt(n.displayName),name:()=>n.displayName,subtitle:()=>{const c=this.svc.friends.get().find(d=>d.id===n.id)??n;return pi(c,s)},hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),remove:{disabled:()=>this.svc.mutating.get(),onclick:()=>{this.svc.remove(n.id)}}},l),n=>n.id),t}isFriendNow(e){return this.svc.results.get().find(t=>t.id===e)?.isFriend===!0}}const Ri=_(`
    <div class="profile">
...
`);class Oi extends T{static styles=`
        .profile {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(Ne);friends=this.inject(fe);auth=this.inject(L);router=this.inject(N);indexDraft=new h("");localErr=new h("");render(){this.auth.currentUser.get()&&this.svc.load();const e=()=>this.auth.currentUser.get()!==null,t=this.wire(Ri,{anon:{className:()=>e()?"profile__anon hidden":"profile__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/profile"}})},body:{className:()=>e()?"profile__body":"profile__body hidden"},name:()=>this.svc.player.get()?.displayName??"…",username:()=>{const n=this.svc.player.get();return n?`@${n.username}`:""},hcp:()=>{const n=this.svc.player.get()?.handicapIndex;return n==null?"–":n.toFixed(1)},index:{value:()=>this.indexDraft.get(),oninput:n=>this.indexDraft.set(n.target.value)},save:{disabled:()=>this.svc.saving.get()||this.indexDraft.get().trim()==="",textContent:()=>this.svc.saving.get()?"Saving…":"Save"},form:{onsubmit:async n=>{n.preventDefault(),this.localErr.set("");const r=this.indexDraft.get().trim().replace(",","."),l=Number.parseFloat(r);if(!Number.isFinite(l)||l<-10||l>54){this.localErr.set("Enter an index between -10 and 54.");return}await this.svc.saveIndex(l)&&this.indexDraft.set("")}},saveErr:{textContent:()=>this.localErr.get()||this.svc.saveError.get()?.message||""},genderErr:{textContent:()=>this.svc.saveError.get()?.message||""},historyEmpty:{className:()=>this.svc.history.get().length===0?"profile__empty":"profile__empty hidden"},signout:{onclick:async()=>{await this.auth.logout(),this.svc.clear(),this.friends.clear(),this.router.navigate("/")}}});this.$each(this.ref(t,"history"),this.svc.history,(n,r,l)=>this.wireEl(ji,{index:()=>n.handicapIndex.toFixed(1),source:()=>n.source,date:()=>n.effectiveDate},l),n=>n.id);const s=[{value:"M",label:"M"},{value:"F",label:"F"},{value:null,label:"Not set"}];return this.$each(this.ref(t,"gender"),()=>s,(n,r,l)=>this.wireEl(_('<button bind="b" type="button"></button>'),{b:{textContent:()=>n.label,className:()=>this.svc.player.get()?.gender===n.value?"on":"",disabled:()=>this.svc.saving.get(),onclick:()=>{this.svc.saveGender(n.value)}}},l),n=>n.label),t}}function Li(i,e){return i?e!==null&&i.ownerPlayerId===e?!0:i.rounds.some(t=>typeof t.shareToken=="string"):!1}class It{list=new h([]);listLoading=new h(!1);listError=new h(null);listLoaded=new h(!1);detail=new h(null);detailId=new h(null);detailLoading=new h(!1);detailError=new h(null);participants=new h([]);board=new h(null);boardRefusal=new h(null);boardLoading=new h(!1);results=new h(null);resultsRefusal=new h(null);mutating=new h(!1);mutateError=new h(null);async loadList(e=!1){if(!e&&(this.listLoaded.get()||this.listLoading.get()))return;const t=await I(this.listLoading,this.listError,()=>b.competitions.list());t&&(this.list.set(t),this.listLoaded.set(!0))}async loadDetail(e,t=!1){if(!t&&this.detailId.get()===e&&this.detail.get()!==null&&!this.detailLoading.get()||this.detailLoading.get()&&this.detailId.get()===e)return;this.detailId.set(e);const s=await I(this.detailLoading,this.detailError,()=>Promise.all([b.competitions.get({id:e}),b.competitions.participants({competitionId:e})]));if(!s)return;const[n,r]=s;this.detailId.get()===e&&(this.detail.set(n),this.participants.set(r),await this.loadBoard(e),n.lifecycle==="finalized"&&await this.loadResults(e))}async loadBoard(e){this.boardLoading.set(!0);try{const t=await b.competitions.leaderboard({id:e});t.ok?(this.board.set(t.value),this.boardRefusal.set(null)):(this.board.set(null),this.boardRefusal.set(t.refusal.message))}catch{this.board.set(null),this.boardRefusal.set(null)}finally{this.boardLoading.set(!1)}}async loadResults(e){try{const t=await b.competitions.results({id:e});t.ok?(this.results.set(t.value),this.resultsRefusal.set(null)):(this.results.set(null),this.resultsRefusal.set(t.refusal.message))}catch{this.results.set(null)}}async create(e){this.mutating.set(!0),this.mutateError.set(null);try{const t=await b.competitions.create({name:e});return this.list.set([t,...this.list.get()]),t}catch(t){return this.mutateError.set(X(t)),null}finally{this.mutating.set(!1)}}transition(e,t){return this.mutate(()=>b.competitions.transition({id:e,to:t}),()=>this.loadDetail(e,!0))}updateConfig(e){return this.mutate(()=>b.competitions.update(e),()=>this.loadDetail(e.id,!0))}async addPlayer(e,t,s){return this.rosterMutate(e,()=>b.competitions.addParticipant({competitionId:e,playerId:t,category:s}))}async addGuest(e,t,s){this.mutating.set(!0),this.mutateError.set(null);let n;try{n=(await b.guestPlayers.create(t)).id}catch(r){return this.mutating.set(!1),this.mutateError.set(X(r)),X(r)}return this.mutating.set(!1),this.rosterMutate(e,()=>b.competitions.addParticipant({competitionId:e,guestPlayerId:n,category:s}))}removeParticipant(e,t){return this.rosterMutate(e,()=>b.competitions.removeParticipant({participantId:t}))}withdrawParticipant(e,t){return this.rosterMutate(e,()=>b.competitions.withdrawParticipant({participantId:t}))}async createRound(e){this.mutating.set(!0),this.mutateError.set(null);try{const t=await b.competitions.createRound(e);if(t.ok)return await this.loadDetail(e.id,!0),{ok:!0,shareToken:t.shareToken};const s="refusal"in t?t.refusal.message:t.diagnostics.map(n=>n.message).join(" · ");return this.mutateError.set(s),{ok:!1,message:s}}catch(t){const s=X(t);return this.mutateError.set(s),{ok:!1,message:s}}finally{this.mutating.set(!1)}}async applyCut(e){this.mutating.set(!0),this.mutateError.set(null);try{const t=await b.competitions.applyCut({id:e});return t.ok?(await this.loadDetail(e,!0),{ok:!0,outcome:t.value}):(this.mutateError.set(t.refusal.message),{ok:!1,message:t.refusal.message})}catch(t){const s=X(t);return this.mutateError.set(s),{ok:!1,message:s}}finally{this.mutating.set(!1)}}async finalize(e){this.mutating.set(!0),this.mutateError.set(null);try{const t=await b.competitions.finalize({id:e});return t.ok?(await this.loadDetail(e,!0),{ok:!0,outcome:t.value}):(this.mutateError.set(t.refusal.message),{ok:!1,message:t.refusal.message})}catch(t){const s=X(t);return this.mutateError.set(s),{ok:!1,message:s}}finally{this.mutating.set(!1)}}clear(){this.list.set([]),this.listLoaded.set(!1),this.detail.set(null),this.detailId.set(null),this.participants.set([]),this.board.set(null),this.boardRefusal.set(null),this.results.set(null),this.resultsRefusal.set(null),this.listError.set(null),this.detailError.set(null),this.mutateError.set(null)}async mutate(e,t){this.mutating.set(!0),this.mutateError.set(null);try{const s=await e();return s.ok?(await t(),null):(this.mutateError.set(s.refusal.message),s.refusal.message)}catch(s){const n=X(s);return this.mutateError.set(n),n}finally{this.mutating.set(!1)}}rosterMutate(e,t){return this.mutate(t,async()=>{const s=await b.competitions.participants({competitionId:e});this.participants.set(s)})}}function X(i){return i&&typeof i=="object"&&"message"in i&&typeof i.message=="string"?i.message:"Something went wrong. Try again."}function Et(i){switch(i){case"draft":return"Draft";case"setup":return"Setup";case"active":return"Live";case"finalized":return"Finalized"}}function Tt(i){return`comp-chip comp-chip--${i}`}function ve(i){switch(i){case"draft":return{to:"setup",label:"Open setup"};case"setup":return{to:"active",label:"Start competition"};default:return null}}function we(i){return i==="draft"||i==="setup"}function Di(i){return i==="setup"||i==="active"}const Hi=_(`
    <div class="comps">
...
`);class Fi extends T{static styles=`
        .comps {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(It);auth=this.inject(L);router=this.inject(N);loggedIn=new w(()=>this.auth.currentUser.get()!==null);nameDraft=new h("");render(){this.loggedIn.get()&&this.svc.loadList();const e=this.wire(Hi,{anon:{className:()=>this.loggedIn.get()?"comps__anon hidden":"comps__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/competitions"}})},body:{className:()=>this.loggedIn.get()?"comps__body":"comps__body hidden"},nameInput:{value:()=>this.nameDraft.get(),oninput:t=>this.nameDraft.set(t.target.value)},createBtn:{disabled:()=>this.svc.mutating.get()||this.nameDraft.get().trim()==="",textContent:()=>this.svc.mutating.get()?"Creating…":"Create"},createForm:{onsubmit:async t=>{t.preventDefault();const s=this.nameDraft.get().trim();if(s==="")return;const n=await this.svc.create(s);n&&(this.nameDraft.set(""),this.router.navigate("/competition",{query:{id:n.id}}))}},createErr:{textContent:()=>this.svc.mutateError.get()??""},loading:{className:()=>this.svc.listLoading.get()&&!this.svc.listLoaded.get()?"comps__loading":"comps__loading hidden"},empty:{className:()=>this.svc.listLoaded.get()&&this.svc.list.get().length===0?"comps__empty":"comps__empty hidden"}});return this.$each(this.ref(e,"list"),this.svc.list,(t,s,n)=>this.wireEl(Mi,{row:{onclick:()=>this.router.navigate("/competition",{query:{id:t.id}})},name:()=>t.name,chip:{textContent:()=>Et(t.lifecycle),className:()=>Tt(t.lifecycle)}},n),t=>t.id),e}}class Ai{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await I(this.loading,this.error,()=>b.setup.aggregations());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=ft()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:typeof e=="string"?e:""}}function Bi(i){const e=[];for(const s of i.rounds)s.value!==null&&(s.status==="counted"?e.push(y(s.value)):s.status==="dropped"&&e.push(`<s>${y(s.value)}</s>`));const t=i.total===null?"—":y(i.total);return e.length===0?`<span class="cb-arith__total">${t}</span>`:`${e.join(" + ")} = <span class="cb-arith__total">${t}</span>`}function Gi(i){return`cb-c cb-c--${i.status}`}function qi(i){return i.value===null?"—":i.status==="dropped"?`<s>${y(i.value)}</s>`:y(i.value)}function Ki(i){return i.findIndex(e=>e.postCut)}function Vi(i,e,t,s){const n=new Map(i.rounds.map(g=>[g.roundNumber,g])),r=i.withdrawn||i.cutAfterRound!==null,l=["cb-row"];i.withdrawn?l.push("cb-row--withdrawn"):i.cutAfterRound!==null?l.push("cb-row--cut"):i.position===1&&l.push("cb-row--lead"),i.incomplete&&l.push("cb-row--incomplete");const c=e.map((g,S)=>{const v=n.get(g.roundNumber),$=S===t?" cb-c--divider":"";return v?`<td class="${Gi(v)}${$}">${qi(v)}</td>`:`<td class="cb-c cb-c--missing${$}">—</td>`}).join(""),d=i.withdrawn?' <span class="cb-tag cb-tag--out">WD</span>':i.cutAfterRound!==null?` <span class="cb-tag cb-tag--out">Cut R${i.cutAfterRound}</span>`:"",u=i.category?` <span class="cb-tag cb-cat">${y(i.category)}</span>`:"",p=r?"—":String(i.position),m=s===null?"":`<td class="cb-points">${s}</td>`;return`<tr class="${l.join(" ")}">
  <td class="cb-pos">${p}</td>
  <td class="cb-who">
    <div class="cb-who__line"><span class="cb-name">${y(i.displayName)}</span>${u}${d}</div>
    <div class="cb-arith">${Bi(i)}</div>
  </td>
  ${c}
  <td class="cb-total">${i.total===null?"—":y(i.total)}</td>
  ${m}
</tr>`}function Ct(i,e,t){if(i.length===0)return'<div class="cb-empty">No scores yet — the board fills in as rounds are played.</div>';const s=Ki(e),n=e.map((c,d)=>`<th class="cb-c${d===s?" cb-c--divider":""}">R${c.roundNumber}</th>`).join(""),r=t?'<th class="cb-points">Pts</th>':"",l=i.map((c,d)=>Vi(c,e,s,t?t(d):null)).join("");return`<table class="cb">
  <thead><tr><th class="cb-pos">#</th><th class="cb-who">Player</th>${n}<th class="cb-total">Total</th>${r}</tr></thead>
  <tbody>${l}</tbody>
</table>`}function Wi(i,e={}){const t=i.rounds.map(l=>({roundNumber:l.roundNumber,postCut:l.postCut})),s=i.operator.kind==="best_n"?`Best ${i.operator.n} of ${t.length}`:"Total across rounds",n=e.defaulted?'<span class="cb-head__hint">· default scoring</span>':"";return`<div class="cb-head">
  <h3 class="cb-head__title">${y(i.metricLabel)}</h3>
  <span class="cb-head__op">${y(s)}</span>${n}
</div>`+Ct(i.entries,t,null)}function Ui(i,e){const t=i.map(n=>n.entry),s=e.length>0?e:Yi(t);return Ct(t,s,n=>y(i[n].points))}function Yi(i){const e=new Set;for(const t of i)for(const s of t.rounds)e.add(s.roundNumber);return[...e].sort((t,s)=>t-s).map(t=>({roundNumber:t,postCut:!1}))}const Qi={not_started:"Not started",active:"Live",complete:"Finished"},Xi=_(`
    <div class="cd">
...
`);class Ji extends T{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
            ...
            & .cd__form {
                ...
                & button[bind="addSlot"], & button[bind="saveSetup"] {
                    ...
                    ${x()} background: ${o("primary")}; color: ${o("primary-text")}; border: none;
                }
            }
            & .cd__slot {
                display: flex; align-items: center; justify-content: space-between;
            ...
            & .cd__friendchip {
                padding: ${a("xs")} ${a("md")}; ${x()} font-family: inherit;
            ...
            & .cd__adminbtns button {
                padding: ${a("md")} ${a("lg")}; font-family: inherit; font-weight: 700; ${x()}
            }
    ...
    `;svc=this.inject(It);catalog=this.inject(oe);aggCatalog=this.inject(Ai);friends=this.inject(fe);profile=this.inject(Ne);auth=this.inject(L);router=this.inject(N);editingSetup=new h(!1);nameDraft=new h("");slotDraft=new h([]);aggStrategy=new h("");aggConfig=new h({});startListDraft=new h("single_group");courseDraft=new h("");teeDraft=new h("");cutAfterDraft=new h("");cutTypeDraft=new h("");cutValueDraft=new h("");formatPickDraft=new h("");guestNameDraft=new h("");guestGenderDraft=new h("M");guestHcpDraft=new h("");roundCourseDraft=new h("");roundDateDraft=new h("");courses=new h([]);tees=new h([]);coursesLoaded=!1;resultSetIdx=new h(0);cutOutcome=new h(null);cutConfirmOpen=new h(!1);finalizeConfirmOpen=new h(!1);admin=new w(()=>Li(this.svc.detail.get(),this.profile.player.get()?.id??null));lifecycle=new w(()=>this.svc.detail.get()?.lifecycle??"draft");render(){const e=this.router.query("id");this.track(k(()=>{const d=e.get();d&&K(()=>{this.svc.loadDetail(d)})})),this.auth.currentUser.get()&&this.profile.load(),this.catalog.load(),this.aggCatalog.load(),this.loadCourses();const t=()=>e.get()??"",s=()=>this.svc.detail.get(),n=this.wire(Xi,{back:{onclick:()=>this.router.navigate("/competitions")},loading:{className:()=>this.svc.detailLoading.get()&&s()===null?"cd__loading":"cd__loading hidden"},loadErr:{textContent:()=>this.svc.detailError.get()?.message??"",className:()=>this.svc.detailError.get()?"cd__loaderr":"cd__loaderr hidden"},body:{className:()=>s()?"cd__body":"cd__body hidden"},name:()=>s()?.name??"",chip:{textContent:()=>Et(this.lifecycle.get()),className:()=>Tt(this.lifecycle.get())},ownerLine:{textContent:()=>this.admin.get()?"You administer this competition.":"Read-only view."},mutateErr:{textContent:()=>this.svc.mutateError.get()??""},transitionRow:{className:()=>this.admin.get()&&ve(this.lifecycle.get())?"cd__transition":"cd__transition hidden"},transitionBtn:{textContent:()=>ve(this.lifecycle.get())?.label??"",disabled:()=>this.svc.mutating.get(),onclick:()=>{const d=ve(this.lifecycle.get());d&&this.svc.transition(t(),d.to)}},setupSection:{className:()=>this.admin.get()&&we(this.lifecycle.get())?"cd__section cd__setup":"cd__section cd__setup hidden"},setupToggle:{textContent:()=>this.editingSetup.get()?"Close":"Edit",onclick:()=>{this.editingSetup.get()?this.editingSetup.set(!1):this.seedEditor()}},setupSummary:{className:()=>this.editingSetup.get()?"cd__summary hidden":"cd__summary",innerHTML:()=>this.setupSummaryHtml()},setupForm:{className:()=>this.editingSetup.get()?"cd__form":"cd__form hidden"},nameInput:{value:()=>this.nameDraft.get(),oninput:d=>this.nameDraft.set(d.target.value)},formatPick:{value:()=>this.formatPickDraft.get(),onchange:d=>this.formatPickDraft.set(d.target.value)},addSlot:{onclick:()=>{const d=this.formatPickDraft.get()||this.catalog.descriptors.get()[0]?.id;d&&this.slotDraft.set([...this.slotDraft.get(),d])}},aggPick:{value:()=>this.aggStrategy.get(),onchange:d=>this.selectStrategy(d.target.value)},aggDesc:{textContent:()=>this.aggCatalog.byId(this.aggStrategy.get())?.description??""},coursePick:{value:()=>this.courseDraft.get(),onchange:d=>{const u=d.target.value;this.courseDraft.set(u),this.teeDraft.set(""),this.loadTees(u)}},teePick:{value:()=>this.teeDraft.get(),onchange:d=>this.teeDraft.set(d.target.value)},startListPick:{value:()=>this.startListDraft.get(),onchange:d=>this.startListDraft.set(d.target.value)},cutAfter:{value:()=>this.cutAfterDraft.get(),oninput:d=>this.cutAfterDraft.set(d.target.value)},cutTypePick:{value:()=>this.cutTypeDraft.get(),onchange:d=>this.cutTypeDraft.set(d.target.value)},cutValue:{value:()=>this.cutValueDraft.get(),oninput:d=>this.cutValueDraft.set(d.target.value)},saveSetup:{disabled:()=>this.svc.mutating.get(),textContent:()=>this.svc.mutating.get()?"Saving…":"Save setup",onclick:()=>{this.saveSetup(t())}},cancelSetup:{onclick:()=>this.editingSetup.set(!1)},rosterCount:()=>{const d=this.svc.participants.get().length;return d===0?"":String(d)},rosterEmpty:{className:()=>this.svc.participants.get().length===0?"cd__empty":"cd__empty hidden"},rosterAdd:{className:()=>this.admin.get()&&we(this.lifecycle.get())?"cd__rosteradd":"cd__rosteradd hidden"},guestName:{value:()=>this.guestNameDraft.get(),oninput:d=>this.guestNameDraft.set(d.target.value)},guestGender:{value:()=>this.guestGenderDraft.get(),onchange:d=>this.guestGenderDraft.set(d.target.value)},guestHcp:{value:()=>this.guestHcpDraft.get(),oninput:d=>this.guestHcpDraft.set(d.target.value)},guestForm:{onsubmit:async d=>{d.preventDefault();const u=this.guestNameDraft.get().trim();if(u==="")return;const p=this.guestHcpDraft.get().trim().replace(",","."),m=p===""?null:Number.parseFloat(p);await this.svc.addGuest(t(),{displayName:u,gender:this.guestGenderDraft.get(),handicapIndex:Number.isFinite(m)?m:null},null)===null&&(this.guestNameDraft.set(""),this.guestHcpDraft.set(""))}},roundsEmpty:{className:()=>(s()?.rounds.length??0)===0?"cd__empty":"cd__empty hidden"},addRoundForm:{className:()=>this.admin.get()&&Di(this.lifecycle.get())?"cd__addround":"cd__addround hidden"},roundCoursePick:{value:()=>this.roundCourseDraft.get(),onchange:d=>this.roundCourseDraft.set(d.target.value)},roundDate:{value:()=>this.roundDateDraft.get(),oninput:d=>this.roundDateDraft.set(d.target.value)},addRound:{disabled:()=>this.svc.mutating.get()},adminActions:{className:()=>this.admin.get()&&this.lifecycle.get()==="active"?"cd__section cd__admin":"cd__section cd__admin hidden"},cutOutcome:{innerHTML:()=>this.cutOutcomeHtml()},applyCut:{disabled:()=>this.svc.mutating.get(),onclick:()=>this.cutConfirmOpen.set(!0)},finalize:{disabled:()=>this.svc.mutating.get(),onclick:()=>this.finalizeConfirmOpen.set(!0)},boardTitle:()=>this.lifecycle.get()==="finalized"?"Official results":"Leaderboard",board:{innerHTML:()=>this.boardHtml()},boardRefusal:{textContent:()=>this.lifecycle.get()==="finalized"?this.svc.resultsRefusal.get()??"":this.svc.board.get()===null?this.svc.boardRefusal.get()??"":""}});this.ref(n,"addRoundForm").addEventListener("submit",d=>{d.preventDefault(),this.doAddRound(t())}),this.$each(this.ref(n,"slotList"),this.slotDraft,(d,u,p)=>this.wireEl(_('<div class="cd__slot"><span bind="label"></span><button bind="rm" type="button" aria-label="Remove">×</button></div>'),{label:()=>`Slot ${u+1}: ${this.catalog.labelOf(d)??d}`,rm:{onclick:()=>this.slotDraft.set(this.slotDraft.get().filter((m,g)=>g!==u))}},p),(d,u)=>`${u}:${d}`),this.$each(this.ref(n,"formatPick"),this.catalog.descriptors,(d,u,p)=>this.wireEl(_('<option bind="o"></option>'),{o:{value:()=>d.id,textContent:()=>this.catalog.labelOf(d)??d.id}},p),d=>d.id),this.$each(this.ref(n,"aggPick"),this.aggCatalog.descriptors,(d,u,p)=>this.wireEl(_('<option bind="o"></option>'),{o:{value:()=>d.id,textContent:()=>this.aggCatalog.labelOf(d)}},p),d=>d.id);const l=new w(()=>this.aggCatalog.byId(this.aggStrategy.get())?.configFields??[]);this.$each(this.ref(n,"aggFields"),l,(d,u,p)=>this.renderConfigField(d,p),d=>d.key);const c=(d,u)=>this.wireEl(_('<option bind="o"></option>'),{o:{value:()=>d.id,textContent:()=>d.name}},u);return this.$each(this.ref(n,"coursePick"),this.courses,(d,u,p)=>c(d,p),d=>d.id),this.$each(this.ref(n,"roundCoursePick"),this.courses,(d,u,p)=>c(d,p),d=>d.id),this.$each(this.ref(n,"teePick"),this.tees,(d,u,p)=>this.wireEl(_('<option bind="o"></option>'),{o:{value:()=>d.id,textContent:()=>d.name}},p),d=>d.id),this.$each(this.ref(n,"roster"),this.svc.participants,(d,u,p)=>this.wireEl(_(`
                    ...
                    `),{rname:()=>d.displayNameSnapshot,rcat:{textContent:()=>d.category??"",className:()=>d.category?"cd__rcat":"cd__rcat hidden"},rout:{textContent:()=>d.withdrawnAt?"Withdrawn":d.cutAfterRound!==null?`Cut R${d.cutAfterRound}`:"",className:()=>d.withdrawnAt||d.cutAfterRound!==null?"cd__rout":"cd__rout hidden"},withdraw:{className:()=>this.admin.get()&&!d.withdrawnAt?"cd__ract":"cd__ract hidden",onclick:()=>{this.svc.withdrawParticipant(t(),d.id)}},remove:{className:()=>this.admin.get()&&we(this.lifecycle.get())?"cd__ract cd__ract--danger":"cd__ract cd__ract--danger hidden",onclick:()=>{this.svc.removeParticipant(t(),d.id)}}},p),d=>d.id),this.$each(this.ref(n,"friendPick"),this.friends.friends,(d,u,p)=>this.wireEl(_('<button bind="chip" class="cd__friendchip" type="button"></button>'),{chip:{textContent:()=>d.displayName,disabled:()=>this.svc.mutating.get()||this.svc.participants.get().some(m=>m.playerId===d.id),onclick:()=>{this.svc.addPlayer(t(),d.id,null)}}},p),d=>d.id),this.auth.currentUser.get()&&this.friends.load(),this.$each(this.ref(n,"rounds"),new w(()=>s()?.rounds??[]),(d,u,p)=>this.wireEl(_(`
                    ...
                    `),{row:{disabled:()=>!d.shareToken,onclick:()=>{d.shareToken&&this.router.navigate("/round",{query:{token:d.shareToken}})}},num:()=>`Round ${d.roundNumber}`,meta:()=>[d.courseNameSnapshot,d.date].filter(Boolean).join(" · ")||(d.shareToken?"Open":"View-only"),status:{textContent:()=>Qi[d.status]??d.status,className:()=>`cd__rstatus s-${d.status}`}},p),d=>d.id),this.$each(this.ref(n,"setSwitch"),new w(()=>this.lifecycle.get()==="finalized"?this.svc.results.get()?.resultSets??[]:[]),(d,u,p)=>this.wireEl(_('<button bind="b" type="button"></button>'),{b:{textContent:()=>d.scoringType.toUpperCase(),className:()=>this.resultSetIdx.get()===u?"on":"",onclick:()=>this.resultSetIdx.set(u)}},p),d=>d.scoringType),this.spawn(B,this.ref(n,"confirmHost"),{open:this.cutConfirmOpen,title:"Apply cut?",message:"This evaluates the configured cut against the current aggregate and marks who advances. Cut players are left out of later rounds.",confirmLabel:"Apply cut",cancelLabel:"Cancel",onconfirm:async()=>{const d=await this.svc.applyCut(t());d.ok&&this.cutOutcome.set(d.outcome)}}),this.spawn(B,this.ref(n,"finalizeHost"),{open:this.finalizeConfirmOpen,title:"Finalize competition?",message:"Finalizing freezes the official results and locks the competition. This cannot be undone.",confirmLabel:"Finalize",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.svc.finalize(t())}}),n}loadCourses(){this.coursesLoaded||(this.coursesLoaded=!0,b.courses.list().then(e=>this.courses.set(e)).catch(()=>{this.coursesLoaded=!1}))}async loadTees(e){if(!e){this.tees.set([]);return}try{this.tees.set(await b.tees.listByCourse({courseId:e}))}catch{this.tees.set([])}}selectStrategy(e){this.applyStrategyConfig(e,{})}applyStrategyConfig(e,t){this.aggStrategy.set(e);const s=this.aggCatalog.byId(e)?.configFields??[],n={};for(const r of s){const l=t[r.key];n[r.key]=l!=null?String(l):String(r.default)}this.aggConfig.set(n)}setConfigValue(e,t){this.aggConfig.set({...this.aggConfig.get(),[e]:t})}buildConfig(e){const t=this.aggCatalog.byId(e)?.configFields??[],s=this.aggConfig.get(),n={};for(const r of t){const l=s[r.key]??String(r.default);n[r.key]=r.kind==="integer"?Number.parseInt(l,10)||Number(r.default):l}return n}renderConfigField(e,t){const s=document.createElement("label");s.className="cd__field";const n=document.createElement("span");if(n.textContent=e.label,s.appendChild(n),e.kind==="select"){const r=document.createElement("select");for(const l of e.options){const c=document.createElement("option");c.value=l.value,c.textContent=l.label,r.appendChild(c)}t(k(()=>{r.value=this.aggConfig.get()[e.key]??String(e.default)})),r.addEventListener("change",()=>this.setConfigValue(e.key,r.value)),s.appendChild(r)}else{const r=document.createElement("input");r.setAttribute("inputmode","numeric"),t(k(()=>{const l=this.aggConfig.get()[e.key]??String(e.default);r.value!==l&&(r.value=l)})),r.addEventListener("input",()=>this.setConfigValue(e.key,r.value)),s.appendChild(r)}return s}seedEditor(){const e=this.svc.detail.get();if(!e)return;this.nameDraft.set(e.name);const t=e.defaultConfig;this.slotDraft.set((t?.slots??[]).map(c=>c.formatId)),this.startListDraft.set(t?.startList??"single_group");const s=t?.fallbackTee?.teeId??"";this.teeDraft.set(s);const n=e.aggregation,r=n?.strategyId??this.aggCatalog.descriptors.get()[0]?.id??"";this.applyStrategyConfig(r,n?.config??{});const l=e.cutRules;l&&l.afterRound!==void 0?(this.cutAfterDraft.set(String(l.afterRound)),this.cutTypeDraft.set(l.cutType??""),this.cutValueDraft.set(l.cutValue!==void 0?String(l.cutValue):"")):(this.cutAfterDraft.set(""),this.cutTypeDraft.set(""),this.cutValueDraft.set("")),this.formatPickDraft.set(this.catalog.descriptors.get()[0]?.id??""),this.editingSetup.set(!0)}async saveSetup(e){const t=this.slotDraft.get().map(g=>({formatId:g})),s=this.teeDraft.get(),n=t.length>0?{slots:t,startList:this.startListDraft.get(),...s?{fallbackTee:{teeId:s}}:{}}:void 0,r=this.aggStrategy.get(),l=r?{strategyId:r,config:this.buildConfig(r)}:void 0,c=Number.parseInt(this.cutAfterDraft.get(),10),d=Number.parseInt(this.cutValueDraft.get(),10),u=this.cutTypeDraft.get(),p=u&&Number.isFinite(c)&&Number.isFinite(d)?{afterRound:c,cutType:u,cutValue:d}:void 0;await this.svc.updateConfig({id:e,name:this.nameDraft.get().trim()||void 0,...n?{defaultConfig:n}:{},...l?{aggregation:l}:{},...p?{cutRules:p}:{}})===null&&this.editingSetup.set(!1)}async doAddRound(e){const t=this.roundCourseDraft.get()||this.courseDraft.get(),s=this.roundDateDraft.get();if(!t||!s){this.svc.mutateError.set("Pick a course and a date for the round.");return}const n=await this.svc.createRound({id:e,courseId:t,playedAt:s});n.ok&&this.router.navigate("/round",{query:{token:n.shareToken}})}setupSummaryHtml(){const e=this.svc.detail.get();if(!e)return"";const t=e.aggregation,s=e.defaultConfig?.slots??[],n=s.length>0?s.map(l=>this.catalog.labelOf(l.formatId)??l.formatId).join(", "):"<em>none set</em>",r=t?y(this.aggCatalog.labelOf(t.strategyId)):"<em>default (chosen automatically)</em>";return`<div>Formats: ${y(n)}</div><div>Scoring: ${r}</div>`}cutOutcomeHtml(){const e=this.cutOutcome.get();if(!e)return"";const t=s=>s.length===0?"—":s.map(n=>y(n.displayName)).join(", ");return`<div class="cd__cutgrp"><strong>Advanced (${e.advanced.length}):</strong> ${t(e.advanced)}</div>
<div class="cd__cutgrp"><strong>Cut (${e.cut.length}):</strong> ${t(e.cut)}</div>`}boardHtml(){if(this.lifecycle.get()==="finalized"){const t=this.svc.results.get();if(!t)return"";const s=Math.min(this.resultSetIdx.get(),t.resultSets.length-1),n=t.resultSets[s];if(!n)return"";const r=(this.svc.board.get()?.view.rounds??[]).map(c=>({roundNumber:c.roundNumber,postCut:c.postCut}));return`<div class="cd__official-banner">Official results · finalized ${y(t.finalizedAt.slice(0,10))}</div>`+`<div class="cb cb--official">${Ui(n.entries,r)}</div>`}const e=this.svc.board.get();return e?Wi(e.view,{defaulted:e.defaulted}):""}}const Zi=_(`
    <div class="app-shell">
        <main bind="content" class="app-shell__content"></main>
        <div bind="nav" class="app-shell__nav"></div>
    </div>
`);class er extends T{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(N);render(){const e=this.wire(Zi,{});return this.spawn(Ts,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Ke,"/history":Bs,"/round":Xn,"/create":Ii,"/login":Ti,"/friends":zi,"/profile":Oi,"/competitions":Fi,"/competition":Ji},Ke),e}}A.get(Kt);const it=A.get(N),rt=A.get(L);await Wt(er,"#app",{hot:void 0,onInit:async()=>{await rt.load(),rt.currentUser.get()&&it.route.get()==="/login"&&it.navigate("/",!0)}});export{T as C,N as R,h as S,Kt as T,f as a,re as b,w as c,k as e,I as r,_ as t};
