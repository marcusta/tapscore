...
`);class Vo extends R{static styles=`
    ...
    `;let ee=Ks;async function Ld(s,e={}){e.everywhere?await s.auth.logoutEverywhere():await s.auth.logout(),s.profile.clear(),s.friends.clear(),s.activity.clear(),s.friendProfile.clear(),s.spectate.clear(),s.admins.clear(),s.homeStats.clear(),s.landing.clear(),s.navigate("/")}const Md="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";function $e(s="avatar"){return`<span class="${s}">
            <img bind="avatarPhoto" class="avatar__photo" alt="" />
            <span bind="avatarInitials" class="avatar__initials"></span>
        </span>`}function we(s){const e=()=>ea(s());return{avatarPhoto:{src:()=>e()??Md,className:()=>e()?"avatar__photo":"avatar__photo hidden"},avatarInitials:{textContent:()=>{const t=s();return ta(t.displayName,t.username)},className:()=>e()?"avatar__initials hidden":"avatar__initials"}}}function Ee(s,e="0.85rem"){return`
        position: relative;
    ...
    `}function Fd(s){if(!s.signedIn)return[];const e=[{kind:"identity",displayName:(s.displayName??"").trim()||(s.username??"").trim()||"Signed in",username:(s.username??"").trim()},{kind:"profile",label:"Profile"}];return s.isSuperAdmin&&e.push({kind:"admin",label:"Admin"}),e.push({kind:"signout",label:"Sign out"}),e.push({kind:"signout-all",label:"Sign out everywhere"}),e}function Bd(s){return Fd(s).map(e=>e.kind)}function mn(s){return s.signedIn?"avatar":"signin"}const jd=b(`
...
`);class Dd extends R{static styles=`
        .acct {
            position: relative;
            display: flex;
            justify-content: flex-end;
...
`),Qd='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',Ms=`
...
`);class bn extends R{static styles=`
        .landing {
...
`),nc='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',ic=b(`
    <div class="round-row">
        <button bind="row" type="button" class="round-summary round-row__main">${Ms}</button>
        <div bind="actions" class="round-row__actions">
            <button bind="menuButton" type="button" class="round-row__menu-button" aria-label="Round actions" aria-haspopup="true" aria-expanded="false">${nc}</button>
            <div bind="menu" class="round-row__menu" role="group" aria-label="Round actions">
                <button bind="action" type="button" class="round-row__menu-action"></button>
            </div>
        </div>
    </div>
`);class rc extends R{static styles=`
        .history {
    ...
    `;svc=this.inject(At);auth=this.inject(j);router=this.inject(B);loggedIn=new $(()=>this.auth.currentUser.get()!==null);rows=new $(()=>tc(this.loggedIn.get()?Ze.fromMyRounds(this.svc.myRounds.get()):Ze.fromDeviceRounds(this.svc.deviceRounds.get())));ongoingRows=new $(()=>this.rows.get().filter(e=>e.status!=="complete"));finishedRows=new $(()=>this.rows.get().filter(e=>e.status==="complete"));deleteOpen=new p(!1);leaveOpen=new p(!1);actionTarget=new p(null);actionError=new p("");openRoundMenu=new p(null);askAction(e,t,n,i){this.openRoundMenu.set(null),this.actionError.set(""),this.actionTarget.set({token:t,roundId:n,name:i,action:e}),e==="delete"?this.deleteOpen.set(!0):this.leaveOpen.set(!0)}render(){this.loggedIn.get()?this.svc.loadMine():this.svc.loadDevice();const e=this.wire(sc,{back:{onclick:()=>this.router.navigate("/")},actionError:{textContent:()=>this.actionError.get()},empty:{className:()=>this.rows.get().length===0?"history__empty":"history__empty hidden"},sections:{className:()=>this.rows.get().length===0?"history__sections hidden":"history__sections"},ongoingSection:{className:()=>this.ongoingRows.get().length===0?"history__section history__ongoing hidden":"history__section history__ongoing"},ongoingCount:()=>String(this.ongoingRows.get().length),finishedSection:{className:()=>this.finishedRows.get().length===0?"history__section history__finished hidden":"history__section history__finished"},finishedCount:()=>String(this.finishedRows.get().length)});this.$each(this.ref(e,"ongoingList"),this.ongoingRows,(r,d,l)=>this.roundRow(r,l,!0),r=>r.key),this.$each(this.ref(e,"finishedList"),this.finishedRows,(r,d,l)=>this.roundRow(r,l),r=>r.key),this.spawn(ee,this.ref(e,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:()=>{const r=this.actionTarget.get();return`Delete ${r?`“${r.name}”`:"this round"}? This permanently removes it and all its scores for everyone. This can't be undone.`},confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const r=this.actionTarget.get();r&&this.svc.remove(r.token,r.roundId).then(d=>{d||this.actionError.set("Could not delete the round. Try again.")})}}),this.spawn(ee,this.ref(e,"confirmHost"),{open:this.leaveOpen,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const r=this.actionTarget.get();r&&this.svc.leave(r.token,r.roundId).then(d=>{d.ok||this.actionError.set(d.message)})}});const t=r=>{r.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1),r.key==="Escape"&&this.leaveOpen.get()&&this.leaveOpen.set(!1),r.key==="Escape"&&this.openRoundMenu.get()!==null&&this.openRoundMenu.set(null)};window.addEventListener("keydown",t),this.track(()=>window.removeEventListener("keydown",t));const n=this.ref(e,"root"),i=r=>{if(this.openRoundMenu.get()===null)return;const d=r.target;d instanceof Node&&n.contains(d)||this.openRoundMenu.set(null)};return document.addEventListener("pointerdown",i,!0),this.track(()=>document.removeEventListener("pointerdown",i,!0)),e}roundRow(e,t,n=!1){return this.wireEl(ic,{row:{disabled:()=>e.token===null,onclick:()=>{e.token!==null&&this.router.navigate("/round",{query:{token:e.token}})}},title:()=>nt(e),course:{textContent:()=>Et(e)??"",className:()=>Et(e)?"round-summary__course":"round-summary__course hidden"},date:()=>Ne(e.date),progress:{textContent:()=>n&&e.holesPlayed&&e.holesPlayed>0?`Thru ${e.holesPlayed}`:"",className:()=>n&&e.holesPlayed&&e.holesPlayed>0?"round-summary__progress":"round-summary__progress hidden"},formats:{textContent:()=>e.formats??"",className:()=>e.formats?"round-summary__formats":"round-summary__formats hidden"},actions:{className:()=>Me(e)===null?"round-row__actions hidden":"round-row__actions"},menuButton:{"aria-expanded":()=>this.openRoundMenu.get()===e.key?"true":"false",onclick:()=>this.openRoundMenu.set(this.openRoundMenu.get()===e.key?null:e.key)},menu:{className:()=>this.openRoundMenu.get()===e.key?"round-row__menu":"round-row__menu hidden"},action:{textContent:()=>{const i=Me(e);return i?Wi(i):""},onclick:()=>{const i=Me(e);!i||e.token===null||this.askAction(i,e.token,e.roundId??"",nt(e))}}},t)}}const oc=700;function ac(s){if(!s.currentHole)return!1;const e=s.balls.filter(t=>!t.pending);return e.length>0&&e.every(t=>t.scored)}function lc(s){return s.currentHole?s.balls.some((e,t)=>t!==s.currentBallIndex&&!e.scored):!1}function ns(s){const e=s.currentHole;if(!e)return{kind:"noop"};const t=s.balls,n=s.currentBallIndex;for(let i=n+1;i<t.length;i++)if(!t[i].scored)return{kind:"moveToBall",ballIndex:i};for(let i=0;i<n;i++)if(!t[i].scored)return{kind:"moveToBall",ballIndex:i};return s.holeIndex>=s.holeCount-1?{kind:"roundComplete",toast:"Round complete"}:{kind:"holeComplete",toast:`Hole ${e.label} done`,fromHoleId:e.id,toHoleIndex:s.holeIndex+1,delayMs:oc}}function dc(s,e){const t=s.currentHole;if(e.kind==="statsDone")return s.holeCompleteOnEntry?{write:null,move:{kind:"stay"}}:{write:null,move:ns(s)};const n=s.balls[s.currentBallIndex];if(!t||!n)return{write:null,move:{kind:"noop"}};if(n.pending)return s.holeCompleteOnEntry?{write:null,move:{kind:"stay"}}:{write:null,move:ns(s)};const i={ballIndex:s.currentBallIndex,holeId:t.id,value:e.value,withMetadata:e.value!==null};return e.value!==null&&e.value>0&&s.collectsStats?{write:i,move:{kind:"openStats"}}:s.holeCompleteOnEntry?{write:i,move:{kind:"stay"}}:{write:i,move:ns(s)}}const Se=60,yn=8,ys=4,cc=Array.from({length:ys*2+1},(s,e)=>e-ys),uc="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",hc=b(`
...
`),_c=b('<button bind="btn" class="se-seg" type="button"></button>'),yc=b(`
    <div class="se-stats__group">
        <span bind="glabel" class="se-stats__group-label"></span>
...
`),vc=b('<div bind="rule" class="se-stats__rule"></div>');class wc extends R{static styles=`
        .se {
            margin-top: ${o("xl")};
            &.hidden { display: none; }
        }
...
</tr>`;const G=vs(n.plan.slotDefId,M.ballIds),C=n.isOpen(G),K=qc(G),ne=tr(N,n.routeSections,e,{mode:n.mode??"product"}),ie=["lb-rank__row--expandable"];return g.lead&&ie.push("lb-rank__lead"),C&&ie.push("lb-rank__row--open"),`<tr class="${ie.join(" ")}" data-expand-key="${L(G)}">
  <td class="lb-rank__pos">${g.position}</td>
  <td class="lb-rank__who"><button type="button" class="lb-rank__toggle" aria-expanded="${C}" aria-controls="${L(K)}"><span class="lb-rank__whobox"><span class="lb-rank__name">${L(g.name)}</span>${w}</span></button></td>${A}
...
<tr class="lb-rank__panel${C?" lb-rank__panel--open":""}" data-panel-key="${L(G)}">
  <td class="lb-rank__panelcell" colspan="${l}"><div class="lb-rank__panelwrap" id="${L(K)}"><div class="lb-rank__panelbox">${ne}</div></div></td>
</tr>`}).join(""),u=r?`
...
</div>`}function Uc(s,e){const t=Nc(s,e),n=t.matches.map(i=>{const r=i.leader==="a"?" lb-mp__team--lead":"",d=i.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const Kc={ranked:Vc,match_summary:(s,e)=>Uc(s,e)},Wc={"default-score-grid":Bc,"compact-match-grid":jc,"category-matrix-grid":Dc};function Yc(s){return`<div class="lb-diag">Unrenderable result section <code>${L(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Qc(s){return`<div class="lb-diag">Unsupported score-grid component <code>${L(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Xc(s,e,t,n=null){const i=Kc[s.kind];return i?i(s,e,t,n):Yc(s.kind)}function tr(s,e,t,n){const i=Ji(s),r=Wc[i];return r?r(s,e,t,n):Qc(i)}function sr(s,e,t=er,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${L(s.formatLabel)}.</div>`:s.leaderboard.map(r=>Xc(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${L(s.formatLabel)}.</div>`}function nr(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>tr(r,e,t,{mode:i})).join(`
`)}const Jc=b(`
    <div bind="root" class="lb">
        <div bind="status" class="lb__status hidden"></div>
        <div bind="body" class="lb__body"></div>
    </div>
`);class Ds extends R{static styles=`
        .lb {
    ...
    `;svc=this.inject(de);expansion=new Hc;slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(n=>n.slotDefId===t)??e[0]??null};render(){return this.wire(Jc,{status:{className:()=>{const t=this.svc.resultLoading.get(),n=this.svc.result.get()===null;return t||n?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody(),onclick:t=>this.onBodyClick(t),onkeydown:t=>this.onBodyKeydown(t)}})}rowFor(e){return e.target?.closest?.("tr[data-expand-key]")??null}onBodyClick(e){const t=this.rowFor(e);if(!t||(window.getSelection?.()?.toString()??"")!=="")return;const n=t.getAttribute("data-expand-key")??"";this.applyOpen(t,this.expansion.toggle(n))}onBodyKeydown(e){if(e.key!=="Escape")return;const t=this.rowFor(e);if(!t)return;const n=t.getAttribute("data-expand-key")??"";this.expansion.isOpen(n)&&(this.applyOpen(t,this.expansion.set(n,!1)),t.querySelector(".lb-rank__toggle")?.focus(),e.stopPropagation())}applyOpen(e,t){e.classList.toggle("lb-rank__row--open",t),e.querySelector(".lb-rank__toggle")?.setAttribute("aria-expanded",String(t));const n=e.nextElementSibling;n?.classList.contains("lb-rank__panel")&&n.classList.toggle("lb-rank__panel--open",t)}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const n=u=>{const f=this.svc.nameOf(u);return this.svc.isPending(u)?`${f} (open seat)`:f},i=u=>this.svc.groupLabelOf(u),r=Oc(t);this.expansion.retain(r.attached.keys());const d=sr(t,n,i,{plan:r,routeSections:e.routeSections,isOpen:u=>this.expansion.isOpen(u)}),l=nr(r.standalone,e.routeSections,n),c=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return d+c}}function Zc(s,e){if(!e)return[];const t=[],n=new Set;for(const i of s)for(const r of i.players){if(r.playerId===e)return[];r.guestPlayerId===null||n.has(r.guestPlayerId)||(n.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const eu=b(`
...
`);class su extends R{static styles=`
        .claim-card {
            margin-top: ${o("lg")};
            padding: ${o("lg")};
            ${P()}
    ...
    `,et.seq=0;let le=et;function iu(s){if(!s)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=s.seats.length>0,t=s.claimedSeats.some(r=>r.viewerMayRelease),n=s.viewer.claimSeat.allowed,i=s.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&n,guestAllowed:e&&i,blockedMessage:e&&!n&&!i?s.viewer.claimSeat.message??s.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function ru(s,e){const t=[];if(s.groupId!==null&&e.length>0){const n=e.findIndex(i=>i.id===s.groupId);if(n>=0){t.push(`Group ${n+1}`);const i=e[n].startTime;i.includes(":")&&t.push(i)}}return s.category!==null&&t.push(s.category),t.join(" · ")}function ou(s){return(s?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const au=b(`
...
`);class mu extends R{static styles=`
    ...
    `;svc=this.inject(de);auth=this.inject(j);router=this.inject(B);tokenQ=this.router.query("token");joining=new p(!1);error=new p("");diagnostics=new p([]);teeId=new p("");tees=new p([]);loadedForCourseId=null;groupChoice=new p("");policyState(){return hu(this.svc.startList.get())}eligible(){return this.policyState().visible&&uu(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,v.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const n=this.groupChoice.get(),i=await v.friendlyRounds.join({token:e,teeId:t,...n?{groupChoice:n}:{}});i.ok?await this.svc.loadByToken(e):this.diagnostics.set(i.diagnostics)}catch(n){this.error.set(n instanceof U&&n.status===409?n.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(I(()=>this.ensureTeesLoaded()));const e=new $(()=>pu(this.svc.groups(),this.svc.startList.get()?.viewer.createGroup.allowed??!0));this.track(I(()=>{const r=e.get(),d=this.groupChoice.get();(!d||!r.options.some(l=>l.value===d&&!l.disabled))&&this.groupChoice.set(r.defaultValue)}));const t=this.wire(fu,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},blocked:{className:()=>this.policyState().blockedMessage!==null?"join-card__blocked":"join-card__blocked hidden",textContent:()=>this.policyState().blockedMessage??""},groupRow:{className:()=>this.svc.groups().length>0&&this.policyState().blockedMessage===null?"join-card__group":"join-card__group hidden"},row:{className:()=>this.policyState().blockedMessage===null?"join-card__row":"join-card__row hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(r=>r.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),n=new le({value:this.teeId,options:{get:()=>this.tees.get().map(r=>({value:r.id,label:r.name}))},placeholder:"Tee"});n.mount(this.ref(t,"teeHost")),this.track(()=>n.destroy());const i=new le({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return i.mount(this.ref(t,"groupHost")),this.track(()=>i.destroy()),t}}const kn=1440*60*1e3;function gu(s,e){if(s===null)return!0;const t=new Date(s).getTime();return!Number.isFinite(t)||t-e>kn?!0:e-t>=kn}function bu(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}function _u(s){const e={visible:!1,index:null};return s.settled||!s.profileLoaded||!s.firstOpen||!bu(s.balls,s.playerId)||!gu(s.handicapConfirmedAt,s.now)?e:{visible:!0,index:s.handicapIndex}}function pe(s){const e=s.trim().replace(",",".");if(e==="")return null;const t=e.startsWith("+"),n=Number.parseFloat(t?e.slice(1):e);return Number.isFinite(n)?t?-n:n:null}function Rt(s){return s<0?`+${String(-s)}`:String(s)}const yu=b(`
...
`);class vu extends R{static styles=`
        .hcp-checkin {
            margin-bottom: ${o("lg")};
            padding: ${o("md")} ${o("lg")};
            ${P()}
    ...
    `;svc=this.inject(de);auth=this.inject(j);profile=this.inject(De);settled=new p(!1);editing=new p(!1);text=new p("");busy=new p(!1);error=new p("");state(){const e=this.profile.player.get();return _u({playerId:this.auth.currentUser.get()?.id??null,balls:this.svc.balls.get(),firstOpen:this.svc.firstOpen.get(),handicapConfirmedAt:e?.handicapConfirmedAt??null,handicapIndex:e?.handicapIndex??null,profileLoaded:e!==null,settled:this.settled.get(),now:Date.now()})}ensureProfileLoaded(){this.settled.get()||this.svc.firstOpen.get()&&this.auth.currentUser.get()&&this.svc.balls.get().length&&this.profile.load()}async confirm(){if(this.busy.get())return;this.error.set(""),this.busy.set(!0);const e=await this.profile.confirmHandicap();this.busy.set(!1),e?this.settled.set(!0):this.error.set("Could not save that right now. Try again.")}startEdit(){const e=this.profile.player.get()?.handicapIndex??null;this.text.set(e===null?"":Rt(e)),this.error.set(""),this.editing.set(!0)}async save(){if(this.busy.get())return;const e=pe(this.text.get());if(e===null){this.error.set("Enter a handicap index, e.g. 18.4 or +2.4.");return}this.error.set(""),this.busy.set(!0);const t=await this.profile.saveIndex(e);this.busy.set(!1),t?(this.editing.set(!1),this.settled.set(!0)):this.error.set("Could not save that right now. Try again.")}render(){return this.track(I(()=>this.ensureProfileLoaded())),this.wire(yu,{root:{className:()=>this.state().visible?"hcp-checkin":"hcp-checkin hidden"},ask:{className:()=>this.editing.get()?"hcp-checkin__ask hidden":"hcp-checkin__ask"},question:{textContent:()=>{const e=this.state().index;return e===null?"No handicap set — add one?":`Handicap ${Rt(e)} — still right?`}},confirm:{textContent:()=>this.state().index===null?"Not now":"Yes",disabled:()=>this.busy.get(),onclick:()=>{this.confirm()}},edit:{textContent:()=>this.state().index===null?"Add":"Update",disabled:()=>this.busy.get(),onclick:()=>this.startEdit()},editor:{className:()=>this.editing.get()?"hcp-checkin__editor":"hcp-checkin__editor hidden"},field:{value:()=>this.text.get(),oninput:e=>this.text.set(e.target.value)},save:{disabled:()=>this.busy.get(),onclick:()=>{this.save()}},cancel:{disabled:()=>this.busy.get(),onclick:()=>this.editing.set(!1)},err:{textContent:()=>this.error.get()}})}}function wu(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}const xu=b(`
...
`);class $u extends R{static styles=`
...
`);class Hu extends R{static styles=`
...
`);class gh extends R{static styles=`
        .story {
            ${P()}
            display: flex; flex-direction: column; gap: ${o("sm")};
            padding: ${o("lg")};
...
`),Th=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),Ih=b('<button bind="pill" class="round-view__grp" type="button"></button>');class Ph extends R{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class sp extends R{static styles=`
        .setup {
            padding: ${o("lg")} ${o("lg")} ${o("2xl")};
            ...
            & .player {
                ...
                & .player__name { ${Z()} flex: 1; padding: ${o("md")}; font-size: 1rem; }
                & .player__remove {
                    ${x()}
                ...
                & .player__index { ${Z()} flex: 1; min-width: 0; padding: ${o("md")}; font-size: 1rem; }
            ...
            & .fslot {
                ...
                & .mrow {
                    ...
                    & .mrow__pct {
                        ...
                        &[hidden] { display: none; }
                        & input { ${Z()} width: 56px; padding: ${o("xs")} ${o("sm")}; font-size: 0.95rem; }
                    }
                }
                ...
                & .grp__start {
                    ...
                    & .grp__time { ${Z()} flex: 1; min-width: 0; padding: ${o("sm")} ${o("md")}; font-size: 1rem; font-family: inherit; }
...
`);class fp extends R{static styles=`
        .fprofile {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
...
`);class gp extends R{static styles=`
        .frounds {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
...
`);class yp extends R{static styles=`
        .fcourses {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
...
`);class kp extends R{static styles=`
        .spectate {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
...
`);class Cp extends R{static styles=`
        .profile {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
            ...
            & .profile__card {
                ...
                & .profile__edit {
                    ...
                    & input { ${Z()} width: 90px; padding: ${o("md")}; font-size: 1rem; text-align: center; }
                    & button {
                        ${x()}
...
`),Lp=b('<div bind="panels" class="statspanels"></div>');class br extends R{static styles=`
        .statspanels {
            display: flex; flex-direction: column; gap: ${o("sm")};
...
`);class Vp extends R{static styles=`
        .stats {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
            ...
            & .rtile {
                ...
                & .rtile__label { font-size: 0.82rem; color: ${a("text-muted")}; }
                & .rtile__qualifier {
                    font-size: 0.72rem; color: ${a("text-muted")};
                    &:empty { display: none; }
                }
            }
...
`),Jp=b('<li bind="text" class="roundstats__legenditem"></li>');function Zp(){const s=[];for(const[e,t]of Object.entries(Bs))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
            `)}class ef extends R{static styles=`
        .roundstats {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
...
`);class mf extends R{static styles=`
        .comps {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
            ...
            & .comps__create {
                ...
                gap: ${o("sm")};
                margin-bottom: ${o("md")};
                & input { ${Z()} flex: 1; padding: ${o("md")}; font-size: 1rem; }
                & button {
                    ${x()}
                    padding: ${o("md")} ${o("lg")};
                    font-family: inherit; font-size: 0.95rem; font-weight: 700;
...
`);class Gf extends R{static styles=`
        .cd {
            padding: ${o("lg")} ${o("lg")} ${o("2xl")};
            ...
            & .cd__form {
                ...
                & .cd__field { display: flex; flex-direction: column; gap: ${o("xs")};
                    ...
                    & input, & select { ${Z()} padding: ${o("sm")} ${o("md")}; font-size: 0.95rem; }
                }
...
`);class Vf extends R{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(B);render(){const e=this.wire(qf,{header:{className:()=>Ts(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(Dd,this.ref(e,"account")),this.spawn(Vo,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":bn,"/history":rc,"/round":Ph,"/create":sp,"/login":lp,"/friends":hp,"/friend":fp,"/friend-rounds":gp,"/friend-courses":yp,"/spectate":kp,"/profile":Cp,"/stats":Vp,"/round-stats":ef,"/admin":cf,...fi.competitions?{"/competitions":mf,"/competition":Gf}:{}},bn),e}}class Uf extends j{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await pr(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await np()),this.error.set(null)}catch(e){e instanceof U&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await ip(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof U&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await rp();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof U&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}q.get(Lr);const Xn=q.get(B);q.set(j,new Uf);const Jn=q.get(j);await jr(Vf,"#app",{hot:void 0,onInit:async()=>{await Jn.load(),Jn.currentUser.get()&&Xn.route.get()==="/login"&&Xn.navigate("/",!0)}});export{ps as A,R as C,B as R,p as S,Lr as T,_ as a,tt as b,$ as c,Pr as d,I as e,Ir as n,F as r,b as t};
