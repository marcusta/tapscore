(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const Ne="modulepreload",He=function(o){return"/tapscore/"+o},ce={},Re=function(e,t,s){let n=Promise.resolve();if(t&&t.length>0){let d=function(u){return Promise.all(u.map(m=>Promise.resolve(m).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};document.getElementsByTagName("link");const l=document.querySelector("meta[property=csp-nonce]"),c=l?.nonce||l?.getAttribute("nonce");n=d(t.map(u=>{if(u=He(u),u in ce)return;ce[u]=!0;const m=u.endsWith(".css"),p=m?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${p}`))return;const g=document.createElement("link");if(g.rel=m?"stylesheet":Ne,m||(g.as="script"),g.crossOrigin="",g.href=u,c&&g.setAttribute("nonce",c),document.head.appendChild(g),m)return new Promise((y,j)=>{g.addEventListener("load",y),g.addEventListener("error",()=>j(new Error(`Unable to preload CSS for ${u}`)))})}))}function r(l){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=l,window.dispatchEvent(c),!c.defaultPrevented)throw l}return n.then(l=>{for(const c of l||[])c.status==="rejected"&&r(c.reason);return e().catch(r)})};class Le{constructor(){this.tracking=null,this.batching=!1,this.pending=new Set}subscribe(e){this.tracking&&(e.add(this.tracking),this.tracking.deps.add(e))}notify(e){for(const t of[...e])this.batching?this.pending.add(t):t.run()}runTracked(e,t){xe(e);const s=this.tracking;this.tracking=e;try{t()}finally{this.tracking=s}}batch(e){this.batching=!0;try{e()}finally{this.batching=!1;const t=[...this.pending];this.pending.clear();for(const s of t)s.run()}}}const B=new Le;function xe(o){for(const e of o.deps)e.delete(o);o.deps.clear()}class h{constructor(e){this.subs=new Set,this.val=e}get(){return B.subscribe(this.subs),this.val}set(e){Object.is(this.val,e)||(this.val=e,B.notify(this.subs))}update(e){this.set(e(this.val))}}class ${constructor(e){this.subs=new Set,this.val=void 0;const t=this,s={run(){B.runTracked(s,()=>{const n=e();Object.is(t.val,n)||(t.val=n,B.notify(t.subs))})},deps:new Set};s.run()}get(){return B.subscribe(this.subs),this.val}}function _(o){const e={run(){B.runTracked(e,o)},deps:new Set};return e.run(),()=>xe(e)}function W(o){B.batch(o)}class Me{constructor(){this.instances=new Map}get(e){let t=this.instances.get(e);return t||(t=new e,this.instances.set(e,t)),t}set(e,t){this.instances.set(e,t)}reset(){this.instances.clear()}}const O=new Me,G="/tapscore/".replace(/\/+$/,"");function te(o){return G?o===G?"/":o.startsWith(G+"/")?o.slice(G.length):o:o}function Be(o){return G+o}class z{constructor(){this.route=new h(te(location.pathname??"/")),this.search=new h(location.search??""),window.addEventListener("popstate",()=>W(()=>{this.route.set(te(location.pathname)),this.search.set(location.search)}))}navigate(e,t){const s=typeof t=="boolean"?{replace:t}:t??{},n=e.indexOf("#"),r=n>=0?e.slice(n):"",l=n>=0?e.slice(0,n):e,c=l.indexOf("?"),d=c>=0?l.slice(0,c):l,u=c>=0?l.slice(c+1):"",m=s.query!==void 0?Fe(s.query):u?"?"+u:"",p=Be(d)+m+r;(s.replace?history.replaceState:history.pushState).call(history,null,"",p),W(()=>{this.route.set(d),this.search.set(m)})}back(){history.back()}link(e,t="active"){const s=e.split("#")[0].split("?")[0];return{onclick:n=>{n.preventDefault(),this.navigate(e)},className:()=>{const n=this.route.get();return n===s||n.startsWith(s+"/")?t:""}}}params(e){const t=e.split("/");return new $(()=>{const s=this.route.get().split("/"),n={};for(const[r,l]of t.entries())l.startsWith(":")&&(n[l.slice(1)]=s[r]??"");return n})}query(e){return new $(()=>new URLSearchParams(this.search.get()).get(e)??void 0)}queries(){return new $(()=>{const e={};for(const[t,s]of new URLSearchParams(this.search.get()))e[t]=s;return e})}}function Fe(o){const e=new URLSearchParams;for(const[s,n]of Object.entries(o))n==null||n===""||e.set(s,String(n));const t=e.toString();return t?"?"+t:""}function Ae(o){return e=>o[e]}function De(o,e){const t=(n,r,l)=>{const c=Object.entries(n).map(([d,u])=>`--${d}:${u}`).join(";");return`${r}{color-scheme:${l};${c}}`},s=document.createElement("style");return s.textContent=t(o,'[data-theme="light"]',"light")+t(e,'[data-theme="dark"]',"dark"),document.head.appendChild(s),n=>`var(--${n})`}const de="basics-js-theme";class qe{constructor(){this.dark=new h(!1);const e=localStorage.getItem(de),t=matchMedia("(prefers-color-scheme: dark)").matches;this.dark.set(e?e==="dark":t),_(()=>{const s=this.dark.get();document.documentElement.setAttribute("data-theme",s?"dark":"light"),localStorage.setItem(de,s?"dark":"light")})}toggle(){this.dark.update(e=>!e)}}function b(o){const e=document.createElement("template");return e.innerHTML=o,e}function Ge(o,e){let t;for(const s of Object.keys(e))o.startsWith(s+"/")&&(!t||s.length>t.length)&&(t=s);return t?e[t]:void 0}const ue=new Set;class T{constructor(e={}){this.props=e,this.disposers=[],this.children=[];const t=this.constructor;if(t.styles&&!ue.has(t)){ue.add(t);const s=document.createElement("style");s.textContent=t.styles,document.head.appendChild(s)}}onMount(){}onDestroy(){}inject(e){return O.get(e)}track(e){this.disposers.push(e)}ref(e,t){return e.querySelector(`[bind="${t}"]`)}spawn(e,t,...s){const n=new e(s[0]);return n.mount(t),this.children.push(n),n}mount(e){e.appendChild(this.render()),this.onMount()}destroy(){this.onDestroy();for(const e of this.children)e.destroy();this.children.length=0;for(const e of this.disposers)e();this.disposers.length=0}wire(e,t,s){const n=s??(l=>this.track(l)),r=e.content.cloneNode(!0);for(const l of r.querySelectorAll("[bind]")){const c=t[l.getAttribute("bind")];if(c)if(typeof c=="function")n(_(()=>{const d=c();l instanceof HTMLInputElement||l instanceof HTMLTextAreaElement?l.value=String(d):l.textContent=String(d)}));else for(const[d,u]of Object.entries(c)){const m=d.includes("-");d.startsWith("on")&&typeof u=="function"?l.addEventListener(d.slice(2),u):typeof u=="function"?n(_(()=>{const p=u();m?l.setAttribute(d,String(p)):l[d]=p})):m?l.setAttribute(d,String(u)):l[d]=u}}return r}wireEl(e,t,s){return this.wire(e,t,s).firstElementChild}slot(e,t){const s=this.props[e];if(s==null)return!1;const n=this.ref(t,e);return n?(typeof s=="string"?n.textContent=s:typeof s=="function"&&s.prototype instanceof T?this.spawn(s,n):typeof s=="function"&&s(n,{spawn:(r,l,...c)=>this.spawn(r,l,...c),track:r=>this.track(r)}),!0):!1}$each(e,t,s,n=(r,l)=>l){const r=typeof t=="function"?t:()=>t.get(),l=new Map,c=new Map;this.track(()=>{for(const d of c.values())d.forEach(u=>u());c.clear()}),this.track(_(()=>{const d=r(),u=new Map;for(const[p,g]of d.entries()){const y=n(g,p);if(l.has(y))u.set(y,l.get(y));else{const j=[];u.set(y,s(g,p,q=>j.push(q))),c.set(y,j)}}for(const[p,g]of l)u.has(p)||(g.remove(),c.get(p)?.forEach(y=>y()),c.delete(p));let m=e.firstChild;for(const p of u.values())p===m?m=m.nextSibling:e.insertBefore(p,m);l.clear();for(const[p,g]of u)l.set(p,g)}))}$condition(e,t,s,n){let r=null;this.track(_(()=>{r&&(r.remove(),r=null),r=t.get()?s():n?.()??null,r&&e.appendChild(r)}))}$swap(e,t,s,n){let r=null;this.track(_(()=>{r&&(r.destroy(),r=null),e.textContent="";const l=t.get(),c=s[l]??Ge(l,s)??n;c&&(r=new c,r.mount(e))})),this.track(()=>r?.destroy())}}async function Ke(o,e,t){const s=document.querySelector(e);s.textContent="";const n=O.get(z);let r=null,l=!1,c=null,d=!!t?.hot?.data.hmr;const u=async m=>{r&&(r.destroy(),r=null,s.textContent=""),m?(c||(c=(await Re(()=>import("./obs-shell.component-O7FAJeUL.js"),[])).ObsShellComponent),r=new c):(!d&&t?.onInit&&(await t.onInit(),d=!0),r=new o),r.mount(s),l=m};await u(te(location.pathname).startsWith("/_obs")),_(()=>{const m=n.route.get().startsWith("/_obs");m!==l&&u(m)}),t?.hot&&(t.hot.data.hmr=!0,t.hot.dispose(()=>r?.destroy()),t.hot.accept())}class D extends Error{constructor(e,t,s,n){super(t),this.status=e,this.details=s,this.traceId=n,this.name="ApiError"}}const We=10,X=[];let V=[],K=null;function Ue(o){X.push(o),X.length>We&&X.shift()}function Qe(o,e,t){const s={code:o,message:e,url:typeof location<"u"?location.href:"",context:[...X],timestamp:new Date().toISOString()};t!==void 0&&(s.traceId=t),V.push(s),Xe()}function Xe(){K||(K=setTimeout(we,5e3))}function we(){if(K&&(clearTimeout(K),K=null),V.length===0)return;const o=V;V=[];for(const e of o){const t=JSON.stringify(e);typeof navigator<"u"&&navigator.sendBeacon?navigator.sendBeacon("/api/_obs/errors",new Blob([t],{type:"application/json"})):typeof fetch<"u"&&fetch("/api/_obs/errors",{method:"POST",headers:{"Content-Type":"application/json"},body:t}).catch(()=>{})}}typeof document<"u"&&document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&we()});const Ve=3e4,Ye=2,U=new Map,$e=new WeakMap;function Ze(o){if(o instanceof D)return o.traceId;if(o!=null&&typeof o=="object")return $e.get(o)}async function f(o){if(o.method==="GET"){const e=U.get(o.url);if(e)return e;const t=he(o,Ye);return U.set(o.url,t),t.then(()=>U.delete(o.url),()=>U.delete(o.url)),t}return he(o,0)}async function he(o,e){const t=o.timeout??Ve;let s;for(let n=0;n<=e;n++){const r=crypto.randomUUID();try{return await et(Je(o,r),t)}catch(l){if(s=l,!(l instanceof D)&&l!=null&&typeof l=="object"&&$e.set(l,r),l instanceof D||n===e)break;await new Promise(c=>setTimeout(c,1e3*2**n))}}throw s}async function Je(o,e){const t={"X-Trace-Id":e},s={method:o.method,headers:t};o.body!==void 0&&(t["Content-Type"]="application/json",s.body=JSON.stringify(o.body));const n=await fetch(o.url,s),r=n.headers.get("x-trace-id")??e;if(Ue({type:"api",detail:`${o.method} ${o.url}`,timestamp:new Date().toISOString()}),!n.ok){const l=await n.json().catch(()=>({error:n.statusText}));throw new D(n.status,l.error??n.statusText,l.details,r)}return n.json()}function et(o,e){let t;const s=new Promise((n,r)=>{t=setTimeout(()=>r(new Error("Request timeout")),e)});return Promise.race([o,s]).finally(()=>clearTimeout(t))}async function w(o,e,t){W(()=>{o.set(!0),e.set(null)});try{const s=await t();return o.set(!1),s}catch(s){const n=tt(s);W(()=>{o.set(!1),e.set(n)}),Qe(n.code,n.message,Ze(s));return}}function tt(o){return o instanceof D?o.status===401?{code:"auth",message:"Unauthorized"}:o.status===409?{code:"conflict",message:"Data has changed — please try again"}:o.status===400?{code:"validation",message:o.message}:{code:"server",message:"Server error"}:o instanceof Error?o.message==="Request timeout"?{code:"timeout",message:"Request timeout"}:{code:"network",message:"Network error"}:{code:"unknown",message:"Unknown error"}}function st(o){return{me:()=>f({method:"GET",url:`${o}/auth/me`}),login:e=>f({method:"POST",url:`${o}/auth/login`,body:e}),logout:()=>f({method:"POST",url:`${o}/auth/logout`,body:{}})}}class re{constructor(){this.api=st("/api"),this.currentUser=new h(null),this.loading=new h(!1),this.error=new h(null)}async load(){const e=await w(this.loading,this.error,()=>this.api.me());e&&this.currentUser.set(e),this.error.get()?.code==="auth"&&this.error.set(null)}async login(e,t){const s=await w(this.loading,this.error,()=>this.api.login({username:e,password:t}));return s?(this.currentUser.set(s),!0):!1}async logout(){await w(this.loading,this.error,()=>this.api.logout());const e=this.error.get();(!e||e.code==="auth")&&this.currentUser.set(null)}}const pe={radius:"12px","radius-pill":"999px","radius-sm":"6px","font-display":"'Fraunces', Georgia, serif",shadow:"0 1px 2px rgba(30, 53, 38, 0.08)","shadow-elevated":"0 4px 16px rgba(30, 53, 38, 0.14)"},i=De({...pe,bg:"#f2eee2",surface:"#fbf9f1","surface-sunken":"#e9e4d4",primary:"#2c5e3f","primary-text":"#f7f4ea","btn-bg":"#fbf9f1","btn-hover":"#efeada",text:"#1e3526","text-muted":"#6b7a6e",border:"#d8d2bf","topbar-bg":"#1e3526","active-bg":"#1e3526","active-text":"#f7f4ea","hover-bg":"#ece7d7","input-bg":"#ffffff",accent:"#b08d3e","accent-soft":"#f0e6cd",error:"#a0463c","under-par":"#a0463c","over-par":"#345b8a","hole-bar":"#e6a23f","hole-bar-text":"#3a2a0d"},{...pe,bg:"#15231a",surface:"#1d2f22","surface-sunken":"#101b14",primary:"#5d9b75","primary-text":"#0f1a13","btn-bg":"#24392b","btn-hover":"#2e4836",text:"#e6e1d2","text-muted":"#8da093",border:"#33493a","topbar-bg":"#0f1a13","active-bg":"#5d9b75","active-text":"#0f1a13","hover-bg":"#273c2e","input-bg":"#101b14",accent:"#cfa84f","accent-soft":"#3a3320",error:"#d48a82","under-par":"#d48a82","over-par":"#8db2e0","hole-bar":"#c08a35","hole-bar-text":"#160f04",shadow:"0 1px 2px rgba(0, 0, 0, 0.3)","shadow-elevated":"0 4px 16px rgba(0, 0, 0, 0.4)"}),I=o=>`var(--${o})`,a=Ae({xs:"0.25rem",sm:"0.5rem",md:"0.75rem",lg:"1rem",xl:"1.5rem","2xl":"2rem"}),k=(o=I("radius"))=>`
...
`);class rt extends T{static styles=`
        .tabbar {
            display: flex;
...
`),xt={not_started:"Not started",active:"Live",complete:"Done"};class me extends T{static styles=`
        .landing {
    ...
    `;svc=this.inject(yt);router=this.inject(z);render(){this.svc.load();const e=this.wire(_t,{createBtn:{onclick:()=>this.router.navigate("/create")},signin:{onclick:()=>this.router.navigate("/login")},count:()=>{const t=this.svc.rounds.get().length;return t===0?"":`${t} on the card`},empty:{className:()=>this.svc.rounds.get().length===0?"landing__empty":"landing__empty hidden"}});return this.$each(this.ref(e,"list"),this.svc.rounds,(t,s,n)=>this.wireEl(vt,{row:{onclick:()=>this.router.navigate("/round",{query:{token:t.friendlyRound.shareToken}})},course:()=>t.round.courseNameSnapshot??"Round",status:{textContent:()=>xt[t.round.status]??t.round.status,className:()=>`round-row__status s-${t.round.status}`},date:()=>t.round.date,formats:()=>t.round.formatSlots.map(oe).join(" · ")},n),t=>t.friendlyRound.id),e}}const wt=180,fe=4,$t=12;function A(o,e){return e<=0?0:Math.max(0,Math.min(e-1,o))}function kt(o){const{dragDistance:e,velocity:t,itemWidth:s}=o;if(Math.abs(e)<$t)return 0;const n=e+t*wt,r=Math.round(-n/s);return Math.max(-fe,Math.min(fe,r))}const St=["1st","2nd","3rd","4th","5th","6th","7th","8th"],F=(o,e)=>`${o}|${e}`;function ke(o){return o.players.map(e=>e.displayName).join(" & ")||o.label||"Ball"}function It(o,e,t){return o?!(o.minPar!==void 0&&e<o.minPar||o.maxPar!==void 0&&e>o.maxPar||o.pars&&!o.pars.includes(e)||o.holes&&!o.holes.includes(t)):!0}class ie{loading=new h(!1);error=new h(null);friendlyRound=new h(null);round=new h(null);balls=new h([]);scorecards=new h([]);cells=new h(new Map);result=new h(null);resultLoading=new h(!1);resultError=new h(null);holeIdx=new h(0);groupIdx=new h(0);token=null;async loadByToken(e){const t=e!==this.token;this.token=e,O.get(Y).load();const s=await w(this.loading,this.error,()=>x.friendlyRounds.byToken({token:e}));if(!s)return;this.friendlyRound.set(s.friendlyRound),this.round.set(s.round);const[n,r]=await Promise.all([x.friendlyRounds.balls({token:e}).catch(()=>[]),x.friendlyRounds.scorecard({token:e}).catch(()=>[])]);this.cells.set(new Map),this.scorecards.set(r),this.balls.set(n),t&&(this.holeIdx.set(0),this.groupIdx.set(0),this.result.set(null))}async loadResult(){if(!this.token)return;const e=await w(this.resultLoading,this.resultError,()=>x.friendlyRounds.result({token:this.token}));e&&this.result.set(e)}ballNameById=new $(()=>{const e=new Map;for(const t of this.balls.get())e.set(t.id,ke(t));return e});nameOf(e){return this.ballNameById.get().get(e)??e}groups(){return this.round.get()?.playingGroups??[]}group(){const e=this.groups();return e[this.groupIdx.get()]??e[0]??null}playedOrder(){return this.group()?.playedOrder??[]}holeIndex(){return A(this.holeIdx.get(),this.playedOrder().length)}currentPlayedHole(){return this.playedOrder()[this.holeIndex()]??null}playHoleById(e){return this.round.get()?.playHoles.find(t=>t.id===e)??null}currentPlayHole(){const e=this.currentPlayedHole();return e?this.playHoleById(e.playHoleId):null}parFor(e){return(e?this.playHoleById(e)?.par:null)??4}occLabel(e){const t=this.round.get(),s=t?.playHoles.find(l=>l.id===e);if(!t||!s)return"";const n=t.playHoles.filter(l=>l.courseHoleNumber===s.courseHoleNumber).sort((l,c)=>l.ordinal-c.ordinal);if(n.length===1)return`${s.courseHoleNumber}`;const r=n.findIndex(l=>l.id===e);return`${s.courseHoleNumber} (${St[r]??`${r+1}th`})`}canPrevHole(){return this.holeIndex()>0}canNextHole(){return this.holeIndex()<this.playedOrder().length-1}prevHole(){this.holeIdx.set(A(this.holeIndex()-1,this.playedOrder().length))}nextHole(){this.holeIdx.set(A(this.holeIndex()+1,this.playedOrder().length))}strokesFor(e,t){const s=this.cells.get().get(F(e,t));return s?s.strokes:this.scorecards.get().find(l=>l.ballId===e)?.holes.find(l=>l.playHoleId===t)?.strokes??null}statusFor(e,t){return this.cells.get().get(F(e,t))?.status??null}metadataFor(e,t,s){const n=this.cells.get().get(F(e,t));return n&&n.metadata!==void 0?n.metadata?.[s]:this.scorecards.get().find(c=>c.ballId===e)?.holes.find(c=>c.playHoleId===t)?.metadata?.[s]}metadataInputs(){const e=O.get(Y),t=this.round.get()?.formatSlots??[],s=[],n=new Set;for(const r of t){const l=e.byId(r.formatId)?.requirements.scoreEntry?.metadata??[];for(const c of l)n.has(c.key)||(n.add(c.key),s.push(c))}return s}metadataInputsForHole(e){return e?this.metadataInputs().filter(t=>It(t.appliesWhen,e.par,e.courseHoleNumber)):[]}async setScore(e,t,s,n){const r=F(e,t),l=crypto.randomUUID();this.patchCell(r,{strokes:s,metadata:n,status:"saving",clientEventId:l}),await this.post(e,t,s,n,l)}async retry(e,t){const s=F(e,t),n=this.cells.get().get(s);n&&(this.patchCell(s,{...n,status:"saving"}),await this.post(e,t,n.strokes,n.metadata,n.clientEventId))}async post(e,t,s,n,r){if(!this.token)return;const l=F(e,t);try{await x.friendlyRounds.score({token:this.token,ballId:e,playHoleId:t,strokes:s,eventType:s===null?"score_cleared":"score_entered",clientEventId:r,...n!=null?{metadata:n}:{}});const c=this.cells.get().get(l);c&&c.clientEventId===r&&this.patchCell(l,{...c,status:"saved"})}catch{const c=this.cells.get().get(l);c&&c.clientEventId===r&&this.patchCell(l,{...c,status:"error"})}}patchCell(e,t){const s=new Map(this.cells.get());s.set(e,t),this.cells.set(s)}}const L=60,ge=8,se=4,Tt=Array.from({length:se*2+1},(o,e)=>e-se),Et="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",Pt=b(`
...
`),Ot=b('<button bind="chip" class="se-chip" type="button"></button>');class Nt extends T{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
...
</div>`}function At(o,e){const t=o.matches.map(s=>{const n=v(s.sideA.ballIds.map(e).join(" & ")),r=v(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,c=s.finished?"Final":`thru ${s.thru}`,d=s.leader==="a"?" lb-mp__team--lead":"",u=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
    <div class="lb-mp__team lb-mp__team--a${d}">${n}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${v(l)}</span><span class="lb-mp__status">${v(c)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${u}">${r}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${v(o.title)}</h4>${t}
</div>`}function Dt(o){return`<div class="lb-diag">Unrenderable result section <code>${v(o)}</code> — no generic view yet. Results are not hidden.</div>`}function qt(o,e){return o.leaderboard.length===0&&o.cards.length===0?`<div class="lb-empty">No scores entered yet for ${v(o.formatLabel)}.</div>`:o.leaderboard.map(s=>s.kind==="ranked"?Ft(s,e):s.kind==="match_summary"?At(s,e):Dt(s.kind)).join("")||`<div class="lb-empty">No leaderboard metric for ${v(o.formatLabel)}.</div>`}function Gt(o,e,t){return o.cards.length===0?"":o.cards.map(s=>Bt(s,e,t)).join(`
`)}const Kt=b(`
...
`),Wt=b('<button bind="pill" class="lb__pill" type="button"></button>');class Ut extends T{static styles=`
        .lb {
            ...
            & .lb-mp {
                ...
                border: 1px solid ${i("border")}; border-radius: 10px; overflow: hidden;
                margin-top: ${a("sm")};
            }
            & .lb-mp__team {
            ...
            & .lb-mp__status { font-size: 0.62rem; text-transform: uppercase; letter-spacing: 0.04em; color: ${i("text-muted")}; }
            ...
            & .lb-grid .lb-r-cat th { font-weight: 400; color: ${i("text-muted")}; }
            & .lb-grid .lb-c-cat { text-align: center; color: ${i("accent")}; }
            ...
            & .lb-card__caption { margin: ${a("sm")} 0 0; font-size: 0.72rem; font-style: italic; color: ${i("text-muted")}; }
    ...
    `;svc=this.inject(ie);selected=new h(0);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots();return e[this.selected.get()]??e[0]??null};render(){this.track(_(()=>{const s=this.slots().length;s>0&&this.selected.get()>=s&&this.selected.set(0)}));const e=this.wire(Kt,{status:{className:()=>{const s=this.svc.resultLoading.get(),n=this.svc.result.get()===null;return s||n?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}}),t=this.ref(e,"selector");return this.track(_(()=>{t.className=this.slots().length>1?"lb__selector":"lb__selector hidden"})),this.$each(t,new $(()=>this.slots().length>1?this.slots():[]),(s,n,r)=>this.slotPill(s,n,r),s=>s.slotDefId),e}slotPill(e,t,s){return this.wireEl(Wt,{pill:{textContent:e.formatLabel,className:()=>this.selected.get()===t?"lb__pill active":"lb__pill",onclick:()=>this.selected.set(t)}},s)}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=c=>this.svc.nameOf(c),n=qt(t,s),r=Gt(t,e.routeSections,s),l=r?`<h3 class="lb-cards__head">Scorecard</h3>${r}`:"";return n+l}}const Qt=b(`
...
`);class Xt extends T{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
    ...
    `;svc=this.inject(ie);router=this.inject(z);tokenQ=this.router.query("token");tab=new h("score");hasRound=new $(()=>this.svc.round.get()!==null);hasScoring=new $(()=>this.svc.balls.get().length>0);shareUrl=new $(()=>{const e=this.tokenQ.get();return e?`${location.origin}/round?token=${e}`:""});render(){this.track(_(()=>{const s=this.tokenQ.get();s&&this.svc.loadByToken(s)}));const e={not_started:"Not started",active:"Live",complete:"Done"},t=this.wire(Qt,{back:{onclick:()=>this.router.navigate("/")},notfound:{className:()=>!this.hasRound.get()&&!this.svc.loading.get()?"round-view__notfound":"round-view__notfound hidden"},body:{className:()=>this.hasRound.get()?"round-view__body":"round-view__body hidden"},course:()=>this.svc.round.get()?.courseNameSnapshot??"Round",status:()=>{const s=this.svc.round.get()?.status??"not_started";return e[s]??s},date:()=>this.svc.round.get()?.date??"",route:()=>{const s=this.svc.round.get();return s?`${s.playHoles.length} holes`:""},formats:{innerHTML:()=>(this.svc.round.get()?.formatSlots??[]).map(s=>`<span class="fmt">${oe(s)}</span>`).join("")},scorePanel:{className:()=>this.tab.get()==="score"?"round-view__panel":"round-view__panel hidden"},lbPanel:{className:()=>this.tab.get()==="leaderboard"?"round-view__panel":"round-view__panel hidden"},shareUrl:{value:()=>this.shareUrl.get()},copy:{onclick:()=>{navigator.clipboard?.writeText(this.shareUrl.get())}},dock:{className:()=>this.hasRound.get()?"round-view__dock":"round-view__dock hidden"},holebar:{className:()=>this.tab.get()==="score"&&this.hasScoring.get()?"round-hole":"round-hole hidden"},holePar:()=>String(this.svc.parFor(this.svc.currentPlayedHole()?.playHoleId??null)),holeNum:()=>{const s=this.svc.currentPlayedHole();return s?this.svc.occLabel(s.playHoleId):""},holeSi:()=>{const s=this.svc.currentPlayHole()?.baseStrokeIndex;return s!=null?String(s):"–"},holePrev:{onclick:()=>this.svc.prevHole(),disabled:()=>!this.svc.canPrevHole()},holeNext:{onclick:()=>this.svc.nextHole(),disabled:()=>!this.svc.canNextHole()},tabScore:{className:()=>this.tab.get()==="score"?"round-tabs__tab active":"round-tabs__tab",onclick:()=>this.tab.set("score")},tabBoard:{className:()=>this.tab.get()==="leaderboard"?"round-tabs__tab active":"round-tabs__tab",onclick:()=>{this.tab.set("leaderboard"),this.svc.loadResult()}}});return this.spawn(Nt,this.ref(t,"scoring")),this.spawn(Ut,this.ref(t,"leaderboard")),t}}function M(o){return typeof o=="object"&&o!==null&&typeof o.get=="function"}const S=o=>`var(--${o})`,ae=class ae extends T{constructor(){super(...arguments),this.open=new h(!1),this.highlightIndex=new h(-1),this.optionEls=[],this.onOutsidePointer=e=>{this.wrapperEl.contains(e.target)||this.open.set(!1)}}render(){const e=document.createElement("div");e.className="ui-select",this.wrapperEl=e;const t=this.props.zIndex??50;this.triggerEl=document.createElement("button"),this.triggerEl.className="ui-select__trigger",this.triggerEl.setAttribute("type","button"),this.triggerEl.setAttribute("role","combobox"),this.triggerEl.setAttribute("aria-haspopup","listbox");const s=document.createElement("span");s.className="ui-select__trigger-label",this.triggerEl.appendChild(s);const n=document.createElement("span");n.className="ui-select__chevron",n.textContent="▾",n.setAttribute("aria-hidden","true"),this.triggerEl.appendChild(n),this.triggerEl.addEventListener("click",l=>{l.stopPropagation(),this.toggle()}),this.triggerEl.addEventListener("keydown",l=>{this.handleTriggerKeydown(l)}),e.appendChild(this.triggerEl),this.dropdownEl=document.createElement("div"),this.dropdownEl.className="ui-select__dropdown",this.dropdownEl.setAttribute("role","listbox"),this.dropdownEl.style.zIndex=String(t),this.dropdownEl.addEventListener("keydown",l=>{this.handleDropdownKeydown(l)}),e.appendChild(this.dropdownEl);const r=l=>{this.optionEls=[],this.dropdownEl.textContent="";for(let c=0;c<l.length;c++){const d=l[c],u=document.createElement("button");if(u.className="ui-select__option",u.setAttribute("type","button"),u.id=`ui-select-opt-${c}`,d.disabled){u.classList.add("ui-select__option--header"),u.disabled=!0,u.setAttribute("role","presentation"),u.setAttribute("aria-disabled","true");const g=document.createElement("span");g.className="ui-select__option-label",g.textContent=d.label,u.appendChild(g),this.dropdownEl.appendChild(u),this.optionEls.push(u);continue}if(u.setAttribute("role","option"),d.icon){const g=document.createElement("span");g.className="ui-select__option-icon",g.textContent=d.icon,u.appendChild(g)}const m=document.createElement("span");m.className="ui-select__option-label",m.textContent=d.label,u.appendChild(m);const p=document.createElement("span");p.className="ui-select__check",p.setAttribute("aria-hidden","true"),u.appendChild(p),u.addEventListener("click",g=>{g.stopPropagation(),this.selectOption(d.value)}),u.addEventListener("mouseenter",()=>{this.highlightIndex.set(c)}),this.dropdownEl.appendChild(u),this.optionEls.push(u)}};return M(this.props.options)?this.track(_(()=>{const l=M(this.props.options)?this.props.options.get():this.props.options;r(l)})):r(this.props.options),this.track(_(()=>{const l=this.props.value.get(),c=M(this.props.options)?this.props.options.get():this.props.options,d=c.find(u=>u.value===l);d?(s.textContent=d.icon?`${d.icon} ${d.label}`:d.label,this.triggerEl.classList.remove("ui-select__trigger--placeholder")):(s.textContent=this.props.placeholder??"",this.triggerEl.classList.toggle("ui-select__trigger--placeholder",!!this.props.placeholder));for(let u=0;u<c.length;u++){const m=this.optionEls[u];if(!m)continue;const p=c[u].value===l;m.setAttribute("aria-selected",String(p)),m.classList.toggle("ui-select__option--selected",p);const g=m.querySelector(".ui-select__check");g&&(g.textContent=p?"✓":"")}})),this.track(_(()=>{const l=this.open.get();if(this.dropdownEl.classList.toggle("open",l),n.classList.toggle("ui-select__chevron--open",l),this.triggerEl.setAttribute("aria-expanded",String(l)),l?document.addEventListener("pointerdown",this.onOutsidePointer,!0):document.removeEventListener("pointerdown",this.onOutsidePointer,!0),l){const c=M(this.props.options)?this.props.options.get():this.props.options,d=this.props.value.get(),u=c.findIndex(p=>p.value===d),m=c.findIndex(p=>!p.disabled);this.highlightIndex.set(u>=0?u:m)}})),this.track(_(()=>{const l=this.highlightIndex.get();for(let c=0;c<this.optionEls.length;c++)this.optionEls[c].classList.toggle("ui-select__option--highlighted",c===l);l>=0&&this.optionEls[l]&&(this.triggerEl.setAttribute("aria-activedescendant",`ui-select-opt-${l}`),this.optionEls[l].scrollIntoView({block:"nearest"}))})),this.props.disabled!=null&&(M(this.props.disabled)?this.track(_(()=>{const l=this.props.disabled.get();this.triggerEl.classList.toggle("ui-select__trigger--disabled",l),this.triggerEl.disabled=l})):this.props.disabled&&(this.triggerEl.classList.add("ui-select__trigger--disabled"),this.triggerEl.disabled=!0)),e}toggle(){this.open.update(e=>!e)}selectOption(e){W(()=>{this.props.value.set(e),this.open.set(!1)}),this.triggerEl.focus()}handleTriggerKeydown(e){switch(e.key){case"Enter":case" ":e.preventDefault(),this.toggle();break;case"ArrowDown":e.preventDefault(),this.open.get()?this.moveHighlight(1):this.open.set(!0);break;case"ArrowUp":e.preventDefault(),this.open.get()?this.moveHighlight(-1):this.open.set(!0);break;case"Escape":this.open.get()&&(e.preventDefault(),this.open.set(!1));break}}handleDropdownKeydown(e){switch(e.key){case"ArrowDown":e.preventDefault(),this.moveHighlight(1);break;case"ArrowUp":e.preventDefault(),this.moveHighlight(-1);break;case"Enter":case" ":{e.preventDefault();const t=this.highlightIndex.get(),s=M(this.props.options)?this.props.options.get():this.props.options;t>=0&&t<s.length&&!s[t].disabled&&this.selectOption(s[t].value);break}case"Escape":e.preventDefault(),this.open.set(!1),this.triggerEl.focus();break;case"Tab":this.open.set(!1);break}}moveHighlight(e){const t=M(this.props.options)?this.props.options.get():this.props.options;if(t.length===0||!t.some(n=>!n.disabled))return;let s=this.highlightIndex.get();do s+=e,s<0&&(s=t.length-1),s>=t.length&&(s=0);while(t[s].disabled);this.highlightIndex.set(s)}onDestroy(){document.removeEventListener("pointerdown",this.onOutsidePointer,!0)}};ae.styles=`
        .ui-select {
            position: relative;
...
`);class as extends T{static styles=`
        .setup {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
            ...
            & .player {
                ...
                & .player__name { flex: 1; padding: ${a("md")}; font-size: 1rem; ${N()} }
                & .player__remove {
                ...
                & .player__index { flex: 1; min-width: 0; padding: ${a("md")}; font-size: 1rem; ${N()} }
            ...
            & .fslot {
                ...
                & .fslot__teammeta {
                    margin: ${a("xs")} 0 0; font-size: 0.78rem; color: ${i("text-muted")};
                    &:empty { display: none; }
                }
                ...
                & .mrow {
                    ...
                    & .mrow__pct {
                        ...
                        &[hidden] { display: none; }
                        & input { width: 56px; padding: ${a("xs")} ${a("sm")}; ${N()} font-size: 0.95rem; }
                    }
                }
                ...
                & .fslot__flat {
                    ...
                    color: ${i("text-muted")};
                    &[hidden] { display: none; }
                    & .fslot__pct { width: 70px; padding: ${a("sm")}; font-size: 1rem; ${N()} }
                }
                & .fslot__bands {
                    display: flex; flex-direction: column; gap: ${a("xs")};
                    &[hidden] { display: none; }
                ...
                & .brow {
                    ...
                    & .brow__pct, & .brow__upto { width: 56px; padding: ${a("sm")}; font-size: 0.95rem; ${N()} }
...
`);class cs extends T{static styles=`
        .login {
            max-width: 340px;
            margin: 0 auto;
            padding: 18vh ${a("xl")} 0;
    ...
    `;auth=this.inject(re);router=this.inject(z);username="";password="";render(){return this.wire(ls,{root:{inert:()=>this.auth.loading.get()},error:{className:()=>this.auth.error.get()?"error show":"error",textContent:()=>this.auth.error.get()?.message??""},form:{onsubmit:async e=>{e.preventDefault(),await this.auth.login(this.username,this.password)&&this.router.navigate("/rounds",!0)}},username:{oninput:e=>{this.username=e.target.value}},password:{oninput:e=>{this.password=e.target.value}},submit:{textContent:()=>this.auth.loading.get()?"Signing in…":"Sign in"}})}}class ds{loading=new h(!1);error=new h(null);guests=new h([]);async load(){const e=await w(this.loading,this.error,()=>x.guestPlayers.list());e&&this.guests.set(e)}async create(e){const t=await w(this.loading,this.error,()=>x.guestPlayers.create(e));return t&&this.guests.update(s=>[...s,t]),t??null}}const us=b(`
    <div class="players">
...
`);function ps(o){return o.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class ms extends T{static styles=`
        .players {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class ys extends T{static styles=`
    ...
    `;svc=this.inject(fs);router=this.inject(z);render(){this.svc.load();const e=this.wire(gs,{subtitle:()=>{const s=this.svc.rounds.get().length;return s===0?"No rounds yet — tee one up.":`${s} round${s===1?"":"s"} on the card.`},newBtn:{onclick:()=>this.router.navigate("/create")}}),t={not_started:"Not started",active:"Live",complete:"Done"};return this.$each(this.ref(e,"list"),this.svc.rounds,(s,n,r)=>this.wireEl(bs,{row:{onclick:()=>this.router.navigate("/score",{query:{roundId:s.id}})},course:()=>s.courseNameSnapshot??"Round",status:{textContent:()=>t[s.status]??s.status,className:()=>`round-row__status s-${s.status}`},date:()=>s.date,formats:()=>s.formatSlots.map(oe).join(" · ")},r),s=>s.id),e}}class _s{loading=new h(!1);error=new h(null);roundId=new h(null);round=new h(null);course=new h(null);balls=new h([]);strokes=new h(new Map);holes=new $(()=>this.course.get()?.holes??[]);playHoleIdByCourseHole=new $(()=>{const e=new Map;for(const t of this.round.get()?.playHoles??[])e.has(t.courseHoleNumber)||e.set(t.courseHoleNumber,t.id);return e});async load(e){if(this.roundId.get()===e&&this.round.get())return;this.roundId.set(e);const t=await w(this.loading,this.error,()=>x.rounds.get({id:e}));if(!t)return;this.round.set(t);const[s,n,r]=await Promise.all([w(this.loading,this.error,()=>x.courses.get({id:t.courseId})),w(this.loading,this.error,()=>x.rounds.balls({roundId:e})),w(this.loading,this.error,()=>x.scorecards.forRound({roundId:e}))]);if(s&&this.course.set(s),n&&this.balls.set(n),r){const l=new Map;for(const c of r)for(const d of c.holes)l.set(`${c.ballId} ${d.holeNumber}`,d.strokes);this.strokes.set(l)}}strokesFor(e,t){return this.strokes.get().get(`${e} ${t}`)??null}async setStrokes(e,t,s){const n=this.roundId.get();if(!n)return;const r=this.playHoleIdByCourseHole.get().get(t);if(!r)return;this.strokes.update(c=>new Map(c).set(`${e.id} ${t}`,s));const l=e.players.length===1?e.players[0]:null;await w(this.loading,this.error,()=>x.scoreEvents.append({roundId:n,ballId:e.id,playHoleId:r,strokes:s,eventType:s===null?"score_cleared":"score_entered",clientEventId:crypto.randomUUID(),sourcePlayerId:l?.playerId??null,sourceGuestPlayerId:l?.guestPlayerId??null}))}}const vs=b(`
    <div class="score">
...
`),xs=b('<button bind="dot" type="button" class="score-dot"></button>'),ws=b(`
    <div class="ball-row">
...
`);class $s extends T{static styles=`
        .score {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(_s);router=this.inject(z);holeQ=this.router.query("hole");roundIdQ=this.router.query("roundId");hole=new $(()=>{const e=Number(this.holeQ.get()??"1");return Number.isFinite(e)&&e>=1?e:1});currentHole=new $(()=>this.svc.holes.get().find(e=>e.holeNumber===this.hole.get())??null);goHole(e){this.router.navigate("/score",{query:{roundId:this.roundIdQ.get(),hole:String(e)}})}render(){this.track(_(()=>{const t=this.roundIdQ.get();t&&this.svc.load(t)}));const e=this.wire(vs,{back:{onclick:()=>this.router.navigate("/rounds")},results:{onclick:()=>this.router.navigate("/results",{query:{roundId:this.roundIdQ.get()}})},course:()=>this.svc.round.get()?.courseNameSnapshot??"",holeNo:()=>String(this.hole.get()),holemeta:()=>{const t=this.currentHole.get();return t?`Par ${t.par} · Index ${t.strokeIndex}`:""},prev:{disabled:()=>this.hole.get()<=1,onclick:()=>this.goHole(this.hole.get()-1)},next:{disabled:()=>this.hole.get()>=(this.svc.holes.get().length||18),onclick:()=>this.goHole(this.hole.get()+1)}});return this.$each(this.ref(e,"dots"),this.svc.holes,(t,s,n)=>this.wireEl(xs,{dot:{className:()=>{const r=this.svc.balls.get().length>0&&this.svc.balls.get().every(c=>this.svc.strokesFor(c.id,t.holeNumber)!==null),l=this.hole.get()===t.holeNumber;return`score-dot${r?" done":""}${l?" now":""}`},onclick:()=>this.goHole(t.holeNumber)}},n),t=>String(t.holeNumber)),this.$each(this.ref(e,"balls"),this.svc.balls,(t,s,n)=>this.ballRow(t,n),t=>t.id),e}ballRow(e,t){const s=()=>this.svc.strokesFor(e.id,this.hole.get()),n=()=>this.currentHole.get()?.par??4;return this.wireEl(ws,{label:()=>e.label??e.players.map(r=>r.displayName).join(" / "),meta:()=>e.players.map(r=>`${r.teeName} · CH ${r.courseHandicap}`).join("  ·  "),minus:{onclick:()=>{const r=s();r===null?this.svc.setStrokes(e,this.hole.get(),n()):r<=1?this.svc.setStrokes(e,this.hole.get(),null):this.svc.setStrokes(e,this.hole.get(),r-1)}},plus:{onclick:()=>{const r=s();this.svc.setStrokes(e,this.hole.get(),r===null?n():r+1)}},value:{textContent:()=>{const r=s();return r===null?"–":String(r)},className:()=>{const r=s();return r===null?"ball-row__value unset":r<n()?"ball-row__value under":r>n()?"ball-row__value over":"ball-row__value"}}},t)}}class ks{loading=new h(!1);error=new h(null);roundId=new h(null);round=new h(null);result=new h(null);balls=new h([]);labelByBall=new $(()=>{const e=new Map;for(const t of this.balls.get())e.set(t.id,t.label??t.players.map(s=>s.displayName).join(" / "));return e});async load(e){this.roundId.set(e);const[t,s]=await Promise.all([w(this.loading,this.error,()=>x.rounds.get({id:e})),w(this.loading,this.error,()=>x.rounds.balls({roundId:e}))]);t&&this.round.set(t),s&&this.balls.set(s);const n=await w(this.loading,this.error,()=>x.leaderboards.forRound({roundId:e}));n&&this.result.set(n)}}const Ss=b(`
    <div class="results">
...
`),Is=b('<div bind="row" class="results-slot"></div>');class Ts extends T{static styles=`
        .results {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class Ps extends T{static styles=`
    ...
    `;router=this.inject(z);render(){const e=this.wire(Es,{});return this.spawn(rt,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":me,"/round":Xt,"/create":as,"/login":cs,"/rounds":ys,"/players":ms,"/score":$s,"/results":Ts},me),e}}O.get(qe);const _e=O.get(z),ve=O.get(re);await Ke(Ps,"#app",{hot:void 0,onInit:async()=>{await ve.load(),ve.currentUser.get()&&_e.route.get()==="/login"&&_e.navigate("/",!0)}});export{T as C,z as R,h as S,qe as T,f as a,W as b,$ as c,_ as e,w as r,b as t};
