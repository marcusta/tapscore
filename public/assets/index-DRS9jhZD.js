...
<tr class="lb-rank__panel${R?" lb-rank__panel--open":""}" data-panel-key="${N(O)}">
  ...
  <table class="lb-rank">
    ...
    <thead><tr><th class="lb-rank__pos">#</th><th class="lb-rank__who">Player</th><th class="lb-rank__total">Total</th>${u}<th class="lb-rank__thru">Thru</th></tr></thead>
    <tbody>${d}</tbody>
  </table>
    ...
    <div class="lb-mp__team lb-mp__team--a${r}">${N(n.sideAName)}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${N(n.standing)}</span><span class="lb-mp__status">${N(n.status)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${o}">${N(n.sideBName)}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${N(t.title)}</h4>${s}
</div>`}const br={ranked:fr,match_summary:(i,e)=>gr(i,e)},yr={"default-score-grid":cr,"compact-match-grid":ur,"category-matrix-grid":hr};function _r(i){return`<div class="lb-diag">Unrenderable result section <code>${N(i)}</code> — no generic view yet. Results are not hidden.</div>`}function vr(i){return`<div class="lb-diag">Unsupported score-grid component <code>${N(i)}</code> — no generic view yet. Results are not hidden.</div>`}function wr(i,e,t,s=null){const n=br[i.kind];return n?n(i,e,t,s):_r(i.kind)}function gs(i,e,t,s){const n=hs(i),r=yr[n];return r?r(i,e,t,s):vr(n)}function xr(i,e,t=fs,s=null){return i.leaderboard.length===0&&i.cards.length===0?`<div class="lb-empty">No scores entered yet for ${N(i.formatLabel)}.</div>`:i.leaderboard.map(r=>wr(r,e,t,s&&r===s.plan.rankedSection?s:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${N(i.formatLabel)}.</div>`}function $r(i,e,t,s={}){if(i.length===0)return"";const n=s.mode??"product";return i.map(r=>gs(r,e,t,{mode:n})).join(`
`)}const kr=_(`
...
`);class Lr extends z{static styles=`
        .seat-card {
            margin-top: ${l("lg")};
            padding: ${l("lg")};
            ${j()}
...
`);class qr extends z{static styles=`
    ...
    `;router=this.inject(L);tokenQ=this.router.query("token");editable=new p(!1);render(){const e=this.tokenQ.get();return e&&v.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(Gr,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function Kr(i,e){if(!e)return!1;for(const t of i)for(const s of t.players)if(s.playerId===e)return!0;return!1}const Vr=_(`
...
`);class Ur extends z{static styles=`
    ...
    `;svc=this.inject(te);auth=this.inject(H);router=this.inject(L);tokenQ=this.router.query("token");open=new p(!1);leaving=new p(!1);error=new p("");diagnostics=new p([]);eligible(){return Kr(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await v.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(Vr,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(U,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function Pt(i){return!(!i.pageVisible||i.status==="complete")}function Wr(i,e){return e&&!i}const Yr=2,zt=3,Qr=75e3;function Xr(i,e=null){const t=new URLSearchParams({token:i});return e!==null&&t.set("since",e),`${M}/friendly-rounds/events?${t.toString()}`}function Jr(i){if(typeof i!="object"||i===null)return!1;const e=i;return e.latestEventId!==null&&typeof e.latestEventId!="string"?!1:e.status==="not_started"||e.status==="active"||e.status==="complete"}function Zr(i){const e=i.eventSourceFactory??(C=>new EventSource(C)),t=i.setTimer??((C,A)=>setTimeout(C,A)),s=i.clearTimer??(C=>clearTimeout(C)),n=i.livenessTimeoutMs??Qr,r=i.isPageVisible??(()=>typeof document>"u"||!document.hidden);let o=!1,d=0,c=null,u=null,f=i.since??null;const m=()=>{u!==null&&(s(u),u=null)},h=()=>{m(),u=t(O,n)},g=()=>{c!==null&&(c.onopen=null,c.onmessage=null,c.onerror=null,c.close(),c=null)},b=()=>{o=!0,m(),g()},x=()=>{if(g(),++d>=zt){b(),i.onDegrade();return}R()};function O(){if(!o){if(!r()){h();return}x()}}function R(){if(o)return;let C;try{C=e(Xr(i.token,f))}catch{b(),i.onDegrade();return}c=C,C.onopen=()=>{d=0},C.onmessage=A=>{if(o||c!==C)return;h();let F;try{F=JSON.parse(A.data)}catch{return}Jr(F)&&(F.latestEventId!==null&&(f=F.latestEventId),i.onEvent({latestEventId:F.latestEventId,status:F.status}))},C.onerror=()=>{o||c!==C||(C.readyState===Yr||++d>=zt)&&(b(),i.onDegrade())},h()}return R(),{stop:()=>{o||b()}}}const eo=2e4;function to(i){if(!(i===null||i===""))return/^\d+$/.test(i)?Number(i):i}const so=_(`
...
`);function At(i){return i.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class Ao extends z{static styles=`
        .friends {
            padding: ${l("xl")} ${l("lg")} ${l("2xl")};
...
`);class Na extends z{static styles=`
    ...
    `;router=this.inject(L);render(){const e=this.wire(Ea,{header:{className:()=>ss(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(ii,this.ref(e,"account")),this.spawn(qn,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":wt,"/history":yi,"/round":ro,"/create":Eo,"/login":Ro,"/friends":Ao,"/profile":Fo,"/admin":Yo,...ts.competitions?{"/competitions":ea,"/competition":Ta}:{}},wt),e}}class Pa extends H{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await _s(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await No()),this.error.set(null)}catch(e){e instanceof K&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Po(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof K&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}}B.get(Ms);const Mt=B.get(L);B.set(H,new Pa);const Ht=B.get(H);await Gs(Na,"#app",{hot:void 0,onInit:async()=>{await Ht.load(),Ht.currentUser.get()&&Mt.route.get()==="/login"&&Mt.navigate("/",!0)}});export{qe as A,z as C,L as R,p as S,Ms as T,y as a,he as b,S as c,Ns as d,P as e,Es as n,D as r,_ as t};
