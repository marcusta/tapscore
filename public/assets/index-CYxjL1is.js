...
`);class Ns extends I{static styles=`
        .tabbar {
            display: flex;
    ...
    `;let B=Le;function Ps(r){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function yt(){return Ps()}class ae{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await T(this.loading,this.error,()=>_.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=yt()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function vt(r){const e=P.get(ae);return e.load(),e.labelOf(r.formatId)??`${r.scoringMode} · ${r.teamShape}`}function zs(r){return r.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:xs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(vt).join(" · ")}))}function Rs(r){return r.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const re={fromMyRounds:zs,fromDeviceRounds:Rs},js=14,Os=1440*60*1e3;function te(r,e){return e(r)}function Ls(r,e,t,s=js){const n=e-s*Os,i=[],l=[];for(const d of r){const u=te(d,t);if(u.status==="complete"){const c=u.completedAt?Date.parse(u.completedAt):NaN;(Number.isNaN(c)||c>=n)&&l.push(d)}else i.push(d)}return i.sort((d,u)=>We(te(d,t).lastActivityAt,te(u,t).lastActivityAt)),l.sort((d,u)=>We(te(d,t).completedAt,te(u,t).completedAt)),{ongoing:i,finished:l}}function We(r,e){const t=r?Date.parse(r):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,i=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===i?0:i-n}const Ds=b(`
    <div class="landing">
...
`),Ms='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',Hs=b(`
    <div class="round-row">
        ...
        <button bind="del" type="button" class="round-row__del" aria-label="Delete round">${Ms}</button>
    </div>
`),wt={not_started:"Not started",active:"Live",complete:"Finished"};class Ue extends I{static styles=`
        .landing {
...
`);class qs extends I{static styles=`
        .history {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);function at(r){return r.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class ji extends I{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(_e);auth=this.inject(D);router=this.inject(N);render(){const e=()=>this.auth.currentUser.get()!==null;e()&&this.svc.load();const t=this.wire(Pi,{anon:{className:()=>e()?"friends__anon hidden":"friends__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/friends"}})},body:{className:()=>e()?"friends__body":"friends__body hidden"},search:{value:()=>this.svc.query.get(),oninput:n=>this.svc.setQuery(n.target.value)},searchHint:{textContent:()=>{const n=this.svc.query.get().trim();return n.length>0&&!nt(n)?"Type at least 2 characters.":this.svc.searching.get()?"Searching…":""}},searchErr:{textContent:()=>this.svc.searchError.get()?.message??""},resultsEmpty:{className:()=>{const n=this.svc.query.get().trim();return nt(n)&&!this.svc.searching.get()&&this.svc.searchError.get()===null&&this.svc.resultsFor.get()===n&&this.svc.results.get().length===0?"friends__empty":"friends__empty hidden"}},friendsEmpty:{className:()=>this.svc.loaded.get()&&this.svc.friends.get().length===0?"friends__empty":"friends__empty hidden"},sortToggle:{className:()=>this.svc.friends.get().length>0?"friends__sort":"friends__sort hidden"},sortFrecency:{"aria-pressed":()=>String(this.svc.sortMode.get()==="frecency"),onclick:()=>this.svc.setSortMode("frecency")},sortAlpha:{"aria-pressed":()=>String(this.svc.sortMode.get()==="alpha"),onclick:()=>this.svc.setSortMode("alpha")}});this.$each(this.ref(t,"results"),this.svc.results,(n,i,l)=>this.wireEl(zi,{initials:()=>at(n.displayName),name:()=>n.displayName,username:()=>`@${n.username}`,hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),add:{className:()=>this.isFriendNow(n.id)?"friend-row__add hidden":"friend-row__add",disabled:()=>this.svc.mutating.get(),onclick:()=>{const d=this.svc.results.get().find(u=>u.id===n.id);d&&!d.isFriend&&this.svc.add(d)}},added:{className:()=>this.isFriendNow(n.id)?"friend-row__added":"friend-row__added hidden"}},l),n=>n.id);const s=new Date().toISOString();return this.$each(this.ref(t,"friends"),()=>je(this.svc.friends.get(),this.svc.sortMode.get()),(n,i,l)=>this.wireEl(Ri,{initials:()=>at(n.displayName),name:()=>n.displayName,subtitle:()=>{const d=this.svc.friends.get().find(u=>u.id===n.id)??n;return fi(d,s)},hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),remove:{disabled:()=>this.svc.mutating.get(),onclick:()=>{this.svc.remove(n.id)}}},l),n=>n.id),t}isFriendNow(e){return this.svc.results.get().find(t=>t.id===e)?.isFriend===!0}}const Oi=b(`
    <div class="profile">
...
`);class _r extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(br,{});return this.spawn(Ns,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Ue,"/history":qs,"/round":Zn,"/create":Ei,"/login":Ni,"/friends":ji,"/profile":Di,"/competitions":Bi,"/competition":gr},Ue),e}}P.get(Wt);const lt=P.get(N),dt=P.get(D);await Yt(_r,"#app",{hot:void 0,onInit:async()=>{await dt.load(),dt.currentUser.get()&&lt.route.get()==="/login"&&lt.navigate("/",!0)}});export{I as C,N as R,h as S,Wt as T,f as a,oe as b,y as c,S as e,T as r,b as t};
