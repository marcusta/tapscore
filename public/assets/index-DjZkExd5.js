(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const a of i.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const xt="modulepreload",$t=function(r){return"/tapscore/"+r},Ne={},kt=function(e,t,s){let n=Promise.resolve();if(t&&t.length>0){let u=function(c){return Promise.all(c.map(m=>Promise.resolve(m).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),d=a?.nonce||a?.getAttribute("nonce");n=u(t.map(c=>{if(c=$t(c),c in Ne)return;Ne[c]=!0;const m=c.endsWith(".css"),p=m?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${p}`))return;const g=document.createElement("link");if(g.rel=m?"stylesheet":xt,m||(g.as="script"),g.crossOrigin="",g.href=c,d&&g.setAttribute("nonce",d),document.head.appendChild(g),m)return new Promise((x,y)=>{g.addEventListener("load",x),g.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${c}`)))})}))}function i(a){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=a,window.dispatchEvent(d),!d.defaultPrevented)throw a}return n.then(a=>{for(const d of a||[])d.status==="rejected"&&i(d.reason);return e().catch(i)})};class St{constructor(){this.tracking=null,this.batching=!1,this.pending=new Set}subscribe(e){this.tracking&&(e.add(this.tracking),this.tracking.deps.add(e))}notify(e){for(const t of[...e])this.batching?this.pending.add(t):t.run()}runTracked(e,t){Je(e);const s=this.tracking;this.tracking=e;try{t()}finally{this.tracking=s}}untrack(e){const t=this.tracking;this.tracking=null;try{return e()}finally{this.tracking=t}}batch(e){this.batching=!0;try{e()}finally{this.batching=!1;const t=[...this.pending];this.pending.clear();for(const s of t)s.run()}}}const q=new St;function Je(r){for(const e of r.deps)e.delete(r);r.deps.clear()}class h{constructor(e){this.subs=new Set,this.val=e}get(){return q.subscribe(this.subs),this.val}peek(){return this.val}set(e){Object.is(this.val,e)||(this.val=e,q.notify(this.subs))}update(e){this.set(e(this.val))}}class k{constructor(e){this.subs=new Set,this.val=void 0;const t=this,s={run(){q.runTracked(s,()=>{const n=e();Object.is(t.val,n)||(t.val=n,q.notify(t.subs))})},deps:new Set};s.run()}get(){return q.subscribe(this.subs),this.val}peek(){return this.val}}function $(r){const e={run(){q.runTracked(e,r)},deps:new Set};return e.run(),()=>Je(e)}function ne(r){q.batch(r)}function Q(r){return q.untrack(r)}class It{constructor(){this.instances=new Map}get(e){let t=this.instances.get(e);return t||(t=new e,this.instances.set(e,t)),t}set(e,t){this.instances.set(e,t)}reset(){this.instances.clear()}}const B=new It,te="/tapscore/".replace(/\/+$/,"");function be(r){return te?r===te?"/":r.startsWith(te+"/")?r.slice(te.length):r:r}function Tt(r){return te+r}class j{constructor(){this.route=new h(be(location.pathname??"/")),this.search=new h(location.search??""),window.addEventListener("popstate",()=>ne(()=>{this.route.set(be(location.pathname)),this.search.set(location.search)}))}navigate(e,t){const s=typeof t=="boolean"?{replace:t}:t??{},n=e.indexOf("#"),i=n>=0?e.slice(n):"",a=n>=0?e.slice(0,n):e,d=a.indexOf("?"),u=d>=0?a.slice(0,d):a,c=d>=0?a.slice(d+1):"",m=s.query!==void 0?Et(s.query):c?"?"+c:"",p=Tt(u)+m+i;(s.replace?history.replaceState:history.pushState).call(history,null,"",p),ne(()=>{this.route.set(u),this.search.set(m)})}back(){history.back()}link(e,t="active"){const s=e.split("#")[0].split("?")[0];return{onclick:n=>{n.preventDefault(),this.navigate(e)},className:()=>{const n=this.route.get();return n===s||n.startsWith(s+"/")?t:""}}}params(e){const t=e.split("/");return new k(()=>{const s=this.route.get().split("/"),n={};for(const[i,a]of t.entries())a.startsWith(":")&&(n[a.slice(1)]=s[i]??"");return n})}query(e){return new k(()=>new URLSearchParams(this.search.get()).get(e)??void 0)}queries(){return new k(()=>{const e={};for(const[t,s]of new URLSearchParams(this.search.get()))e[t]=s;return e})}}function Et(r){const e=new URLSearchParams;for(const[s,n]of Object.entries(r))n==null||n===""||e.set(s,String(n));const t=e.toString();return t?"?"+t:""}function Ct(r){return e=>r[e]}function Nt(r,e){const t=(n,i,a)=>{const d=Object.entries(n).map(([u,c])=>`--${u}:${c}`).join(";");return`${i}{color-scheme:${a};${d}}`},s=document.createElement("style");return s.textContent=t(r,'[data-theme="light"]',"light")+t(e,'[data-theme="dark"]',"dark"),document.head.appendChild(s),n=>`var(--${n})`}const Pe="basics-js-theme";class Pt{constructor(){this.dark=new h(!1);const e=localStorage.getItem(Pe),t=matchMedia("(prefers-color-scheme: dark)").matches;this.dark.set(e?e==="dark":t),$(()=>{const s=this.dark.get();document.documentElement.setAttribute("data-theme",s?"dark":"light"),localStorage.setItem(Pe,s?"dark":"light")})}toggle(){this.dark.update(e=>!e)}}function b(r){const e=document.createElement("template");return e.innerHTML=r,e}function zt(r,e){let t;for(const s of Object.keys(e))r.startsWith(s+"/")&&(!t||s.length>t.length)&&(t=s);return t?e[t]:void 0}const ze=new Set;class T{constructor(e={}){this.props=e,this.disposers=[],this.children=[];const t=this.constructor;if(t.styles&&!ze.has(t)){ze.add(t);const s=document.createElement("style");s.textContent=t.styles,document.head.appendChild(s)}}onMount(){}onDestroy(){}inject(e){return B.get(e)}track(e){this.disposers.push(e)}ref(e,t){return e.querySelector(`[bind="${t}"]`)}spawn(e,t,...s){const n=Q(()=>{const i=new e(s[0]);return i.mount(t),i});return this.children.push(n),n}mount(e){e.appendChild(this.render()),this.onMount()}destroy(){this.onDestroy();for(const e of this.children)e.destroy();this.children.length=0;for(const e of this.disposers)e();this.disposers.length=0}wire(e,t,s){const n=s??(a=>this.track(a)),i=e.content.cloneNode(!0);for(const a of i.querySelectorAll("[bind]")){const d=t[a.getAttribute("bind")];if(d)if(typeof d=="function")n($(()=>{const u=d();a instanceof HTMLInputElement||a instanceof HTMLTextAreaElement?a.value=String(u):a.textContent=String(u)}));else for(const[u,c]of Object.entries(d)){const m=u.includes("-");u.startsWith("on")&&typeof c=="function"?a.addEventListener(u.slice(2),c):typeof c=="function"?n($(()=>{const p=c();m?a.setAttribute(u,String(p)):a[u]=p})):m?a.setAttribute(u,String(c)):a[u]=c}}return i}wireEl(e,t,s){return this.wire(e,t,s).firstElementChild}slot(e,t){const s=this.props[e];if(s==null)return!1;const n=this.ref(t,e);return n?(typeof s=="string"?n.textContent=s:typeof s=="function"&&s.prototype instanceof T?this.spawn(s,n):typeof s=="function"&&s(n,{spawn:(i,a,...d)=>this.spawn(i,a,...d),track:i=>this.track(i)}),!0):!1}$each(e,t,s,n=(i,a)=>a){const i=typeof t=="function"?t:()=>t.get(),a=new Map,d=new Map;this.track(()=>{for(const u of d.values())u.forEach(c=>c());d.clear()}),this.track($(()=>{const u=i(),c=new Map;for(const[p,g]of u.entries()){const x=n(g,p);if(a.has(x))c.set(x,a.get(x));else{const y=[];c.set(x,Q(()=>s(g,p,v=>y.push(v)))),d.set(x,y)}}for(const[p,g]of a)c.has(p)||(g.remove(),d.get(p)?.forEach(x=>x()),d.delete(p));let m=e.firstChild;for(const p of c.values())p===m?m=m.nextSibling:e.insertBefore(p,m);a.clear();for(const[p,g]of c)a.set(p,g)}))}$condition(e,t,s,n){let i=null;this.track($(()=>{i&&(i.remove(),i=null);const a=t.get();i=Q(()=>a?s():n?.()??null),i&&e.appendChild(i)}))}$swap(e,t,s,n){let i=null;this.track($(()=>{i&&(i.destroy(),i=null),e.textContent="";const a=t.get(),d=s[a]??zt(a,s)??n;d&&(i=Q(()=>{const u=new d;return u.mount(e),u}))})),this.track(()=>i?.destroy())}}async function jt(r,e,t){const s=document.querySelector(e);s.textContent="";const n=B.get(j);let i=null,a=!1,d=null,u=!!t?.hot?.data.hmr;const c=async m=>{i&&(i.destroy(),i=null,s.textContent=""),m?(d||(d=(await kt(()=>import("./obs-shell.component-BYMYw6gw.js"),[])).ObsShellComponent),i=Q(()=>new d)):(!u&&t?.onInit&&(await t.onInit(),u=!0),i=Q(()=>new r)),Q(()=>i.mount(s)),a=m};await c(be(location.pathname).startsWith("/_obs")),$(()=>{const m=n.route.get().startsWith("/_obs");m!==a&&c(m)}),t?.hot&&(t.hot.data.hmr=!0,t.hot.dispose(()=>i?.destroy()),t.hot.accept())}class M extends Error{constructor(e,t,s,n){super(t),this.status=e,this.details=s,this.traceId=n,this.name="ApiError"}}const Ot=10,le=[];let de=[],se=null;function Rt(r){le.push(r),le.length>Ot&&le.shift()}function Ht(r,e,t){const s={code:r,message:e,url:typeof location<"u"?location.href:"",context:[...le],timestamp:new Date().toISOString()};t!==void 0&&(s.traceId=t),de.push(s),Mt()}function Mt(){se||(se=setTimeout(Ze,5e3))}function Ze(){if(se&&(clearTimeout(se),se=null),de.length===0)return;const r=de;de=[];for(const e of r){const t=JSON.stringify(e);typeof navigator<"u"&&navigator.sendBeacon?navigator.sendBeacon("/api/_obs/errors",new Blob([t],{type:"application/json"})):typeof fetch<"u"&&fetch("/api/_obs/errors",{method:"POST",headers:{"Content-Type":"application/json"},body:t}).catch(()=>{})}}typeof document<"u"&&document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&Ze()});const Lt=3e4,Ft=2,ie=new Map,et=new WeakMap;function At(r){if(r instanceof M)return r.traceId;if(r!=null&&typeof r=="object")return et.get(r)}async function f(r){if(r.method==="GET"){const e=ie.get(r.url);if(e)return e;const t=je(r,Ft);return ie.set(r.url,t),t.then(()=>ie.delete(r.url),()=>ie.delete(r.url)),t}return je(r,0)}async function je(r,e){const t=r.timeout??Lt;let s;for(let n=0;n<=e;n++){const i=crypto.randomUUID();try{return await Dt(Bt(r,i),t)}catch(a){if(s=a,!(a instanceof M)&&a!=null&&typeof a=="object"&&et.set(a,i),a instanceof M||n===e)break;await new Promise(d=>setTimeout(d,1e3*2**n))}}throw s}async function Bt(r,e){const t={"X-Trace-Id":e},s={method:r.method,headers:t};r.body!==void 0&&(t["Content-Type"]="application/json",s.body=JSON.stringify(r.body));const n=await fetch(r.url,s),i=n.headers.get("x-trace-id")??e;if(Rt({type:"api",detail:`${r.method} ${r.url}`,timestamp:new Date().toISOString()}),!n.ok){const a=await n.json().catch(()=>({error:n.statusText}));throw new M(n.status,a.error??n.statusText,a.details,i)}return n.json()}function Dt(r,e){let t;const s=new Promise((n,i)=>{t=setTimeout(()=>i(new Error("Request timeout")),e)});return Promise.race([r,s]).finally(()=>clearTimeout(t))}async function E(r,e,t){ne(()=>{r.set(!0),e.set(null)});try{const s=await t();return r.set(!1),s}catch(s){const n=Gt(s);ne(()=>{r.set(!1),e.set(n)}),Ht(n.code,n.message,At(s));return}}function Gt(r){return r instanceof M?r.status===401?{code:"auth",message:"Unauthorized"}:r.status===409?{code:"conflict",message:"Data has changed — please try again"}:r.status===400?{code:"validation",message:r.message}:{code:"server",message:"Server error"}:r instanceof Error?r.message==="Request timeout"?{code:"timeout",message:"Request timeout"}:{code:"network",message:"Network error"}:{code:"unknown",message:"Unknown error"}}function qt(r){return{me:()=>f({method:"GET",url:`${r}/auth/me`}),login:e=>f({method:"POST",url:`${r}/auth/login`,body:e}),logout:()=>f({method:"POST",url:`${r}/auth/logout`,body:{}})}}class L{constructor(){this.api=qt("/api"),this.currentUser=new h(null),this.loading=new h(!1),this.error=new h(null)}async load(){const e=await E(this.loading,this.error,()=>this.api.me());e&&this.currentUser.set(e),this.error.get()?.code==="auth"&&this.error.set(null)}async login(e,t){const s=await E(this.loading,this.error,()=>this.api.login({username:e,password:t}));return s?(this.currentUser.set(s),!0):!1}async logout(){await E(this.loading,this.error,()=>this.api.logout());const e=this.error.get();(!e||e.code==="auth")&&this.currentUser.set(null)}}const Oe={radius:"12px","radius-pill":"999px","radius-sm":"6px","font-display":"'Fraunces', Georgia, serif",shadow:"0 1px 2px rgba(30, 53, 38, 0.08)","shadow-elevated":"0 4px 16px rgba(30, 53, 38, 0.14)"},l=Nt({...Oe,bg:"#f2eee2",surface:"#fbf9f1","surface-sunken":"#e9e4d4",primary:"#2c5e3f","primary-text":"#f7f4ea","btn-bg":"#fbf9f1","btn-hover":"#efeada",text:"#1e3526","text-muted":"#6b7a6e",border:"#d8d2bf","topbar-bg":"#1e3526","active-bg":"#1e3526","active-text":"#f7f4ea","hover-bg":"#ece7d7","input-bg":"#ffffff",accent:"#b08d3e","accent-soft":"#f0e6cd",error:"#a0463c","under-par":"#a0463c","over-par":"#345b8a","hole-bar":"#e6a23f","hole-bar-text":"#3a2a0d"},{...Oe,bg:"#15231a",surface:"#1d2f22","surface-sunken":"#101b14",primary:"#5d9b75","primary-text":"#0f1a13","btn-bg":"#24392b","btn-hover":"#2e4836",text:"#e6e1d2","text-muted":"#8da093",border:"#33493a","topbar-bg":"#0f1a13","active-bg":"#5d9b75","active-text":"#0f1a13","hover-bg":"#273c2e","input-bg":"#101b14",accent:"#cfa84f","accent-soft":"#3a3320",error:"#d48a82","under-par":"#d48a82","over-par":"#8db2e0","hole-bar":"#c08a35","hole-bar-text":"#160f04",shadow:"0 1px 2px rgba(0, 0, 0, 0.3)","shadow-elevated":"0 4px 16px rgba(0, 0, 0, 0.4)"}),P=r=>`var(--${r})`,o=Ct({xs:"0.25rem",sm:"0.5rem",md:"0.75rem",lg:"1rem",xl:"1.5rem","2xl":"2rem"}),S=(r=P("radius"))=>`
...
`);class Ut extends T{static styles=`
    ...
    `;let ye=Se;const N=r=>`var(--${r})`,Ie=class Ie extends T{render(){const e=document.createElement("div");this.spawn(ye,e,{open:this.props.open,bg:"rgba(0,0,0,0.4)",zIndex:199,scrollLock:!0,onclose:()=>this.handleCancel()}),this.dialogEl=document.createElement("div"),this.dialogEl.className="ui-confirm",this.dialogEl.style.zIndex="200";const t=document.createElement("h2");t.className="ui-confirm__title",t.textContent=this.props.title??"Confirm",this.dialogEl.appendChild(t);const s=document.createElement("p");if(s.className="ui-confirm__message",typeof this.props.message=="function"){const d=this.props.message;this.track($(()=>{s.textContent=d()}))}else s.textContent=this.props.message;this.dialogEl.appendChild(s);const n=document.createElement("div");n.className="ui-confirm__actions";const i=document.createElement("button");i.className="ui-confirm__btn ui-confirm__btn--cancel",i.textContent=this.props.cancelLabel??"Cancel",i.addEventListener("click",d=>{d.stopPropagation(),this.handleCancel()}),n.appendChild(i);const a=document.createElement("button");return a.className=this.props.danger?"ui-confirm__btn ui-confirm__btn--danger":"ui-confirm__btn ui-confirm__btn--confirm",a.textContent=this.props.confirmLabel??"Confirm",a.addEventListener("click",d=>{d.stopPropagation(),this.props.open.set(!1),this.props.onconfirm()}),n.appendChild(a),this.dialogEl.appendChild(n),this.dialogEl.addEventListener("click",d=>d.stopPropagation()),e.appendChild(this.dialogEl),this.track($(()=>{this.dialogEl.classList.toggle("open",this.props.open.get())})),e}handleCancel(){this.props.open.set(!1),this.props.oncancel&&this.props.oncancel()}};Ie.styles=`
        .ui-confirm {
            position: fixed;
            top: 50%;
            left: 50%;
...
`),ot={not_started:"Not started",active:"Live",complete:"Finished"};class Me extends T{static styles=`
        .landing {
...
`),$s='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',ks=b(`
    <div class="round-row">
        <button bind="row" type="button" class="round-row__main">
            <div class="round-row__top">
                <span bind="course" class="round-row__course"></span>
        ...
        <button bind="del" type="button" class="round-row__del" aria-label="Delete round">${$s}</button>
    </div>
`);class Ss extends T{static styles=`
    ...
    `;svc=this.inject(rt);auth=this.inject(L);router=this.inject(j);loggedIn=new k(()=>this.auth.currentUser.get()!==null);rows=new k(()=>ws(this.loggedIn.get()?he.fromMyRounds(this.svc.myRounds.get()):he.fromDeviceRounds(this.svc.deviceRounds.get())));deleteOpen=new h(!1);deleteTarget=new h(null);askDelete(e,t,s){this.deleteTarget.set({token:e,roundId:t,name:s}),this.deleteOpen.set(!0)}render(){this.loggedIn.get()?this.svc.loadMine():this.svc.loadDevice();const e=this.wire(xs,{back:{onclick:()=>this.router.navigate("/")},empty:{className:()=>this.rows.get().length===0?"history__empty":"history__empty hidden"}});this.$each(this.ref(e,"list"),this.rows,(s,n,i)=>this.roundRow(s,i),s=>s.key),this.spawn(Y,this.ref(e,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:()=>{const s=this.deleteTarget.get();return`Delete ${s?`“${s.name}”`:"this round"}? This permanently removes it and all its scores for everyone. This can't be undone.`},confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const s=this.deleteTarget.get();s&&this.svc.remove(s.token,s.roundId)}});const t=s=>{s.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1)};return window.addEventListener("keydown",t),this.track(()=>window.removeEventListener("keydown",t)),e}roundRow(e,t){return this.wireEl(ks,{row:{disabled:()=>e.token===null,onclick:()=>{e.token!==null&&this.router.navigate("/round",{query:{token:e.token}})}},course:()=>e.courseName||"Round",role:{textContent:()=>e.roleLabel??"",className:()=>e.roleLabel?"round-row__role":"round-row__role hidden"},status:{textContent:()=>ot[e.status]??e.status,className:()=>`round-row__status s-${e.status}`},date:()=>e.date??"",formats:()=>e.formats??"",del:{className:()=>e.token===null?"round-row__del hidden":"round-row__del",onclick:()=>{e.token!==null&&this.askDelete(e.token,e.roundId??"",e.courseName||"this round")}}},t)}}const Is=180,Fe=4,Ts=12;function X(r,e){return e<=0?0:Math.max(0,Math.min(e-1,r))}function Es(r){const{dragDistance:e,velocity:t,itemWidth:s}=r;if(Math.abs(e)<Ts)return 0;const n=e+t*Is,i=Math.round(-n/s);return Math.max(-Fe,Math.min(Fe,i))}const Ae="tapscore:pending-scores:v1",Cs=336*60*60*1e3,Be=200;function Ns(){try{return globalThis.localStorage??null}catch{return null}}function Ps(r){if(typeof r!="object"||r===null)return!1;const e=r;return typeof e.token=="string"&&typeof e.ballId=="string"&&typeof e.playHoleId=="string"&&(typeof e.strokes=="number"||e.strokes===null)&&(e.eventType==="score_entered"||e.eventType==="score_cleared")&&typeof e.clientEventId=="string"&&typeof e.queuedAt=="number"}class zs{entries=[];storage;constructor(e=Ns(),t=Date.now()){this.storage=e,this.entries=this.load();const s=this.applyHygiene(t);s.length!==this.entries.length&&(this.entries=s,this.persist())}enqueue(e){const t=this.entries.findIndex(s=>s.token===e.token&&s.ballId===e.ballId&&s.playHoleId===e.playHoleId);t>=0?this.entries[t]=e:this.entries.push(e),this.entries=this.applyHygiene(e.queuedAt),this.persist()}remove(e){const t=this.entries.filter(s=>s.clientEventId!==e);t.length!==this.entries.length&&(this.entries=t,this.persist())}entriesFor(e){return this.entries.filter(t=>t.token===e)}size(){return this.entries.length}applyHygiene(e){const t=this.entries.filter(s=>e-s.queuedAt<=Cs);return t.length>Be?t.slice(t.length-Be):t}load(){if(!this.storage)return[];try{const e=this.storage.getItem(Ae);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t.filter(Ps):[]}catch{return[]}}persist(){if(this.storage)try{this.storage.setItem(Ae,JSON.stringify(this.entries))}catch{}}}const js=["1st","2nd","3rd","4th","5th","6th","7th","8th"],U=(r,e)=>`${r}|${e}`;function at(r){return r.players.map(e=>e.displayName).join(" & ")||r.label||"Ball"}function Os(r,e,t){return r?!(r.minPar!==void 0&&e<r.minPar||r.maxPar!==void 0&&e>r.maxPar||r.pars&&!r.pars.includes(e)||r.holes&&!r.holes.includes(t)):!0}class J{constructor(e=new zs){this.queue=e}queue;loading=new h(!1);error=new h(null);friendlyRound=new h(null);round=new h(null);balls=new h([]);scorecards=new h([]);cells=new h(new Map);result=new h(null);resultLoading=new h(!1);resultError=new h(null);resultCursor=null;holeIdx=new h(0);groupIdx=new h(0);selectedSlot=new h(null);token=null;loadSeq=0;resultSeq=0;flushing=!1;pendingSlotIndex=null;async loadByToken(e,t){const s=e!==this.token;this.token=e;const n=++this.loadSeq;s&&this.resetForNewToken(t),B.get(ue).load();const i=await E(this.loading,this.error,()=>_.friendlyRounds.byToken({token:e}));if(!i||n!==this.loadSeq||e!==this.token)return;if(this.friendlyRound.set(i.friendlyRound),this.round.set(i.round),ce({token:e,courseName:i.round.courseNameSnapshot??"",status:i.round.status,completedAt:i.round.completedAt,lastSeenAt:new Date().toISOString()}),this.pendingSlotIndex!==null){const c=i.round.formatSlots[this.pendingSlotIndex]?.slotDefId??null;this.pendingSlotIndex=null,c!==null&&this.selectedSlot.set(c)}const[a,d]=await Promise.all([_.friendlyRounds.balls({token:e}).catch(()=>[]),_.friendlyRounds.scorecard({token:e}).catch(()=>[])]);n!==this.loadSeq||e!==this.token||(this.cells.set(new Map),this.scorecards.set(d),this.balls.set(a),await this.flushPending())}deleting=new h(!1);async deleteRound(){const e=this.token;if(!e||this.deleting.get())return!1;this.deleting.set(!0);try{return await _.friendlyRounds.remove({token:e}),nt(e),!0}catch{return!1}finally{this.deleting.set(!1)}}finishing=new h(!1);async finishRound(){const e=this.token;if(!e||this.finishing.get())return null;this.finishing.set(!0);try{const t=await _.friendlyRounds.finish({token:e}),s=this.round.get();return e===this.token&&s&&(this.round.set({...s,status:t.status,completedAt:t.completedAt}),ce({token:e,courseName:s.courseNameSnapshot??"",status:t.status,completedAt:t.completedAt,lastSeenAt:new Date().toISOString()})),{status:t.status}}catch{return null}finally{this.finishing.set(!1)}}async reopenRound(){const e=this.token;if(!e||this.finishing.get())return null;this.finishing.set(!0);try{const t=await _.friendlyRounds.reopen({token:e}),s=this.round.get();return e===this.token&&s&&(this.round.set({...s,status:t.status,completedAt:null}),ce({token:e,courseName:s.courseNameSnapshot??"",status:t.status,completedAt:null,lastSeenAt:new Date().toISOString()})),{status:t.status}}catch{return null}finally{this.finishing.set(!1)}}async loadResult(){const e=this.token;if(!e)return;const t=++this.resultSeq,s=await E(this.resultLoading,this.resultError,()=>_.friendlyRounds.result({token:e}));t!==this.resultSeq||e!==this.token||s&&(this.resultCursor=s.cursor,s.unchanged||this.result.set(s.result))}async pollResult(){const e=this.token;if(!e)return;const t=++this.resultSeq;let s;try{s=await _.friendlyRounds.result({token:e,...this.resultCursor!==null?{cursor:this.resultCursor}:{}})}catch{return}t!==this.resultSeq||e!==this.token||(this.resultCursor=s.cursor,s.unchanged||this.result.set(s.result))}ballNameById=new k(()=>{const e=new Map;for(const t of this.balls.get())e.set(t.id,at(t));for(const t of this.result.get()?.slots??[])for(const s of t.subjectLabels??[])e.set(s.ballId,s.label);return e});nameOf(e){return this.ballNameById.get().get(e)??e}groupLabelByBallId=new k(()=>{const e=new Map,t=this.groups();return t.length<2||t.forEach((s,n)=>{for(const i of s.ballIds)e.set(i,`Group ${n+1}`)}),e});groupLabelOf(e){return this.groupLabelByBallId.get().get(e)??null}selectedSlotDefId(){const e=this.round.get()?.formatSlots??[];if(e.length===0)return null;const t=this.selectedSlot.get();return t!==null&&e.some(s=>s.slotDefId===t)?t:e[0]?.slotDefId??null}selectSlot(e){this.selectedSlot.set(e)}groups(){return this.round.get()?.playingGroups??[]}group(){const e=this.groups();return e[this.groupIdx.get()]??e[0]??null}playedOrder(){return this.group()?.playedOrder??[]}holeIndex(){return X(this.holeIdx.get(),this.playedOrder().length)}currentPlayedHole(){return this.playedOrder()[this.holeIndex()]??null}playHoleById(e){return this.round.get()?.playHoles.find(t=>t.id===e)??null}currentPlayHole(){const e=this.currentPlayedHole();return e?this.playHoleById(e.playHoleId):null}parFor(e){return(e?this.playHoleById(e)?.par:null)??4}occLabel(e){const t=this.round.get(),s=t?.playHoles.find(a=>a.id===e);if(!t||!s)return"";const n=t.playHoles.filter(a=>a.courseHoleNumber===s.courseHoleNumber).sort((a,d)=>a.ordinal-d.ordinal);if(n.length===1)return`${s.courseHoleNumber}`;const i=n.findIndex(a=>a.id===e);return`${s.courseHoleNumber} (${js[i]??`${i+1}th`})`}canPrevHole(){return this.holeIndex()>0}canNextHole(){return this.holeIndex()<this.playedOrder().length-1}prevHole(){this.holeIdx.set(X(this.holeIndex()-1,this.playedOrder().length))}nextHole(){this.holeIdx.set(X(this.holeIndex()+1,this.playedOrder().length))}strokesFor(e,t){const s=this.cells.get().get(U(e,t));return s?s.strokes:this.scorecards.get().find(a=>a.ballId===e)?.holes.find(a=>a.playHoleId===t)?.strokes??null}statusFor(e,t){return this.cells.get().get(U(e,t))?.status??null}metadataFor(e,t,s){const n=this.cells.get().get(U(e,t));return n&&n.metadata!==void 0?n.metadata?.[s]:this.scorecards.get().find(d=>d.ballId===e)?.holes.find(d=>d.playHoleId===t)?.metadata?.[s]}metadataInputs(){const e=B.get(ue),t=this.round.get()?.formatSlots??[],s=[],n=new Set;for(const i of t){const a=e.byId(i.formatId)?.requirements.scoreEntry?.metadata??[];for(const d of a)n.has(d.key)||(n.add(d.key),s.push(d))}return s}metadataInputsForHole(e){return e?this.metadataInputs().filter(t=>Os(t.appliesWhen,e.par,e.courseHoleNumber)):[]}async setScore(e,t,s,n){const i=U(e,t),a=crypto.randomUUID();this.patchCell(i,{strokes:s,metadata:n,status:"saving",clientEventId:a});const d=this.token;d&&(this.enqueue(d,e,t,s,n,a),await this.post(d,e,t,s,n,a))}async retry(e,t){const s=U(e,t),n=this.cells.get().get(s);if(!n)return;this.patchCell(s,{...n,status:"saving"});const i=this.token;i&&(this.enqueue(i,e,t,n.strokes,n.metadata,n.clientEventId),await this.post(i,e,t,n.strokes,n.metadata,n.clientEventId))}async flushPending(){const e=this.token;if(!(!e||this.flushing)){this.flushing=!0;try{for(const t of this.queue.entriesFor(e)){if(e!==this.token)return;this.patchCell(U(t.ballId,t.playHoleId),{strokes:t.strokes,metadata:t.metadata,status:"saving",clientEventId:t.clientEventId}),await this.post(e,t.ballId,t.playHoleId,t.strokes,t.metadata,t.clientEventId)}}finally{this.flushing=!1}}}enqueue(e,t,s,n,i,a){this.queue.enqueue({token:e,ballId:t,playHoleId:s,strokes:n,eventType:n===null?"score_cleared":"score_entered",clientEventId:a,...i!==void 0?{metadata:i}:{},queuedAt:Date.now()})}async post(e,t,s,n,i,a){const d=U(t,s);try{await _.friendlyRounds.score({token:e,ballId:t,playHoleId:s,strokes:n,eventType:n===null?"score_cleared":"score_entered",clientEventId:a,...i!=null?{metadata:i}:{}}),this.queue.remove(a);const u=this.cells.get().get(d);u&&u.clientEventId===a&&this.patchCell(d,{...u,status:"saved"});const c=this.round.get();e===this.token&&c&&c.status==="not_started"&&this.round.set({...c,status:"active"})}catch{const u=this.cells.get().get(d);u&&u.clientEventId===a&&this.patchCell(d,{...u,status:"error"})}}patchCell(e,t){const s=new Map(this.cells.get());s.set(e,t),this.cells.set(s)}resetForNewToken(e){this.resultSeq++,this.resultCursor=null,this.friendlyRound.set(null),this.round.set(null),this.balls.set([]),this.scorecards.set([]),this.cells.set(new Map),this.result.set(null),this.resultError.set(null),this.holeIdx.set(e?.holeIdx??0),this.groupIdx.set(e?.groupIdx??0);const t=e?.selectedSlot;this.pendingSlotIndex=null,typeof t=="string"?this.selectedSlot.set(t):typeof t=="number"?(this.pendingSlotIndex=t,this.selectedSlot.set(null)):this.selectedSlot.set(null)}}const W=60,De=8,_e=4,Rs=Array.from({length:_e*2+1},(r,e)=>e-_e),Hs="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",Ms=b(`
...
`);class Ds extends T{static styles=`
        ...
        .se-row {
            ...
            & .se-row__scores { display: flex; align-items: center; padding-right: ${De}px; flex-shrink: 0; }
        ...
        .se-toast {
            ...
            padding: ${o("md")} ${o("xl")}; border-radius: ${l("radius")};
            box-shadow: ${l("shadow-elevated")};
            &.hidden { display: none; }
        }
    `;svc=this.inject(J);holeIdx=this.svc.holeIdx;modalOpen=new h(!1);currentBallIdx=new h(0);extendedOpen=new h(!1);extendedScore=new h(10);statsOpen=new h(!1);pendingMeta=new h({});lastMetaKey=null;toastMsg=new h(null);dragOffset=new h(0);transitioning=new h(!1);ptr=null;pendingSteps=null;settleTimer=null;advanceTimer=null;flashTimer=null;hasScoring=new k(()=>this.svc.balls.get().length>0);group=()=>this.svc.group();playedOrder=()=>this.svc.playedOrder();holeIndex=()=>this.svc.holeIndex();currentHole=()=>this.svc.currentPlayedHole();occAtOffset=e=>{const t=this.playedOrder();return t[X(this.holeIndex()+e,t.length)]??null};ballsInGroup=()=>{const e=this.group();if(!e)return[];const t=new Map(this.svc.balls.get().map(s=>[s.id,s]));return e.ballIds.map(s=>t.get(s)).filter(s=>!!s)};parFor=e=>this.svc.parFor(e);occLabel=e=>this.svc.occLabel(e);ballName=e=>at(e);metaInputs=()=>this.svc.metadataInputsForHole(this.svc.currentPlayHole()).filter(e=>e.kind==="boolean");displayScore=e=>e===null?"–":String(e);toParValue=e=>{let t=0,s=0,n=!1;for(const i of this.playedOrder()){const a=this.svc.strokesFor(e.id,i.playHoleId);a!==null&&a>0&&(t+=a,s+=this.parFor(i.playHoleId),n=!0)}return n?t-s:null};toParText=e=>{const t=this.toParValue(e);return t===null?"–":t===0?"E":t>0?`+${t}`:`${t}`};toParClass=e=>{const t=this.toParValue(e);return`se-row__topar ${t===null||t===0?"even":t<0?"under":"over"}`};scoreLabel=(e,t)=>{if(e===1)return"HIO";const s=e-t;return s<=-4||s>=5?"OTHER":{"-3":"ALBA","-2":"EAGLE","-1":"BIRDIE",0:"PAR",1:"BOGEY",2:"DOUBLE",3:"TRIPLE",4:"QUAD"}[String(s)]??""};render(){this.track(()=>{this.advanceTimer&&clearTimeout(this.advanceTimer),this.flashTimer&&clearTimeout(this.flashTimer),this.settleTimer&&clearTimeout(this.settleTimer)}),this.track($(()=>{const i=this.ballsInGroup().length;i>0&&this.currentBallIdx.get()>=i&&this.currentBallIdx.set(0)}));const e=this.wire(Ms,{root:{className:()=>this.hasScoring.get()?"se":"se hidden"},close:{onclick:()=>{this.statsOpen.set(!1),this.modalOpen.set(!1)}},modal:{className:()=>this.modalOpen.get()?"se-modal":"se-modal hidden"},modalTitle:()=>{const i=this.currentHole();return i?`Hole ${this.occLabel(i.playHoleId)} · Par ${this.parFor(i.playHoleId)}`:""},extended:{className:()=>this.extendedOpen.get()?"se-pad__ext":"se-pad__ext hidden"},extVal:()=>String(this.extendedScore.get()),extMinus:{onclick:()=>this.extendedScore.set(Math.max(10,this.extendedScore.get()-1))},extPlus:{onclick:()=>this.extendedScore.set(this.extendedScore.get()+1)},extCancel:{onclick:()=>this.extendedOpen.set(!1)},extOk:{onclick:()=>{this.extendedOpen.set(!1),this.commit(this.extendedScore.get())}},toast:{className:()=>this.toastMsg.get()?"se-toast":"se-toast hidden",textContent:()=>this.toastMsg.get()??""},stats:{className:()=>this.statsOpen.get()?"se-stats":"se-stats hidden"},statsBack:{onclick:()=>this.statsOpen.set(!1)},statsHole:()=>{const i=this.currentHole();return i?`Hole ${this.occLabel(i.playHoleId)} · Par ${this.parFor(i.playHoleId)}`:""},statsTitle:()=>{const i=this.ballsInGroup()[this.currentBallIdx.get()];return i?this.ballName(i):""},statsScore:()=>{const i=this.ballsInGroup()[this.currentBallIdx.get()],a=this.currentHole();return!i||!a?"":this.displayScore(this.svc.strokesFor(i.id,a.playHoleId))},statsNext:{textContent:()=>this.hasMoreUnscored()?"Next ›":"Done ›",onclick:()=>{this.statsOpen.set(!1),this.advance()}}}),t=this.ref(e,"viewport"),s=this.ref(e,"track");this.bindCarouselPointer(t,s),this.track($(()=>{s.style.transition=this.transitioning.get()?Hs:"none",s.style.transform=`translateX(${this.dragOffset.get()}px)`})),this.$each(s,new k(()=>Rs),(i,a,d)=>this.holeItem(i,d),i=>i),this.$each(this.ref(e,"rows"),new k(()=>{const i=this.playedOrder(),a=this.holeIndex(),d=i[a];if(!d)return[];const u=a>0?i[a-1].playHoleId:null;return this.ballsInGroup().map(c=>({ball:c,ph:d.playHoleId,prevPh:u}))}),(i,a,d)=>this.playerRow(i.ball,i.ph,i.prevPh,d),i=>`${i.ball.id}|${i.ph}`),this.$each(this.ref(e,"modalList"),new k(()=>this.ballsInGroup()),(i,a,d)=>this.modalRow(i,a,d),i=>i.id);const n=this.ref(e,"keys");for(const i of[1,2,3,4,5,6,7,8,9])n.appendChild(this.numberKey(i));return n.appendChild(this.specialKey("10+","","se-key",()=>this.openExtended())),n.appendChild(this.specialKey("✕","clear","se-key clear",()=>this.commit(null))),n.appendChild(this.specialKey("0","pick up","se-key muted",()=>this.commit(0))),this.$each(this.ref(e,"statsBody"),new k(()=>this.metaInputs()),(i,a,d)=>this.metaChip(i,d),i=>i.key),this.track($(()=>{if(!this.modalOpen.get()){this.lastMetaKey=null;return}const i=this.ballsInGroup()[this.currentBallIdx.get()],a=this.currentHole();if(!i||!a)return;const d=`${i.id}|${a.playHoleId}`;if(d===this.lastMetaKey)return;this.lastMetaKey=d;const u={};for(const c of this.metaInputs())u[c.key]=this.svc.metadataFor(i.id,a.playHoleId,c.key)===!0;this.pendingMeta.set(u)})),e}holeItem(e,t){return this.wireEl(Ls,{item:{className:()=>{const s=e===-1&&this.holeIndex()<=0;return`se-hole${e===0?" active":""}${s?" gone":""}`}},hnum:{textContent:()=>{const s=this.occAtOffset(e);return s?this.occLabel(s.playHoleId):""}},hpar:{textContent:()=>{const s=this.occAtOffset(e);return s?`Par ${this.parFor(s.playHoleId)}`:""}}},t)}playerRow(e,t,s,n){return this.wireEl(Fs,{name:{textContent:this.ballName(e)},topar:{textContent:()=>this.toParText(e),className:()=>this.toParClass(e)},prev:{textContent:()=>s?this.displayScore(this.svc.strokesFor(e.id,s)):""},cval:{textContent:()=>this.displayScore(this.svc.strokesFor(e.id,t))},circle:{className:()=>this.svc.strokesFor(e.id,t)===null?"se-row__circle empty":"se-row__circle",onclick:()=>this.openModalForBall(e.id)}},n)}modalRow(e,t,s){const n=e.players.length>1?`Team · CH ${e.courseHandicap}`:`CH ${e.players[0]?.courseHandicap??e.courseHandicap}`;return this.wireEl(As,{mrow:{className:()=>this.currentBallIdx.get()===t?"se-mrow sel":"se-mrow",onclick:()=>this.currentBallIdx.set(t)},mname:{textContent:this.ballName(e)},mhcp:{textContent:n},mval:{textContent:()=>{const i=this.currentHole();return i?this.displayScore(this.svc.strokesFor(e.id,i.playHoleId)):"–"}}},s)}numberKey(e){return this.wireEl(Ge,{key:{className:()=>{const t=this.currentHole();return(t?e===this.parFor(t.playHoleId):!1)?"se-key par":"se-key"},onclick:()=>this.commit(e)},num:{textContent:String(e)},lbl:{textContent:()=>{const t=this.currentHole();return t?this.scoreLabel(e,this.parFor(t.playHoleId)):""}}})}specialKey(e,t,s,n){return this.wireEl(Ge,{key:{className:s,onclick:n},num:{textContent:e},lbl:{textContent:t}})}openModalForBall(e){const t=this.ballsInGroup().findIndex(s=>s.id===e);this.currentBallIdx.set(t<0?0:t),this.extendedOpen.set(!1),this.statsOpen.set(!1),this.modalOpen.set(!0)}openExtended(){this.extendedScore.set(10),this.extendedOpen.set(!0)}commit(e){const t=this.ballsInGroup(),s=this.currentHole(),n=t[this.currentBallIdx.get()];if(!s||!n)return;const i=e===null?void 0:this.metaSnapshot();this.svc.setScore(n.id,s.playHoleId,e,i),e!==null&&e>0&&this.metaInputs().length>0?this.statsOpen.set(!0):this.advance()}hasMoreUnscored=()=>{const e=this.ballsInGroup(),t=this.currentHole();if(!t)return!1;const s=this.currentBallIdx.get();return e.some((n,i)=>i!==s&&this.svc.strokesFor(n.id,t.playHoleId)===null)};metaSnapshot(){const e=this.metaInputs();if(e.length===0)return;const t=this.pendingMeta.get(),s={};for(const n of e)s[n.key]=t[n.key]===!0;return s}setMeta(e,t){const s=this.pendingMeta.get();this.pendingMeta.set({...s,[e]:t});const n=this.ballsInGroup()[this.currentBallIdx.get()],i=this.currentHole();if(!n||!i)return;const a=this.svc.strokesFor(n.id,i.playHoleId);a!==null&&this.svc.setScore(n.id,i.playHoleId,a,this.metaSnapshot())}metaChip(e,t){return this.wireEl(Bs,{glabel:{textContent:e.label},miss:{className:()=>this.pendingMeta.get()[e.key]?"se-seg":"se-seg on-miss",onclick:()=>this.setMeta(e.key,!1)},hit:{className:()=>this.pendingMeta.get()[e.key]?"se-seg on-hit":"se-seg",onclick:()=>this.setMeta(e.key,!0)}},t)}advance(){const e=this.ballsInGroup(),t=this.currentHole();if(!t)return;const s=u=>this.svc.strokesFor(e[u].id,t.playHoleId)!==null,n=this.currentBallIdx.get();for(let u=n+1;u<e.length;u++)if(!s(u))return this.currentBallIdx.set(u);for(let u=0;u<n;u++)if(!s(u))return this.currentBallIdx.set(u);const i=this.playedOrder();if(this.holeIndex()>=i.length-1){this.flash("Round complete"),this.modalOpen.set(!1);return}this.flash(`Hole ${this.occLabel(t.playHoleId)} done`);const d=t.playHoleId;this.advanceTimer&&clearTimeout(this.advanceTimer),this.advanceTimer=setTimeout(()=>{this.advanceTimer=null,this.currentHole()?.playHoleId===d&&(this.holeIdx.set(X(this.holeIndex()+1,this.playedOrder().length)),this.currentBallIdx.set(0))},700)}flash(e){this.toastMsg.set(e),this.flashTimer&&clearTimeout(this.flashTimer),this.flashTimer=setTimeout(()=>{this.flashTimer=null,this.toastMsg.get()===e&&this.toastMsg.set(null)},1100)}snap(e){this.pendingSteps=e,this.transitioning.set(!0),this.dragOffset.set(-e*W),this.settleTimer&&clearTimeout(this.settleTimer),this.settleTimer=setTimeout(()=>this.finishSettle(),420)}finishSettle(){if(this.pendingSteps===null)return;const e=this.pendingSteps;this.pendingSteps=null,this.settleTimer&&(clearTimeout(this.settleTimer),this.settleTimer=null),this.transitioning.set(!1),e!==0&&this.holeIdx.set(X(this.holeIndex()+e,this.playedOrder().length)),this.dragOffset.set(0)}bindCarouselPointer(e,t){t.addEventListener("transitionend",n=>{n.propertyName==="transform"&&this.finishSettle()}),e.addEventListener("pointerdown",n=>{this.ptr||this.transitioning.get()||this.playedOrder().length<=1||(this.ptr={id:n.pointerId,startX:n.clientX,startY:n.clientY,lastX:n.clientX,lastTime:Date.now(),velocity:0,horiz:!1},this.dragOffset.set(0),e.setPointerCapture?.(n.pointerId))}),e.addEventListener("pointermove",n=>{const i=this.ptr;if(!i||i.id!==n.pointerId)return;const a=n.clientX-i.startX,d=n.clientY-i.startY;if(!i.horiz){if(Math.abs(d)>Math.abs(a)&&Math.abs(d)>8||Math.abs(a)<=8)return;i.horiz=!0}const u=Date.now(),c=Math.max(1,u-i.lastTime);i.velocity=(n.clientX-i.lastX)/c,i.lastX=n.clientX,i.lastTime=u,this.dragOffset.set(a)});const s=n=>{const i=this.ptr;if(!i||i.id!==n.pointerId)return;const a=n.clientX-i.startX,d=i.horiz;if(this.ptr=null,e.releasePointerCapture?.(n.pointerId),!d){this.dragOffset.set(0);return}this.snap(Es({dragDistance:a,velocity:i.velocity,itemWidth:W}))};e.addEventListener("pointerup",s),e.addEventListener("pointercancel",n=>{!this.ptr||this.ptr.id!==n.pointerId||(this.ptr=null,e.releasePointerCapture?.(n.pointerId),this.snap(0))})}}const lt=()=>null;function w(r){return String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Gs(r,e){const t=[...r].sort((i,a)=>i.canonicalOrdinal-a.canonicalOrdinal);if(e.length===0)return[{label:"TOT",holes:t,playHoleIds:new Set(t.map(i=>i.playHoleId))}];const s=[...e].sort((i,a)=>i.fromCanonicalOrdinal-a.fromCanonicalOrdinal),n=[];for(const i of s){const a=t.filter(d=>d.canonicalOrdinal>=i.fromCanonicalOrdinal&&d.canonicalOrdinal<=i.toCanonicalOrdinal);a.length!==0&&n.push({label:i.label,holes:a,playHoleIds:new Set(a.map(d=>d.playHoleId))})}return n}function qs(r){return r.kind==="si"?"lb-c-si":r.kind==="given"?"lb-c-given":r.kind==="status"?"lb-c-status":r.kind==="category"?"lb-c-cat":""}function Ks(r){const e=[r.kind==="category"?"lb-r-cat":`lb-r-${r.kind}`];return(r.kind==="si"||r.kind==="given")&&e.push("lb-r-dim"),r.team&&e.push(`lb-team-${r.team}`),e.join(" ")}function Us(r){return r&&r.marker?r.marker.template:null}function Ws(r){const e=r?.marker?.tone;return e==="success"||e==="warning"||e==="danger"?` lb-mark-tone--${e}`:""}function Vs(r,e){const t=r.cells.filter(s=>e.has(s.playHoleId));if(r.aggregate==="sum"){const s=t.map(n=>n.value).filter(n=>n!==null);return s.length===0?"—":String(s.reduce((n,i)=>n+i,0))}if(r.aggregate==="last"){for(let s=t.length-1;s>=0;s--){const n=t[s].value;if(n!==null)return Number.isInteger(n)?String(n):n.toFixed(1)}return"—"}return"—"}function Qs(r){return r.filter(e=>!(e.startsWith("slot #")||/^CH -?\d/.test(e)||/^PH -?\d/.test(e)))}function xe(r,e,t,s){const n=Gs(r.holes,e),i=v=>{const H=`<tr><th class="lb-rowlabel">Hole</th>${v.holes.map(I=>`<th>${w(I.occurrenceLabel)}</th>`).join("")}<th class="lb-sum">${w(v.label)}</th></tr>`,F=r.rows.map(I=>{const O=new Map(I.cells.map(K=>[K.playHoleId,K])),D=K=>I.emphasis?`<strong>${K}</strong>`:K,gt=v.holes.map(K=>{const G=O.get(K.playHoleId),_t=G?.title?` title="${w(G.title)}"`:"",pe=D(w(G?.display??"")),Ee=Us(G),vt=Ws(G),me=G?.marker?.label,wt=me?` title="${w(me)}" aria-label="${w(me)}"`:"";let Ce=Ee?`<span class="lb-mark lb-mark--${Ee}${vt}"${wt}>${pe}</span>`:pe;return G?.team&&(Ce=`<span class="lb-pill lb-pill--${G.team}">${pe}</span>`),`<td class="${qs(I)}"${_t}>${Ce}</td>`}).join(""),bt=`<td class="lb-sum">${D(Vs(I,v.playHoleIds))}</td>`,yt=I.subjectBallId?w(t(I.subjectBallId))+(I.label?" "+w(I.label):""):w(I.label);return`<tr class="${Ks(I)}"><th class="lb-rowlabel">${yt}</th>${gt}${bt}</tr>`}).join("");return`<div class="lb-card__scroll"><table class="lb-grid"><thead>${H}</thead><tbody>${F}</tbody></table></div>`},a=n.map(v=>i(v)).join(""),d=r.title.groups.map(v=>v.map(H=>w(t(H))).join(" & ")).filter(Boolean).join(r.title.joiner),u=s.mode==="verification"?r.subtitleFacts:Qs(r.subtitleFacts),c=u.length?`<div class="lb-card__sub">${u.map(w).join(" · ")}</div>`:"",m=s.mode==="verification"&&r.footnotes.length?`<div class="lb-card__notes"><span class="lb-card__notes-label">Points breakdown</span>${r.footnotes.map(v=>`<span class="lb-card__note">${w(v)}</span>`).join("")}</div>`:"",p=s.mode==="verification"&&r.caption?`<p class="lb-card__caption">${w(r.caption)}</p>`:"",g=r.totals.length?`<ul class="lb-card__totals">${r.totals.map(v=>`<li>${w(v.label)} = <strong>${v.value??"—"}</strong></li>`).join("")}</ul>`:"",x=d?`<header class="lb-card__head"><h4>${d}</h4>${c}</header>`:c;return`<article class="${s.cardModifier?`lb-card ${s.cardModifier}`:"lb-card"}">
  ${x}
  ${a}
  ${m}${p}${g}
</article>`}function Ys(r,e,t,s){return xe(r,e,t,s)}function Xs(r,e,t,s){return xe(r,e,t,{...s,cardModifier:"lb-card--compact-match"})}function Js(r,e,t,s){return xe(r,e,t,{...s,cardModifier:"lb-card--category-matrix"})}function Zs(r,e){const t=new Set(r.map(e));return t.size!==1?null:[...t][0]??null}function en(r){if(r===void 0)return"";const e=r===0?"E":r>0?`+${r}`:`−${Math.abs(r)}`;return` <span class="lb-rank__pace lb-rank__pace--${r===0?"even":r>0?"over":"under"}">${w(e)}</span>`}function tn(r,e,t=lt){const s=r.entries.map(n=>{const i=Zs(n.ballIds,t),a=i?` <span class="lb-rank__group">${w(i)}</span>`:"";return`<tr class="${n.position===1?"lb-rank__lead":""}">
  <td class="lb-rank__pos">${n.position}</td>
  <td class="lb-rank__who"><span class="lb-rank__name">${w(n.ballIds.map(e).join(" & "))}</span>${a}</td>
  <td class="lb-rank__total">${n.total??"—"}${en(n.paceDelta)}</td>
  <td class="lb-rank__thru">${n.holesPlayed}</td>
</tr>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${w(r.metricLabel)}</h4>
  <table class="lb-rank">
    <colgroup>
...
</div>`}function sn(r,e){const t=r.matches.map(s=>{const n=w(s.sideA.ballIds.map(e).join(" & ")),i=w(s.sideB.ballIds.map(e).join(" & ")),a=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,u=s.leader==="a"?" lb-mp__team--lead":"",c=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
    <div class="lb-mp__team lb-mp__team--a${u}">${n}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${w(a)}</span><span class="lb-mp__status">${w(d)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${c}">${i}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${w(r.title)}</h4>${t}
</div>`}const nn={ranked:tn,match_summary:(r,e)=>sn(r,e)},rn={"default-score-grid":Ys,"compact-match-grid":Xs,"category-matrix-grid":Js};function on(r){return r.componentId??"default-score-grid"}function an(r){return`<div class="lb-diag">Unrenderable result section <code>${w(r)}</code> — no generic view yet. Results are not hidden.</div>`}function ln(r){return`<div class="lb-diag">Unsupported score-grid component <code>${w(r)}</code> — no generic view yet. Results are not hidden.</div>`}function dn(r,e,t){const s=nn[r.kind];return s?s(r,e,t):an(r.kind)}function cn(r,e,t,s){const n=on(r),i=rn[n];return i?i(r,e,t,s):ln(n)}function un(r,e,t=lt){return r.leaderboard.length===0&&r.cards.length===0?`<div class="lb-empty">No scores entered yet for ${w(r.formatLabel)}.</div>`:r.leaderboard.map(n=>dn(n,e,t)).join("")||`<div class="lb-empty">No leaderboard metric for ${w(r.formatLabel)}.</div>`}function hn(r,e,t,s={}){if(r.cards.length===0)return"";const n=s.mode??"product";return r.cards.map(i=>cn(i,e,t,{mode:n})).join(`
`)}const pn=b(`
...
`);class mn extends T{static styles=`
        .lb {
            ...
            & .lb-card__caption { margin: ${o("sm")} 0 0; font-size: 0.72rem; font-style: italic; color: ${l("text-muted")}; }
            & .lb-card__notes { margin: ${o("sm")} 0 0; font-size: 0.72rem; color: ${l("text-muted")}; }
    ...
    `;svc=this.inject(J);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(pn,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=u=>this.svc.nameOf(u),i=un(t,s,u=>this.svc.groupLabelOf(u)),a=hn(t,e.routeSections,s),d=a?`<h3 class="lb-cards__head">Scorecard</h3>${a}`:"";return i+d}}function fn(r,e){if(!e)return[];const t=[],s=new Set;for(const n of r)for(const i of n.players){if(i.playerId===e)return[];i.guestPlayerId===null||s.has(i.guestPlayerId)||(s.add(i.guestPlayerId),t.push({guestPlayerId:i.guestPlayerId,displayName:i.displayName}))}return t}const gn=b(`
...
`);class yn extends T{static styles=`
        .claim-card {
            margin-top: ${o("lg")};
            padding: ${o("lg")};
            ${z()}
    ...
    `;svc=this.inject(J);auth=this.inject(L);router=this.inject(j);tokenQ=this.router.query("token");claiming=new h(!1);error=new h("");claimable(){return fn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async claim(e){const t=this.tokenQ.get();if(!(!t||this.claiming.get())){this.error.set(""),this.claiming.set(!0);try{await _.friendlyRounds.claimGuest({token:t,guestPlayerId:e}),await this.svc.loadByToken(t)}catch(s){this.error.set(s instanceof M&&s.status===409?"Already claimed — or you already play in this round under your account.":s instanceof M&&s.status===404?"That guest is no longer claimable on this round.":"Could not claim right now. Try again.")}finally{this.claiming.set(!1)}}}render(){const e=this.wire(gn,{root:{className:()=>this.claimable().length>0?"claim-card":"claim-card hidden"},err:{textContent:()=>this.error.get()}});return this.$each(this.ref(e,"rows"),()=>this.claimable(),(t,s,n)=>this.wireEl(bn,{name:()=>t.displayName,claim:{disabled:()=>this.claiming.get(),onclick:()=>{this.claim(t.guestPlayerId)}}},n),t=>t.guestPlayerId),e}}function V(r){return typeof r=="object"&&r!==null&&typeof r.get=="function"}const C=r=>`var(--${r})`,Te=class Te extends T{constructor(){super(...arguments),this.open=new h(!1),this.highlightIndex=new h(-1),this.optionEls=[],this.onOutsidePointer=e=>{this.wrapperEl.contains(e.target)||this.open.set(!1)}}render(){const e=document.createElement("div");e.className="ui-select",this.wrapperEl=e;const t=this.props.zIndex??50;this.triggerEl=document.createElement("button"),this.triggerEl.className="ui-select__trigger",this.triggerEl.setAttribute("type","button"),this.triggerEl.setAttribute("role","combobox"),this.triggerEl.setAttribute("aria-haspopup","listbox");const s=document.createElement("span");s.className="ui-select__trigger-label",this.triggerEl.appendChild(s);const n=document.createElement("span");n.className="ui-select__chevron",n.textContent="▾",n.setAttribute("aria-hidden","true"),this.triggerEl.appendChild(n),this.triggerEl.addEventListener("click",a=>{a.stopPropagation(),this.toggle()}),this.triggerEl.addEventListener("keydown",a=>{this.handleTriggerKeydown(a)}),e.appendChild(this.triggerEl),this.dropdownEl=document.createElement("div"),this.dropdownEl.className="ui-select__dropdown",this.dropdownEl.setAttribute("role","listbox"),this.dropdownEl.style.zIndex=String(t),this.dropdownEl.addEventListener("keydown",a=>{this.handleDropdownKeydown(a)}),e.appendChild(this.dropdownEl);const i=a=>{this.optionEls=[],this.dropdownEl.textContent="";for(let d=0;d<a.length;d++){const u=a[d],c=document.createElement("button");if(c.className="ui-select__option",c.setAttribute("type","button"),c.id=`ui-select-opt-${d}`,u.disabled){c.classList.add("ui-select__option--header"),c.disabled=!0,c.setAttribute("role","presentation"),c.setAttribute("aria-disabled","true");const g=document.createElement("span");g.className="ui-select__option-label",g.textContent=u.label,c.appendChild(g),this.dropdownEl.appendChild(c),this.optionEls.push(c);continue}if(c.setAttribute("role","option"),u.icon){const g=document.createElement("span");g.className="ui-select__option-icon",g.textContent=u.icon,c.appendChild(g)}const m=document.createElement("span");m.className="ui-select__option-label",m.textContent=u.label,c.appendChild(m);const p=document.createElement("span");p.className="ui-select__check",p.setAttribute("aria-hidden","true"),c.appendChild(p),c.addEventListener("click",g=>{g.stopPropagation(),this.selectOption(u.value)}),c.addEventListener("mouseenter",()=>{this.highlightIndex.set(d)}),this.dropdownEl.appendChild(c),this.optionEls.push(c)}};return V(this.props.options)?this.track($(()=>{const a=V(this.props.options)?this.props.options.get():this.props.options;i(a)})):i(this.props.options),this.track($(()=>{const a=this.props.value.get(),d=V(this.props.options)?this.props.options.get():this.props.options,u=d.find(c=>c.value===a);u?(s.textContent=u.icon?`${u.icon} ${u.label}`:u.label,this.triggerEl.classList.remove("ui-select__trigger--placeholder")):(s.textContent=this.props.placeholder??"",this.triggerEl.classList.toggle("ui-select__trigger--placeholder",!!this.props.placeholder));for(let c=0;c<d.length;c++){const m=this.optionEls[c];if(!m)continue;const p=d[c].value===a;m.setAttribute("aria-selected",String(p)),m.classList.toggle("ui-select__option--selected",p);const g=m.querySelector(".ui-select__check");g&&(g.textContent=p?"✓":"")}})),this.track($(()=>{const a=this.open.get();if(this.dropdownEl.classList.toggle("open",a),n.classList.toggle("ui-select__chevron--open",a),this.triggerEl.setAttribute("aria-expanded",String(a)),a?document.addEventListener("pointerdown",this.onOutsidePointer,!0):document.removeEventListener("pointerdown",this.onOutsidePointer,!0),a){const d=V(this.props.options)?this.props.options.get():this.props.options,u=this.props.value.get(),c=d.findIndex(p=>p.value===u),m=d.findIndex(p=>!p.disabled);this.highlightIndex.set(c>=0?c:m)}})),this.track($(()=>{const a=this.highlightIndex.get();for(let d=0;d<this.optionEls.length;d++)this.optionEls[d].classList.toggle("ui-select__option--highlighted",d===a);a>=0&&this.optionEls[a]&&(this.triggerEl.setAttribute("aria-activedescendant",`ui-select-opt-${a}`),this.optionEls[a].scrollIntoView({block:"nearest"}))})),this.props.disabled!=null&&(V(this.props.disabled)?this.track($(()=>{const a=this.props.disabled.get();this.triggerEl.classList.toggle("ui-select__trigger--disabled",a),this.triggerEl.disabled=a})):this.props.disabled&&(this.triggerEl.classList.add("ui-select__trigger--disabled"),this.triggerEl.disabled=!0)),e}toggle(){this.open.update(e=>!e)}selectOption(e){ne(()=>{this.props.value.set(e),this.open.set(!1)}),this.triggerEl.focus()}handleTriggerKeydown(e){switch(e.key){case"Enter":case" ":e.preventDefault(),this.toggle();break;case"ArrowDown":e.preventDefault(),this.open.get()?this.moveHighlight(1):this.open.set(!0);break;case"ArrowUp":e.preventDefault(),this.open.get()?this.moveHighlight(-1):this.open.set(!0);break;case"Escape":this.open.get()&&(e.preventDefault(),this.open.set(!1));break}}handleDropdownKeydown(e){switch(e.key){case"ArrowDown":e.preventDefault(),this.moveHighlight(1);break;case"ArrowUp":e.preventDefault(),this.moveHighlight(-1);break;case"Enter":case" ":{e.preventDefault();const t=this.highlightIndex.get(),s=V(this.props.options)?this.props.options.get():this.props.options;t>=0&&t<s.length&&!s[t].disabled&&this.selectOption(s[t].value);break}case"Escape":e.preventDefault(),this.open.set(!1),this.triggerEl.focus();break;case"Tab":this.open.set(!1);break}}moveHighlight(e){const t=V(this.props.options)?this.props.options.get():this.props.options;if(t.length===0||!t.some(n=>!n.disabled))return;let s=this.highlightIndex.get();do s+=e,s<0&&(s=t.length-1),s>=t.length&&(s=0);while(t[s].disabled);this.highlightIndex.set(s)}onDestroy(){document.removeEventListener("pointerdown",this.onOutsidePointer,!0)}};Te.styles=`
        .ui-select {
            position: relative;
    ...
    `;let re=Te;function _n(r,e,t){if(!e||t!=="not_started")return!1;for(const s of r)for(const n of s.players)if(n.playerId===e)return!1;return!0}const qe="new";function vn(r){const e=r.map((s,n)=>{const i=s.ballIds.length,a=[`Group ${n+1}`];return s.startTime.includes(":")&&a.push(s.startTime),{value:s.id,label:`${a.join(" · ")} — ${i} of ${s.capacity}`,disabled:i>=s.capacity}}),t=e.find(s=>!s.disabled);return e.push({value:qe,label:"Start a new group",disabled:!1}),{options:e,defaultValue:t?.value??qe}}const wn=b(`
...
`);class xn extends T{static styles=`
        .join-card {
            margin-top: ${o("lg")};
            padding: ${o("lg")};
            ${z()}
    ...
    `;svc=this.inject(J);auth=this.inject(L);router=this.inject(j);tokenQ=this.router.query("token");joining=new h(!1);error=new h("");diagnostics=new h([]);teeId=new h("");tees=new h([]);loadedForCourseId=null;groupChoice=new h("");eligible(){return _n(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,_.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}diagnosticMessage(e){return e.code==="missing_gender"||e.code==="missing_handicap_index"?`${e.message} — <a data-nav="/profile">update your profile</a>.`:e.message}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const s=this.groupChoice.get(),n=await _.friendlyRounds.join({token:e,teeId:t,...s?{groupChoice:s}:{}});n.ok?await this.svc.loadByToken(e):this.diagnostics.set(n.diagnostics)}catch(s){this.error.set(s instanceof M&&s.status===409?s.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track($(()=>this.ensureTeesLoaded()));const e=new k(()=>vn(this.svc.groups()));this.track($(()=>{const i=e.get(),a=this.groupChoice.get();(!a||!i.options.some(d=>d.value===a&&!d.disabled))&&this.groupChoice.set(i.defaultValue)}));const t=this.wire(wn,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},groupRow:{className:()=>this.svc.groups().length>0?"join-card__group":"join-card__group hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{innerHTML:()=>this.diagnostics.get().map(i=>this.diagnosticMessage(i)).join(" · "),onclick:i=>{const d=i.target.closest("[data-nav]")?.getAttribute("data-nav");d&&(i.preventDefault(),this.router.navigate(d))}},err:{textContent:()=>this.error.get()}}),s=new re({value:this.teeId,options:{get:()=>this.tees.get().map(i=>({value:i.id,label:i.name}))},placeholder:"Tee"});s.mount(this.ref(t,"teeHost")),this.track(()=>s.destroy());const n=new re({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return n.mount(this.ref(t,"groupHost")),this.track(()=>n.destroy()),t}}const $n=b(`
...
`);class kn extends T{static styles=`
        .edit-card {
            margin-top: ${o("lg")};
            padding: ${o("lg")};
            ${z()}
    ...
    `;router=this.inject(j);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&_.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire($n,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function Sn(r,e){if(!e)return!1;for(const t of r)for(const s of t.players)if(s.playerId===e)return!0;return!1}const In=b(`
...
`);class Tn extends T{static styles=`
    ...
    `;svc=this.inject(J);auth=this.inject(L);router=this.inject(j);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return Sn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await _.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(In,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(Y,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function En(r){return!(r.tab!=="leaderboard"||!r.pageVisible||r.status==="complete")}const Cn=2e4;function Nn(r){if(!(r===null||r===""))return/^\d+$/.test(r)?Number(r):r}const Pn=b(`
...
`),zn=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),jn=b('<button bind="pill" class="round-view__grp" type="button"></button>');class On extends T{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class dr extends T{static styles=`
        .setup {
            padding: ${o("lg")} ${o("lg")} ${o("2xl")};
            ...
            & .setup__head {
                ...
                & p { margin: ${o("xs")} 0 0; color: ${l("text-muted")}; font-size: 0.9rem; }
            }

            & .setup__section {
                margin-bottom: ${o("xl")};
                & h2 {
                    margin: 0 0 ${o("sm")}; font-family: ${l("font-display")};
                    font-weight: 600; font-size: 1.2rem;
                }
            }

            & .setup__hint { margin: 0 0 ${o("md")}; color: ${l("text-muted")}; font-size: 0.82rem; }

            & .setup__note {
                margin: ${o("sm")} 0 0; font-size: 0.82rem; color: ${l("text-muted")};
                &:empty { display: none; }
            }

            & .setup__warn {
                margin: ${o("sm")} 0 0; font-size: 0.82rem; color: ${l("error")};
            ...
            & .setup__startrow {
                display: flex; align-items: center; justify-content: space-between;
                gap: ${o("md")}; font-size: 0.9rem; color: ${l("text-muted")};
            }
            ...
            & .player {
                ...
                & .player__name { flex: 1; padding: ${o("md")}; font-size: 1rem; ${A()} }
                & .player__remove {
                ...
                & .player__index { flex: 1; min-width: 0; padding: ${o("md")}; font-size: 1rem; ${A()} }
            ...
            & .setup__add {
                width: 100%; margin-top: ${o("md")}; padding: ${o("md")}; ${S()}
                font-family: inherit; font-weight: 700; font-size: 0.95rem;
            }
            & .setup__add.hidden { display: none; }

            & .setup__friends {
                margin-top: ${o("sm")}; padding: ${o("sm")}; ${z()}
                &.hidden { display: none; }
            ...
            & .setup__banner {
                color: ${l("error")}; font-size: 0.875rem; margin-bottom: ${o("md")};
                white-space: pre-line;
                &:empty { display: none; }
            }
            ...
            & .fslot {
                ...
                & .fslot__teammeta {
                    margin: ${o("xs")} 0 0; font-size: 0.78rem; color: ${l("text-muted")};
                    &:empty { display: none; }
                }
                ...
                & .mrow {
                    ...
                    & .mrow__pct {
                        ...
                        &[hidden] { display: none; }
                        & input { width: 56px; padding: ${o("xs")} ${o("sm")}; ${A()} font-size: 0.95rem; }
                    }
                }
                ...
                & .fslot__seg {
                    ...
                    & button {
                        flex: 1; padding: ${o("sm")} 0; ${S()}
                        font-family: inherit; font-weight: 700; font-size: 0.82rem;
                        &.on { background: ${l("primary")}; color: ${l("primary-text")}; border-color: ${l("primary")}; }
                    }
                }
                & .fslot__flat {
                    display: flex; align-items: center; gap: ${o("xs")}; font-size: 0.9rem;
                    color: ${l("text-muted")};
                    &[hidden] { display: none; }
                    & .fslot__pct { width: 70px; padding: ${o("sm")}; font-size: 1rem; ${A()} }
                }
                & .fslot__bands {
                    display: flex; flex-direction: column; gap: ${o("xs")};
                    &[hidden] { display: none; }
                }
                & .fslot__bandrows { display: flex; flex-direction: column; gap: ${o("xs")}; }
                & .brow {
                    display: flex; align-items: center; gap: ${o("xs")};
                    font-size: 0.82rem; color: ${l("text-muted")};
                    & .brow__pct, & .brow__upto { width: 56px; padding: ${o("sm")}; font-size: 0.95rem; ${A()} }
                    & .brow__del { margin-left: auto; width: 30px; height: 30px; ${S()} font-size: 0.8rem; color: ${l("text-muted")}; }
                }
                & .fslot__addband {
                    align-self: flex-start; padding: ${o("xs")} ${o("sm")}; ${S()}
                ...
                & .grp__start {
                    ...
                    & .grp__time { flex: 1; min-width: 0; padding: ${o("sm")} ${o("md")}; font-size: 1rem; font-family: inherit; ${A()} }
...
`)},create:{disabled:()=>this.svc.submitting.get(),textContent:()=>this.svc.submitting.get()?t?"Saving…":"Creating…":t?"Save changes":"Create round",onclick:async()=>{const c=await this.svc.submit();c.ok&&this.router.navigate("/round",{query:{token:c.token}})}}});this.$each(this.ref(d,"presets"),()=>sr,(c,m,p)=>this.wireEl(b('<button bind="b" type="button"></button>'),{b:{textContent:()=>this.svc.presetLabel(c),className:()=>this.svc.preset.get()===c?"on":"",disabled:()=>n(),onclick:()=>{n()||this.svc.setPreset(c)}}},p),c=>c);const u=c=>this.track(c);return this.mountSelect(this.ref(d,"course"),u,{value:this.bound(u,()=>this.svc.courseId.get(),c=>{c&&c!==this.svc.courseId.get()&&this.svc.selectCourse(c)}),options:{get:()=>{const c=[];let m="";for(const p of this.svc.courses.get())p.clubName!==m&&(c.push({value:`__club:${p.clubName}`,label:p.clubName,disabled:!0}),m=p.clubName),c.push({value:p.id,label:p.name});return c}},placeholder:"Select a course",disabled:{get:()=>n()}}),this.mountSelect(this.ref(d,"startHole"),u,{value:this.bound(u,()=>String(this.svc.startHole.get()),c=>this.svc.startHole.set(Number(c))),options:{get:()=>this.svc.startHoleOptions().map(c=>({value:String(c),label:String(c)}))},disabled:{get:()=>n()}}),this.$each(this.ref(d,"friendRows"),()=>$e(this.friends.friends.get().filter(c=>!this.svc.hasPlayer(c.id)),"frecency"),(c,m,p)=>this.wireEl(lr,{row:{onclick:()=>this.svc.addFriend({id:c.id,displayName:c.displayName,handicapIndex:c.handicapIndex,gender:c.gender})},name:()=>c.displayName,username:()=>`@${c.username}`,hcp:()=>c.handicapIndex===null?"–":c.handicapIndex.toFixed(1)},p),c=>c.id),this.$each(this.ref(d,"players"),this.svc.players,(c,m,p)=>this.playerRow(c.key,p),c=>c.key),this.$each(this.ref(d,"teams"),this.svc.teams,(c,m,p)=>this.teamCard(c.key,p),c=>c.key),this.$each(this.ref(d,"groups"),this.svc.groups,(c,m,p)=>this.groupCard(c.key,p),c=>c.key),this.$each(this.ref(d,"formats"),this.svc.formatSlots,(c,m,p)=>this.formatCard(c.key,m,p),c=>c.key),d}mountSelect(e,t,s){const n=new re(s);n.mount(e),t(()=>n.destroy())}bound(e,t,s){const n=new h(t());return e($(()=>n.set(t()))),e($(()=>{const i=n.get();queueMicrotask(()=>s(i))})),n}eachInto(e,t,s,n,i){const a=new Map,d=new Map;t(()=>{for(const u of d.values())u.forEach(c=>c());d.clear()}),t($(()=>{const u=s(),c=new Map;for(const[p,g]of u.entries()){const x=i(g,p);if(a.has(x))c.set(x,a.get(x));else{const y=[];c.set(x,n(g,p,v=>y.push(v))),d.set(x,y)}}for(const[p,g]of a)c.has(p)||(g.remove(),d.get(p)?.forEach(x=>x()),d.delete(p));let m=e.firstChild;for(const p of c.values())p===m?m=m.nextSibling:e.insertBefore(p,m);a.clear();for(const[p,g]of c)a.set(p,g)}))}formatCard(e,t,s){const n=()=>this.svc.slotByKey(e),i=()=>n()?.formatId??"",a=this.wireEl(ir,{remove:{onclick:()=>this.svc.removeFormatSlot(e)},desc:{textContent:()=>this.svc.catalog.byId(i())?.description??""},allowance:{value:this.svc.slotByKey(e)?.allowancePct??"100",oninput:u=>this.svc.setSlotAllowance(e,u.target.value)},allowanceHint:{textContent:()=>this.svc.isSideFormat(i())?"applied to each side member’s ball":"of each player’s course handicap"},err:{textContent:()=>this.svc.humanizedForFormat(t).join(" · ")}},s);this.mountSelect(this.ref(a,"format"),s,{value:this.bound(s,()=>i(),u=>{u&&u!==this.svc.slotByKey(e)?.formatId&&this.svc.setSlotFormat(e,u)}),options:{get:()=>this.svc.catalog.descriptors.get().map(u=>({value:u.id,label:this.svc.catalog.labelOf(u)??u.label}))}});const d=()=>{const u=this.svc.isSideFormat(i()),c=[];u||c.push(...this.svc.players.get().map(m=>({kind:"player",subKey:m.key})));for(const m of this.svc.teams.get())this.svc.teamKindFitsFormat(i(),m.kind)&&c.push({kind:"team",subKey:m.key});return c};return this.eachInto(this.ref(a,"subjectRows"),s,d,(u,c,m)=>this.subjectRow(e,u.kind,u.subKey,m),u=>`${u.kind}${u.subKey}`),a}subjectRow(e,t,s,n){const i=()=>{if(t==="player")return this.svc.players.get().find(c=>c.key===s)?.name?.trim()||"Player";const u=this.svc.teamByKey(s);return u?`${this.svc.teamLabel(u)} (${u.kind==="multi_ball"?"side":"team"})`:"Team"},a=()=>t==="player"?this.svc.subjectPlayerIn(e,s):this.svc.subjectTeamIn(e,s),d=u=>t==="player"?this.svc.setSubjectPlayer(e,s,u):this.svc.setSubjectTeam(e,s,u);return this.wireEl(We,{chk:{checked:()=>a(),onchange:u=>d(u.target.checked)},name:{textContent:()=>i()}},n)}groupCard(e,t){const s=this.wireEl(ar,{remove:{onclick:()=>this.svc.removeGroup(e)},groupName:{textContent:()=>{const n=this.svc.groupByKey(e);return n?this.svc.groupLabel(n):"Group"}},time:{value:this.svc.groupByKey(e)?.startTime??"",oninput:n=>this.svc.setGroupStartTime(e,n.target.value)},meta:{textContent:()=>{const n=this.svc.groupSize(e);return n===0?"Tick the players who walk with this group.":`${n} player${n===1?"":"s"}`}}},t);return this.mountSelect(this.ref(s,"hole"),t,{value:this.bound(t,()=>{const n=this.svc.groupByKey(e)?.startHole;return n==null?"":String(n)},n=>this.svc.setGroupStartHole(e,n===""?null:Number(n))),options:{get:()=>[{value:"",label:"First hole"},...this.svc.startHoleOptions().map(n=>({value:String(n),label:`Hole ${n}`}))]}}),this.eachInto(this.ref(s,"memberRows"),t,()=>this.svc.players.get(),(n,i,a)=>this.groupMemberRow(e,n.key,a),n=>n.key),s}groupMemberRow(e,t,s){return this.wireEl(We,{chk:{checked:()=>this.svc.groupMemberIn(e,t),onchange:n=>this.svc.setGroupMember(e,t,n.target.checked)},name:{textContent:()=>this.svc.players.get().find(n=>n.key===t)?.name?.trim()||"Player"}},s)}teamCard(e,t){const s=()=>this.svc.teamKindOf(e)==="multi_ball",n=this.wireEl(or,{remove:{onclick:()=>this.svc.removeTeam(e)},teamName:{textContent:()=>{const i=this.svc.teamByKey(e);return i?this.svc.teamLabel(i):"Team"}},compGroup:{hidden:()=>s()},membersLabel:{textContent:()=>s()?"Members (each a ball)":"Members & allowance"},teamMeta:{textContent:()=>{const i=this.svc.teamSize(e);if(i===0)return s()?"Tick at least 2 members — a side needs ≥2 balls.":"Tick at least 2 players to form a team ball.";if(i<2)return"Add one more member — a team needs at least 2.";if(s())return`${i} balls · a side (scored together by a side format)`;const a=this.svc.teamBallCh(e);return a===null?`${i} players`:`${i} players · plays off CH ${a}`}}},t);return this.mountSelect(this.ref(n,"kindSel"),t,{value:this.bound(t,()=>this.svc.teamKindOf(e),i=>this.svc.setTeamKind(e,i==="multi_ball"?"multi_ball":"single_ball")),options:{get:()=>[{value:"single_ball",label:"One combined ball"},{value:"multi_ball",label:"Separate balls (a side)"}]}}),this.mountSelect(this.ref(n,"formation"),t,{value:this.bound(t,()=>this.svc.teamByKey(e)?.formation??"scramble",i=>this.svc.setTeamFormation(e,i)),options:{get:()=>this.svc.formations.map(i=>({value:i,label:i[0].toUpperCase()+i.slice(1)}))}}),this.eachInto(this.ref(n,"memberRows"),t,()=>{const i=this.svc.players.get().map(a=>({kind:"player",mKey:a.key}));if(s())for(const a of this.svc.eligibleNestedTeams(e))i.push({kind:"team",mKey:a.key});return i},(i,a,d)=>i.kind==="player"?this.teamMemberRow(e,i.mKey,d):this.teamNestedRow(e,i.mKey,d),i=>`${i.kind}${i.mKey}`),n}teamNestedRow(e,t,s){const n=()=>this.svc.teamHasTeamMember(e,t);return this.wireEl(Ve,{chk:{checked:()=>n(),disabled:()=>!n()&&this.svc.teamAtMaxSize(e),onchange:i=>this.svc.setTeamMemberTeam(e,t,i.target.checked)},name:{textContent:()=>{const i=this.svc.teamByKey(t);return i?`${this.svc.teamLabel(i)} (combined ball)`:"Team"}},pctWrap:{hidden:()=>!0},pct:{value:"100",oninput:()=>{}}},s)}teamMemberRow(e,t,s){const n=()=>this.svc.players.get().find(a=>a.key===t)??null,i=()=>this.svc.teamMemberIn(e,t);return this.wireEl(Ve,{chk:{checked:()=>i(),disabled:()=>!i()&&this.svc.teamAtMaxSize(e),onchange:a=>this.svc.setTeamMember(e,t,a.target.checked)},name:{textContent:()=>n()?.name?.trim()||"Player"},pctWrap:{hidden:()=>!i()||this.svc.teamKindOf(e)==="multi_ball"},pct:{value:this.svc.teamByKey(e)?.pctByPlayer[t]??"100",oninput:a=>this.svc.setTeamPct(e,t,a.target.value)}},s)}playerRow(e,t){const s=()=>this.svc.players.get().find(a=>a.key===e)??null,n=()=>this.svc.players.get().findIndex(a=>a.key===e),i=this.wireEl(rr,{name:{value:s()?.name??"",readOnly:()=>!!s()?.playerId,oninput:a=>this.svc.patchPlayer(e,{name:a.target.value})},index:{value:s()?.handicapIndex??"",oninput:a=>this.svc.patchPlayer(e,{handicapIndex:a.target.value})},remove:{onclick:()=>this.svc.removePlayer(e)},ch:{textContent:()=>{const a=s();if(!a)return"";const d=this.svc.derivedCH(a);if(!d)return"";const u=d.rating;return`Course handicap ${d.ch}  ·  ${a.handicapIndex} × ${u.slope}/113 + (${u.courseRating} − ${u.par}) = ${d.raw.toFixed(1)}`}},err:{textContent:()=>this.svc.diagnosticsForPlayer(n()).map(a=>a.message).join(" · ")}},t);return this.mountSelect(this.ref(i,"gender"),t,{value:this.bound(t,()=>s()?.gender??"M",a=>this.svc.patchPlayer(e,{gender:a})),options:{get:()=>[{value:"M",label:"M"},{value:"F",label:"F"}]},disabled:{get:()=>s()?.genderKnown===!0}}),this.mountSelect(this.ref(i,"tee"),t,{value:this.bound(t,()=>s()?.teeId??"",a=>this.svc.patchPlayer(e,{teeId:a})),options:{get:()=>this.svc.tees.get().map(a=>({value:a.id,label:a.name}))},placeholder:"Tee"}),i}}const cr=b(`
...
`);class ur extends T{static styles=`
        .login {
            max-width: 340px;
            margin: 0 auto;
            padding: 14vh ${o("xl")} 0;
...
`);function Qe(r){return r.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class fr extends T{static styles=`
        .friends {
            padding: ${o("xl")} ${o("lg")} ${o("2xl")};
            ...
            & .friends__err {
                margin: ${o("sm")} 0 0; font-size: 0.85rem; color: ${l("error")};
                &:empty { display: none; }
            }

            & .friends__empty {
                color: ${l("text-muted")}; font-size: 0.9rem; padding: ${o("md")} 0;
                &.hidden { display: none; }
            }
    ...
    `;svc=this.inject(ke);auth=this.inject(L);router=this.inject(j);render(){const e=()=>this.auth.currentUser.get()!==null;e()&&this.svc.load();const t=this.wire(hr,{anon:{className:()=>e()?"friends__anon hidden":"friends__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/friends"}})},body:{className:()=>e()?"friends__body":"friends__body hidden"},search:{value:()=>this.svc.query.get(),oninput:n=>this.svc.setQuery(n.target.value)},searchHint:{textContent:()=>{const n=this.svc.query.get().trim();return n.length>0&&!Ke(n)?"Type at least 2 characters.":this.svc.searching.get()?"Searching…":""}},searchErr:{textContent:()=>this.svc.searchError.get()?.message??""},resultsEmpty:{className:()=>{const n=this.svc.query.get().trim();return Ke(n)&&!this.svc.searching.get()&&this.svc.searchError.get()===null&&this.svc.resultsFor.get()===n&&this.svc.results.get().length===0?"friends__empty":"friends__empty hidden"}},friendsEmpty:{className:()=>this.svc.loaded.get()&&this.svc.friends.get().length===0?"friends__empty":"friends__empty hidden"},sortToggle:{className:()=>this.svc.friends.get().length>0?"friends__sort":"friends__sort hidden"},sortFrecency:{"aria-pressed":()=>String(this.svc.sortMode.get()==="frecency"),onclick:()=>this.svc.setSortMode("frecency")},sortAlpha:{"aria-pressed":()=>String(this.svc.sortMode.get()==="alpha"),onclick:()=>this.svc.setSortMode("alpha")}});this.$each(this.ref(t,"results"),this.svc.results,(n,i,a)=>this.wireEl(pr,{initials:()=>Qe(n.displayName),name:()=>n.displayName,username:()=>`@${n.username}`,hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),add:{className:()=>this.isFriendNow(n.id)?"friend-row__add hidden":"friend-row__add",disabled:()=>this.svc.mutating.get(),onclick:()=>{const d=this.svc.results.get().find(u=>u.id===n.id);d&&!d.isFriend&&this.svc.add(d)}},added:{className:()=>this.isFriendNow(n.id)?"friend-row__added":"friend-row__added hidden"}},a),n=>n.id);const s=new Date().toISOString();return this.$each(this.ref(t,"friends"),()=>$e(this.svc.friends.get(),this.svc.sortMode.get()),(n,i,a)=>this.wireEl(mr,{initials:()=>Qe(n.displayName),name:()=>n.displayName,subtitle:()=>{const d=this.svc.friends.get().find(u=>u.id===n.id)??n;return Yn(d,s)},hcp:()=>n.handicapIndex===null?"–":n.handicapIndex.toFixed(1),remove:{disabled:()=>this.svc.mutating.get(),onclick:()=>{this.svc.remove(n.id)}}},a),n=>n.id),t}isFriendNow(e){return this.svc.results.get().find(t=>t.id===e)?.isFriend===!0}}const gr=b(`
    <div class="profile">
...
`);class yr extends T{static styles=`
        .profile {
            ...
            & .profile__card {
                ...
                & .profile__edit {
                    ...
                    & input { width: 90px; padding: ${o("md")}; font-size: 1rem; text-align: center; ${A()} }
                    & button {
                ...
                & .profile__hint { margin: ${o("sm")} 0 0; font-size: 0.8rem; color: ${l("text-muted")}; }
                & .profile__err {
                    margin: ${o("sm")} 0 0; font-size: 0.85rem; color: ${l("error")};
                    &:empty { display: none; }
                }
    ...
    `;svc=this.inject(ut);friends=this.inject(ke);auth=this.inject(L);router=this.inject(j);indexDraft=new h("");localErr=new h("");render(){this.auth.currentUser.get()&&this.svc.load();const e=()=>this.auth.currentUser.get()!==null,t=this.wire(gr,{anon:{className:()=>e()?"profile__anon hidden":"profile__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/profile"}})},body:{className:()=>e()?"profile__body":"profile__body hidden"},name:()=>this.svc.player.get()?.displayName??"…",username:()=>{const n=this.svc.player.get();return n?`@${n.username}`:""},hcp:()=>{const n=this.svc.player.get()?.handicapIndex;return n==null?"–":n.toFixed(1)},index:{value:()=>this.indexDraft.get(),oninput:n=>this.indexDraft.set(n.target.value)},save:{disabled:()=>this.svc.saving.get()||this.indexDraft.get().trim()==="",textContent:()=>this.svc.saving.get()?"Saving…":"Save"},form:{onsubmit:async n=>{n.preventDefault(),this.localErr.set("");const i=this.indexDraft.get().trim().replace(",","."),a=Number.parseFloat(i);if(!Number.isFinite(a)||a<-10||a>54){this.localErr.set("Enter an index between -10 and 54.");return}await this.svc.saveIndex(a)&&this.indexDraft.set("")}},saveErr:{textContent:()=>this.localErr.get()||this.svc.saveError.get()?.message||""},genderErr:{textContent:()=>this.svc.saveError.get()?.message||""},historyEmpty:{className:()=>this.svc.history.get().length===0?"profile__empty":"profile__empty hidden"},signout:{onclick:async()=>{await this.auth.logout(),this.svc.clear(),this.friends.clear(),this.router.navigate("/")}}});this.$each(this.ref(t,"history"),this.svc.history,(n,i,a)=>this.wireEl(br,{index:()=>n.handicapIndex.toFixed(1),source:()=>n.source,date:()=>n.effectiveDate},a),n=>n.id);const s=[{value:"M",label:"M"},{value:"F",label:"F"},{value:null,label:"Not set"}];return this.$each(this.ref(t,"gender"),()=>s,(n,i,a)=>this.wireEl(b('<button bind="b" type="button"></button>'),{b:{textContent:()=>n.label,className:()=>this.svc.player.get()?.gender===n.value?"on":"",disabled:()=>this.svc.saving.get(),onclick:()=>{this.svc.saveGender(n.value)}}},a),n=>n.label),t}}const _r=b(`
    <div class="app-shell">
...
`);class vr extends T{static styles=`
    ...
    `;router=this.inject(j);render(){const e=this.wire(_r,{});return this.spawn(Ut,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Me,"/history":Ss,"/round":On,"/create":dr,"/login":ur,"/friends":fr,"/profile":yr},Me),e}}B.get(Pt);const Ye=B.get(j),Xe=B.get(L);await jt(vr,"#app",{hot:void 0,onInit:async()=>{await Xe.load(),Xe.currentUser.get()&&Ye.route.get()==="/login"&&Ye.navigate("/",!0)}});export{T as C,j as R,h as S,Pt as T,f as a,ne as b,k as c,$ as e,E as r,b as t};
