...
`);class un extends I{static styles=`
  ...
  ${l}
  ${m}${p}${f}
</article>`}function yn(r,e,t,s){return ze(r,e,t,s)}function vn(r,e,t,s){return ze(r,e,t,{...s,cardModifier:"lb-card--compact-match"})}function wn(r,e,t,s){return ze(r,e,t,{...s,cardModifier:"lb-card--category-matrix"})}function xn(r,e){const t=new Set(r.map(e));return t.size!==1?null:[...t][0]??null}function $n(r){if(r===void 0)return"";const e=r===0?"E":r>0?`+${r}`:`−${Math.abs(r)}`;return` <span class="lb-rank__pace lb-rank__pace--${r===0?"even":r>0?"over":"under"}">${S(e)}</span>`}function kn(r,e,t=St){const s=r.entries.map(n=>{const i=xn(n.ballIds,t),l=i?` <span class="lb-rank__group">${S(i)}</span>`:"";return`<tr class="${n.position===1?"lb-rank__lead":""}">
  <td class="lb-rank__pos">${n.position}</td>
  <td class="lb-rank__who"><span class="lb-rank__name">${S(n.ballIds.map(e).join(" & "))}</span>${l}</td>
  <td class="lb-rank__total">${n.total??"—"}${$n(n.paceDelta)}</td>
...
</div>`}function Sn(r,e){const t=r.matches.map(s=>{const n=S(s.sideA.ballIds.map(e).join(" & ")),i=S(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,u=s.leader==="a"?" lb-mp__team--lead":"",c=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const In={ranked:kn,match_summary:(r,e)=>Sn(r,e)},Tn={"default-score-grid":yn,"compact-match-grid":vn,"category-matrix-grid":wn};function Cn(r){return r.componentId??"default-score-grid"}function En(r){return`<div class="lb-diag">Unrenderable result section <code>${S(r)}</code> — no generic view yet. Results are not hidden.</div>`}function Nn(r){return`<div class="lb-diag">Unsupported score-grid component <code>${S(r)}</code> — no generic view yet. Results are not hidden.</div>`}function Pn(r,e,t){const s=In[r.kind];return s?s(r,e,t):En(r.kind)}function zn(r,e,t,s){const n=Cn(r),i=Tn[n];return i?i(r,e,t,s):Nn(n)}function Rn(r,e,t=St){return r.leaderboard.length===0&&r.cards.length===0?`<div class="lb-empty">No scores entered yet for ${S(r.formatLabel)}.</div>`:r.leaderboard.map(n=>Pn(n,e,t)).join("")||`<div class="lb-empty">No leaderboard metric for ${S(r.formatLabel)}.</div>`}function jn(r,e,t,s={}){if(r.cards.length===0)return"";const n=s.mode??"product";return r.cards.map(i=>zn(i,e,t,{mode:n})).join(`
`)}const On=b(`
...
`);class Ln extends I{static styles=`
    ...
    `;svc=this.inject(Z);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(On,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=u=>{const c=this.svc.nameOf(u);return this.svc.isPending(u)?`${c} (open seat)`:c},i=Rn(t,s,u=>this.svc.groupLabelOf(u)),l=jn(t,e.routeSections,s),d=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return i+d}}function Dn(r,e){if(!e)return[];const t=[],s=new Set;for(const n of r)for(const i of n.players){if(i.playerId===e)return[];i.guestPlayerId===null||s.has(i.guestPlayerId)||(s.add(i.guestPlayerId),t.push({guestPlayerId:i.guestPlayerId,displayName:i.displayName}))}return t}const Mn=b(`
...
`);class An extends I{static styles=`
    ...
    `;let J=De;function Fn(r){if(!r)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=r.seats.length>0,t=r.claimedSeats.some(i=>i.viewerMayRelease),s=r.viewer.claimSeat.allowed,n=r.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&s,guestAllowed:e&&n,blockedMessage:e&&!s&&!n?r.viewer.claimSeat.message??r.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function Bn(r,e){const t=[];if(r.groupId!==null&&e.length>0){const s=e.findIndex(n=>n.id===r.groupId);if(s>=0){t.push(`Group ${s+1}`);const n=e[s].startTime;n.includes(":")&&t.push(n)}}return r.category!==null&&t.push(r.category),t.join(" · ")}function Gn(r){return(r?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const qn=b(`
...
`);class ei extends I{static styles=`
    ...
    `;router=this.inject(N);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&_.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(Zn,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function ti(r,e){if(!e)return!1;for(const t of r)for(const s of t.players)if(s.playerId===e)return!0;return!1}const si=b(`
...
`);class ni extends I{static styles=`
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return ti(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await _.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(si,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(B,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function ii(r){return!(r.tab!=="leaderboard"||!r.pageVisible||r.status==="complete")}const ri=2e4;function oi(r){if(!(r===null||r===""))return/^\d+$/.test(r)?Number(r):r}const ai=b(`
...
`),li=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),di=b('<button bind="pill" class="round-view__grp" type="button"></button>');class ci extends I{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);function lt(r){return r.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class Ki extends I{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),mr=b('<option bind="option"></option>');class gr extends I{competitions=this.inject(ee);state=this.inject(de);router=this.inject(N);render(){const e=new y(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(hr,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Yi(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:s=>{s.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:s=>this.state.roundCourseDraft.set(s.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:s=>this.state.roundDateDraft.set(s.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(s,n,i)=>this.wireEl(mr,{option:{value:()=>s.id,textContent:()=>s.name}},i),s=>s.id),this.$each(this.ref(t,"rounds"),e,(s,n,i)=>this.wireEl(pr,{row:{disabled:()=>!s.shareToken,onclick:()=>{s.shareToken&&this.router.navigate("/round",{query:{token:s.shareToken}})}},number:()=>`Round ${s.roundNumber}`,meta:()=>[s.courseNameSnapshot,s.date].filter(Boolean).join(" · ")||(s.shareToken?"Open":"View-only"),status:{textContent:()=>ur[s.status]??s.status,className:()=>`cd__rstatus s-${s.status}`}},i),s=>JSON.stringify({id:s.id,status:s.status,shareToken:s.shareToken,courseName:s.courseNameSnapshot,date:s.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function fr(r,e,t){return JSON.stringify({entry:r,points:e,columns:t})}function br(r){return r.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const _r=b(`
    <div>
...
`),yr=b('<button bind="button" type="button"></button>'),vr=b('<th bind="cell"></th>'),wr=b('<tr bind="row"></tr>'),xr=b('<td bind="cell"><span bind="value"></span></td>'),$r=b(`
...
`);class Er extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Cr,{});return this.spawn(js,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Qe,"/history":Ws,"/round":ci,"/create":Hi,"/login":Fi,"/friends":Ki,"/profile":Wi,...vt.competitions?{"/competitions":Zi,"/competition":Tr}:{}},Qe),e}}P.get(Qt);const dt=P.get(N),ct=P.get(L);await Xt(Er,"#app",{hot:void 0,onInit:async()=>{await ct.load(),ct.currentUser.get()&&dt.route.get()==="/login"&&dt.navigate("/",!0)}});export{I as C,N as R,h as S,Qt as T,g as a,ae as b,y as c,k as e,C as r,b as t};
