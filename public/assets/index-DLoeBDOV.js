...
</div>`}function Ht(o,e){const t=o.lines.map(s=>{const n=s.segments.map(r=>"text"in r?x(r.text):x(r.ballIds.map(e).join(" & "))).join("");return`<li class="lb-match__line lb-match__line--${s.result}">${n}</li>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${x(o.title)}</h4>
  <ul class="lb-match">${t}</ul>
</div>`}function Rt(o){return`<div class="lb-diag">Unrenderable result section <code>${x(o)}</code> — no generic view yet. Results are not hidden.</div>`}function Lt(o,e){return o.leaderboard.length===0&&o.cards.length===0?`<div class="lb-empty">No scores entered yet for ${x(o.formatLabel)}.</div>`:o.leaderboard.map(s=>s.kind==="ranked"?Nt(s,e):s.kind==="match_summary"?Ht(s,e):Rt(s.kind)).join("")||`<div class="lb-empty">No leaderboard metric for ${x(o.formatLabel)}.</div>`}function Mt(o,e,t){return o.cards.length===0?"":o.cards.map(s=>Ot(s,e,t)).join(`
`)}const Bt=y(`
...
`),vs=y('<div bind="row" class="results-slot"></div>');class xs extends T{static styles=`
        .results {
            padding: ${l("lg")} ${l("lg")} ${l("2xl")};
...
`);class $s extends T{static styles=`
    ...
    `;router=this.inject(z);render(){const e=this.wire(ws,{});return this.spawn(Ze,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":de,"/round":qt,"/create":ts,"/login":ns,"/rounds":hs,"/players":ls,"/score":bs,"/results":xs},de),e}}j.get(Le);const fe=j.get(z),ge=j.get(te);await Be($s,"#app",{hot:void 0,onInit:async()=>{await ge.load(),ge.currentUser.get()&&fe.route.get()==="/login"&&fe.navigate("/",!0)}});export{T as C,z as R,h as S,Le as T,f as a,G as b,$ as c,_ as e,w as r,y as t};
