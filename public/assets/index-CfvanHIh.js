(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(r){if(r.ep)return;r.ep=!0;const n=t(r);fetch(r.href,n)}})();const qe="modulepreload",Ge=function(i){return"/tapscore/"+i},me={},Ke=function(e,t,s){let r=Promise.resolve();if(t&&t.length>0){let c=function(u){return Promise.all(u.map(f=>Promise.resolve(f).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),d=o?.nonce||o?.getAttribute("nonce");r=c(t.map(u=>{if(u=Ge(u),u in me)return;me[u]=!0;const f=u.endsWith(".css"),p=f?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${p}`))return;const g=document.createElement("link");if(g.rel=f?"stylesheet":qe,f||(g.as="script"),g.crossOrigin="",g.href=u,d&&g.setAttribute("nonce",d),document.head.appendChild(g),f)return new Promise((_,M)=>{g.addEventListener("load",_),g.addEventListener("error",()=>M(new Error(`Unable to preload CSS for ${u}`)))})}))}function n(o){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=o,window.dispatchEvent(d),!d.defaultPrevented)throw o}return r.then(o=>{for(const d of o||[])d.status==="rejected"&&n(d.reason);return e().catch(n)})};class Ue{constructor(){this.tracking=null,this.batching=!1,this.pending=new Set}subscribe(e){this.tracking&&(e.add(this.tracking),this.tracking.deps.add(e))}notify(e){for(const t of[...e])this.batching?this.pending.add(t):t.run()}runTracked(e,t){Pe(e);const s=this.tracking;this.tracking=e;try{t()}finally{this.tracking=s}}untrack(e){const t=this.tracking;this.tracking=null;try{return e()}finally{this.tracking=t}}batch(e){this.batching=!0;try{e()}finally{this.batching=!1;const t=[...this.pending];this.pending.clear();for(const s of t)s.run()}}}const L=new Ue;function Pe(i){for(const e of i.deps)e.delete(i);i.deps.clear()}class h{constructor(e){this.subs=new Set,this.val=e}get(){return L.subscribe(this.subs),this.val}peek(){return this.val}set(e){Object.is(this.val,e)||(this.val=e,L.notify(this.subs))}update(e){this.set(e(this.val))}}class E{constructor(e){this.subs=new Set,this.val=void 0;const t=this,s={run(){L.runTracked(s,()=>{const r=e();Object.is(t.val,r)||(t.val=r,L.notify(t.subs))})},deps:new Set};s.run()}get(){return L.subscribe(this.subs),this.val}peek(){return this.val}}function v(i){const e={run(){L.runTracked(e,i)},deps:new Set};return e.run(),()=>Pe(e)}function X(i){L.batch(i)}function q(i){return L.untrack(i)}class We{constructor(){this.instances=new Map}get(e){let t=this.instances.get(e);return t||(t=new e,this.instances.set(e,t)),t}set(e,t){this.instances.set(e,t)}reset(){this.instances.clear()}}const H=new We,U="/tapscore/".replace(/\/+$/,"");function oe(i){return U?i===U?"/":i.startsWith(U+"/")?i.slice(U.length):i:i}function Xe(i){return U+i}class N{constructor(){this.route=new h(oe(location.pathname??"/")),this.search=new h(location.search??""),window.addEventListener("popstate",()=>X(()=>{this.route.set(oe(location.pathname)),this.search.set(location.search)}))}navigate(e,t){const s=typeof t=="boolean"?{replace:t}:t??{},r=e.indexOf("#"),n=r>=0?e.slice(r):"",o=r>=0?e.slice(0,r):e,d=o.indexOf("?"),c=d>=0?o.slice(0,d):o,u=d>=0?o.slice(d+1):"",f=s.query!==void 0?Qe(s.query):u?"?"+u:"",p=Xe(c)+f+n;(s.replace?history.replaceState:history.pushState).call(history,null,"",p),X(()=>{this.route.set(c),this.search.set(f)})}back(){history.back()}link(e,t="active"){const s=e.split("#")[0].split("?")[0];return{onclick:r=>{r.preventDefault(),this.navigate(e)},className:()=>{const r=this.route.get();return r===s||r.startsWith(s+"/")?t:""}}}params(e){const t=e.split("/");return new E(()=>{const s=this.route.get().split("/"),r={};for(const[n,o]of t.entries())o.startsWith(":")&&(r[o.slice(1)]=s[n]??"");return r})}query(e){return new E(()=>new URLSearchParams(this.search.get()).get(e)??void 0)}queries(){return new E(()=>{const e={};for(const[t,s]of new URLSearchParams(this.search.get()))e[t]=s;return e})}}function Qe(i){const e=new URLSearchParams;for(const[s,r]of Object.entries(i))r==null||r===""||e.set(s,String(r));const t=e.toString();return t?"?"+t:""}function Ve(i){return e=>i[e]}function Ye(i,e){const t=(r,n,o)=>{const d=Object.entries(r).map(([c,u])=>`--${c}:${u}`).join(";");return`${n}{color-scheme:${o};${d}}`},s=document.createElement("style");return s.textContent=t(i,'[data-theme="light"]',"light")+t(e,'[data-theme="dark"]',"dark"),document.head.appendChild(s),r=>`var(--${r})`}const fe="basics-js-theme";class Je{constructor(){this.dark=new h(!1);const e=localStorage.getItem(fe),t=matchMedia("(prefers-color-scheme: dark)").matches;this.dark.set(e?e==="dark":t),v(()=>{const s=this.dark.get();document.documentElement.setAttribute("data-theme",s?"dark":"light"),localStorage.setItem(fe,s?"dark":"light")})}toggle(){this.dark.update(e=>!e)}}function b(i){const e=document.createElement("template");return e.innerHTML=i,e}function Ze(i,e){let t;for(const s of Object.keys(e))i.startsWith(s+"/")&&(!t||s.length>t.length)&&(t=s);return t?e[t]:void 0}const ge=new Set;class T{constructor(e={}){this.props=e,this.disposers=[],this.children=[];const t=this.constructor;if(t.styles&&!ge.has(t)){ge.add(t);const s=document.createElement("style");s.textContent=t.styles,document.head.appendChild(s)}}onMount(){}onDestroy(){}inject(e){return H.get(e)}track(e){this.disposers.push(e)}ref(e,t){return e.querySelector(`[bind="${t}"]`)}spawn(e,t,...s){const r=q(()=>{const n=new e(s[0]);return n.mount(t),n});return this.children.push(r),r}mount(e){e.appendChild(this.render()),this.onMount()}destroy(){this.onDestroy();for(const e of this.children)e.destroy();this.children.length=0;for(const e of this.disposers)e();this.disposers.length=0}wire(e,t,s){const r=s??(o=>this.track(o)),n=e.content.cloneNode(!0);for(const o of n.querySelectorAll("[bind]")){const d=t[o.getAttribute("bind")];if(d)if(typeof d=="function")r(v(()=>{const c=d();o instanceof HTMLInputElement||o instanceof HTMLTextAreaElement?o.value=String(c):o.textContent=String(c)}));else for(const[c,u]of Object.entries(d)){const f=c.includes("-");c.startsWith("on")&&typeof u=="function"?o.addEventListener(c.slice(2),u):typeof u=="function"?r(v(()=>{const p=u();f?o.setAttribute(c,String(p)):o[c]=p})):f?o.setAttribute(c,String(u)):o[c]=u}}return n}wireEl(e,t,s){return this.wire(e,t,s).firstElementChild}slot(e,t){const s=this.props[e];if(s==null)return!1;const r=this.ref(t,e);return r?(typeof s=="string"?r.textContent=s:typeof s=="function"&&s.prototype instanceof T?this.spawn(s,r):typeof s=="function"&&s(r,{spawn:(n,o,...d)=>this.spawn(n,o,...d),track:n=>this.track(n)}),!0):!1}$each(e,t,s,r=(n,o)=>o){const n=typeof t=="function"?t:()=>t.get(),o=new Map,d=new Map;this.track(()=>{for(const c of d.values())c.forEach(u=>u());d.clear()}),this.track(v(()=>{const c=n(),u=new Map;for(const[p,g]of c.entries()){const _=r(g,p);if(o.has(_))u.set(_,o.get(_));else{const M=[];u.set(_,q(()=>s(g,p,w=>M.push(w)))),d.set(_,M)}}for(const[p,g]of o)u.has(p)||(g.remove(),d.get(p)?.forEach(_=>_()),d.delete(p));let f=e.firstChild;for(const p of u.values())p===f?f=f.nextSibling:e.insertBefore(p,f);o.clear();for(const[p,g]of u)o.set(p,g)}))}$condition(e,t,s,r){let n=null;this.track(v(()=>{n&&(n.remove(),n=null);const o=t.get();n=q(()=>o?s():r?.()??null),n&&e.appendChild(n)}))}$swap(e,t,s,r){let n=null;this.track(v(()=>{n&&(n.destroy(),n=null),e.textContent="";const o=t.get(),d=s[o]??Ze(o,s)??r;d&&(n=q(()=>{const c=new d;return c.mount(e),c}))})),this.track(()=>n?.destroy())}}async function et(i,e,t){const s=document.querySelector(e);s.textContent="";const r=H.get(N);let n=null,o=!1,d=null,c=!!t?.hot?.data.hmr;const u=async f=>{n&&(n.destroy(),n=null,s.textContent=""),f?(d||(d=(await Ke(()=>import("./obs-shell.component-CSkZF4OI.js"),[])).ObsShellComponent),n=q(()=>new d)):(!c&&t?.onInit&&(await t.onInit(),c=!0),n=q(()=>new i)),q(()=>n.mount(s)),o=f};await u(oe(location.pathname).startsWith("/_obs")),v(()=>{const f=r.route.get().startsWith("/_obs");f!==o&&u(f)}),t?.hot&&(t.hot.data.hmr=!0,t.hot.dispose(()=>n?.destroy()),t.hot.accept())}class j extends Error{constructor(e,t,s,r){super(t),this.status=e,this.details=s,this.traceId=r,this.name="ApiError"}}const tt=10,Y=[];let J=[],W=null;function st(i){Y.push(i),Y.length>tt&&Y.shift()}function nt(i,e,t){const s={code:i,message:e,url:typeof location<"u"?location.href:"",context:[...Y],timestamp:new Date().toISOString()};t!==void 0&&(s.traceId=t),J.push(s),rt()}function rt(){W||(W=setTimeout(Ce,5e3))}function Ce(){if(W&&(clearTimeout(W),W=null),J.length===0)return;const i=J;J=[];for(const e of i){const t=JSON.stringify(e);typeof navigator<"u"&&navigator.sendBeacon?navigator.sendBeacon("/api/_obs/errors",new Blob([t],{type:"application/json"})):typeof fetch<"u"&&fetch("/api/_obs/errors",{method:"POST",headers:{"Content-Type":"application/json"},body:t}).catch(()=>{})}}typeof document<"u"&&document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&Ce()});const it=3e4,ot=2,Q=new Map,ze=new WeakMap;function at(i){if(i instanceof j)return i.traceId;if(i!=null&&typeof i=="object")return ze.get(i)}async function m(i){if(i.method==="GET"){const e=Q.get(i.url);if(e)return e;const t=be(i,ot);return Q.set(i.url,t),t.then(()=>Q.delete(i.url),()=>Q.delete(i.url)),t}return be(i,0)}async function be(i,e){const t=i.timeout??it;let s;for(let r=0;r<=e;r++){const n=crypto.randomUUID();try{return await dt(lt(i,n),t)}catch(o){if(s=o,!(o instanceof j)&&o!=null&&typeof o=="object"&&ze.set(o,n),o instanceof j||r===e)break;await new Promise(d=>setTimeout(d,1e3*2**r))}}throw s}async function lt(i,e){const t={"X-Trace-Id":e},s={method:i.method,headers:t};i.body!==void 0&&(t["Content-Type"]="application/json",s.body=JSON.stringify(i.body));const r=await fetch(i.url,s),n=r.headers.get("x-trace-id")??e;if(st({type:"api",detail:`${i.method} ${i.url}`,timestamp:new Date().toISOString()}),!r.ok){const o=await r.json().catch(()=>({error:r.statusText}));throw new j(r.status,o.error??r.statusText,o.details,n)}return r.json()}function dt(i,e){let t;const s=new Promise((r,n)=>{t=setTimeout(()=>n(new Error("Request timeout")),e)});return Promise.race([i,s]).finally(()=>clearTimeout(t))}async function I(i,e,t){X(()=>{i.set(!0),e.set(null)});try{const s=await t();return i.set(!1),s}catch(s){const r=ct(s);X(()=>{i.set(!1),e.set(r)}),nt(r.code,r.message,at(s));return}}function ct(i){return i instanceof j?i.status===401?{code:"auth",message:"Unauthorized"}:i.status===409?{code:"conflict",message:"Data has changed — please try again"}:i.status===400?{code:"validation",message:i.message}:{code:"server",message:"Server error"}:i instanceof Error?i.message==="Request timeout"?{code:"timeout",message:"Request timeout"}:{code:"network",message:"Network error"}:{code:"unknown",message:"Unknown error"}}function ut(i){return{me:()=>m({method:"GET",url:`${i}/auth/me`}),login:e=>m({method:"POST",url:`${i}/auth/login`,body:e}),logout:()=>m({method:"POST",url:`${i}/auth/logout`,body:{}})}}class G{constructor(){this.api=ut("/api"),this.currentUser=new h(null),this.loading=new h(!1),this.error=new h(null)}async load(){const e=await I(this.loading,this.error,()=>this.api.me());e&&this.currentUser.set(e),this.error.get()?.code==="auth"&&this.error.set(null)}async login(e,t){const s=await I(this.loading,this.error,()=>this.api.login({username:e,password:t}));return s?(this.currentUser.set(s),!0):!1}async logout(){await I(this.loading,this.error,()=>this.api.logout());const e=this.error.get();(!e||e.code==="auth")&&this.currentUser.set(null)}}const ye={radius:"12px","radius-pill":"999px","radius-sm":"6px","font-display":"'Fraunces', Georgia, serif",shadow:"0 1px 2px rgba(30, 53, 38, 0.08)","shadow-elevated":"0 4px 16px rgba(30, 53, 38, 0.14)"},l=Ye({...ye,bg:"#f2eee2",surface:"#fbf9f1","surface-sunken":"#e9e4d4",primary:"#2c5e3f","primary-text":"#f7f4ea","btn-bg":"#fbf9f1","btn-hover":"#efeada",text:"#1e3526","text-muted":"#6b7a6e",border:"#d8d2bf","topbar-bg":"#1e3526","active-bg":"#1e3526","active-text":"#f7f4ea","hover-bg":"#ece7d7","input-bg":"#ffffff",accent:"#b08d3e","accent-soft":"#f0e6cd",error:"#a0463c","under-par":"#a0463c","over-par":"#345b8a","hole-bar":"#e6a23f","hole-bar-text":"#3a2a0d"},{...ye,bg:"#15231a",surface:"#1d2f22","surface-sunken":"#101b14",primary:"#5d9b75","primary-text":"#0f1a13","btn-bg":"#24392b","btn-hover":"#2e4836",text:"#e6e1d2","text-muted":"#8da093",border:"#33493a","topbar-bg":"#0f1a13","active-bg":"#5d9b75","active-text":"#0f1a13","hover-bg":"#273c2e","input-bg":"#101b14",accent:"#cfa84f","accent-soft":"#3a3320",error:"#d48a82","under-par":"#d48a82","over-par":"#8db2e0","hole-bar":"#c08a35","hole-bar-text":"#160f04",shadow:"0 1px 2px rgba(0, 0, 0, 0.3)","shadow-elevated":"0 4px 16px rgba(0, 0, 0, 0.4)"}),S=i=>`var(--${i})`,a=Ve({xs:"0.25rem",sm:"0.5rem",md:"0.75rem",lg:"1rem",xl:"1.5rem","2xl":"2rem"}),k=(i=S("radius"))=>`
...
`,O=()=>`
    ...
    color: ${S("text")};
    font-family: inherit;
    &::placeholder { color: ${S("text-muted")}; }
`,z=i=>`
    ...
    &:hover { box-shadow: ${S("shadow-elevated")}; }`:""}
`,ht=b(`
...
`);class pt extends T{static styles=`
        .tabbar {
            display: flex;
    ...
    `;router=this.inject(N);auth=this.inject(G);render(){return this.wire(ht,{root:{className:()=>this.auth.currentUser.get()&&this.router.route.get()!=="/login"?"tabbar":"tabbar hidden"},roundsLink:this.router.link("/rounds"),playersLink:this.router.link("/players"),profileLink:this.router.link("/profile")})}}function mt(i){return{async me(){return m({method:"GET",url:`${i}/players/me`})},async register(e){return m({method:"POST",url:`${i}/players/register`,body:e})},async updateHandicap(e){return m({method:"POST",url:`${i}/players/me/handicap`,body:e})},async myHandicapHistory(){return m({method:"GET",url:`${i}/players/me/handicap-history`})}}}function ft(i){return{async list(){return m({method:"GET",url:`${i}/clubs`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/clubs/get${s?"?"+s:""}`})},async create(e){return m({method:"POST",url:`${i}/clubs`,body:e})},async update(e){return m({method:"POST",url:`${i}/clubs/update`,body:e})},async remove(e){return m({method:"DELETE",url:`${i}/clubs/${e.id}`})}}}function gt(i){return{async list(){return m({method:"GET",url:`${i}/courses`})},async listByClub(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/courses/by-club${s?"?"+s:""}`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/courses/get${s?"?"+s:""}`})},async create(e){return m({method:"POST",url:`${i}/courses`,body:e})},async update(e){return m({method:"POST",url:`${i}/courses/update`,body:e})},async updateHole(e){return m({method:"POST",url:`${i}/courses/holes/update`,body:e})},async validate(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/courses/validate${s?"?"+s:""}`})},async remove(e){return m({method:"DELETE",url:`${i}/courses/${e.id}`})}}}function bt(i){return{async listByCourse(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/tees/by-course${s?"?"+s:""}`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/tees/get${s?"?"+s:""}`})},async create(e){return m({method:"POST",url:`${i}/tees`,body:e})},async update(e){return m({method:"POST",url:`${i}/tees/update`,body:e})},async remove(e){return m({method:"DELETE",url:`${i}/tees/${e.id}`})}}}function yt(i){return{async list(){return m({method:"GET",url:`${i}/guest-players`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/guest-players/get${s?"?"+s:""}`})},async create(e){return m({method:"POST",url:`${i}/guest-players`,body:e})}}}function _t(i){return{async latest(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/handicap/latest${s?"?"+s:""}`})},async history(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/handicap/history${s?"?"+s:""}`})},async record(e){return m({method:"POST",url:`${i}/handicap/record`,body:e})}}}function vt(i){return{async list(){return m({method:"GET",url:`${i}/rounds`})},async balls(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/rounds/balls${s?"?"+s:""}`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/rounds/get${s?"?"+s:""}`})},async create(e){return m({method:"POST",url:`${i}/rounds`,body:e})},async createFromDraft(e){return m({method:"POST",url:`${i}/rounds/from-draft`,body:e})},async update(e){return m({method:"POST",url:`${i}/rounds/update`,body:e})},async remove(e){return m({method:"DELETE",url:`${i}/rounds/${e.id}`})}}}function xt(i){return{async listByRound(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/score-events/by-round${s?"?"+s:""}`})},async append(e){return m({method:"POST",url:`${i}/score-events`,body:e})}}}function wt(i){return{async forBall(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/scorecards/for-ball${s?"?"+s:""}`})},async forRound(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/scorecards/for-round${s?"?"+s:""}`})}}}function $t(i){return{async forRound(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/leaderboards/for-round${s?"?"+s:""}`})}}}function kt(i){return{async list(){return m({method:"GET",url:`${i}/friendly-rounds`})},async create(e){return m({method:"POST",url:`${i}/friendly-rounds`,body:e})},async byToken(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/friendly-rounds/by-token${s?"?"+s:""}`})},async get(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/friendly-rounds/get${s?"?"+s:""}`})},async balls(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/friendly-rounds/balls${s?"?"+s:""}`})},async scorecard(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/friendly-rounds/scorecard${s?"?"+s:""}`})},async result(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/friendly-rounds/result${s?"?"+s:""}`})},async score(e){return m({method:"POST",url:`${i}/friendly-rounds/score`,body:e})},async claimGuest(e){return m({method:"POST",url:`${i}/friendly-rounds/claim-guest`,body:e})}}}function St(i){return{async myRounds(){return m({method:"GET",url:`${i}/dashboard/my-rounds`})}}}function It(i){return{async courses(){return m({method:"GET",url:`${i}/setup/courses`})},async teesByCourse(e){const t=new URLSearchParams;for(const[r,n]of Object.entries(e))n!==void 0&&t.set(r,String(n));const s=t.toString();return m({method:"GET",url:`${i}/setup/tees/by-course${s?"?"+s:""}`})},async formats(){return m({method:"GET",url:`${i}/setup/formats`})}}}const P="/tapscore/".replace(/\/+$/,"")+"/api",x={players:mt(P),clubs:ft(P),courses:gt(P),tees:bt(P),guestPlayers:yt(P),handicap:_t(P),rounds:vt(P),scoreEvents:xt(P),scorecards:wt(P),leaderboards:$t(P),friendlyRounds:kt(P),dashboard:St(P),setup:It(P)};function Tt(i){return[...i.played?["Played"]:[],...i.created?["Created"]:[]].join(" · ")}function Et(i,e){const t=new Map;for(const s of e)t.set(s.round.id,{round:s.round,token:s.friendlyRound.shareToken,played:!1,created:!0});for(const s of i){const r=t.get(s.round.id);r?r.played=!0:t.set(s.round.id,{round:s.round,token:s.shareToken,played:!0,created:!1})}return[...t.values()].sort((s,r)=>r.round.date.localeCompare(s.round.date)||s.round.id.localeCompare(r.round.id))}class Pt{loading=new h(!1);error=new h(null);rounds=new h([]);mine=new h(null);mineLoading=new h(!1);mineError=new h(null);myRounds=new E(()=>{const e=this.mine.get();return e?Et(e.produced,e.created):[]});async load(){const e=await I(this.loading,this.error,()=>x.friendlyRounds.list());e&&this.rounds.set(e)}async loadMine(){const e=await I(this.mineLoading,this.mineError,()=>x.dashboard.myRounds());e&&this.mine.set(e)}}function Ct(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function zt(){return Ct()}class Z{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await I(this.loading,this.error,()=>x.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=zt()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}}function ee(i){const e=H.get(Z);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}const Nt=b(`
    <div class="landing">
...
`),_e={not_started:"Not started",active:"Live",complete:"Done"};class ve extends T{static styles=`
        .landing {
    ...
    `;svc=this.inject(Pt);auth=this.inject(G);router=this.inject(N);render(){this.svc.load();const e=()=>this.auth.currentUser.get()!==null;e()&&this.svc.loadMine();const t=this.wire(Nt,{createBtn:{onclick:()=>this.router.navigate("/create")},signin:{textContent:()=>e()?"Profile":"Sign in",onclick:()=>this.router.navigate(e()?"/profile":"/login")},mySection:{className:()=>e()?"landing__mine":"landing__mine hidden"},myCount:()=>{const s=this.svc.myRounds.get().length;return s===0?"":`${s} on the card`},myEmpty:{className:()=>e()&&this.svc.myRounds.get().length===0&&!this.svc.mineLoading.get()?"landing__empty":"landing__empty hidden"},count:()=>{const s=this.svc.rounds.get().length;return s===0?"":`${s} on the card`},empty:{className:()=>this.svc.rounds.get().length===0?"landing__empty":"landing__empty hidden"}});return this.$each(this.ref(t,"list"),this.svc.rounds,(s,r,n)=>this.wireEl(jt,{row:{onclick:()=>this.router.navigate("/round",{query:{token:s.friendlyRound.shareToken}})},course:()=>s.round.courseNameSnapshot??"Round",status:{textContent:()=>_e[s.round.status]??s.round.status,className:()=>`round-row__status s-${s.round.status}`},date:()=>s.round.date,formats:()=>s.round.formatSlots.map(ee).join(" · ")},n),s=>s.friendlyRound.id),this.$each(this.ref(t,"myList"),this.svc.myRounds,(s,r,n)=>this.wireEl(Ot,{row:{disabled:()=>s.token===null,onclick:()=>{s.token!==null&&this.router.navigate("/round",{query:{token:s.token}})}},course:()=>s.round.courseNameSnapshot??"Round",role:()=>Tt(s),status:{textContent:()=>_e[s.round.status]??s.round.status,className:()=>`round-row__status s-${s.round.status}`},date:()=>s.round.date,formats:()=>s.round.formatSlots.map(ee).join(" · ")},n),s=>s.round.id),t}}const Ht=180,xe=4,Mt=12;function K(i,e){return e<=0?0:Math.max(0,Math.min(e-1,i))}function Rt(i){const{dragDistance:e,velocity:t,itemWidth:s}=i;if(Math.abs(e)<Mt)return 0;const r=e+t*Ht,n=Math.round(-r/s);return Math.max(-xe,Math.min(xe,n))}const we="tapscore:pending-scores:v1",Lt=336*60*60*1e3,$e=200;function Bt(){try{return globalThis.localStorage??null}catch{return null}}function Ft(i){if(typeof i!="object"||i===null)return!1;const e=i;return typeof e.token=="string"&&typeof e.ballId=="string"&&typeof e.playHoleId=="string"&&(typeof e.strokes=="number"||e.strokes===null)&&(e.eventType==="score_entered"||e.eventType==="score_cleared")&&typeof e.clientEventId=="string"&&typeof e.queuedAt=="number"}class At{entries=[];storage;constructor(e=Bt(),t=Date.now()){this.storage=e,this.entries=this.load();const s=this.applyHygiene(t);s.length!==this.entries.length&&(this.entries=s,this.persist())}enqueue(e){const t=this.entries.findIndex(s=>s.token===e.token&&s.ballId===e.ballId&&s.playHoleId===e.playHoleId);t>=0?this.entries[t]=e:this.entries.push(e),this.entries=this.applyHygiene(e.queuedAt),this.persist()}remove(e){const t=this.entries.filter(s=>s.clientEventId!==e);t.length!==this.entries.length&&(this.entries=t,this.persist())}entriesFor(e){return this.entries.filter(t=>t.token===e)}size(){return this.entries.length}applyHygiene(e){const t=this.entries.filter(s=>e-s.queuedAt<=Lt);return t.length>$e?t.slice(t.length-$e):t}load(){if(!this.storage)return[];try{const e=this.storage.getItem(we);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t.filter(Ft):[]}catch{return[]}}persist(){if(this.storage)try{this.storage.setItem(we,JSON.stringify(this.entries))}catch{}}}const Dt=["1st","2nd","3rd","4th","5th","6th","7th","8th"],F=(i,e)=>`${i}|${e}`;function Ne(i){return i.players.map(e=>e.displayName).join(" & ")||i.label||"Ball"}function qt(i,e,t){return i?!(i.minPar!==void 0&&e<i.minPar||i.maxPar!==void 0&&e>i.maxPar||i.pars&&!i.pars.includes(e)||i.holes&&!i.holes.includes(t)):!0}class te{constructor(e=new At){this.queue=e}queue;loading=new h(!1);error=new h(null);friendlyRound=new h(null);round=new h(null);balls=new h([]);scorecards=new h([]);cells=new h(new Map);result=new h(null);resultLoading=new h(!1);resultError=new h(null);holeIdx=new h(0);groupIdx=new h(0);selectedSlot=new h(null);token=null;loadSeq=0;resultSeq=0;flushing=!1;pendingSlotIndex=null;async loadByToken(e,t){const s=e!==this.token;this.token=e;const r=++this.loadSeq;s&&this.resetForNewToken(t),H.get(Z).load();const n=await I(this.loading,this.error,()=>x.friendlyRounds.byToken({token:e}));if(!n||r!==this.loadSeq||e!==this.token)return;if(this.friendlyRound.set(n.friendlyRound),this.round.set(n.round),this.pendingSlotIndex!==null){const u=n.round.formatSlots[this.pendingSlotIndex]?.slotDefId??null;this.pendingSlotIndex=null,u!==null&&this.selectedSlot.set(u)}const[o,d]=await Promise.all([x.friendlyRounds.balls({token:e}).catch(()=>[]),x.friendlyRounds.scorecard({token:e}).catch(()=>[])]);r!==this.loadSeq||e!==this.token||(this.cells.set(new Map),this.scorecards.set(d),this.balls.set(o),await this.flushPending())}async loadResult(){const e=this.token;if(!e)return;const t=++this.resultSeq,s=await I(this.resultLoading,this.resultError,()=>x.friendlyRounds.result({token:e}));t!==this.resultSeq||e!==this.token||s&&this.result.set(s)}ballNameById=new E(()=>{const e=new Map;for(const t of this.balls.get())e.set(t.id,Ne(t));return e});nameOf(e){return this.ballNameById.get().get(e)??e}selectedSlotDefId(){const e=this.round.get()?.formatSlots??[];if(e.length===0)return null;const t=this.selectedSlot.get();return t!==null&&e.some(s=>s.slotDefId===t)?t:e[0]?.slotDefId??null}selectSlot(e){this.selectedSlot.set(e)}groups(){return this.round.get()?.playingGroups??[]}group(){const e=this.groups();return e[this.groupIdx.get()]??e[0]??null}playedOrder(){return this.group()?.playedOrder??[]}holeIndex(){return K(this.holeIdx.get(),this.playedOrder().length)}currentPlayedHole(){return this.playedOrder()[this.holeIndex()]??null}playHoleById(e){return this.round.get()?.playHoles.find(t=>t.id===e)??null}currentPlayHole(){const e=this.currentPlayedHole();return e?this.playHoleById(e.playHoleId):null}parFor(e){return(e?this.playHoleById(e)?.par:null)??4}occLabel(e){const t=this.round.get(),s=t?.playHoles.find(o=>o.id===e);if(!t||!s)return"";const r=t.playHoles.filter(o=>o.courseHoleNumber===s.courseHoleNumber).sort((o,d)=>o.ordinal-d.ordinal);if(r.length===1)return`${s.courseHoleNumber}`;const n=r.findIndex(o=>o.id===e);return`${s.courseHoleNumber} (${Dt[n]??`${n+1}th`})`}canPrevHole(){return this.holeIndex()>0}canNextHole(){return this.holeIndex()<this.playedOrder().length-1}prevHole(){this.holeIdx.set(K(this.holeIndex()-1,this.playedOrder().length))}nextHole(){this.holeIdx.set(K(this.holeIndex()+1,this.playedOrder().length))}strokesFor(e,t){const s=this.cells.get().get(F(e,t));return s?s.strokes:this.scorecards.get().find(o=>o.ballId===e)?.holes.find(o=>o.playHoleId===t)?.strokes??null}statusFor(e,t){return this.cells.get().get(F(e,t))?.status??null}metadataFor(e,t,s){const r=this.cells.get().get(F(e,t));return r&&r.metadata!==void 0?r.metadata?.[s]:this.scorecards.get().find(d=>d.ballId===e)?.holes.find(d=>d.playHoleId===t)?.metadata?.[s]}metadataInputs(){const e=H.get(Z),t=this.round.get()?.formatSlots??[],s=[],r=new Set;for(const n of t){const o=e.byId(n.formatId)?.requirements.scoreEntry?.metadata??[];for(const d of o)r.has(d.key)||(r.add(d.key),s.push(d))}return s}metadataInputsForHole(e){return e?this.metadataInputs().filter(t=>qt(t.appliesWhen,e.par,e.courseHoleNumber)):[]}async setScore(e,t,s,r){const n=F(e,t),o=crypto.randomUUID();this.patchCell(n,{strokes:s,metadata:r,status:"saving",clientEventId:o});const d=this.token;d&&(this.enqueue(d,e,t,s,r,o),await this.post(d,e,t,s,r,o))}async retry(e,t){const s=F(e,t),r=this.cells.get().get(s);if(!r)return;this.patchCell(s,{...r,status:"saving"});const n=this.token;n&&(this.enqueue(n,e,t,r.strokes,r.metadata,r.clientEventId),await this.post(n,e,t,r.strokes,r.metadata,r.clientEventId))}async flushPending(){const e=this.token;if(!(!e||this.flushing)){this.flushing=!0;try{for(const t of this.queue.entriesFor(e)){if(e!==this.token)return;this.patchCell(F(t.ballId,t.playHoleId),{strokes:t.strokes,metadata:t.metadata,status:"saving",clientEventId:t.clientEventId}),await this.post(e,t.ballId,t.playHoleId,t.strokes,t.metadata,t.clientEventId)}}finally{this.flushing=!1}}}enqueue(e,t,s,r,n,o){this.queue.enqueue({token:e,ballId:t,playHoleId:s,strokes:r,eventType:r===null?"score_cleared":"score_entered",clientEventId:o,...n!==void 0?{metadata:n}:{},queuedAt:Date.now()})}async post(e,t,s,r,n,o){const d=F(t,s);try{await x.friendlyRounds.score({token:e,ballId:t,playHoleId:s,strokes:r,eventType:r===null?"score_cleared":"score_entered",clientEventId:o,...n!=null?{metadata:n}:{}}),this.queue.remove(o);const c=this.cells.get().get(d);c&&c.clientEventId===o&&this.patchCell(d,{...c,status:"saved"});const u=this.round.get();e===this.token&&u&&u.status==="not_started"&&this.round.set({...u,status:"active"})}catch{const c=this.cells.get().get(d);c&&c.clientEventId===o&&this.patchCell(d,{...c,status:"error"})}}patchCell(e,t){const s=new Map(this.cells.get());s.set(e,t),this.cells.set(s)}resetForNewToken(e){this.resultSeq++,this.friendlyRound.set(null),this.round.set(null),this.balls.set([]),this.scorecards.set([]),this.cells.set(new Map),this.result.set(null),this.resultError.set(null),this.holeIdx.set(e?.holeIdx??0),this.groupIdx.set(e?.groupIdx??0);const t=e?.selectedSlot;this.pendingSlotIndex=null,typeof t=="string"?this.selectedSlot.set(t):typeof t=="number"?(this.pendingSlotIndex=t,this.selectedSlot.set(null)):this.selectedSlot.set(null)}}const A=60,ke=8,ae=4,Gt=Array.from({length:ae*2+1},(i,e)=>e-ae),Kt="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",Ut=b(`
...
`);class Yt extends T{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
        ...
        .se-row {
            ...
            & .se-row__scores { display: flex; align-items: center; padding-right: ${ke}px; flex-shrink: 0; }
...
</div>`}function ds(i,e){const t=i.matches.map(s=>{const r=y(s.sideA.ballIds.map(e).join(" & ")),n=y(s.sideB.ballIds.map(e).join(" & ")),o=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,c=s.leader==="a"?" lb-mp__team--lead":"",u=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const cs={ranked:ls,match_summary:ds},us={"default-score-grid":is,"compact-match-grid":os,"category-matrix-grid":as};function hs(i){return i.componentId??"default-score-grid"}function ps(i){return`<div class="lb-diag">Unrenderable result section <code>${y(i)}</code> — no generic view yet. Results are not hidden.</div>`}function ms(i){return`<div class="lb-diag">Unsupported score-grid component <code>${y(i)}</code> — no generic view yet. Results are not hidden.</div>`}function fs(i,e){const t=cs[i.kind];return t?t(i,e):ps(i.kind)}function gs(i,e,t,s){const r=hs(i),n=us[r];return n?n(i,e,t,s):ms(r)}function bs(i,e){return i.leaderboard.length===0&&i.cards.length===0?`<div class="lb-empty">No scores entered yet for ${y(i.formatLabel)}.</div>`:i.leaderboard.map(s=>fs(s,e)).join("")||`<div class="lb-empty">No leaderboard metric for ${y(i.formatLabel)}.</div>`}function ys(i,e,t,s={}){if(i.cards.length===0)return"";const r=s.mode??"product";return i.cards.map(n=>gs(n,e,t,{mode:r})).join(`
`)}const _s=b(`
...
`);class vs extends T{static styles=`
    ...
    `;svc=this.inject(te);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(_s,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=d=>this.svc.nameOf(d),r=bs(t,s),n=ys(t,e.routeSections,s),o=n?`<h3 class="lb-cards__head">Scorecard</h3>${n}`:"";return r+o}}function xs(i,e){if(!e)return[];const t=[],s=new Set;for(const r of i)for(const n of r.players){if(n.playerId===e)return[];n.guestPlayerId===null||s.has(n.guestPlayerId)||(s.add(n.guestPlayerId),t.push({guestPlayerId:n.guestPlayerId,displayName:n.displayName}))}return t}const ws=b(`
...
`);class ks extends T{static styles=`
        .claim-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${z()}
    ...
    `;svc=this.inject(te);auth=this.inject(G);router=this.inject(N);tokenQ=this.router.query("token");claiming=new h(!1);error=new h("");claimable(){return xs(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async claim(e){const t=this.tokenQ.get();if(!(!t||this.claiming.get())){this.error.set(""),this.claiming.set(!0);try{await x.friendlyRounds.claimGuest({token:t,guestPlayerId:e}),await this.svc.loadByToken(t)}catch(s){this.error.set(s instanceof j&&s.status===409?"Already claimed — or you already play in this round under your account.":s instanceof j&&s.status===404?"That guest is no longer claimable on this round.":"Could not claim right now. Try again.")}finally{this.claiming.set(!1)}}}render(){const e=this.wire(ws,{root:{className:()=>this.claimable().length>0?"claim-card":"claim-card hidden"},err:{textContent:()=>this.error.get()}});return this.$each(this.ref(e,"rows"),()=>this.claimable(),(t,s,r)=>this.wireEl($s,{name:()=>t.displayName,claim:{disabled:()=>this.claiming.get(),onclick:()=>{this.claim(t.guestPlayerId)}}},r),t=>t.guestPlayerId),e}}function Ss(i){if(!(i===null||i===""))return/^\d+$/.test(i)?Number(i):i}const Is=b(`
...
`),Ts=b('<button bind="pill" class="round-view__fmt" type="button"></button>');class Es extends T{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class Fs extends T{static styles=`
        .setup {
            ...
            & .player {
                ...
                & .player__name { flex: 1; padding: ${a("md")}; font-size: 1rem; ${O()} }
                & .player__remove {
                ...
                & .player__index { flex: 1; min-width: 0; padding: ${a("md")}; font-size: 1rem; ${O()} }
            ...
            & .fslot {
                ...
                & .mrow {
                    ...
                    & .mrow__pct {
                        ...
                        &[hidden] { display: none; }
                        & input { width: 56px; padding: ${a("xs")} ${a("sm")}; ${O()} font-size: 0.95rem; }
                    }
                }
                ...
                & .fslot__flat {
                    ...
                    color: ${l("text-muted")};
                    &[hidden] { display: none; }
                    & .fslot__pct { width: 70px; padding: ${a("sm")}; font-size: 1rem; ${O()} }
                }
                & .fslot__bands {
                    display: flex; flex-direction: column; gap: ${a("xs")};
                    &[hidden] { display: none; }
                ...
                & .brow {
                    ...
                    & .brow__pct, & .brow__upto { width: 56px; padding: ${a("sm")}; font-size: 0.95rem; ${O()} }
...
`);class Ds extends T{static styles=`
    ...
    `;auth=this.inject(G);router=this.inject(N);nextQ=this.router.query("next");mode=new h("login");busy=new h(!1);registerError=new h("");username="";password="";displayName="";hcp="";destination(e){const t=this.nextQ.get();return t&&t.startsWith("/")?t:e}async submit(){if(this.registerError.set(""),this.mode.get()==="login"){await this.auth.login(this.username,this.password)&&this.router.navigate(this.destination("/rounds"),!0);return}const e=this.hcp.trim().replace(",","."),t=e===""?null:Number.parseFloat(e);if(t!==null&&!Number.isFinite(t)){this.registerError.set("Handicap index must be a number (or leave it empty).");return}if(this.password.length<8){this.registerError.set("Password must be at least 8 characters.");return}if(!this.username.trim()||!this.displayName.trim()){this.registerError.set("Username and display name are required.");return}this.busy.set(!0);try{const s=await x.players.register({username:this.username.trim(),password:this.password,displayName:this.displayName.trim(),handicapIndex:t});this.auth.currentUser.set({id:s.id,username:s.username}),this.router.navigate(this.destination("/"),!0)}catch(s){this.registerError.set(s instanceof j&&s.status===409?"That username is taken.":s instanceof j?s.message:"Could not create the account. Try again.")}finally{this.busy.set(!1)}}render(){const e=()=>this.mode.get()==="register",t=()=>this.auth.loading.get()||this.busy.get();return this.wire(As,{root:{inert:()=>t()},error:{className:()=>this.registerError.get()||this.auth.error.get()?"error show":"error",textContent:()=>this.registerError.get()||this.auth.error.get()?.message||""},form:{onsubmit:async s=>{s.preventDefault(),await this.submit()}},username:{oninput:s=>{this.username=s.target.value}},password:{autocomplete:()=>e()?"new-password":"current-password",oninput:s=>{this.password=s.target.value}},registerFields:{className:()=>e()?"login__register":"login__register hidden"},displayName:{oninput:s=>{this.displayName=s.target.value}},hcp:{oninput:s=>{this.hcp=s.target.value}},submit:{textContent:()=>t()?e()?"Creating account…":"Signing in…":e()?"Create account":"Sign in"},toggle:{textContent:()=>e()?"Have an account? Sign in":"New here? Create an account",onclick:()=>{this.registerError.set(""),this.auth.error.set(null),this.mode.set(e()?"login":"register")}}})}}class qs{loading=new h(!1);error=new h(null);guests=new h([]);async load(){const e=await I(this.loading,this.error,()=>x.guestPlayers.list());e&&this.guests.set(e)}async create(e){const t=await I(this.loading,this.error,()=>x.guestPlayers.create(e));return t&&this.guests.update(s=>[...s,t]),t??null}}const Gs=b(`
    <div class="players">
...
`);function Us(i){return i.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class Ws extends T{static styles=`
        .players {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Ys extends T{static styles=`
        .rounds {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    `;svc=this.inject(Xs);router=this.inject(N);render(){this.svc.load();const e=this.wire(Qs,{subtitle:()=>{const s=this.svc.rounds.get().length;return s===0?"No rounds yet — tee one up.":`${s} round${s===1?"":"s"} on the card.`},newBtn:{onclick:()=>this.router.navigate("/create")}}),t={not_started:"Not started",active:"Live",complete:"Done"};return this.$each(this.ref(e,"list"),this.svc.rounds,(s,r,n)=>this.wireEl(Vs,{row:{disabled:!0},course:()=>s.courseNameSnapshot??"Round",status:{textContent:()=>t[s.status]??s.status,className:()=>`round-row__status s-${s.status}`},date:()=>s.date,formats:()=>s.formatSlots.map(ee).join(" · ")},n),s=>s.id),e}}const Js=b(`
    <div class="profile">
...
`);class en extends T{static styles=`
        .profile {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
            ...
            & .profile__card {
                ...
                & .profile__edit {
                    ...
                    & input { width: 90px; padding: ${a("md")}; font-size: 1rem; text-align: center; ${O()} }
                    & button {
    ...
    `;svc=this.inject(Oe);auth=this.inject(G);router=this.inject(N);indexDraft=new h("");localErr=new h("");render(){this.auth.currentUser.get()&&this.svc.load();const e=()=>this.auth.currentUser.get()!==null,t=this.wire(Js,{anon:{className:()=>e()?"profile__anon hidden":"profile__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/profile"}})},body:{className:()=>e()?"profile__body":"profile__body hidden"},name:()=>this.svc.player.get()?.displayName??"…",username:()=>{const s=this.svc.player.get();return s?`@${s.username}`:""},hcp:()=>{const s=this.svc.player.get()?.handicapIndex;return s==null?"–":s.toFixed(1)},index:{value:()=>this.indexDraft.get(),oninput:s=>this.indexDraft.set(s.target.value)},save:{disabled:()=>this.svc.saving.get()||this.indexDraft.get().trim()==="",textContent:()=>this.svc.saving.get()?"Saving…":"Save"},form:{onsubmit:async s=>{s.preventDefault(),this.localErr.set("");const r=this.indexDraft.get().trim().replace(",","."),n=Number.parseFloat(r);if(!Number.isFinite(n)||n<-10||n>54){this.localErr.set("Enter an index between -10 and 54.");return}await this.svc.saveIndex(n)&&this.indexDraft.set("")}},saveErr:{textContent:()=>this.localErr.get()||this.svc.saveError.get()?.message||""},historyEmpty:{className:()=>this.svc.history.get().length===0?"profile__empty":"profile__empty hidden"},signout:{onclick:async()=>{await this.auth.logout(),this.svc.clear(),this.router.navigate("/")}}});return this.$each(this.ref(t,"history"),this.svc.history,(s,r,n)=>this.wireEl(Zs,{index:()=>s.handicapIndex.toFixed(1),source:()=>s.source,date:()=>s.effectiveDate},n),s=>s.id),t}}const tn=b(`
    <div class="app-shell">
        <main bind="content" class="app-shell__content"></main>
        <div bind="nav" class="app-shell__nav"></div>
    </div>
`);class sn extends T{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(N);render(){const e=this.wire(tn,{});return this.spawn(pt,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":ve,"/round":Es,"/create":Fs,"/login":Ds,"/rounds":Ys,"/players":Ws,"/profile":en},ve),e}}H.get(Je);const Te=H.get(N),Ee=H.get(G);await et(sn,"#app",{hot:void 0,onInit:async()=>{await Ee.load(),Ee.currentUser.get()&&Te.route.get()==="/login"&&Te.navigate("/",!0)}});export{T as C,N as R,h as S,Je as T,m as a,X as b,E as c,v as e,I as r,b as t};
