...
`);class ja extends R{static styles=`
    ...
    `}function Od(s){if(!s.signedIn)return[];const e=[{kind:"identity",displayName:(s.displayName??"").trim()||(s.username??"").trim()||"Signed in",username:(s.username??"").trim()},{kind:"profile",label:"Profile"}];return s.isSuperAdmin&&e.push({kind:"admin",label:"Admin"}),e.push({kind:"signout",label:"Sign out"}),e.push({kind:"signout-all",label:"Sign out everywhere"}),e}function zd(s){return Od(s).map(e=>e.kind)}function pn(s){return s.signedIn?"avatar":"signin"}const Hd=b(`
...
`),qd='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',zs=`
...
`),Ud=b(`
    <button bind="row" type="button" class="round-summary finished-row">${zs}</button>
`),Kd=b(`
...
`),Qd={not_started:"Not started",active:"Live",complete:"Finished"};class mn extends R{static styles=`
        .landing {
            ...
            & .round-summary {
                ...
                & .round-summary__formats::before { content: '·'; margin-right: ${a("sm")}; }
...
`),Zd='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',ec=b(`
    <div class="round-row">
        ...
        <button bind="del" type="button" class="round-row__del" aria-label="Delete round">${Zd}</button>
    </div>
`);class tc extends R{static styles=`
        .history {
    ...
    `;svc=this.inject(Rt);auth=this.inject(j);router=this.inject(B);loggedIn=new $(()=>this.auth.currentUser.get()!==null);rows=new $(()=>Xd(this.loggedIn.get()?Ze.fromMyRounds(this.svc.myRounds.get()):Ze.fromDeviceRounds(this.svc.deviceRounds.get())));deleteOpen=new p(!1);deleteTarget=new p(null);askDelete(e,t,n){this.deleteTarget.set({token:e,roundId:t,name:n}),this.deleteOpen.set(!0)}render(){this.loggedIn.get()?this.svc.loadMine():this.svc.loadDevice();const e=this.wire(Jd,{back:{onclick:()=>this.router.navigate("/")},empty:{className:()=>this.rows.get().length===0?"history__empty":"history__empty hidden"},list:{className:()=>this.rows.get().length===0?"history__list hidden":"history__list"}});this.$each(this.ref(e,"list"),this.rows,(n,i,r)=>this.roundRow(n,r),n=>n.key),this.spawn(ee,this.ref(e,"confirmHost"),{open:this.deleteOpen,title:"Delete round?",message:()=>{const n=this.deleteTarget.get();return`Delete ${n?`“${n.name}”`:"this round"}? This permanently removes it and all its scores for everyone. This can't be undone.`},confirmLabel:"Delete",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{const n=this.deleteTarget.get();n&&this.svc.remove(n.token,n.roundId)}});const t=n=>{n.key==="Escape"&&this.deleteOpen.get()&&this.deleteOpen.set(!1)};return window.addEventListener("keydown",t),this.track(()=>window.removeEventListener("keydown",t)),e}roundRow(e,t){return this.wireEl(ec,{row:{disabled:()=>e.token===null,onclick:()=>{e.token!==null&&this.router.navigate("/round",{query:{token:e.token}})}},title:()=>Ae(e),course:{textContent:()=>ze(e)??"",className:()=>ze(e)?"round-row__course":"round-row__course hidden"},role:{textContent:()=>e.roleLabel??"",className:()=>e.roleLabel?"round-row__role":"round-row__role hidden"},status:{textContent:()=>Qd[e.status]??e.status,className:()=>`round-row__status s-${e.status}`},date:()=>ve(e.date),formats:()=>e.formats??"",del:{className:()=>e.token===null?"round-row__del hidden":"round-row__del",onclick:()=>{e.token!==null&&this.askDelete(e.token,e.roundId??"",Ae(e))}}},t)}}const sc=700;function nc(s){if(!s.currentHole)return!1;const e=s.balls.filter(t=>!t.pending);return e.length>0&&e.every(t=>t.scored)}function ic(s){return s.currentHole?s.balls.some((e,t)=>t!==s.currentBallIndex&&!e.scored):!1}function Jt(s){const e=s.currentHole;if(!e)return{kind:"noop"};const t=s.balls,n=s.currentBallIndex;for(let i=n+1;i<t.length;i++)if(!t[i].scored)return{kind:"moveToBall",ballIndex:i};for(let i=0;i<n;i++)if(!t[i].scored)return{kind:"moveToBall",ballIndex:i};return s.holeIndex>=s.holeCount-1?{kind:"roundComplete",toast:"Round complete"}:{kind:"holeComplete",toast:`Hole ${e.label} done`,fromHoleId:e.id,toHoleIndex:s.holeIndex+1,delayMs:sc}}function rc(s,e){const t=s.currentHole;if(e.kind==="statsDone")return s.holeCompleteOnEntry?{write:null,move:{kind:"stay"}}:{write:null,move:Jt(s)};const n=s.balls[s.currentBallIndex];if(!t||!n)return{write:null,move:{kind:"noop"}};if(n.pending)return s.holeCompleteOnEntry?{write:null,move:{kind:"stay"}}:{write:null,move:Jt(s)};const i={ballIndex:s.currentBallIndex,holeId:t.id,value:e.value,withMetadata:e.value!==null};return e.value!==null&&e.value>0&&s.collectsStats?{write:i,move:{kind:"openStats"}}:s.holeCompleteOnEntry?{write:i,move:{kind:"stay"}}:{write:i,move:Jt(s)}}const Se=60,bn=8,fs=4,ac=Array.from({length:fs*2+1},(s,e)=>e-fs),oc="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",lc=b(`
...
`),fc=b('<button bind="btn" class="se-seg" type="button"></button>'),mc=b(`
    <div class="se-stats__group">
        <span bind="glabel" class="se-stats__group-label"></span>
...
`),gc=b('<div bind="rule" class="se-stats__rule"></div>');class bc extends R{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
...
</tr>`;const G=ms(n.plan.slotDefId,L.ballIds),E=n.isOpen(G),K=Bc(G),te=Zi(N,n.routeSections,e,{mode:n.mode??"product"}),se=["lb-rank__row--expandable"];return g.lead&&se.push("lb-rank__lead"),E&&se.push("lb-rank__row--open"),`<tr class="${se.join(" ")}" data-expand-key="${M(G)}">
  <td class="lb-rank__pos">${g.position}</td>
...
<tr class="lb-rank__panel${E?" lb-rank__panel--open":""}" data-panel-key="${M(G)}">
  <td class="lb-rank__panelcell" colspan="${o}"><div class="lb-rank__panelwrap" id="${M(K)}"><div class="lb-rank__panelbox">${te}</div></div></td>
</tr>`}).join(""),u=r?`
...
</div>`}function Dc(s,e){const t=Ic(s,e),n=t.matches.map(i=>{const r=i.leader==="a"?" lb-mp__team--lead":"",d=i.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const Gc={ranked:jc,match_summary:(s,e)=>Dc(s,e)},qc={"default-score-grid":Ac,"compact-match-grid":Lc,"category-matrix-grid":Mc};function Vc(s){return`<div class="lb-diag">Unrenderable result section <code>${M(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Uc(s){return`<div class="lb-diag">Unsupported score-grid component <code>${M(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Kc(s,e,t,n=null){const i=Gc[s.kind];return i?i(s,e,t,n):Vc(s.kind)}function Zi(s,e,t,n){const i=Qi(s),r=qc[i];return r?r(s,e,t,n):Uc(i)}function er(s,e,t=Ji,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${M(s.formatLabel)}.</div>`:s.leaderboard.map(r=>Kc(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${M(s.formatLabel)}.</div>`}function tr(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>Zi(r,e,t,{mode:i})).join(`
`)}const Wc=b(`
...
`);class Jc extends R{static styles=`
        .claim-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${C()}
    ...
    `,tt.seq=0;let de=tt;function eu(s){if(!s)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=s.seats.length>0,t=s.claimedSeats.some(r=>r.viewerMayRelease),n=s.viewer.claimSeat.allowed,i=s.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&n,guestAllowed:e&&i,blockedMessage:e&&!n&&!i?s.viewer.claimSeat.message??s.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function tu(s,e){const t=[];if(s.groupId!==null&&e.length>0){const n=e.findIndex(i=>i.id===s.groupId);if(n>=0){t.push(`Group ${n+1}`);const i=e[n].startTime;i.includes(":")&&t.push(i)}}return s.category!==null&&t.push(s.category),t.join(" · ")}function su(s){return(s?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const nu=b(`
...
`);class au extends R{static styles=`
        .seat-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${C()}
...
`);class uu extends R{static styles=`
        .join-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${C()}
    ...
    `;svc=this.inject(ce);auth=this.inject(j);router=this.inject(B);tokenQ=this.router.query("token");joining=new p(!1);error=new p("");diagnostics=new p([]);teeId=new p("");tees=new p([]);loadedForCourseId=null;groupChoice=new p("");policyState(){return lu(this.svc.startList.get())}eligible(){return this.policyState().visible&&ou(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,v.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const n=this.groupChoice.get(),i=await v.friendlyRounds.join({token:e,teeId:t,...n?{groupChoice:n}:{}});i.ok?await this.svc.loadByToken(e):this.diagnostics.set(i.diagnostics)}catch(n){this.error.set(n instanceof U&&n.status===409?n.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(I(()=>this.ensureTeesLoaded()));const e=new $(()=>du(this.svc.groups(),this.svc.startList.get()?.viewer.createGroup.allowed??!0));this.track(I(()=>{const r=e.get(),d=this.groupChoice.get();(!d||!r.options.some(o=>o.value===d&&!o.disabled))&&this.groupChoice.set(r.defaultValue)}));const t=this.wire(cu,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},blocked:{className:()=>this.policyState().blockedMessage!==null?"join-card__blocked":"join-card__blocked hidden",textContent:()=>this.policyState().blockedMessage??""},groupRow:{className:()=>this.svc.groups().length>0&&this.policyState().blockedMessage===null?"join-card__group":"join-card__group hidden"},row:{className:()=>this.policyState().blockedMessage===null?"join-card__row":"join-card__row hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(r=>r.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),n=new de({value:this.teeId,options:{get:()=>this.tees.get().map(r=>({value:r.id,label:r.name}))},placeholder:"Tee"});n.mount(this.ref(t,"teeHost")),this.track(()=>n.destroy());const i=new de({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return i.mount(this.ref(t,"groupHost")),this.track(()=>i.destroy()),t}}const xn=1440*60*1e3;function hu(s,e){if(s===null)return!0;const t=new Date(s).getTime();return!Number.isFinite(t)||t-e>xn?!0:e-t>=xn}function pu(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}function fu(s){const e={visible:!1,index:null};return s.settled||!s.profileLoaded||!s.firstOpen||!pu(s.balls,s.playerId)||!hu(s.handicapConfirmedAt,s.now)?e:{visible:!0,index:s.handicapIndex}}function pe(s){const e=s.trim().replace(",",".");if(e==="")return null;const t=e.startsWith("+"),n=Number.parseFloat(t?e.slice(1):e);return Number.isFinite(n)?t?-n:n:null}function Pt(s){return s<0?`+${String(-s)}`:String(s)}const mu=b(`
...
`);class gu extends R{static styles=`
        .hcp-checkin {
            margin-bottom: ${a("lg")};
            padding: ${a("md")} ${a("lg")};
            ${C()}
    ...
    `;svc=this.inject(ce);auth=this.inject(j);profile=this.inject(Be);settled=new p(!1);editing=new p(!1);text=new p("");busy=new p(!1);error=new p("");state(){const e=this.profile.player.get();return fu({playerId:this.auth.currentUser.get()?.id??null,balls:this.svc.balls.get(),firstOpen:this.svc.firstOpen.get(),handicapConfirmedAt:e?.handicapConfirmedAt??null,handicapIndex:e?.handicapIndex??null,profileLoaded:e!==null,settled:this.settled.get(),now:Date.now()})}ensureProfileLoaded(){this.settled.get()||this.svc.firstOpen.get()&&this.auth.currentUser.get()&&this.svc.balls.get().length&&this.profile.load()}async confirm(){if(this.busy.get())return;this.error.set(""),this.busy.set(!0);const e=await this.profile.confirmHandicap();this.busy.set(!1),e?this.settled.set(!0):this.error.set("Could not save that right now. Try again.")}startEdit(){const e=this.profile.player.get()?.handicapIndex??null;this.text.set(e===null?"":Pt(e)),this.error.set(""),this.editing.set(!0)}async save(){if(this.busy.get())return;const e=pe(this.text.get());if(e===null){this.error.set("Enter a handicap index, e.g. 18.4 or +2.4.");return}this.error.set(""),this.busy.set(!0);const t=await this.profile.saveIndex(e);this.busy.set(!1),t?(this.editing.set(!1),this.settled.set(!0)):this.error.set("Could not save that right now. Try again.")}render(){return this.track(I(()=>this.ensureProfileLoaded())),this.wire(mu,{root:{className:()=>this.state().visible?"hcp-checkin":"hcp-checkin hidden"},ask:{className:()=>this.editing.get()?"hcp-checkin__ask hidden":"hcp-checkin__ask"},question:{textContent:()=>{const e=this.state().index;return e===null?"No handicap set — add one?":`Handicap ${Pt(e)} — still right?`}},confirm:{textContent:()=>this.state().index===null?"Not now":"Yes",disabled:()=>this.busy.get(),onclick:()=>{this.confirm()}},edit:{textContent:()=>this.state().index===null?"Add":"Update",disabled:()=>this.busy.get(),onclick:()=>this.startEdit()},editor:{className:()=>this.editing.get()?"hcp-checkin__editor":"hcp-checkin__editor hidden"},field:{value:()=>this.text.get(),oninput:e=>this.text.set(e.target.value)},save:{disabled:()=>this.busy.get(),onclick:()=>{this.save()}},cancel:{disabled:()=>this.busy.get(),onclick:()=>this.editing.set(!1)},err:{textContent:()=>this.error.get()}})}}function bu(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}const _u=b(`
...
`);class yu extends R{static styles=`
...
`);class Eu extends R{static styles=`
...
`);class ch extends R{static styles=`
    ...
    `;round=this.inject(ce);stats=this.inject(rt);auth=this.inject(j);router=this.inject(B);colors=Bt;render(){this.track(I(()=>{const i=this.eligibleRoundId();Y(()=>{i!==null&&this.stats.load(i).catch(()=>{})})}));const e=()=>this.shows()?this.stats.model.get():null,t=this.wire(oh,{story:{className:()=>this.shows()?"story":"story hidden"},title:()=>Rn.title,score:()=>{const i=e();return i===null?"":or(i.strokes,i.vsPar)??""},hint:()=>e()===null?"":this.hint(),open:{textContent:()=>Rn.seeWholeRound,onclick:()=>{const i=this.stats.roundId.get();i!==null&&this.router.navigate("/round-stats",{query:{id:i}})}}}),n=i=>{const r=e();return r===null?null:Fe(r.waterfall,i)};return this.$each(this.ref(t,"values"),()=>e()===null?[]:[...le],(i,r,d)=>this.wireEl(dh,{label:()=>Lt(i),amount:{textContent:()=>{const o=n(i);return o===null?P.notRecorded:je(o)},className:()=>n(i)===null?"story__valueamount story__valueamount--absent":"story__valueamount",style:()=>{const o=n(i);return o===null?"":`color:${De(Ft(o),this.colors)}`}}},d),i=>i),this.$each(this.ref(t,"lines"),()=>e()?.insights??[],(i,r,d)=>this.wireEl(lh,{text:()=>ah(i)},d),i=>i.id),t}eligibleRoundId(){const e=this.round.round.get();return e===null?null:sr({signedInPlayerId:this.auth.currentUser.get()?.id??null,statConfigPlayerIds:new Set(this.round.statModules.get().keys()),statRows:this.round.statRows.get(),holesUnscored:nr({playerId:this.auth.currentUser.get()?.id??"",balls:this.round.balls.get(),groups:this.round.groups(),strokesFor:(n,i)=>this.round.strokesFor(n,i)})}).reason==="eligible"?e.id:null}shows(){const e=this.eligibleRoundId();return e===null?!1:this.stats.phase.get()==="ready"&&this.stats.roundId.get()===e}hint(){const e=this.stats.model.get();if(e===null)return"";if(e.deltas===null)return W.waterfallHint;let t=null;for(const n of le){const i=Is(e.deltas,n);i!==null&&(t===null||Math.abs(i)>Math.abs(t))&&(t=i)}return t===null?W.waterfallHint:lr(t,e.windowCount)}}function An(s){return!(!s.pageVisible||s.status==="complete")}function uh(s,e){return e&&!s}const hh=2,Ln=3,ph=75e3;function fh(s,e=null){const t=new URLSearchParams({token:s});return e!==null&&t.set("since",e),`${D}/friendly-rounds/events?${t.toString()}`}function mh(s){if(typeof s!="object"||s===null)return!1;const e=s;return e.latestEventId!==null&&typeof e.latestEventId!="string"?!1:e.status==="not_started"||e.status==="active"||e.status==="complete"}function gh(s){const e=s.eventSourceFactory??(N=>new EventSource(N)),t=s.setTimer??((N,G)=>setTimeout(N,G)),n=s.clearTimer??(N=>clearTimeout(N)),i=s.livenessTimeoutMs??ph,r=s.isPageVisible??(()=>typeof document>"u"||!document.hidden);let d=!1,o=0,c=null,u=null,f=s.since??null;const m=()=>{u!==null&&(n(u),u=null)},h=()=>{m(),u=t(z,i)},g=()=>{c!==null&&(c.onopen=null,c.onmessage=null,c.onerror=null,c.close(),c=null)},y=()=>{d=!0,m(),g()},w=()=>{if(g(),++o>=Ln){y(),s.onDegrade();return}L()};function z(){if(!d){if(!r()){h();return}w()}}function L(){if(d)return;let N;try{N=e(fh(s.token,f))}catch{y(),s.onDegrade();return}c=N,N.onopen=()=>{o=0},N.onmessage=G=>{if(d||c!==N)return;h();let E;try{E=JSON.parse(G.data)}catch{return}mh(E)&&(E.latestEventId!==null&&(f=E.latestEventId),s.onEvent({latestEventId:E.latestEventId,status:E.status}))},N.onerror=()=>{d||c!==N||(N.readyState===hh||++o>=Ln)&&(y(),s.onDegrade())},h()}return L(),{stop:()=>{d||y()}}}const bh=2e4;function _h(s){if(!(s===null||s===""))return/^\d+$/.test(s)?Number(s):s}const yh=b(`
...
`),vh=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),wh=b('<button bind="pill" class="round-view__grp" type="button"></button>');class xh extends R{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class Qh extends R{static styles=`
        .setup {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`),ip='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8.5" cy="7.5" r="3.25"/><path d="M2.5 20c.5-3.5 2.7-5.5 6-5.5 2.7 0 5.1 1.8 5.8 4.5"/><circle cx="17.5" cy="16.5" r="4.25"/><path d="M15.5 16.5h4"/></svg>',rp=b(`
...
`),Un=b(`
    <div class="friend-row">
        ...
        <span bind="hcp" class="friend-row__hcp"></span>
        <button bind="remove" class="friend-row__remove" type="button" aria-label="Remove friend">${ip}</button>
    </div>
`);class ap extends R{static styles=`
        .friends {
...
`);class pp extends R{static styles=`
        .fcourses {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
    ...
    <div class="spectate">
        ...
        <div bind="body" class="spectate__body">
            ...
            <p class="spectate__note">${gp}</p>
        </div>
    </div>
`);class _p extends R{static styles=`
        .spectate {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
        ...
        ${Ms.styles}
    `;svc=this.inject(Ii);auth=this.inject(j);router=this.inject(B);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(I(()=>{const c=e.get();Y(()=>{!c||!n()||(this.svc.setRound(c),this.svc.load())})}));const i=()=>this.svc.view.get(),r=()=>this.svc.unavailable.get(),d=()=>(t.get()??"").trim()||null;return this.wire(bp,{back:{onclick:()=>{window.history.length>1?window.history.back():this.router.navigate("/")}},anon:{className:()=>n()?"spectate__anon hidden":"spectate__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/spectate?id=${e.get()??""}&name=${encodeURIComponent(d()??"")}`}})},refusal:{className:()=>n()&&r()!==null?"spectate__refusal":"spectate__refusal hidden"},refusalTitle:{textContent:()=>{const c=r();return c?Kn[c].title:""}},refusalMsg:{textContent:()=>{const c=r();return c?Kn[c].message:""}},state:{textContent:()=>r()!==null||!n()?"":this.svc.loading.get()&&i()===null?"Loading…":i()===null?this.svc.error.get()??"":""},retry:{className:()=>n()&&r()===null&&this.svc.error.get()!==null&&i()===null&&!this.svc.loading.get()?"spectate__retry":"spectate__retry hidden",onclick:()=>{this.svc.load(!0)}},body:{className:()=>n()&&r()===null&&i()!==null?"spectate__body":"spectate__body hidden"},title:{textContent:()=>{const c=i();return c?fp(d(),c.round.name,c.round.courseNameSnapshot):""}},status:{textContent:()=>{const c=i();return c&&c.status==="active"?"Live":""},className:()=>`spectate__status s-${i()?.status??""}`},subtitle:{textContent:()=>{const c=i();return c?mp(c.round.name,c.round.courseNameSnapshot,c.status,c.round.playHoles.length||null)??"":""}},date:{textContent:()=>ve(i()?.round.date??null)},board:{innerHTML:()=>this.renderBoards()}})}renderBoards(){const e=this.svc.view.get();if(!e)return"";const t=r=>this.svc.nameOf(r),n=r=>this.svc.groupLabelOf(r),i=e.result.slots;return i.length===0?'<div class="lb-empty">No formats in this round.</div>':i.map(r=>{const d=i.length>1?`<h2 class="spectate__slot-head">${yp(r.formatLabel)}</h2>`:"",o=er(r,t,n),c=tr(r.cards,e.result.routeSections,t),u=c?`<h3 class="lb-cards__head">Scorecard</h3>${c}`:"";return d+o+u}).join("")}}function yp(s){return s.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}const vp=b(`
    <div class="profile">
...
`);class $p extends R{static styles=`
        .profile {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),Sp=b('<h3 bind="text" class="block__subhead"></h3>'),Tp=b('<p bind="text" class="block__note"></p>'),Ip=b(`
...
`),Np=b('<div bind="panels" class="statspanels"></div>');class fr extends R{static styles=`
        .statspanels {
            display: flex; flex-direction: column; gap: ${a("sm")};
...
`);class Mp extends R{static styles=`
        .stats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),Vp=b('<li bind="text" class="roundstats__legenditem"></li>');function Up(){const s=[];for(const[e,t]of Object.entries(As))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
    ...
    `;svc=this.inject(rt);auth=this.inject(j);router=this.inject(B);colors=Bt;openHole=new p(null);render(){const e=this.router.query("id"),t=()=>this.svc.model.get(),n=()=>this.svc.phase.get()==="ready"&&t()!==null;this.track(I(()=>{const o=e.get();Y(()=>{this.openHole.set(null),o&&this.svc.load(o)})}));const i=this.router.query("finish"),r=()=>i.get()==="1",d=this.wire(jp,{back:{className:()=>r()?"roundstats__back hidden":"roundstats__back",onclick:()=>this.router.navigate("/stats")},finishKicker:{className:()=>r()?"roundstats__kicker":"roundstats__kicker hidden"},finishClose:{className:()=>r()?"roundstats__closebtn":"roundstats__closebtn hidden",onclick:()=>this.router.navigate("/")},state:()=>this.stateLine(),body:{className:()=>n()?"roundstats__body":"roundstats__body hidden"},title:()=>t()===null?"":kn(t()),subtitle:()=>this.subtitle(),score:()=>{const o=t();return o===null?"":or(o.strokes,o.vsPar)??""},stripSec:{className:()=>(t()?.cells.length??0)>0?"roundstats__section":"roundstats__section hidden"},wfHeading:()=>W.waterfallHeading,wfHint:()=>W.waterfallHint,legendHeading:()=>W.legendHeading,detail:{className:()=>this.selected()===null?"holedetail hidden":"holedetail"},detailTitle:()=>{const o=this.selected();return o===null?"":Ju(o)},detailEmpty:()=>{const o=this.selected();return o===null?"":js(o).length===0?W.nothingRecordedOnHole:""}});return this.setHeading(d,"stripSec",W.holeStripHeading),this.$each(this.ref(d,"strip"),()=>t()?.cells??[],(o,c,u)=>this.wireEl(Dp,{cell:{className:()=>{const f=this.cellNow(o.id)??o;return f.marker===null?"cell":`cell cell--${f.marker}`},"aria-pressed":()=>String(this.openHole.get()===o.id),"aria-label":()=>On(this.cellNow(o.id)??o),title:()=>On(this.cellNow(o.id)??o),onclick:()=>this.openHole.set(this.openHole.get()===o.id?null:o.id)},hole:()=>String((this.cellNow(o.id)??o).holeNumber),score:()=>Zu(this.cellNow(o.id)??o),tee:{className:()=>(this.cellNow(o.id)??o).tee===null?"cell__tee cell__tee--absent":"cell__tee",style:()=>{const f=this.cellNow(o.id)??o;return f.tee===null?"":`background:${this.teeColor(f.tee)}`}},gir:{className:()=>{const f=this.cellNow(o.id)??o;return f.gir===null?"cell__gir cell__gir--absent":f.gir?"cell__gir cell__gir--hit":"cell__gir"}},putts:()=>{const f=this.cellNow(o.id)??o;return f.putts===null?"":String(f.putts)},pen:{className:()=>ku(this.cellNow(o.id)??o)?"cell__pen":"cell__pen cell__pen--absent"}},u),o=>o.id),this.$each(this.ref(d,"detailLines"),()=>rh(this.selected()),(o,c,u)=>this.wireEl(Gp,{label:()=>o.label,value:()=>o.value},u),o=>o.key),this.$each(this.ref(d,"deltas"),()=>{const o=t();return o===null?[]:le.filter(c=>Fe(o.waterfall,c)!==null)},(o,c,u)=>this.wireEl(qp,{title:()=>Lt(o),sentence:()=>{const f=t();if(f===null||f.deltas===null)return"";const m=Is(f.deltas,o);return m===null?"":lr(m,f.windowCount)},value:{textContent:()=>{const f=this.componentValue(o);return f===null?"":je(f)},style:()=>{const f=this.componentValue(o);return f===null?"":`color:${De(Ft(f),this.colors)}`}},bar:{innerHTML:()=>{const f=t(),m=this.componentValue(o);return f===null||m===null?"":rr(m,ji([f.waterfall]),this.colors)}}},u),o=>o),this.spawn(fr,this.ref(d,"panels"),{model:()=>t()?.panels??Bi}),this.$each(this.ref(d,"legend"),()=>[W.legendTee,W.legendGir,W.legendPutts,W.legendPenalty,W.legendAbsence],(o,c,u)=>this.wireEl(Vp,{text:()=>o},u),o=>o),d}cellNow(e){return this.svc.model.get()?.cells.find(t=>t.id===e)}selected(){const e=this.openHole.get();return e===null?null:this.cellNow(e)??null}teeColor(e){switch(e){case"fairway":return this.colors.gain;case"in_play":return this.colors.neutral;case"trouble":return this.colors.loss}}componentValue(e){const t=this.svc.model.get();return t===null?null:Fe(t.waterfall,e)}subtitle(){const e=this.svc.model.get();return e===null?"":Xu({...e,title:kn(e)})}stateLine(){if(this.auth.currentUser.get()===null)return W.notSignedIn;switch(this.svc.phase.get()){case"loading":case"idle":return W.loading;case"notFound":return W.noStatsInRound;case"notAuthorized":return W.notSignedIn;case"failed":return`${W.failedPrefix}${this.svc.failure.get()??""}`;case"ready":return this.svc.model.get()?.cells.length===0?W.noHoleStrip:""}}setHeading(e,t,n){const i=this.ref(e,t).querySelector("h2");i&&(i.textContent=n)}}const Wp=b(`
    <div class="admin">
...
`),Jp={not_started:"Not started",active:"Playing",complete:"Done"},Zp={private:"Private",friends:"Friends",link:"Link"};function ef(s){const e=[`${s.participants.length} players`,`${s.scoreEventCount} scores`];return s.lastEventAt?e.push(`last ${s.lastEventAt.replace("T"," ").slice(0,16)}`):e.push("never played"),e.join(" · ")}function tf(s){const e=[`@${s.username}`,`${s.roundCount} rounds`];return s.lastRoundDate&&e.push(`last ${s.lastRoundDate}`),s.handicapIndex!==null&&e.push(`hcp ${s.handicapIndex}`),s.deletedAt&&e.push("DELETED"),e.join(" · ")}class sf extends R{static styles=`
        .admin {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),$f=b('<option bind="option"></option>');class kf extends R{competitions=this.inject(Oe);state=this.inject(lt);router=this.inject(B);render(){const e=new $(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(wf,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&rf(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:n=>{n.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:n=>this.state.roundCourseDraft.set(n.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:n=>this.state.roundDateDraft.set(n.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(n,i,r)=>this.wireEl($f,{option:{value:()=>n.id,textContent:()=>n.name}},r),n=>n.id),this.$each(this.ref(t,"rounds"),e,(n,i,r)=>this.wireEl(xf,{row:{disabled:()=>!n.shareToken,onclick:()=>{n.shareToken&&this.router.navigate("/round",{query:{token:n.shareToken}})}},number:()=>`Round ${n.roundNumber}`,meta:()=>[n.courseNameSnapshot,n.date].filter(Boolean).join(" · ")||(n.shareToken?"Open":"View-only"),status:{textContent:()=>vf[n.status]??n.status,className:()=>`cd__rstatus s-${n.status}`}},r),n=>JSON.stringify({id:n.id,status:n.status,shareToken:n.shareToken,courseName:n.courseNameSnapshot,date:n.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function Sf(s,e,t){return JSON.stringify({entry:s,points:e,columns:t})}function Tf(s){return s.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const If=b(`
    <div>
...
`),Pf=b('<button bind="button" type="button"></button>'),Cf=b('<th bind="cell"></th>'),Ef=b('<tr bind="row"></tr>'),Nf=b('<td bind="cell"><span bind="value"></span></td>'),Rf=b(`
...
`);class Af extends R{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class Mf extends R{static styles=`
    ...
    `;router=this.inject(B);render(){const e=this.wire(Lf,{header:{className:()=>ws(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(Ad,this.ref(e,"account")),this.spawn(ja,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":mn,"/history":tc,"/round":xh,"/create":Qh,"/login":sp,"/friends":ap,"/friend":lp,"/friend-rounds":cp,"/friend-courses":pp,"/spectate":_p,"/profile":$p,"/stats":Mp,"/round-stats":Kp,"/admin":sf,...pi.competitions?{"/competitions":lf,"/competition":Af}:{}},mn),e}}class Ff extends j{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await cr(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Xh()),this.error.set(null)}catch(e){e instanceof U&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Jh(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof U&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await Zh();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof U&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}V.get(zr);const Qn=V.get(B);V.set(j,new Ff);const Xn=V.get(j);await Mr(Mf,"#app",{hot:void 0,onInit:async()=>{await Xn.load(),Xn.currentUser.get()&&Qn.route.get()==="/login"&&Qn.navigate("/",!0)}});export{os as A,R as C,B as R,p as S,zr as T,_ as a,st as b,$ as c,Sr as d,I as e,kr as n,F as r,b as t};
