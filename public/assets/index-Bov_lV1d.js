...
`),mt={not_started:"Not started",active:"Live",complete:"Done"};class ce extends T{static styles=`
        .landing {
...
</div>`}function Nt(o,e){const t=o.lines.map(s=>{const n=s.segments.map(r=>"text"in r?w(r.text):w(r.ballIds.map(e).join(" & "))).join("");return`<li class="lb-match__line lb-match__line--${s.result}">${n}</li>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${w(o.title)}</h4>
  <ul class="lb-match">${t}</ul>
</div>`}function Ht(o){return`<div class="lb-diag">Unrenderable result section <code>${w(o)}</code> — no generic view yet. Results are not hidden.</div>`}function Rt(o,e){return o.leaderboard.length===0&&o.cards.length===0?`<div class="lb-empty">No scores entered yet for ${w(o.formatLabel)}.</div>`:o.leaderboard.map(s=>s.kind==="ranked"?Ot(s,e):s.kind==="match_summary"?Nt(s,e):Ht(s.kind)).join("")||`<div class="lb-empty">No leaderboard metric for ${w(o.formatLabel)}.</div>`}function Lt(o,e,t){return o.cards.length===0?"":o.cards.map(s=>jt(s,e,t)).join(`
`)}const Mt=b(`
...
`);class ws extends T{static styles=`
    ...
    `;router=this.inject(z);render(){const e=this.wire(xs,{});return this.spawn(Ye,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":ce,"/round":Dt,"/create":es,"/login":ss,"/rounds":us,"/players":as,"/score":gs,"/results":vs},ce),e}}j.get(Re);const fe=j.get(z),ge=j.get(te);await Me(ws,"#app",{hot:void 0,onInit:async()=>{await ge.load(),ge.currentUser.get()&&fe.route.get()==="/login"&&fe.navigate("/",!0)}});export{T as C,z as R,h as S,Re as T,f as a,G as b,$ as c,y as e,x as r,b as t};
