...
`);class Qo extends M{static styles=`
    ...
    `;let le=kn;async function Gu(s,e={}){e.everywhere?await s.auth.logoutEverywhere():await s.auth.logout(),s.profile.clear(),s.friends.clear(),s.activity.clear(),s.friendProfile.clear(),s.spectate.clear(),s.admins.clear(),s.homeStats.clear(),s.landing.clear(),s.navigate("/")}const ju="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";function Ee(s="avatar"){return`<span class="${s}">
            <img bind="avatarPhoto" class="avatar__photo" alt="" />
            <span bind="avatarInitials" class="avatar__initials"></span>
        </span>`}function Se(s){const e=()=>al(s());return{avatarPhoto:{src:()=>e()??ju,className:()=>e()?"avatar__photo":"avatar__photo hidden"},avatarInitials:{textContent:()=>{const t=s();return ol(t.displayName,t.username)},className:()=>e()?"avatar__initials hidden":"avatar__initials"}}}function ze(s,e="0.85rem"){return`
        position: relative;
    ...
    `}const Du="/tapscore/";function qu(s=Du){const e=s.trim().replace(/\/+$/,"");return`${e===""?"":e}/manage/`}function Vu(s){if(!s.signedIn)return[];const e=[{kind:"identity",displayName:(s.displayName??"").trim()||(s.username??"").trim()||"Signed in",username:(s.username??"").trim()},{kind:"profile",label:"Profile"}];return s.canManageCourses&&e.push({kind:"course-setup",label:"Course setup"}),s.isSuperAdmin&&e.push({kind:"admin",label:"Admin"}),e.push({kind:"signout",label:"Sign out"}),e.push({kind:"signout-all",label:"Sign out everywhere"}),e}function Uu(s){return Vu(s).map(e=>e.kind)}function Jn(s){return s.signedIn?"avatar":"signin"}const Ku=b(`
...
`);class Wu extends M{static styles=`
        .acct {
            position: relative;
            display: flex;
            justify-content: flex-end;
...
`),sh='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',mn=`
...
`),dh='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',ch=b(`
    <div class="round-row">
        <button bind="row" type="button" class="round-summary round-row__main">${mn}</button>
        <div bind="actions" class="round-row__actions">
            <button bind="menuButton" type="button" class="round-row__menu-button" aria-label="Round actions" aria-haspopup="true" aria-expanded="false">${dh}</button>
            <div bind="menu" class="round-row__menu" role="group" aria-label="Round actions">
                <button bind="action" type="button" class="round-row__menu-action"></button>
            </div>
        </div>
    </div>
`);class uh extends M{static styles=`
        .history {
...
`;function Wr(s,e,t=""){const n=t?`info-dot ${t}`:"info-dot";return`<button bind="${s}" class="${n}" type="button" aria-label="${e}"><span aria-hidden="true">i</span></button>`}const Yr=`
        .info-dot {
            ...
            &:hover { color: ${l("text")}; }
            &.hidden { display: none; }
        }`,Ne=60,si=8,Us=4,yh=Array.from({length:Us*2+1},(s,e)=>e-Us),_h="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",vh=b(`
...
`),Th=b('<button bind="btn" class="se-seg" type="button"></button>'),ri=b(`
    <div class="se-stats__group">
        <span bind="glabel" class="se-stats__group-label"></span>
...
`),Ph=b('<div bind="rule" class="se-stats__rule"></div>');class Ch extends M{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
...
</tr>`;const U=Ks(n.plan.slotDefId,N.ballIds),O=n.isOpen(U),ee=Qh(U),ce=ea(B,n.routeSections,e,{mode:n.mode??"product"}),ue=["lb-rank__row--expandable"];return _.lead&&ue.push("lb-rank__lead"),O&&ue.push("lb-rank__row--open"),`<tr class="${ue.join(" ")}" data-expand-key="${G(U)}">
  <td class="lb-rank__pos">${_.position}</td>
...
</div>`}function Zh(s,e){const t=Lh(s,e),n=t.matches.map(i=>{const r=i.leader==="a"?" lb-mp__team--lead":"",o=i.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const ep={ranked:Jh,match_summary:(s,e)=>Zh(s,e)},tp={"default-score-grid":Kh,"compact-match-grid":Wh,"category-matrix-grid":Yh};function sp(s){return`<div class="lb-diag">Unrenderable result section <code>${G(s)}</code> — no generic view yet. Results are not hidden.</div>`}function np(s){return`<div class="lb-diag">Unsupported score-grid component <code>${G(s)}</code> — no generic view yet. Results are not hidden.</div>`}function ip(s,e,t,n=null){const i=ep[s.kind];return i?i(s,e,t,n):sp(s.kind)}function ea(s,e,t,n){const i=Qr(s),r=tp[i];return r?r(s,e,t,n):np(i)}function ta(s,e,t=Zr,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${G(s.formatLabel)}.</div>`:s.leaderboard.map(r=>ip(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${G(s.formatLabel)}.</div>`}function sa(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>ea(r,e,t,{mode:i})).join(`
`)}const rp=b(`
...
`);class _n extends M{static styles=`
    ...
    `;svc=this.inject(ge);expansion=new Gh;slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(n=>n.slotDefId===t)??e[0]??null};render(){return this.wire(rp,{status:{className:()=>{const t=this.svc.resultLoading.get(),n=this.svc.result.get()===null;return t||n?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody(),onclick:t=>this.onBodyClick(t),onkeydown:t=>this.onBodyKeydown(t)}})}rowFor(e){return e.target?.closest?.("tr[data-expand-key]")??null}onBodyClick(e){const t=this.rowFor(e);if(!t||(window.getSelection?.()?.toString()??"")!=="")return;const n=t.getAttribute("data-expand-key")??"";this.applyOpen(t,this.expansion.toggle(n))}onBodyKeydown(e){if(e.key!=="Escape")return;const t=this.rowFor(e);if(!t)return;const n=t.getAttribute("data-expand-key")??"";this.expansion.isOpen(n)&&(this.applyOpen(t,this.expansion.set(n,!1)),t.querySelector(".lb-rank__toggle")?.focus(),e.stopPropagation())}applyOpen(e,t){e.classList.toggle("lb-rank__row--open",t),e.querySelector(".lb-rank__toggle")?.setAttribute("aria-expanded",String(t));const n=e.nextElementSibling;n?.classList.contains("lb-rank__panel")&&n.classList.toggle("lb-rank__panel--open",t)}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const n=u=>{const h=this.svc.nameOf(u);return this.svc.isPending(u)?`${h} (open seat)`:h},i=u=>this.svc.groupLabelOf(u),r=Fh(t);this.expansion.retain(r.attached.keys());const o=ta(t,n,i,{plan:r,routeSections:e.routeSections,isOpen:u=>this.expansion.isOpen(u)}),d=sa(r.standalone,e.routeSections,n),c=d?`<h3 class="lb-cards__head">Scorecard</h3>${d}`:"";return o+c}}function ap(s,e){if(!e)return[];const t=[],n=new Set;for(const i of s)for(const r of i.players){if(r.playerId===e)return[];r.guestPlayerId===null||n.has(r.guestPlayerId)||(n.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const op=b(`
...
`);class dp extends M{static styles=`
    ...
    `,lt.seq=0;let pe=lt;function up(s){if(!s)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=s.seats.length>0,t=s.claimedSeats.some(r=>r.viewerMayRelease),n=s.viewer.claimSeat.allowed,i=s.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&n,guestAllowed:e&&i,blockedMessage:e&&!n&&!i?s.viewer.claimSeat.message??s.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function hp(s,e){const t=[];if(s.groupId!==null&&e.length>0){const n=e.findIndex(i=>i.id===s.groupId);if(n>=0){t.push(`Group ${n+1}`);const i=e[n].startTime;i.includes(":")&&t.push(i)}}return s.category!==null&&t.push(s.category),t.join(" · ")}function pp(s){return(s?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const fp=b(`
...
`);class bp extends M{static styles=`
        .seat-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${R()}
...
`);class Pp extends M{static styles=`
    ...
    `;svc=this.inject(ge);auth=this.inject(V);profile=this.inject(Ie);settled=new p(!1);editing=new p(!1);text=new p("");busy=new p(!1);error=new p("");state(){const e=this.profile.player.get();return Sp({playerId:this.auth.currentUser.get()?.id??null,balls:this.svc.balls.get(),firstOpen:this.svc.firstOpen.get(),handicapConfirmedAt:e?.handicapConfirmedAt??null,handicapIndex:e?.handicapIndex??null,profileLoaded:e!==null,settled:this.settled.get(),now:Date.now()})}ensureProfileLoaded(){this.settled.get()||this.svc.firstOpen.get()&&this.auth.currentUser.get()&&this.svc.balls.get().length&&this.profile.load()}async confirm(){if(this.busy.get())return;this.error.set(""),this.busy.set(!0);const e=await this.profile.confirmHandicap();this.busy.set(!1),e?this.settled.set(!0):this.error.set("Could not save that right now. Try again.")}startEdit(){const e=this.profile.player.get()?.handicapIndex??null;this.text.set(e===null?"":Ut(e)),this.error.set(""),this.editing.set(!0)}async save(){if(this.busy.get())return;const e=ve(this.text.get());if(e===null){this.error.set("Enter a handicap index, e.g. 18.4 or +2.4.");return}this.error.set(""),this.busy.set(!0);const t=await this.profile.saveIndex(e);this.busy.set(!1),t?(this.editing.set(!1),this.settled.set(!0)):this.error.set("Could not save that right now. Try again.")}render(){return this.track(C(()=>this.ensureProfileLoaded())),this.wire(Tp,{root:{className:()=>this.state().visible?"hcp-checkin":"hcp-checkin hidden"},ask:{className:()=>this.editing.get()?"hcp-checkin__ask hidden":"hcp-checkin__ask"},question:{textContent:()=>{const e=this.state().index;return e===null?"No handicap set — add one?":`Handicap ${Ut(e)} — still right?`}},confirm:{textContent:()=>this.state().index===null?"Not now":"Yes",disabled:()=>this.busy.get(),onclick:()=>{this.confirm()}},edit:{textContent:()=>this.state().index===null?"Add":"Update",disabled:()=>this.busy.get(),onclick:()=>this.startEdit()},editor:{className:()=>this.editing.get()?"hcp-checkin__editor":"hcp-checkin__editor hidden"},field:{value:()=>this.text.get(),oninput:e=>this.text.set(e.target.value)},save:{disabled:()=>this.busy.get(),onclick:()=>{this.save()}},cancel:{disabled:()=>this.busy.get(),onclick:()=>this.editing.set(!1)},err:{textContent:()=>this.error.get()}})}}function Cp(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}const Ip=b(`
...
`);class Ep extends M{static styles=`
...
`),qf=b('<li bind="text" class="story__line"></li>'),Vf=b(`
...
`),sm=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),nm=b('<button bind="pill" class="round-view__grp" type="button"></button>');class im extends M{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`),zm='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8.5" cy="7.5" r="3.25"/><path d="M2.5 20c.5-3.5 2.7-5.5 6-5.5 2.7 0 5.1 1.8 5.8 4.5"/><circle cx="17.5" cy="16.5" r="4.25"/><path d="M15.5 16.5h4"/></svg>',Fm=b(`
...
`),Ai=b(`
    <div class="friend-row">
        ...
        <span bind="hcp" class="friend-row__hcp"></span>
        <button bind="remove" class="friend-row__remove" type="button" aria-label="Remove friend">${zm}</button>
    </div>
...
`);class Vm extends M{static styles=`
        .frounds {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Wm extends M{static styles=`
    ...
    `;svc=this.inject(ss);auth=this.inject(V);router=this.inject(q);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(C(()=>{const c=e.get();se(()=>{!c||!n()||(this.svc.setPlayer(c),this.svc.loadCourses())})}));const i=()=>this.svc.unavailable.get(),r=()=>i()===null?this.svc.courses.get():[],o=()=>(t.get()??"").trim(),d=this.wire(Um,{back:{onclick:()=>this.router.navigate("/friend",{query:{id:e.get()??"",name:o()}})},subtitle:{textContent:()=>{const c=o();return c?`Where ${c} has played the rounds they share.`:"Where the rounds they share were played."}},refusal:{className:()=>i()!==null?"fcourses__refusal":"fcourses__refusal hidden"},refusalTitle:{textContent:()=>{const c=i();return c?Ke[c].title:""}},refusalMsg:{textContent:()=>{const c=i();return c?Ke[c].message:""}},anon:{className:()=>n()?"fcourses__anon hidden":"fcourses__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/friend-courses?id=${e.get()??""}&name=${encodeURIComponent(o())}`}})},state:{textContent:()=>i()!==null||!n()?"":this.svc.coursesLoading.get()?"Loading…":this.svc.coursesError.get()??""},retry:{className:()=>i()===null&&this.svc.coursesError.get()!==null&&!this.svc.coursesLoading.get()?"fcourses__retry":"fcourses__retry hidden",onclick:()=>{this.svc.loadCourses(!0)}},empty:{className:()=>i()===null&&this.svc.coursesLoaded.get()&&r().length===0&&this.svc.coursesError.get()===null?"fcourses__state":"fcourses__state hidden"},listCard:{className:()=>r().length>0?"fcourses__listcard":"fcourses__listcard hidden"},truncated:{className:()=>i()===null&&this.svc.coursesHasMore.get()&&r().length>0?"fcourses__state":"fcourses__state hidden"}});return this.$each(this.ref(d,"list"),r,(c,u,h)=>this.wireEl(Km,{name:()=>c.courseName??"Course",facts:()=>Al(c,Fe)},h),c=>c.courseId),d}}function Ym(s,e,t){const n=ft(e);if(n){const d=Li(s);return d?`Watching · ${d} ${n}`:`Watching · ${n}`}const i=Li(s)??"this",r=ft(t),o=r?` at ${r}`:"";return`Watching · ${i} round${o}`}function Xm(s,e,t,n){const i=ft(s)===null?null:ft(e),r=n!==null?`${n} holes`:null;return[i,r,t==="complete"?"Finished":t==="not_started"?"Not started":null].filter(c=>c!==null).join(" · ")||null}const Qm="You're watching this round. Only its players can enter scores.",Bi={forbidden:{title:"Round not available",message:"This round is no longer shared with you."},not_found:{title:"Round not found",message:"This round doesn't exist anymore."}};function Li(s){const e=ft(s);return e?e.toLowerCase().endsWith("s")?`${e}'`:`${e}'s`:null}function ft(s){return(s??"").trim()||null}const Jm=b(`
    <div class="spectate">
...
`);class Zm extends M{static styles=`
        ...
        ${_n.styles}
    `;svc=this.inject(br);auth=this.inject(V);router=this.inject(q);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(C(()=>{const c=e.get();se(()=>{!c||!n()||(this.svc.setRound(c),this.svc.load())})}));const i=()=>this.svc.view.get(),r=()=>this.svc.unavailable.get(),o=()=>(t.get()??"").trim()||null;return this.wire(Jm,{back:{onclick:()=>{window.history.length>1?window.history.back():this.router.navigate("/")}},anon:{className:()=>n()?"spectate__anon hidden":"spectate__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/spectate?id=${e.get()??""}&name=${encodeURIComponent(o()??"")}`}})},refusal:{className:()=>n()&&r()!==null?"spectate__refusal":"spectate__refusal hidden"},refusalTitle:{textContent:()=>{const c=r();return c?Bi[c].title:""}},refusalMsg:{textContent:()=>{const c=r();return c?Bi[c].message:""}},state:{textContent:()=>r()!==null||!n()?"":this.svc.loading.get()&&i()===null?"Loading…":i()===null?this.svc.error.get()??"":""},retry:{className:()=>n()&&r()===null&&this.svc.error.get()!==null&&i()===null&&!this.svc.loading.get()?"spectate__retry":"spectate__retry hidden",onclick:()=>{this.svc.load(!0)}},body:{className:()=>n()&&r()===null&&i()!==null?"spectate__body":"spectate__body hidden"},title:{textContent:()=>{const c=i();return c?Ym(o(),c.round.name,c.round.courseNameSnapshot):""}},status:{textContent:()=>{const c=i();return c&&c.status==="active"?"Live":""},className:()=>`spectate__status s-${i()?.status??""}`},subtitle:{textContent:()=>{const c=i();return c?Xm(c.round.name,c.round.courseNameSnapshot,c.status,c.round.playHoles.length||null)??"":""}},date:{textContent:()=>Fe(i()?.round.date??null)},board:{innerHTML:()=>this.renderBoards()}})}renderBoards(){const e=this.svc.view.get();if(!e)return"";const t=r=>this.svc.nameOf(r),n=r=>this.svc.groupLabelOf(r),i=e.result.slots;return i.length===0?'<div class="lb-empty">No formats in this round.</div>':i.map(r=>{const o=i.length>1?`<h2 class="spectate__slot-head">${eg(r.formatLabel)}</h2>`:"",d=ta(r,t,n),c=sa(r.cards,e.result.routeSections,t),u=c?`<h3 class="lb-cards__head">Scorecard</h3>${c}`:"";return o+d+u}).join("")}}function eg(s){return s.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}const tg=b(`
    <div class="profile">
...
`);class ig extends M{static styles=`
        .profile {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),dg=b('<h3 bind="text" class="block__subhead"></h3>'),cg=b(`
...
`);class Tg extends M{static styles=`
        .stats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),Og=b('<li bind="text" class="roundstats__legenditem"></li>');function Hg(){const s=[];for(const[e,t]of Object.entries(bn))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
            `)}class Mg extends M{static styles=`
        .roundstats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
${cs}
    `;svc=this.inject(pt);auth=this.inject(V);router=this.inject(q);colors=ps;openHole=new p(null);infoOpen=new p(!1);render(){const e=this.router.query("id"),t=()=>this.svc.model.get(),n=()=>this.svc.phase.get()==="ready"&&t()!==null;this.track(C(()=>{const d=e.get();se(()=>{this.openHole.set(null),d&&this.svc.load(d)})}));const i=this.router.query("finish"),r=()=>i.get()==="1",o=this.wire(Ig,{back:{className:()=>r()?"roundstats__back hidden":"roundstats__back",onclick:()=>this.router.navigate("/stats")},finishKicker:{className:()=>r()?"roundstats__kicker":"roundstats__kicker hidden"},finishClose:{className:()=>r()?"roundstats__closebtn":"roundstats__closebtn hidden",onclick:()=>this.router.navigate("/")},state:()=>this.stateLine(),body:{className:()=>n()?"roundstats__body":"roundstats__body hidden"},title:()=>t()===null?"":ci(t()),subtitle:()=>this.subtitle(),score:()=>{const d=t();return d===null?"":ca(d.strokes,d.vsPar)??""},stripSec:{className:()=>(t()?.cells.length??0)>0?"roundstats__section":"roundstats__section hidden"},wfHeading:()=>te.waterfallHeading,wfHint:()=>te.waterfallHint,infoTrigger:{textContent:()=>$.prioritiesInfo,onclick:()=>this.infoOpen.set(!0)},infoSheet:{className:()=>this.infoOpen.get()?"stats-info":"stats-info hidden",onclick:d=>{d.target===d.currentTarget&&this.infoOpen.set(!1)}},infoTitle:()=>ye.title,infoDone:{onclick:()=>this.infoOpen.set(!1)},legendHeading:()=>te.legendHeading,detail:{className:()=>this.selected()===null?"holedetail hidden":"holedetail"},detailTitle:()=>{const d=this.selected();return d===null?"":Hf(d)},detailEmpty:()=>{const d=this.selected();return d===null?"":wn(d).length===0?te.nothingRecordedOnHole:""}});return this.setHeading(o,"stripSec",te.holeStripHeading),this.$each(this.ref(o,"strip"),()=>t()?.cells??[],(d,c,u)=>this.wireEl(Eg,{cell:{className:()=>{const h=this.cellNow(d.id)??d;return h.marker===null?"cell":`cell cell--${h.marker}`},"aria-pressed":()=>String(this.openHole.get()===d.id),"aria-label":()=>xi(this.cellNow(d.id)??d),title:()=>xi(this.cellNow(d.id)??d),onclick:()=>this.openHole.set(this.openHole.get()===d.id?null:d.id)},hole:()=>String((this.cellNow(d.id)??d).holeNumber),score:()=>Mf(this.cellNow(d.id)??d),tee:{className:()=>(this.cellNow(d.id)??d).tee===null?"cell__tee cell__tee--absent":"cell__tee",style:()=>{const h=this.cellNow(d.id)??d;return h.tee===null?"":`background:${this.teeColor(h.tee)}`}},gir:{className:()=>{const h=this.cellNow(d.id)??d;return h.gir===null?"cell__gir cell__gir--absent":h.gir?"cell__gir cell__gir--hit":"cell__gir"}},putts:()=>{const h=this.cellNow(d.id)??d;return h.putts===null?"":String(h.putts)},pen:{className:()=>Mp(this.cellNow(d.id)??d)?"cell__pen":"cell__pen cell__pen--absent"}},u),d=>d.id),this.$each(this.ref(o,"detailLines"),()=>Gf(this.selected()),(d,c,u)=>this.wireEl(Rg,{label:()=>d.label,value:()=>d.value},u),d=>d.key),this.$each(this.ref(o,"deltas"),()=>{const d=t();return d===null?[]:he.filter(c=>Ye(d.waterfall,c)!==null)},(d,c,u)=>this.wireEl(Ng,{title:()=>os(d),sentence:()=>{const h=t();if(h===null||h.deltas===null)return"";const f=dn(h.deltas,d);return f===null?"":ua(f,h.windowCount)},value:{textContent:()=>{const h=this.componentValue(d);return h===null?"":we(h)},style:()=>{const h=this.componentValue(d);return h===null?"":`color:${Qe(us(h),this.colors)}`}},bar:{innerHTML:()=>{const h=t(),f=this.componentValue(d);return h===null||f===null?"":oa(f,Br([h.waterfall]),this.colors)}}},u),d=>d),this.$each(this.ref(o,"infoCards"),()=>{const d=t();return d===null?[]:Xt({attributed:d.waterfall.coverage.attributed,holesScored:d.waterfall.coverage.holesScored,windowRounds:0,rowsPer18:he.map(c=>Te(d.waterfall,c)),penaltySource:Yt(d.panels.totals),baseline:this.svc.sgInfo.get()})},(d,c,u)=>this.wireEl(yt,{ctitle:()=>d.title,ctext:()=>d.body},u),d=>d.id),this.spawn(_a,this.ref(o,"panels"),{model:()=>t()?.panels??Ar,baseline:()=>this.svc.sgInfo.get()}),this.$each(this.ref(o,"legend"),()=>[te.legendTee,te.legendGir,te.legendPutts,te.legendPenalty,te.legendAbsence],(d,c,u)=>this.wireEl(Og,{text:()=>d},u),d=>d),o}cellNow(e){return this.svc.model.get()?.cells.find(t=>t.id===e)}selected(){const e=this.openHole.get();return e===null?null:this.cellNow(e)??null}teeColor(e){switch(e){case"fairway":return this.colors.gain;case"in_play":return this.colors.neutral;case"trouble":return this.colors.loss}}componentValue(e){const t=this.svc.model.get();return t===null?null:Ye(t.waterfall,e)}subtitle(){const e=this.svc.model.get();return e===null?"":Of({...e,title:ci(e)})}stateLine(){if(this.auth.currentUser.get()===null)return te.notSignedIn;switch(this.svc.phase.get()){case"loading":case"idle":return te.loading;case"notFound":return te.noStatsInRound;case"notAuthorized":return te.notSignedIn;case"failed":return`${te.failedPrefix}${this.svc.failure.get()??""}`;case"ready":return this.svc.model.get()?.cells.length===0?te.noHoleStrip:""}}setHeading(e,t,n){const i=this.ref(e,t).querySelector("h2");i&&(i.textContent=n)}}const Ag=b(`
    <div class="admin">
...
`),Fg={not_started:"Not started",active:"Playing",complete:"Done"},Gg={private:"Private",friends:"Friends",link:"Link"};function jg(s){const e=[`${s.participants.length} players`,`${s.scoreEventCount} scores`];return s.lastEventAt?e.push(`last ${s.lastEventAt.replace("T"," ").slice(0,16)}`):e.push("never played"),e.join(" · ")}function Dg(s){const e=[`@${s.username}`,`${s.roundCount} rounds`];return s.lastRoundDate&&e.push(`last ${s.lastRoundDate}`),s.handicapIndex!==null&&e.push(`hcp ${s.handicapIndex}`),s.deletedAt&&e.push("DELETED"),e.join(" · ")}class qg extends M{static styles=`
        .admin {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Yg extends M{static styles=`
        .comps {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),cb=b('<option bind="option"></option>');class ub extends M{competitions=this.inject(je);state=this.inject(_t);router=this.inject(q);render(){const e=new T(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(lb,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Ug(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:n=>{n.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:n=>this.state.roundCourseDraft.set(n.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:n=>this.state.roundDateDraft.set(n.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(n,i,r)=>this.wireEl(cb,{option:{value:()=>n.id,textContent:()=>n.name}},r),n=>n.id),this.$each(this.ref(t,"rounds"),e,(n,i,r)=>this.wireEl(db,{row:{disabled:()=>!n.shareToken,onclick:()=>{n.shareToken&&this.router.navigate("/round",{query:{token:n.shareToken}})}},number:()=>`Round ${n.roundNumber}`,meta:()=>[n.courseNameSnapshot,n.date].filter(Boolean).join(" · ")||(n.shareToken?"Open":"View-only"),status:{textContent:()=>ob[n.status]??n.status,className:()=>`cd__rstatus s-${n.status}`}},r),n=>JSON.stringify({id:n.id,status:n.status,shareToken:n.shareToken,courseName:n.courseNameSnapshot,date:n.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function hb(s,e,t){return JSON.stringify({entry:s,points:e,columns:t})}function pb(s){return s.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const fb=b(`
    <div>
...
`),mb=b('<button bind="button" type="button"></button>'),gb=b('<th bind="cell"></th>'),bb=b('<tr bind="row"></tr>'),yb=b('<td bind="cell"><span bind="value"></span></td>'),_b=b(`
...
`);class Sb extends M{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(q);render(){const e=this.wire($b,{header:{className:()=>tn(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(Wu,this.ref(e,"account")),this.spawn(Qo,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":ei,"/history":uh,"/round":im,"/create":Om,"/login":Bm,"/friends":Gm,"/friend":Dm,"/friend-rounds":Vm,"/friend-courses":Wm,"/spectate":Zm,"/profile":ig,"/stats":Tg,"/round-stats":Mg,"/admin":qg,...nr.competitions?{"/competitions":Yg,"/competition":kb}:{}},ei),e}}class Tb extends V{constructor(e){super(),this.client=e}client;async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await this.client.login(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await this.client.me()),this.error.set(null)}catch(e){e instanceof J&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await this.client.logout(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof J&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await this.client.logoutAll();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof J&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}Y.get(ja);const Fi=Y.get(q);Y.set(V,new Tb(ma));const Gi=Y.get(V);await Ua(Sb,"#app",{hot:void 0,onInit:async()=>{await Gi.load(),Gi.currentUser.get()&&Fi.route.get()==="/login"&&Fi.navigate("/",!0)}});export{Bs as A,M as C,q as R,p as S,ja as T,y as a,dt as b,T as c,Oa as d,C as e,Na as n,F as r,b as t};
