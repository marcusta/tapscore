...
`);class Xo extends M{static styles=`
    ...
    `;let oe=xn;async function zu(s,e={}){e.everywhere?await s.auth.logoutEverywhere():await s.auth.logout(),s.profile.clear(),s.friends.clear(),s.activity.clear(),s.friendProfile.clear(),s.spectate.clear(),s.admins.clear(),s.homeStats.clear(),s.landing.clear(),s.navigate("/")}const Fu="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";function Ee(s="avatar"){return`<span class="${s}">
            <img bind="avatarPhoto" class="avatar__photo" alt="" />
            <span bind="avatarInitials" class="avatar__initials"></span>
        </span>`}function Te(s){const e=()=>rl(s());return{avatarPhoto:{src:()=>e()??Fu,className:()=>e()?"avatar__photo":"avatar__photo hidden"},avatarInitials:{textContent:()=>{const t=s();return al(t.displayName,t.username)},className:()=>e()?"avatar__initials hidden":"avatar__initials"}}}function ze(s,e="0.85rem"){return`
        position: relative;
...
</div>`}function Qh(s,e){const t=Ah(s,e),n=t.matches.map(i=>{const r=i.leader==="a"?" lb-mp__team--lead":"",l=i.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
`);class zp extends M{static styles=`
...
`),Bm='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8.5" cy="7.5" r="3.25"/><path d="M2.5 20c.5-3.5 2.7-5.5 6-5.5 2.7 0 5.1 1.8 5.8 4.5"/><circle cx="17.5" cy="16.5" r="4.25"/><path d="M15.5 16.5h4"/></svg>',Lm=b(`
...
`);class Um extends M{static styles=`
    ...
    `;svc=this.inject(ts);auth=this.inject(V);router=this.inject(q);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(C(()=>{const c=e.get();se(()=>{!c||!n()||(this.svc.setPlayer(c),this.svc.loadCourses())})}));const i=()=>this.svc.unavailable.get(),r=()=>i()===null?this.svc.courses.get():[],l=()=>(t.get()??"").trim(),d=this.wire(qm,{back:{onclick:()=>this.router.navigate("/friend",{query:{id:e.get()??"",name:l()}})},subtitle:{textContent:()=>{const c=l();return c?`Where ${c} has played the rounds they share.`:"Where the rounds they share were played."}},refusal:{className:()=>i()!==null?"fcourses__refusal":"fcourses__refusal hidden"},refusalTitle:{textContent:()=>{const c=i();return c?Ke[c].title:""}},refusalMsg:{textContent:()=>{const c=i();return c?Ke[c].message:""}},anon:{className:()=>n()?"fcourses__anon hidden":"fcourses__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/friend-courses?id=${e.get()??""}&name=${encodeURIComponent(l())}`}})},state:{textContent:()=>i()!==null||!n()?"":this.svc.coursesLoading.get()?"Loading…":this.svc.coursesError.get()??""},retry:{className:()=>i()===null&&this.svc.coursesError.get()!==null&&!this.svc.coursesLoading.get()?"fcourses__retry":"fcourses__retry hidden",onclick:()=>{this.svc.loadCourses(!0)}},empty:{className:()=>i()===null&&this.svc.coursesLoaded.get()&&r().length===0&&this.svc.coursesError.get()===null?"fcourses__state":"fcourses__state hidden"},listCard:{className:()=>r().length>0?"fcourses__listcard":"fcourses__listcard hidden"},truncated:{className:()=>i()===null&&this.svc.coursesHasMore.get()&&r().length>0?"fcourses__state":"fcourses__state hidden"}});return this.$each(this.ref(d,"list"),r,(c,u,h)=>this.wireEl(Vm,{name:()=>c.courseName??"Course",facts:()=>Ml(c,Fe)},h),c=>c.courseId),d}}function Km(s,e,t){const n=ft(e);if(n){const d=Bi(s);return d?`Watching · ${d} ${n}`:`Watching · ${n}`}const i=Bi(s)??"this",r=ft(t),l=r?` at ${r}`:"";return`Watching · ${i} round${l}`}function Wm(s,e,t,n){const i=ft(s)===null?null:ft(e),r=n!==null?`${n} holes`:null;return[i,r,t==="complete"?"Finished":t==="not_started"?"Not started":null].filter(c=>c!==null).join(" · ")||null}const Ym="You're watching this round. Only its players can enter scores.",Ai={forbidden:{title:"Round not available",message:"This round is no longer shared with you."},not_found:{title:"Round not found",message:"This round doesn't exist anymore."}};function Bi(s){const e=ft(s);return e?e.toLowerCase().endsWith("s")?`${e}'`:`${e}'s`:null}function ft(s){return(s??"").trim()||null}const Xm=b(`
    <div class="spectate">
