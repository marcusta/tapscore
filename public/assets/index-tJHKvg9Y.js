...
`);class Ds extends I{static styles=`
    ...
    `;let B=Me;function Hs(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function St(){return Hs()}class ce{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await C(this.loading,this.error,()=>b.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=St()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function It(i){const e=z.get(ce);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}function Ms(i){return i.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:Cs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(It).join(" · ")}))}function As(i){return i.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const le={fromMyRounds:Ms,fromDeviceRounds:As},Fs=14,Bs=1440*60*1e3;function ie(i,e){return e(i)}function Gs(i,e,t,s=Fs){const n=e-s*Bs,r=[],l=[];for(const d of i){const u=ie(d,t);if(u.status==="complete"){const c=u.completedAt?Date.parse(u.completedAt):NaN;(Number.isNaN(c)||c>=n)&&l.push(d)}else r.push(d)}return r.sort((d,u)=>Xe(ie(d,t).lastActivityAt,ie(u,t).lastActivityAt)),l.sort((d,u)=>Xe(ie(d,t).completedAt,ie(u,t).completedAt)),{ongoing:r,finished:l}}function Xe(i,e){const t=i?Date.parse(i):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,r=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===r?0:r-n}const qs=_(`
    <div class="landing">
...
`);function ht(i){return i.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class Zi extends I{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Dr extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Lr,{});return this.spawn(Ds,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Je,"/history":Xs,"/round":fi,"/create":qi,"/login":Qi,"/friends":Zi,"/profile":sr,...kt.competitions?{"/competitions":ar,"/competition":Or}:{}},Je),e}}class Hr extends R{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await Lt(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Ki()),this.error.set(null)}catch(e){e instanceof M&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Vi(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof M&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}}z.get(Jt);const pt=z.get(N);z.set(R,new Hr);const mt=z.get(R);await es(Dr,"#app",{hot:void 0,onInit:async()=>{await mt.load(),mt.currentUser.get()&&pt.route.get()==="/login"&&pt.navigate("/",!0)}});export{I as C,N as R,h as S,Jt as T,f as a,de as b,y as c,$ as e,C as r,_ as t};
