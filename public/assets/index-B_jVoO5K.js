...
`);class Bs extends I{static styles=`
    ...
    `;let G=Ge;function Gs(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function He(){return Gs()}class ue{loading=new p(!1);error=new p(null);descriptors=new p([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await C(this.loading,this.error,()=>_.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=He()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function Et(i){const e=z.get(ue);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}function qs(i){return i.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:zs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(Et).join(" · ")}))}function Ks(i){return i.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const de={fromMyRounds:qs,fromDeviceRounds:Ks},Vs=14,Us=1440*60*1e3;function oe(i,e){return e(i)}function Ws(i,e,t,s=Vs){const n=e-s*Us,r=[],l=[];for(const d of i){const u=oe(d,t);if(u.status==="complete"){const h=u.completedAt?Date.parse(u.completedAt):NaN;(Number.isNaN(h)||h>=n)&&l.push(d)}else r.push(d)}return r.sort((d,u)=>et(oe(d,t).lastActivityAt,oe(u,t).lastActivityAt)),l.sort((d,u)=>et(oe(d,t).completedAt,oe(u,t).completedAt)),{ongoing:r,finished:l}}function et(i,e){const t=i?Date.parse(i):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,r=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===r?0:r-n}const Qs=b(`
    <div class="landing">
...
</div>`}function Ln(i,e){const t=i.matches.map(s=>{const n=S(s.sideA.ballIds.map(e).join(" & ")),r=S(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,u=s.leader==="a"?" lb-mp__team--lead":"",h=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
`),Hr=b('<span bind="part"><span bind="separator"></span><span bind="value"></span></span>');class Mr extends I{competitions=this.inject(ne);state=this.inject(he);render(){const e=new v(()=>{if(this.state.lifecycle.get()!=="finalized")return(this.competitions.board.get()?.view.entries??[]).map(c=>({entry:c,points:null}));const h=this.competitions.results.get()?.resultSets??[],f=Math.min(this.state.resultSetIndex.get(),h.length-1);return(h[f]?.entries??[]).map(c=>({entry:c.entry,points:c.points}))}),t=new v(()=>{const h=this.competitions.board.get()?.view.rounds??[];if(h.length>0)return h;const f=new Set;for(const c of e.get())for(const m of c.entry.rounds)f.add(m.roundNumber);return[...f].sort((c,m)=>c-m).map(c=>({roundNumber:c,postCut:!1}))}),s=()=>this.state.lifecycle.get()==="finalized",n=()=>s()?(this.competitions.results.get()?.resultSets.length??0)>0:this.competitions.board.get()!==null,r=()=>this.state.cutOutcome.get(),l=h=>h.length===0?"—":h.map(f=>f.displayName).join(", "),d=this.wire(zr,{admin:{className:()=>this.state.admin.get()&&this.state.lifecycle.get()==="active"?"cd__section cd__admin":"cd__section cd__admin hidden"},cutOutcome:{className:()=>r()?"cd__cutoutcome":"cd__cutoutcome hidden"},advancedLabel:()=>`Advanced (${r()?.advanced.length??0}):`,advanced:()=>l(r()?.advanced??[]),cutLabel:()=>`Cut (${r()?.cut.length??0}):`,cut:()=>l(r()?.cut??[]),applyCut:{disabled:()=>this.competitions.mutating.get(),onclick:()=>this.state.cutConfirmOpen.set(!0)},finalize:{disabled:()=>this.competitions.mutating.get(),onclick:()=>this.state.finalizeConfirmOpen.set(!0)},title:()=>s()?"Official results":"Leaderboard",board:{className:()=>s()?"cd__board cb cb--official":"cd__board"},official:{textContent:()=>{const h=this.competitions.results.get()?.finalizedAt.slice(0,10)??"";return s()&&h?`Official results · finalized ${h}`:""},className:()=>s()?"cd__official-banner":"cd__official-banner hidden"},boardHead:{className:()=>s()?"cb-head hidden":"cb-head"},metric:()=>this.competitions.board.get()?.view.metricLabel??"",operator:()=>{const h=this.competitions.board.get();return h?h.view.operator.kind==="best_n"?`Best ${h.view.operator.n} of ${h.view.rounds.length}`:"Total across rounds":""},defaulted:{className:()=>this.competitions.board.get()?.defaulted?"cb-head__hint":"cb-head__hint hidden"},empty:{className:()=>n()&&e.get().length===0?"cb-empty":"cb-empty hidden"},table:{className:()=>n()&&e.get().length>0?"cb":"cb hidden"},refusal:{textContent:()=>s()?this.competitions.resultsRefusal.get()??"":this.competitions.board.get()===null?this.competitions.boardRefusal.get()??"":""}}),u=new v(()=>[{text:"#",className:"cb-pos"},{text:"Player",className:"cb-who"},...t.get().map((h,f,c)=>({text:`R${h.roundNumber}`,className:`cb-c${h.postCut&&!c.slice(0,f).some(m=>m.postCut)?" cb-c--divider":""}`})),{text:"Total",className:"cb-total"},...s()?[{text:"Pts",className:"cb-points"}]:[]]);return this.$each(this.ref(d,"headers"),u,(h,f,c)=>this.wireEl(jr,{cell:{textContent:()=>h.text,className:()=>h.className}},c),h=>`${h.text}:${h.className}`),this.$each(this.ref(d,"rows"),e,(h,f,c)=>this.boardRow(h,t.get(),c),h=>Nr(h.entry,h.points,t.get())),this.$each(this.ref(d,"switcher"),new v(()=>s()?this.competitions.results.get()?.resultSets??[]:[]),(h,f,c)=>this.wireEl(Rr,{button:{textContent:()=>h.scoringType.toUpperCase(),className:()=>this.state.resultSetIndex.get()===f?"on":"",onclick:()=>this.state.resultSetIndex.set(f)}},c),h=>h.scoringType),this.spawn(G,this.ref(d,"cutConfirm"),{open:this.state.cutConfirmOpen,title:"Apply cut?",message:"This evaluates the configured cut against the current aggregate and marks who advances. Cut players are left out of later rounds.",confirmLabel:"Apply cut",cancelLabel:"Cancel",onconfirm:async()=>{const h=await this.competitions.applyCut(this.state.id.get()??"");h.ok&&this.state.cutOutcome.set(h.outcome)}}),this.spawn(G,this.ref(d,"finalizeConfirm"),{open:this.state.finalizeConfirmOpen,title:"Finalize competition?",message:"Finalizing freezes the official results and locks the competition. This cannot be undone.",confirmLabel:"Finalize",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.competitions.finalize(this.state.id.get()??"")}}),d}boardRow(e,t,s){const n=e.entry,r=n.withdrawn||n.cutAfterRound!==null,l=["cb-row"];n.withdrawn?l.push("cb-row--withdrawn"):n.cutAfterRound!==null?l.push("cb-row--cut"):n.position===1&&l.push("cb-row--lead"),n.incomplete&&l.push("cb-row--incomplete");const d=t.findIndex(c=>c.postCut),u=new Map(n.rounds.map(c=>[c.roundNumber,c])),h=[{kind:"position",text:r?"—":String(n.position)},{kind:"who",entry:n},...t.map((c,m)=>({kind:"round",cell:u.get(c.roundNumber)??null,divider:m===d})),{kind:"total",text:n.total===null?"—":String(n.total)},...e.points===null?[]:[{kind:"points",text:String(e.points)}]],f=this.wireEl(Or,{row:{className:()=>l.join(" ")}},s);return this.$each(f,new v(()=>h),(c,m,y)=>this.boardCell(c,y),(c,m)=>m),f}boardCell(e,t){if(e.kind==="who")return this.whoCell(e.entry,t);const s=e.kind==="position"?"cb-pos":e.kind==="total"?"cb-total":e.kind==="points"?"cb-points":`cb-c cb-c--${e.cell?.status??"missing"}${e.divider?" cb-c--divider":""}`,n=e.kind==="round"?e.cell?.value===null||!e.cell?"—":String(e.cell.value):e.text;return this.wireEl(Dr,{cell:{className:()=>s},value:{textContent:()=>n,className:()=>e.kind==="round"&&e.cell?.status==="dropped"?"cb-struck":""}},t)}whoCell(e,t){const s=e.withdrawn?"WD":e.cutAfterRound!==null?`Cut R${e.cutAfterRound}`:"",n=Pr(e),r=this.wireEl(Lr,{cell:{},name:()=>e.displayName,category:{textContent:()=>e.category??"",className:()=>e.category?"cb-tag cb-cat":"cb-tag cb-cat hidden"},status:{textContent:()=>s,className:()=>s?"cb-tag cb-tag--out":"cb-tag cb-tag--out hidden"},equals:{className:()=>n.length===0?"hidden":""},total:()=>e.total===null?"—":String(e.total)},t);return this.$each(r.querySelector('[bind="parts"]'),new v(()=>n),(l,d,u)=>this.wireEl(Hr,{separator:()=>d===0?"":" + ",value:{textContent:()=>l.text,className:()=>l.dropped?"cb-struck":""}},u),(l,d)=>d),r}}const Fr=b(`
    <div class="cd">
...
`);class Gr extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Br,{});return this.spawn(Bs,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":tt,"/history":sn,"/round":xi,"/create":Yi,"/login":tr,"/friends":rr,"/profile":lr,...Tt.competitions?{"/competitions":pr,"/competition":Ar}:{}},tt),e}}class qr extends R{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await Ft(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Xi()),this.error.set(null)}catch(e){e instanceof M&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Ji(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof M&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}}z.get(ns);const bt=z.get(N);z.set(R,new qr);const _t=z.get(R);await rs(Gr,"#app",{hot:void 0,onInit:async()=>{await _t.load(),_t.currentUser.get()&&bt.route.get()==="/login"&&bt.navigate("/",!0)}});export{I as C,N as R,p as S,ns as T,g as a,ce as b,v as c,$ as e,C as r,b as t};
