...
`),ah=b('<div bind="rule" class="se-stats__rule"></div>');class oh extends M{static styles=`
  ...
  <td class="lb-rank__who"><span class="lb-rank__whobox"><span class="lb-rank__name">${L(g.name)}</span>${w}</span></td>${T}
</tr>`;if(!N||!z)return`<tr class="${g.lead?"lb-rank__lead":""}">
  <td class="lb-rank__pos">${g.position}</td>
  <td class="lb-rank__who"><span class="lb-rank__whobox"><span class="lb-rank__name">${L(g.name)}</span>${w}</span></td>${T}
  <td class="lb-rank__disclosure"></td>
</tr>`;const K=Us(n.plan.slotDefId,N.ballIds),O=n.isOpen(K),J=Eh(K),de=Kr(z,n.routeSections,e,{mode:n.mode??"product"}),ce=["lb-rank__row--expandable"];return g.lead&&ce.push("lb-rank__lead"),O&&ce.push("lb-rank__row--open"),`<tr class="${ce.join(" ")}" data-expand-key="${L(K)}">
  <td class="lb-rank__pos">${g.position}</td>
  <td class="lb-rank__who"><button type="button" class="lb-rank__toggle" aria-expanded="${O}" aria-controls="${L(J)}"><span class="lb-rank__whobox"><span class="lb-rank__name">${L(g.name)}</span>${w}</span></button></td>${T}
  <td class="lb-rank__disclosure"><span class="lb-rank__chev" aria-hidden="true"></span></td>
</tr>
<tr class="lb-rank__panel${O?" lb-rank__panel--open":""}" data-panel-key="${L(K)}">
  <td class="lb-rank__panelcell" colspan="${d}"><div class="lb-rank__panelwrap" id="${L(J)}"><div class="lb-rank__panelbox">${de}</div></div></td>
</tr>`}).join(""),u=r?`
...
</div>`}const Oh={ranked:Rh,match_summary:(s,e)=>Nh(s,e)},Hh={"default-score-grid":Th,"compact-match-grid":Ph,"category-matrix-grid":Ih};function Mh(s){return`<div class="lb-diag">Unrenderable result section <code>${L(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Ah(s){return`<div class="lb-diag">Unsupported score-grid component <code>${L(s)}</code> — no generic view yet. Results are not hidden.</div>`}function zh(s,e,t,n=null){const i=Oh[s.kind];return i?i(s,e,t,n):Mh(s.kind)}function Kr(s,e,t,n){const i=qr(s),r=Hh[i];return r?r(s,e,t,n):Ah(i)}function Wr(s,e,t=Ur,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${L(s.formatLabel)}.</div>`:s.leaderboard.map(r=>zh(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${L(s.formatLabel)}.</div>`}function Yr(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>Kr(r,e,t,{mode:i})).join(`
`)}const Bh=b(`
