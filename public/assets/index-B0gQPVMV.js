...
`);class Ln extends I{static styles=`
    ...
    `;let le=De;function Dn(r,e,t){if(!e||t!=="not_started")return!1;for(const s of r)for(const n of s.players)if(n.playerId===e)return!1;return!0}const tt="new";function Mn(r){const e=r.map((s,n)=>{const i=s.ballIds.length,l=[`Group ${n+1}`];return s.startTime.includes(":")&&l.push(s.startTime),{value:s.id,label:`${l.join(" · ")} — ${i} of ${s.capacity}`,disabled:i>=s.capacity}}),t=e.find(s=>!s.disabled);return e.push({value:tt,label:"Start a new group",disabled:!1}),{options:e,defaultValue:t?.value??tt}}const Hn=b(`
...
`);class Fn extends I{static styles=`
    ...
    `;svc=this.inject(ee);auth=this.inject(D);router=this.inject(N);tokenQ=this.router.query("token");joining=new h(!1);error=new h("");diagnostics=new h([]);teeId=new h("");tees=new h([]);loadedForCourseId=null;groupChoice=new h("");eligible(){return Dn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,_.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const s=this.groupChoice.get(),n=await _.friendlyRounds.join({token:e,teeId:t,...s?{groupChoice:s}:{}});n.ok?await this.svc.loadByToken(e):this.diagnostics.set(n.diagnostics)}catch(s){this.error.set(s instanceof F&&s.status===409?s.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(S(()=>this.ensureTeesLoaded()));const e=new y(()=>Mn(this.svc.groups()));this.track(S(()=>{const i=e.get(),l=this.groupChoice.get();(!l||!i.options.some(d=>d.value===l&&!d.disabled))&&this.groupChoice.set(i.defaultValue)}));const t=this.wire(Hn,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},groupRow:{className:()=>this.svc.groups().length>0?"join-card__group":"join-card__group hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(i=>i.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),s=new le({value:this.teeId,options:{get:()=>this.tees.get().map(i=>({value:i.id,label:i.name}))},placeholder:"Tee"});s.mount(this.ref(t,"teeHost")),this.track(()=>s.destroy());const n=new le({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return n.mount(this.ref(t,"groupHost")),this.track(()=>n.destroy()),t}}const An=b(`
...
`);class Bn extends I{static styles=`
    ...
    `;router=this.inject(N);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&_.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(An,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function Gn(r,e){if(!e)return!1;for(const t of r)for(const s of t.players)if(s.playerId===e)return!0;return!1}const qn=b(`
...
`);class Kn extends I{static styles=`
    ...
    `;svc=this.inject(ee);auth=this.inject(D);router=this.inject(N);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return Gn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await _.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(qn,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(B,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function Vn(r){return!(r.tab!=="leaderboard"||!r.pageVisible||r.status==="complete")}const Wn=2e4;function Un(r){if(!(r===null||r===""))return/^\d+$/.test(r)?Number(r):r}const Yn=b(`
...
`),Qn=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),Xn=b('<button bind="pill" class="round-view__grp" type="button"></button>');class Jn extends I{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class Ti extends I{static styles=`
        .setup {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`),sr=b('<option bind="option"></option>');class nr extends I{competitions=this.inject(J);state=this.inject(de);router=this.inject(N);render(){const e=new y(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(er,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Mi(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:s=>{s.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:s=>this.state.roundCourseDraft.set(s.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:s=>this.state.roundDateDraft.set(s.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(s,n,i)=>this.wireEl(sr,{option:{value:()=>s.id,textContent:()=>s.name}},i),s=>s.id),this.$each(this.ref(t,"rounds"),e,(s,n,i)=>this.wireEl(tr,{row:{disabled:()=>!s.shareToken,onclick:()=>{s.shareToken&&this.router.navigate("/round",{query:{token:s.shareToken}})}},number:()=>`Round ${s.roundNumber}`,meta:()=>[s.courseNameSnapshot,s.date].filter(Boolean).join(" · ")||(s.shareToken?"Open":"View-only"),status:{textContent:()=>Zi[s.status]??s.status,className:()=>`cd__rstatus s-${s.status}`}},i),s=>JSON.stringify({id:s.id,status:s.status,shareToken:s.shareToken,courseName:s.courseNameSnapshot,date:s.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function ir(r,e,t){return JSON.stringify({entry:r,points:e,columns:t})}function rr(r){return r.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const or=b(`
    <div>
...
`),ar=b('<button bind="button" type="button"></button>'),lr=b('<th bind="cell"></th>'),dr=b('<tr bind="row"></tr>'),cr=b('<td bind="cell"><span bind="value"></span></td>'),ur=b(`
...
`);class fr extends I{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class br extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(gr,{});return this.spawn(Cs,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Ue,"/history":Gs,"/round":Jn,"/create":Ti,"/login":Ci,"/friends":Ri,"/profile":Li,"/competitions":Ai,"/competition":fr},Ue),e}}P.get(Vt);const at=P.get(N),lt=P.get(D);await Ut(br,"#app",{hot:void 0,onInit:async()=>{await lt.load(),lt.currentUser.get()&&at.route.get()==="/login"&&at.navigate("/",!0)}});export{I as C,N as R,h as S,Vt as T,f as a,oe as b,y as c,S as e,T as r,b as t};
