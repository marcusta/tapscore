...
</tr>`;const K=Ks(n.plan.slotDefId,N.ballIds),O=n.isOpen(K),J=Lh(K),ce=Jr(L,n.routeSections,e,{mode:n.mode??"product"}),ue=["lb-rank__row--expandable"];return f.lead&&ue.push("lb-rank__lead"),O&&ue.push("lb-rank__row--open"),`<tr class="${ue.join(" ")}" data-expand-key="${F(K)}">
  <td class="lb-rank__pos">${f.position}</td>
...
`);class Om extends M{static styles=`
    ...
    `;svc=this.inject(ts);auth=this.inject(j);router=this.inject(G);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(C(()=>{const c=e.get();se(()=>{!c||!n()||(this.svc.setPlayer(c),this.svc.loadCourses())})}));const i=()=>this.svc.unavailable.get(),r=()=>i()===null?this.svc.courses.get():[],o=()=>(t.get()??"").trim(),d=this.wire(Rm,{back:{onclick:()=>this.router.navigate("/friend",{query:{id:e.get()??"",name:o()}})},subtitle:{textContent:()=>{const c=o();return c?`Where ${c} has played the rounds they share.`:"Where the rounds they share were played."}},refusal:{className:()=>i()!==null?"fcourses__refusal":"fcourses__refusal hidden"},refusalTitle:{textContent:()=>{const c=i();return c?Ue[c].title:""}},refusalMsg:{textContent:()=>{const c=i();return c?Ue[c].message:""}},anon:{className:()=>n()?"fcourses__anon hidden":"fcourses__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/friend-courses?id=${e.get()??""}&name=${encodeURIComponent(o())}`}})},state:{textContent:()=>i()!==null||!n()?"":this.svc.coursesLoading.get()?"Loading…":this.svc.coursesError.get()??""},retry:{className:()=>i()===null&&this.svc.coursesError.get()!==null&&!this.svc.coursesLoading.get()?"fcourses__retry":"fcourses__retry hidden",onclick:()=>{this.svc.loadCourses(!0)}},empty:{className:()=>i()===null&&this.svc.coursesLoaded.get()&&r().length===0&&this.svc.coursesError.get()===null?"fcourses__state":"fcourses__state hidden"},listCard:{className:()=>r().length>0?"fcourses__listcard":"fcourses__listcard hidden"},truncated:{className:()=>i()===null&&this.svc.coursesHasMore.get()&&r().length>0?"fcourses__state":"fcourses__state hidden"}});return this.$each(this.ref(d,"list"),r,(c,u,h)=>this.wireEl(Nm,{name:()=>c.courseName??"Course",facts:()=>Ol(c,Fe)},h),c=>c.courseId),d}}function Mm(s,e,t){const n=pt(e);if(n){const d=zi(s);return d?`Watching · ${d} ${n}`:`Watching · ${n}`}const i=zi(s)??"this",r=pt(t),o=r?` at ${r}`:"";return`Watching · ${i} round${o}`}function Hm(s,e,t,n){const i=pt(s)===null?null:pt(e),r=n!==null?`${n} holes`:null;return[i,r,t==="complete"?"Finished":t==="not_started"?"Not started":null].filter(c=>c!==null).join(" · ")||null}const Am="You're watching this round. Only its players can enter scores.",Ai={forbidden:{title:"Round not available",message:"This round is no longer shared with you."},not_found:{title:"Round not found",message:"This round doesn't exist anymore."}};function zi(s){const e=pt(s);return e?e.toLowerCase().endsWith("s")?`${e}'`:`${e}'s`:null}function pt(s){return(s??"").trim()||null}const zm=b(`
    <div class="spectate">
...
`),gg=b('<li bind="text" class="roundstats__legenditem"></li>');function bg(){const s=[];for(const[e,t]of Object.entries(_n))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
            `)}class _g extends M{static styles=`
        .roundstats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),nb=b('<button bind="button" type="button"></button>'),ib=b('<th bind="cell"></th>'),rb=b('<tr bind="row"></tr>'),ab=b('<td bind="cell"><span bind="value"></span></td>'),ob=b(`
...
`);class pb extends M{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(G);render(){const e=this.wire(hb,{header:{className:()=>tn(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(Mu,this.ref(e,"account")),this.spawn(Wo,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":ei,"/history":Qu,"/round":Gf,"/create":gm,"/login":xm,"/friends":Tm,"/friend":Cm,"/friend-rounds":Em,"/friend-courses":Om,"/spectate":Lm,"/profile":Dm,"/stats":dg,"/round-stats":_g,"/admin":Pg,"/course-setup":Ng,...sr.competitions?{"/competitions":zg,"/competition":ub}:{}},ei),e}}class fb extends j{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await pa(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await bm()),this.error.set(null)}catch(e){e instanceof X&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await _m(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof X&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await ym();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof X&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}W.get(Fa);const Bi=W.get(G);W.set(j,new fb);const Fi=W.get(j);await qa(pb,"#app",{hot:void 0,onInit:async()=>{await Fi.load(),Fi.currentUser.get()&&Bi.route.get()==="/login"&&Bi.navigate("/",!0)}});export{zs as A,M as C,G as R,p as S,Fa as T,_ as a,lt as b,S as c,Ra as d,C as e,Ea as n,z as r,b as t};
