...
`);class Vh extends N{static styles=`
    ...
    `;svc=this.inject(Et);auth=this.inject(D);router=this.inject(B);render(){const e=()=>this.auth.currentUser.get()!==null;e()&&this.svc.load();const t=this.wire(Dh,{anon:{className:()=>e()?"friends__anon hidden":"friends__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:"/friends"}})},body:{className:()=>e()?"friends__body":"friends__body hidden"},search:{value:()=>this.svc.query.get(),oninput:i=>this.svc.setQuery(i.target.value)},searchHint:{textContent:()=>{const i=this.svc.query.get().trim();return i.length>0&&!Ks(i)?"Type at least 2 characters.":this.svc.searching.get()?"Searching…":""}},searchErr:{textContent:()=>this.svc.searchError.get()?.message??""},resultsEmpty:{className:()=>{const i=this.svc.query.get().trim();return Ks(i)&&!this.svc.searching.get()&&this.svc.searchError.get()===null&&this.svc.resultsFor.get()===i&&this.svc.results.get().length===0?"friends__empty":"friends__empty hidden"}},friendsEmpty:{className:()=>this.svc.loaded.get()&&ht(this.svc.friends.get()).mutual.length===0?"friends__empty":"friends__empty hidden"},connectionsSection:{className:()=>ht(this.svc.friends.get()).addedByMe.length>0?"friends__section":"friends__section hidden"},sortToggle:{className:()=>this.svc.friends.get().length>0?"friends__sort":"friends__sort hidden"},sortFrecency:{"aria-pressed":()=>String(this.svc.sortMode.get()==="frecency"),onclick:()=>this.svc.setSortMode("frecency")},sortAlpha:{"aria-pressed":()=>String(this.svc.sortMode.get()==="alpha"),onclick:()=>this.svc.setSortMode("alpha")}});this.$each(this.ref(t,"results"),this.svc.results,(i,r,l)=>this.wireEl(qh,{...Se(()=>this.svc.results.get().find(d=>d.id===i.id)??i),name:()=>i.displayName,username:()=>i.homeClubName?`@${i.username} · ${i.homeClubName}`:`@${i.username}`,hcp:()=>i.handicapIndex===null?"–":i.handicapIndex.toFixed(1),add:{className:()=>this.isFriendNow(i.id)?"friend-row__add hidden":"friend-row__add",disabled:()=>this.svc.mutating.get(),onclick:()=>{const d=this.svc.results.get().find(c=>c.id===i.id);d&&!d.isFriend&&this.svc.add(d)}},added:{className:()=>this.isFriendNow(i.id)?"friend-row__added":"friend-row__added hidden"}},l),i=>i.id);const n=new Date().toISOString();return this.$each(this.ref(t,"friends"),()=>vt(ht(this.svc.friends.get()).mutual,this.svc.sortMode.get()),(i,r,l)=>this.wireEl(jn,this.friendRowBindings(i,n),l),i=>i.id),this.$each(this.ref(t,"connections"),()=>vt(ht(this.svc.friends.get()).addedByMe,this.svc.sortMode.get()),(i,r,l)=>this.wireEl(jn,this.friendRowBindings(i,n),l),i=>i.id),t}liveFriend(e){return this.svc.friends.get().find(t=>t.id===e.id)??e}friendRowBindings(e,t){return{...Se(()=>this.svc.friends.get().find(n=>n.id===e.id)??e),open:{disabled:()=>!this.liveFriend(e).isMutual,onclick:()=>{const n=this.liveFriend(e);n.isMutual&&this.router.navigate("/friend",{query:{id:e.id,name:n.displayName}})}},name:()=>e.displayName,subtitle:()=>so(this.liveFriend(e),t),connection:()=>no(this.liveFriend(e))??"",hcp:()=>e.handicapIndex===null?"–":e.handicapIndex.toFixed(1),remove:{"aria-label":()=>`Remove ${this.liveFriend(e).displayName} from friends`,title:()=>`Remove ${this.liveFriend(e).displayName} from friends`,disabled:()=>this.svc.mutating.get(),onclick:()=>{this.svc.remove(e.id)}}}}isFriendNow(e){return this.svc.results.get().find(t=>t.id===e)?.isFriend===!0}}const ar=b(`
