...
</div>`}function jt(o){return`<div class="lb-diag">Unrenderable result section <code>${w(o)}</code> — no generic view yet. Results are not hidden.</div>`}function Ot(o,e){return o.leaderboard.length===0&&o.cards.length===0?`<div class="lb-empty">No scores entered yet for ${w(o.formatLabel)}.</div>`:o.leaderboard.map(s=>s.kind==="ranked"?Ct(s,e):s.kind==="match_summary"?zt(s,e):jt(s.kind)).join("")||`<div class="lb-empty">No leaderboard metric for ${w(o.formatLabel)}.</div>`}function Nt(o,e,t){return o.cards.length===0?"":o.cards.map(s=>Pt(s,e,t)).join(`
`)}const Ht=b(`
...
`),Rt=b('<button bind="pill" class="lb__pill" type="button"></button>');class Lt extends T{static styles=`
        .lb {
...
`);class vs extends T{static styles=`
    ...
    `;router=this.inject(z);render(){const e=this.wire(_s,{});return this.spawn(Qe,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":le,"/round":Bt,"/create":Jt,"/login":es,"/rounds":ds,"/players":os,"/score":ms,"/results":ys},le),e}}j.get(Oe);const he=j.get(z),pe=j.get(Z);await He(vs,"#app",{hot:void 0,onInit:async()=>{await pe.load(),pe.currentUser.get()&&he.route.get()==="/login"&&he.navigate("/",!0)}});export{T as C,z as R,h as S,Oe as T,m as a,G as b,$ as c,y as e,x as r,b as t};
