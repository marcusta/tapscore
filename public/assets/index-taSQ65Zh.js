...
`);class Hs extends I{static styles=`
    ...
    `;let G=Fe;function Ms(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function St(){return Ms()}class ce{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await T(this.loading,this.error,()=>b.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=St()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function It(i){const e=z.get(ce);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}function Fs(i){return i.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:Cs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(It).join(" · ")}))}function As(i){return i.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const le={fromMyRounds:Fs,fromDeviceRounds:As},Bs=14,Gs=1440*60*1e3;function ie(i,e){return e(i)}function qs(i,e,t,s=Bs){const n=e-s*Gs,r=[],l=[];for(const d of i){const u=ie(d,t);if(u.status==="complete"){const c=u.completedAt?Date.parse(u.completedAt):NaN;(Number.isNaN(c)||c>=n)&&l.push(d)}else r.push(d)}return r.sort((d,u)=>Xe(ie(d,t).lastActivityAt,ie(u,t).lastActivityAt)),l.sort((d,u)=>Xe(ie(d,t).completedAt,ie(u,t).completedAt)),{ongoing:r,finished:l}}function Xe(i,e){const t=i?Date.parse(i):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,r=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===r?0:r-n}const Ks=_(`
    <div class="landing">
...
`),Tt={not_started:"Not started",active:"Live",complete:"Finished"};class Je extends I{static styles=`
        .landing {
...
</div>`}function Rn(i,e){const t=i.matches.map(s=>{const n=S(s.sideA.ballIds.map(e).join(" & ")),r=S(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,u=s.leader==="a"?" lb-mp__team--lead":"",c=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const jn={ranked:zn,match_summary:(i,e)=>Rn(i,e)},On={"default-score-grid":Sn,"compact-match-grid":In,"category-matrix-grid":Tn};function Ln(i){return i.componentId??"default-score-grid"}function Dn(i){return`<div class="lb-diag">Unrenderable result section <code>${S(i)}</code> — no generic view yet. Results are not hidden.</div>`}function Hn(i){return`<div class="lb-diag">Unsupported score-grid component <code>${S(i)}</code> — no generic view yet. Results are not hidden.</div>`}function Mn(i,e,t){const s=jn[i.kind];return s?s(i,e,t):Dn(i.kind)}function Fn(i,e,t,s){const n=Ln(i),r=On[n];return r?r(i,e,t,s):Hn(n)}function An(i,e,t=Nt){return i.leaderboard.length===0&&i.cards.length===0?`<div class="lb-empty">No scores entered yet for ${S(i.formatLabel)}.</div>`:i.leaderboard.map(n=>Mn(n,e,t)).join("")||`<div class="lb-empty">No leaderboard metric for ${S(i.formatLabel)}.</div>`}function Bn(i,e,t,s={}){if(i.cards.length===0)return"";const n=s.mode??"product";return i.cards.map(r=>Fn(r,e,t,{mode:n})).join(`
`)}const Gn=_(`
...
`);class qn extends I{static styles=`
    ...
    `;svc=this.inject(ee);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(Gn,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=u=>{const c=this.svc.nameOf(u);return this.svc.isPending(u)?`${c} (open seat)`:c},r=An(t,s,u=>this.svc.groupLabelOf(u)),l=Bn(t,e.routeSections,s),d=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return r+d}}function Kn(i,e){if(!e)return[];const t=[],s=new Set;for(const n of i)for(const r of n.players){if(r.playerId===e)return[];r.guestPlayerId===null||s.has(r.guestPlayerId)||(s.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const Vn=_(`
...
`);class Wn extends I{static styles=`
    ...
    `;let q=Ae;function Qn(i){if(!i)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=i.seats.length>0,t=i.claimedSeats.some(r=>r.viewerMayRelease),s=i.viewer.claimSeat.allowed,n=i.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&s,guestAllowed:e&&n,blockedMessage:e&&!s&&!n?i.viewer.claimSeat.message??i.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function Yn(i,e){const t=[];if(i.groupId!==null&&e.length>0){const s=e.findIndex(n=>n.id===i.groupId);if(s>=0){t.push(`Group ${s+1}`);const n=e[s].startTime;n.includes(":")&&t.push(n)}}return i.category!==null&&t.push(i.category),t.join(" · ")}function Xn(i){return(i?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const Jn=_(`
...
`);class oi extends I{static styles=`
    ...
    `;svc=this.inject(ee);auth=this.inject(R);router=this.inject(N);tokenQ=this.router.query("token");joining=new h(!1);error=new h("");diagnostics=new h([]);teeId=new h("");tees=new h([]);loadedForCourseId=null;groupChoice=new h("");policyState(){return ni(this.svc.startList.get())}eligible(){return this.policyState().visible&&si(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,b.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const s=this.groupChoice.get(),n=await b.friendlyRounds.join({token:e,teeId:t,...s?{groupChoice:s}:{}});n.ok?await this.svc.loadByToken(e):this.diagnostics.set(n.diagnostics)}catch(s){this.error.set(s instanceof M&&s.status===409?s.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track($(()=>this.ensureTeesLoaded()));const e=new y(()=>ii(this.svc.groups(),this.svc.startList.get()?.viewer.createGroup.allowed??!0));this.track($(()=>{const r=e.get(),l=this.groupChoice.get();(!l||!r.options.some(d=>d.value===l&&!d.disabled))&&this.groupChoice.set(r.defaultValue)}));const t=this.wire(ri,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},blocked:{className:()=>this.policyState().blockedMessage!==null?"join-card__blocked":"join-card__blocked hidden",textContent:()=>this.policyState().blockedMessage??""},groupRow:{className:()=>this.svc.groups().length>0&&this.policyState().blockedMessage===null?"join-card__group":"join-card__group hidden"},row:{className:()=>this.policyState().blockedMessage===null?"join-card__row":"join-card__row hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(r=>r.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),s=new q({value:this.teeId,options:{get:()=>this.tees.get().map(r=>({value:r.id,label:r.name}))},placeholder:"Tee"});s.mount(this.ref(t,"teeHost")),this.track(()=>s.destroy());const n=new q({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return n.mount(this.ref(t,"groupHost")),this.track(()=>n.destroy()),t}}const ai=_(`
...
`);class li extends I{static styles=`
    ...
    `;router=this.inject(N);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&b.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(ai,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function di(i,e){if(!e)return!1;for(const t of i)for(const s of t.players)if(s.playerId===e)return!0;return!1}const ci=_(`
...
`);class ui extends I{static styles=`
    ...
    `;svc=this.inject(ee);auth=this.inject(R);router=this.inject(N);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return di(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await b.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(ci,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(G,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function hi(i){return!(i.tab!=="leaderboard"||!i.pageVisible||i.status==="complete")}const pi=2e4;function mi(i){if(!(i===null||i===""))return/^\d+$/.test(i)?Number(i):i}const fi=_(`
...
`),gi=_('<button bind="pill" class="round-view__fmt" type="button"></button>'),bi=_('<button bind="pill" class="round-view__grp" type="button"></button>');class _i extends I{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class Vi extends I{static styles=`
        .setup {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class Xi extends I{static styles=`
        .login {
            max-width: 340px;
            margin: 0 auto;
            padding: 14vh ${a("xl")} 0;
...
`);function ht(i){return i.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class tr extends I{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),kr=_('<option bind="option"></option>');class Sr extends I{competitions=this.inject(te);state=this.inject(ue);router=this.inject(N);render(){const e=new y(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(xr,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&or(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:s=>{s.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:s=>this.state.roundCourseDraft.set(s.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:s=>this.state.roundDateDraft.set(s.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(s,n,r)=>this.wireEl(kr,{option:{value:()=>s.id,textContent:()=>s.name}},r),s=>s.id),this.$each(this.ref(t,"rounds"),e,(s,n,r)=>this.wireEl($r,{row:{disabled:()=>!s.shareToken,onclick:()=>{s.shareToken&&this.router.navigate("/round",{query:{token:s.shareToken}})}},number:()=>`Round ${s.roundNumber}`,meta:()=>[s.courseNameSnapshot,s.date].filter(Boolean).join(" · ")||(s.shareToken?"Open":"View-only"),status:{textContent:()=>wr[s.status]??s.status,className:()=>`cd__rstatus s-${s.status}`}},r),s=>JSON.stringify({id:s.id,status:s.status,shareToken:s.shareToken,courseName:s.courseNameSnapshot,date:s.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function Ir(i,e,t){return JSON.stringify({entry:i,points:e,columns:t})}function Tr(i){return i.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const Cr=_(`
    <div>
...
`),Er=_('<button bind="button" type="button"></button>'),Nr=_('<th bind="cell"></th>'),Pr=_('<tr bind="row"></tr>'),zr=_('<td bind="cell"><span bind="value"></span></td>'),Rr=_(`
...
`);class Mr extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Hr,{});return this.spawn(Hs,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Je,"/history":Js,"/round":_i,"/create":Vi,"/login":Xi,"/friends":tr,"/profile":ir,...kt.competitions?{"/competitions":dr,"/competition":Dr}:{}},Je),e}}class Fr extends R{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await Lt(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Ui()),this.error.set(null)}catch(e){e instanceof M&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Wi(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof M&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}}z.get(Zt);const pt=z.get(N);z.set(R,new Fr);const mt=z.get(R);await ts(Mr,"#app",{hot:void 0,onInit:async()=>{await mt.load(),mt.currentUser.get()&&pt.route.get()==="/login"&&pt.navigate("/",!0)}});export{I as C,N as R,h as S,Zt as T,f as a,de as b,y as c,$ as e,T as r,_ as t};
