...
`);class Xo extends M{static styles=`
    ...
    `;let oe=xn;async function Fu(s,e={}){e.everywhere?await s.auth.logoutEverywhere():await s.auth.logout(),s.profile.clear(),s.friends.clear(),s.activity.clear(),s.friendProfile.clear(),s.spectate.clear(),s.admins.clear(),s.homeStats.clear(),s.landing.clear(),s.navigate("/")}const Gu="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";function Ee(s="avatar"){return`<span class="${s}">
            <img bind="avatarPhoto" class="avatar__photo" alt="" />
            <span bind="avatarInitials" class="avatar__initials"></span>
        </span>`}function Te(s){const e=()=>rl(s());return{avatarPhoto:{src:()=>e()??Gu,className:()=>e()?"avatar__photo":"avatar__photo hidden"},avatarInitials:{textContent:()=>{const t=s();return al(t.displayName,t.username)},className:()=>e()?"avatar__initials hidden":"avatar__initials"}}}function ze(s,e="0.85rem"){return`
        position: relative;
    ...
    `}const ju="/tapscore/";function Du(s=ju){const e=s.trim().replace(/\/+$/,"");return`${e===""?"":e}/manage/`}function qu(s){if(!s.signedIn)return[];const e=[{kind:"identity",displayName:(s.displayName??"").trim()||(s.username??"").trim()||"Signed in",username:(s.username??"").trim()},{kind:"profile",label:"Profile"}];return s.canManageCourses&&e.push({kind:"course-setup",label:"Course setup"}),s.isSuperAdmin&&e.push({kind:"admin",label:"Admin"}),e.push({kind:"signout",label:"Sign out"}),e.push({kind:"signout-all",label:"Sign out everywhere"}),e}function Vu(s){return qu(s).map(e=>e.kind)}function Qn(s){return s.signedIn?"avatar":"signin"}const Uu=b(`
...
`);class Ku extends M{static styles=`
        .acct {
            position: relative;
            display: flex;
            justify-content: flex-end;
...
`),th='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',fn=`
...
`,sh=b(`
    <div class="round-row">
        ...
        <div bind="actions" class="round-row__actions">
            <button bind="menuButton" type="button" class="round-row__menu-button" aria-label="Round actions" aria-haspopup="true" aria-expanded="false">${th}</button>
...
`),lh='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',dh=b(`
    <div class="round-row">
        <button bind="row" type="button" class="round-summary round-row__main">${fn}</button>
        <div bind="actions" class="round-row__actions">
...
`);class ch extends M{static styles=`
        .history {
...
`;function Kr(s,e,t=""){const n=t?`info-dot ${t}`:"info-dot";return`<button bind="${s}" class="${n}" type="button" aria-label="${e}"><span aria-hidden="true">i</span></button>`}const Wr=`
        .info-dot {
            ...
            &:hover { color: ${o("text")}; }
            &.hidden { display: none; }
        }`,Ne=60,ti=8,Vs=4,bh=Array.from({length:Vs*2+1},(s,e)=>e-Vs),yh="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",_h=b(`
...
`),Sh=b('<button bind="btn" class="se-seg" type="button"></button>'),ii=b(`
    <div class="se-stats__group">
        <span bind="glabel" class="se-stats__group-label"></span>
...
`),Th=b('<div bind="rule" class="se-stats__rule"></div>');class Ph extends M{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
...
</tr>`;const U=Us(n.plan.slotDefId,N.ballIds),O=n.isOpen(U),ee=Xh(U),ce=Zr(B,n.routeSections,e,{mode:n.mode??"product"}),ue=["lb-rank__row--expandable"];return _.lead&&ue.push("lb-rank__lead"),O&&ue.push("lb-rank__row--open"),`<tr class="${ue.join(" ")}" data-expand-key="${G(U)}">
  <td class="lb-rank__pos">${_.position}</td>
...
<tr class="lb-rank__panel${O?" lb-rank__panel--open":""}" data-panel-key="${G(U)}">
  <td class="lb-rank__panelcell" colspan="${d}"><div class="lb-rank__panelwrap" id="${G(ee)}"><div class="lb-rank__panelbox">${ce}</div></div></td>
      ...
      <col class="lb-rank__col-pace">`:"",h=r?'<th class="lb-rank__pace">Pace</th>':"",f=l?`
  ...
  <table class="lb-rank">
...
</div>`}function Jh(s,e){const t=Bh(s,e),n=t.matches.map(i=>{const r=i.leader==="a"?" lb-mp__team--lead":"",l=i.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
    ...
    <div class="lb-mp__team lb-mp__team--b${l}">${G(i.sideBName)}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${G(t.title)}</h4>${n}
</div>`}const Zh={ranked:Qh,match_summary:(s,e)=>Jh(s,e)},ep={"default-score-grid":Uh,"compact-match-grid":Kh,"category-matrix-grid":Wh};function tp(s){return`<div class="lb-diag">Unrenderable result section <code>${G(s)}</code> — no generic view yet. Results are not hidden.</div>`}function sp(s){return`<div class="lb-diag">Unsupported score-grid component <code>${G(s)}</code> — no generic view yet. Results are not hidden.</div>`}function np(s,e,t,n=null){const i=Zh[s.kind];return i?i(s,e,t,n):tp(s.kind)}function Zr(s,e,t,n){const i=Xr(s),r=ep[i];return r?r(s,e,t,n):sp(i)}function ea(s,e,t=Jr,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${G(s.formatLabel)}.</div>`:s.leaderboard.map(r=>np(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${G(s.formatLabel)}.</div>`}function ta(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>Zr(r,e,t,{mode:i})).join(`
`)}const ip=b(`
...
`);class yn extends M{static styles=`
    ...
    `;svc=this.inject(ge);expansion=new Fh;slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(n=>n.slotDefId===t)??e[0]??null};render(){return this.wire(ip,{status:{className:()=>{const t=this.svc.resultLoading.get(),n=this.svc.result.get()===null;return t||n?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody(),onclick:t=>this.onBodyClick(t),onkeydown:t=>this.onBodyKeydown(t)}})}rowFor(e){return e.target?.closest?.("tr[data-expand-key]")??null}onBodyClick(e){const t=this.rowFor(e);if(!t||(window.getSelection?.()?.toString()??"")!=="")return;const n=t.getAttribute("data-expand-key")??"";this.applyOpen(t,this.expansion.toggle(n))}onBodyKeydown(e){if(e.key!=="Escape")return;const t=this.rowFor(e);if(!t)return;const n=t.getAttribute("data-expand-key")??"";this.expansion.isOpen(n)&&(this.applyOpen(t,this.expansion.set(n,!1)),t.querySelector(".lb-rank__toggle")?.focus(),e.stopPropagation())}applyOpen(e,t){e.classList.toggle("lb-rank__row--open",t),e.querySelector(".lb-rank__toggle")?.setAttribute("aria-expanded",String(t));const n=e.nextElementSibling;n?.classList.contains("lb-rank__panel")&&n.classList.toggle("lb-rank__panel--open",t)}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const n=u=>{const h=this.svc.nameOf(u);return this.svc.isPending(u)?`${h} (open seat)`:h},i=u=>this.svc.groupLabelOf(u),r=zh(t);this.expansion.retain(r.attached.keys());const l=ea(t,n,i,{plan:r,routeSections:e.routeSections,isOpen:u=>this.expansion.isOpen(u)}),d=ta(r.standalone,e.routeSections,n),c=d?`<h3 class="lb-cards__head">Scorecard</h3>${d}`:"";return l+c}}function rp(s,e){if(!e)return[];const t=[],n=new Set;for(const i of s)for(const r of i.players){if(r.playerId===e)return[];r.guestPlayerId===null||n.has(r.guestPlayerId)||(n.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const ap=b(`
...
`);class lp extends M{static styles=`
    ...
    `,lt.seq=0;let pe=lt;function cp(s){if(!s)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=s.seats.length>0,t=s.claimedSeats.some(r=>r.viewerMayRelease),n=s.viewer.claimSeat.allowed,i=s.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&n,guestAllowed:e&&i,blockedMessage:e&&!n&&!i?s.viewer.claimSeat.message??s.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function up(s,e){const t=[];if(s.groupId!==null&&e.length>0){const n=e.findIndex(i=>i.id===s.groupId);if(n>=0){t.push(`Group ${n+1}`);const i=e[n].startTime;i.includes(":")&&t.push(i)}}return s.category!==null&&t.push(s.category),t.join(" · ")}function hp(s){return(s?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const pp=b(`
...
`);class wp extends M{static styles=`
        .join-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${R()}
    ...
    `;svc=this.inject(ge);auth=this.inject(V);router=this.inject(q);tokenQ=this.router.query("token");joining=new p(!1);error=new p("");diagnostics=new p([]);teeId=new p("");tees=new p([]);loadedForCourseId=null;groupChoice=new p("");policyState(){return yp(this.svc.startList.get())}eligible(){return this.policyState().visible&&bp(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,w.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const n=this.groupChoice.get(),i=await w.friendlyRounds.join({token:e,teeId:t,...n?{groupChoice:n}:{}});i.ok?await this.svc.loadByToken(e):this.diagnostics.set(i.diagnostics)}catch(n){this.error.set(n instanceof Q&&n.status===409?n.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(C(()=>this.ensureTeesLoaded()));const e=new T(()=>_p(this.svc.groups(),this.svc.startList.get()?.viewer.createGroup.allowed??!0));this.track(C(()=>{const r=e.get(),l=this.groupChoice.get();(!l||!r.options.some(d=>d.value===l&&!d.disabled))&&this.groupChoice.set(r.defaultValue)}));const t=this.wire(vp,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},blocked:{className:()=>this.policyState().blockedMessage!==null?"join-card__blocked":"join-card__blocked hidden",textContent:()=>this.policyState().blockedMessage??""},groupRow:{className:()=>this.svc.groups().length>0&&this.policyState().blockedMessage===null?"join-card__group":"join-card__group hidden"},row:{className:()=>this.policyState().blockedMessage===null?"join-card__row":"join-card__row hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(r=>r.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),n=new pe({value:this.teeId,options:{get:()=>this.tees.get().map(r=>({value:r.id,label:r.name}))},placeholder:"Tee"});n.mount(this.ref(t,"teeHost")),this.track(()=>n.destroy());const i=new pe({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return i.mount(this.ref(t,"groupHost")),this.track(()=>i.destroy()),t}}const oi=1440*60*1e3;function xp(s,e){if(s===null)return!0;const t=new Date(s).getTime();return!Number.isFinite(t)||t-e>oi?!0:e-t>=oi}function kp(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}function $p(s){const e={visible:!1,index:null};return s.settled||!s.profileLoaded||!s.firstOpen||!kp(s.balls,s.playerId)||!xp(s.handicapConfirmedAt,s.now)?e:{visible:!0,index:s.handicapIndex}}function ve(s){const e=s.trim().replace(",",".");if(e==="")return null;const t=e.startsWith("+"),n=Number.parseFloat(t?e.slice(1):e);return Number.isFinite(n)?t?-n:n:null}function Vt(s){return s<0?`+${String(-s)}`:String(s)}const Sp=b(`
...
`);class Tp extends M{static styles=`
    ...
    `;svc=this.inject(ge);auth=this.inject(V);profile=this.inject(Ie);settled=new p(!1);editing=new p(!1);text=new p("");busy=new p(!1);error=new p("");state(){const e=this.profile.player.get();return $p({playerId:this.auth.currentUser.get()?.id??null,balls:this.svc.balls.get(),firstOpen:this.svc.firstOpen.get(),handicapConfirmedAt:e?.handicapConfirmedAt??null,handicapIndex:e?.handicapIndex??null,profileLoaded:e!==null,settled:this.settled.get(),now:Date.now()})}ensureProfileLoaded(){this.settled.get()||this.svc.firstOpen.get()&&this.auth.currentUser.get()&&this.svc.balls.get().length&&this.profile.load()}async confirm(){if(this.busy.get())return;this.error.set(""),this.busy.set(!0);const e=await this.profile.confirmHandicap();this.busy.set(!1),e?this.settled.set(!0):this.error.set("Could not save that right now. Try again.")}startEdit(){const e=this.profile.player.get()?.handicapIndex??null;this.text.set(e===null?"":Vt(e)),this.error.set(""),this.editing.set(!0)}async save(){if(this.busy.get())return;const e=ve(this.text.get());if(e===null){this.error.set("Enter a handicap index, e.g. 18.4 or +2.4.");return}this.error.set(""),this.busy.set(!0);const t=await this.profile.saveIndex(e);this.busy.set(!1),t?(this.editing.set(!1),this.settled.set(!0)):this.error.set("Could not save that right now. Try again.")}render(){return this.track(C(()=>this.ensureProfileLoaded())),this.wire(Sp,{root:{className:()=>this.state().visible?"hcp-checkin":"hcp-checkin hidden"},ask:{className:()=>this.editing.get()?"hcp-checkin__ask hidden":"hcp-checkin__ask"},question:{textContent:()=>{const e=this.state().index;return e===null?"No handicap set — add one?":`Handicap ${Vt(e)} — still right?`}},confirm:{textContent:()=>this.state().index===null?"Not now":"Yes",disabled:()=>this.busy.get(),onclick:()=>{this.confirm()}},edit:{textContent:()=>this.state().index===null?"Add":"Update",disabled:()=>this.busy.get(),onclick:()=>this.startEdit()},editor:{className:()=>this.editing.get()?"hcp-checkin__editor":"hcp-checkin__editor hidden"},field:{value:()=>this.text.get(),oninput:e=>this.text.set(e.target.value)},save:{disabled:()=>this.busy.get(),onclick:()=>{this.save()}},cancel:{disabled:()=>this.busy.get(),onclick:()=>this.editing.set(!1)},err:{textContent:()=>this.error.get()}})}}function Pp(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}const Cp=b(`
...
`),Df=b('<li bind="text" class="story__line"></li>'),qf=b(`
...
${ds}
    `;round=this.inject(ge);stats=this.inject(pt);auth=this.inject(V);router=this.inject(q);colors=hs;infoOpen=new p(!1);render(){this.track(C(()=>{const i=this.eligibleRoundId();se(()=>{i!==null&&this.stats.load(i).catch(()=>{})})}));const e=()=>this.shows()?this.stats.model.get():null,t=this.wire(jf,{story:{className:()=>this.shows()?"story":"story hidden"},title:()=>vi.title,score:()=>{const i=e();return i===null?"":da(i.strokes,i.vsPar)??""},hint:()=>e()===null?"":this.hint(),infoTrigger:{textContent:()=>$.prioritiesInfo,onclick:()=>this.infoOpen.set(!0)},infoSheet:{className:()=>this.infoOpen.get()?"stats-info":"stats-info hidden",onclick:i=>{i.target===i.currentTarget&&this.infoOpen.set(!1)}},infoTitle:()=>ye.title,infoDone:{onclick:()=>this.infoOpen.set(!1)},open:{textContent:()=>vi.seeWholeRound,onclick:()=>{const i=this.stats.roundId.get();i!==null&&this.router.navigate("/round-stats",{query:{id:i}})}}}),n=i=>{const r=e();return r===null?null:Ye(r.waterfall,i)};return this.$each(this.ref(t,"values"),()=>e()===null?[]:[...he],(i,r,l)=>this.wireEl(qf,{label:()=>as(i),amount:{textContent:()=>{const d=n(i);return d===null?$.notRecorded:xe(d)},className:()=>n(i)===null?"story__valueamount story__valueamount--absent":"story__valueamount",style:()=>{const d=n(i);return d===null?"":`color:${Qe(cs(d),this.colors)}`}}},l),i=>i),this.$each(this.ref(t,"infoCards"),()=>{const i=e();return i===null?[]:Yt({attributed:i.waterfall.coverage.attributed,holesScored:i.waterfall.coverage.holesScored,windowRounds:0,rowsPer18:he.map(r=>Pe(i.waterfall,r)),penaltySource:Wt(i.panels.totals),baseline:this.stats.sgInfo.get()})},(i,r,l)=>this.wireEl(yt,{ctitle:()=>i.title,ctext:()=>i.body},l),i=>i.id),this.$each(this.ref(t,"lines"),()=>e()?.insights??[],(i,r,l)=>this.wireEl(Df,{text:()=>Gf(i)},l),i=>i.id),t}eligibleRoundId(){const e=this.round.round.get();return e===null?null:sa({signedInPlayerId:this.auth.currentUser.get()?.id??null,statConfigPlayerIds:new Set(this.round.statModules.get().keys()),statRows:this.round.statRows.get(),holesUnscored:na({playerId:this.auth.currentUser.get()?.id??"",balls:this.round.balls.get(),groups:this.round.groups(),strokesFor:(n,i)=>this.round.strokesFor(n,i)})}).reason==="eligible"?e.id:null}shows(){const e=this.eligibleRoundId();return e===null?!1:this.stats.phase.get()==="ready"&&this.stats.roundId.get()===e}hint(){const e=this.stats.model.get();if(e===null)return"";if(e.deltas===null)return te.waterfallHint;let t=null;for(const n of he){const i=ln(e.deltas,n);i!==null&&(t===null||Math.abs(i)>Math.abs(t))&&(t=i)}return t===null?te.waterfallHint:ca(t,e.windowCount)}}function $i(s){return!(!s.pageVisible||s.status==="complete")}function Uf(s,e){return e&&!s}const Kf=2,Si=3,Wf=75e3;function Yf(s,e=null){const t=new URLSearchParams({token:s});return e!==null&&t.set("since",e),`${W}/friendly-rounds/events?${t.toString()}`}function Xf(s){if(typeof s!="object"||s===null)return!1;const e=s;return e.latestEventId!==null&&typeof e.latestEventId!="string"?!1:e.status==="not_started"||e.status==="active"||e.status==="complete"}function Qf(s){const e=s.eventSourceFactory??(B=>new EventSource(B)),t=s.setTimer??((B,U)=>setTimeout(B,U)),n=s.clearTimer??(B=>clearTimeout(B)),i=s.livenessTimeoutMs??Wf,r=s.isPageVisible??(()=>typeof document>"u"||!document.hidden);let l=!1,d=0,c=null,u=null,h=s.since??null;const f=()=>{u!==null&&(n(u),u=null)},m=()=>{f(),u=t(k,i)},_=()=>{c!==null&&(c.onopen=null,c.onmessage=null,c.onerror=null,c.close(),c=null)},g=()=>{l=!0,f(),_()},v=()=>{if(_(),++d>=Si){g(),s.onDegrade();return}N()};function k(){if(!l){if(!r()){m();return}v()}}function N(){if(l)return;let B;try{B=e(Yf(s.token,h))}catch{g(),s.onDegrade();return}c=B,B.onopen=()=>{d=0},B.onmessage=U=>{if(l||c!==B)return;m();let O;try{O=JSON.parse(U.data)}catch{return}Xf(O)&&(O.latestEventId!==null&&(h=O.latestEventId),s.onEvent({latestEventId:O.latestEventId,status:O.status}))},B.onerror=()=>{l||c!==B||(B.readyState===Kf||++d>=Si)&&(g(),s.onDegrade())},m()}return N(),{stop:()=>{l||g()}}}const Jf=2e4;function Zf(s){if(!(s===null||s===""))return/^\d+$/.test(s)?Number(s):s}const em=b(`
...
`);class Km extends M{static styles=`
    ...
    `;svc=this.inject(ts);auth=this.inject(V);router=this.inject(q);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(C(()=>{const c=e.get();se(()=>{!c||!n()||(this.svc.setPlayer(c),this.svc.loadCourses())})}));const i=()=>this.svc.unavailable.get(),r=()=>i()===null?this.svc.courses.get():[],l=()=>(t.get()??"").trim(),d=this.wire(Vm,{back:{onclick:()=>this.router.navigate("/friend",{query:{id:e.get()??"",name:l()}})},subtitle:{textContent:()=>{const c=l();return c?`Where ${c} has played the rounds they share.`:"Where the rounds they share were played."}},refusal:{className:()=>i()!==null?"fcourses__refusal":"fcourses__refusal hidden"},refusalTitle:{textContent:()=>{const c=i();return c?Ke[c].title:""}},refusalMsg:{textContent:()=>{const c=i();return c?Ke[c].message:""}},anon:{className:()=>n()?"fcourses__anon hidden":"fcourses__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/friend-courses?id=${e.get()??""}&name=${encodeURIComponent(l())}`}})},state:{textContent:()=>i()!==null||!n()?"":this.svc.coursesLoading.get()?"Loading…":this.svc.coursesError.get()??""},retry:{className:()=>i()===null&&this.svc.coursesError.get()!==null&&!this.svc.coursesLoading.get()?"fcourses__retry":"fcourses__retry hidden",onclick:()=>{this.svc.loadCourses(!0)}},empty:{className:()=>i()===null&&this.svc.coursesLoaded.get()&&r().length===0&&this.svc.coursesError.get()===null?"fcourses__state":"fcourses__state hidden"},listCard:{className:()=>r().length>0?"fcourses__listcard":"fcourses__listcard hidden"},truncated:{className:()=>i()===null&&this.svc.coursesHasMore.get()&&r().length>0?"fcourses__state":"fcourses__state hidden"}});return this.$each(this.ref(d,"list"),r,(c,u,h)=>this.wireEl(Um,{name:()=>c.courseName??"Course",facts:()=>Ml(c,Fe)},h),c=>c.courseId),d}}function Wm(s,e,t){const n=ft(e);if(n){const d=Bi(s);return d?`Watching · ${d} ${n}`:`Watching · ${n}`}const i=Bi(s)??"this",r=ft(t),l=r?` at ${r}`:"";return`Watching · ${i} round${l}`}function Ym(s,e,t,n){const i=ft(s)===null?null:ft(e),r=n!==null?`${n} holes`:null;return[i,r,t==="complete"?"Finished":t==="not_started"?"Not started":null].filter(c=>c!==null).join(" · ")||null}const Xm="You're watching this round. Only its players can enter scores.",Ai={forbidden:{title:"Round not available",message:"This round is no longer shared with you."},not_found:{title:"Round not found",message:"This round doesn't exist anymore."}};function Bi(s){const e=ft(s);return e?e.toLowerCase().endsWith("s")?`${e}'`:`${e}'s`:null}function ft(s){return(s??"").trim()||null}const Qm=b(`
    <div class="spectate">
...
`),Ng=b('<li bind="text" class="roundstats__legenditem"></li>');function Og(){const s=[];for(const[e,t]of Object.entries(gn))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
...
${ds}
    `;svc=this.inject(pt);auth=this.inject(V);router=this.inject(q);colors=hs;openHole=new p(null);infoOpen=new p(!1);render(){const e=this.router.query("id"),t=()=>this.svc.model.get(),n=()=>this.svc.phase.get()==="ready"&&t()!==null;this.track(C(()=>{const d=e.get();se(()=>{this.openHole.set(null),d&&this.svc.load(d)})}));const i=this.router.query("finish"),r=()=>i.get()==="1",l=this.wire(Cg,{back:{className:()=>r()?"roundstats__back hidden":"roundstats__back",onclick:()=>this.router.navigate("/stats")},finishKicker:{className:()=>r()?"roundstats__kicker":"roundstats__kicker hidden"},finishClose:{className:()=>r()?"roundstats__closebtn":"roundstats__closebtn hidden",onclick:()=>this.router.navigate("/")},state:()=>this.stateLine(),body:{className:()=>n()?"roundstats__body":"roundstats__body hidden"},title:()=>t()===null?"":di(t()),subtitle:()=>this.subtitle(),score:()=>{const d=t();return d===null?"":da(d.strokes,d.vsPar)??""},stripSec:{className:()=>(t()?.cells.length??0)>0?"roundstats__section":"roundstats__section hidden"},wfHeading:()=>te.waterfallHeading,wfHint:()=>te.waterfallHint,infoTrigger:{textContent:()=>$.prioritiesInfo,onclick:()=>this.infoOpen.set(!0)},infoSheet:{className:()=>this.infoOpen.get()?"stats-info":"stats-info hidden",onclick:d=>{d.target===d.currentTarget&&this.infoOpen.set(!1)}},infoTitle:()=>ye.title,infoDone:{onclick:()=>this.infoOpen.set(!1)},legendHeading:()=>te.legendHeading,detail:{className:()=>this.selected()===null?"holedetail hidden":"holedetail"},detailTitle:()=>{const d=this.selected();return d===null?"":Of(d)},detailEmpty:()=>{const d=this.selected();return d===null?"":vn(d).length===0?te.nothingRecordedOnHole:""}});return this.setHeading(l,"stripSec",te.holeStripHeading),this.$each(this.ref(l,"strip"),()=>t()?.cells??[],(d,c,u)=>this.wireEl(Ig,{cell:{className:()=>{const h=this.cellNow(d.id)??d;return h.marker===null?"cell":`cell cell--${h.marker}`},"aria-pressed":()=>String(this.openHole.get()===d.id),"aria-label":()=>wi(this.cellNow(d.id)??d),title:()=>wi(this.cellNow(d.id)??d),onclick:()=>this.openHole.set(this.openHole.get()===d.id?null:d.id)},hole:()=>String((this.cellNow(d.id)??d).holeNumber),score:()=>Hf(this.cellNow(d.id)??d),tee:{className:()=>(this.cellNow(d.id)??d).tee===null?"cell__tee cell__tee--absent":"cell__tee",style:()=>{const h=this.cellNow(d.id)??d;return h.tee===null?"":`background:${this.teeColor(h.tee)}`}},gir:{className:()=>{const h=this.cellNow(d.id)??d;return h.gir===null?"cell__gir cell__gir--absent":h.gir?"cell__gir cell__gir--hit":"cell__gir"}},putts:()=>{const h=this.cellNow(d.id)??d;return h.putts===null?"":String(h.putts)},pen:{className:()=>Hp(this.cellNow(d.id)??d)?"cell__pen":"cell__pen cell__pen--absent"}},u),d=>d.id),this.$each(this.ref(l,"detailLines"),()=>Ff(this.selected()),(d,c,u)=>this.wireEl(Eg,{label:()=>d.label,value:()=>d.value},u),d=>d.key),this.$each(this.ref(l,"deltas"),()=>{const d=t();return d===null?[]:he.filter(c=>Ye(d.waterfall,c)!==null)},(d,c,u)=>this.wireEl(Rg,{title:()=>as(d),sentence:()=>{const h=t();if(h===null||h.deltas===null)return"";const f=ln(h.deltas,d);return f===null?"":ca(f,h.windowCount)},value:{textContent:()=>{const h=this.componentValue(d);return h===null?"":xe(h)},style:()=>{const h=this.componentValue(d);return h===null?"":`color:${Qe(cs(h),this.colors)}`}},bar:{innerHTML:()=>{const h=t(),f=this.componentValue(d);return h===null||f===null?"":aa(f,Ar([h.waterfall]),this.colors)}}},u),d=>d),this.$each(this.ref(l,"infoCards"),()=>{const d=t();return d===null?[]:Yt({attributed:d.waterfall.coverage.attributed,holesScored:d.waterfall.coverage.holesScored,windowRounds:0,rowsPer18:he.map(c=>Pe(d.waterfall,c)),penaltySource:Wt(d.panels.totals),baseline:this.svc.sgInfo.get()})},(d,c,u)=>this.wireEl(yt,{ctitle:()=>d.title,ctext:()=>d.body},u),d=>d.id),this.spawn(ya,this.ref(l,"panels"),{model:()=>t()?.panels??Mr,baseline:()=>this.svc.sgInfo.get()}),this.$each(this.ref(l,"legend"),()=>[te.legendTee,te.legendGir,te.legendPutts,te.legendPenalty,te.legendAbsence],(d,c,u)=>this.wireEl(Ng,{text:()=>d},u),d=>d),l}cellNow(e){return this.svc.model.get()?.cells.find(t=>t.id===e)}selected(){const e=this.openHole.get();return e===null?null:this.cellNow(e)??null}teeColor(e){switch(e){case"fairway":return this.colors.gain;case"in_play":return this.colors.neutral;case"trouble":return this.colors.loss}}componentValue(e){const t=this.svc.model.get();return t===null?null:Ye(t.waterfall,e)}subtitle(){const e=this.svc.model.get();return e===null?"":Nf({...e,title:di(e)})}stateLine(){if(this.auth.currentUser.get()===null)return te.notSignedIn;switch(this.svc.phase.get()){case"loading":case"idle":return te.loading;case"notFound":return te.noStatsInRound;case"notAuthorized":return te.notSignedIn;case"failed":return`${te.failedPrefix}${this.svc.failure.get()??""}`;case"ready":return this.svc.model.get()?.cells.length===0?te.noHoleStrip:""}}setHeading(e,t,n){const i=this.ref(e,t).querySelector("h2");i&&(i.textContent=n)}}const Mg=b(`
    <div class="admin">
...
`),zg={not_started:"Not started",active:"Playing",complete:"Done"},Fg={private:"Private",friends:"Friends",link:"Link"};function Gg(s){const e=[`${s.participants.length} players`,`${s.scoreEventCount} scores`];return s.lastEventAt?e.push(`last ${s.lastEventAt.replace("T"," ").slice(0,16)}`):e.push("never played"),e.join(" · ")}function jg(s){const e=[`@${s.username}`,`${s.roundCount} rounds`];return s.lastRoundDate&&e.push(`last ${s.lastRoundDate}`),s.handicapIndex!==null&&e.push(`hcp ${s.handicapIndex}`),s.deletedAt&&e.push("DELETED"),e.join(" · ")}class Dg extends M{static styles=`
        .admin {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Wg extends M{static styles=`
        .comps {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),db=b('<option bind="option"></option>');class cb extends M{competitions=this.inject(je);state=this.inject(_t);router=this.inject(q);render(){const e=new T(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(ob,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Vg(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:n=>{n.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:n=>this.state.roundCourseDraft.set(n.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:n=>this.state.roundDateDraft.set(n.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(n,i,r)=>this.wireEl(db,{option:{value:()=>n.id,textContent:()=>n.name}},r),n=>n.id),this.$each(this.ref(t,"rounds"),e,(n,i,r)=>this.wireEl(lb,{row:{disabled:()=>!n.shareToken,onclick:()=>{n.shareToken&&this.router.navigate("/round",{query:{token:n.shareToken}})}},number:()=>`Round ${n.roundNumber}`,meta:()=>[n.courseNameSnapshot,n.date].filter(Boolean).join(" · ")||(n.shareToken?"Open":"View-only"),status:{textContent:()=>ab[n.status]??n.status,className:()=>`cd__rstatus s-${n.status}`}},r),n=>JSON.stringify({id:n.id,status:n.status,shareToken:n.shareToken,courseName:n.courseNameSnapshot,date:n.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function ub(s,e,t){return JSON.stringify({entry:s,points:e,columns:t})}function hb(s){return s.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const pb=b(`
    <div>
...
`),fb=b('<button bind="button" type="button"></button>'),mb=b('<th bind="cell"></th>'),gb=b('<tr bind="row"></tr>'),bb=b('<td bind="cell"><span bind="value"></span></td>'),yb=b(`
...
`);class xb extends M{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class $b extends M{static styles=`
        .app-shell {
            display: grid;
    ...
    `;router=this.inject(q);render(){const e=this.wire(kb,{header:{className:()=>en(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(Ku,this.ref(e,"account")),this.spawn(Xo,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Zn,"/history":ch,"/round":nm,"/create":Nm,"/login":Am,"/friends":Fm,"/friend":jm,"/friend-rounds":qm,"/friend-courses":Km,"/spectate":Jm,"/profile":ng,"/stats":Sg,"/round-stats":Hg,"/admin":Dg,...sr.competitions?{"/competitions":Wg,"/competition":xb}:{}},Zn),e}}class Sb extends V{constructor(e){super(),this.client=e}client;async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await this.client.login(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await this.client.me()),this.error.set(null)}catch(e){e instanceof Q&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await this.client.logout(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof Q&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await this.client.logoutAll();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof Q&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}Y.get(Ga);const zi=Y.get(q);Y.set(V,new Sb(fa));const Fi=Y.get(V);await Va($b,"#app",{hot:void 0,onInit:async()=>{await Fi.load(),Fi.currentUser.get()&&zi.route.get()==="/login"&&zi.navigate("/",!0)}});export{As as A,M as C,q as R,p as S,Ga as T,y as a,dt as b,T as c,Na as d,C as e,Ra as n,F as r,b as t};
