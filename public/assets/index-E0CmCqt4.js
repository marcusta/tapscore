...
`);class Ha extends C{static styles=`
        .dock {
    ...
    `;let ee=Hs;async function xd(s,e={}){e.everywhere?await s.auth.logoutEverywhere():await s.auth.logout(),s.profile.clear(),s.friends.clear(),s.activity.clear(),s.friendProfile.clear(),s.spectate.clear(),s.admins.clear(),s.homeStats.clear(),s.landing.clear(),s.navigate("/")}const $d="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";function Pe(s="avatar"){return`<span class="${s}">
            <img bind="avatarPhoto" class="avatar__photo" alt="" />
            <span bind="avatarInitials" class="avatar__initials"></span>
        </span>`}function Se(s){const e=()=>Ka(s());return{avatarPhoto:{src:()=>e()??$d,className:()=>e()?"avatar__photo":"avatar__photo hidden"},avatarInitials:{textContent:()=>{const t=s();return Wa(t.displayName,t.username)},className:()=>e()?"avatar__initials hidden":"avatar__initials"}}}function He(s,e="0.85rem"){return`
        position: relative;
...
`,Vi={not_started:"Not started",active:"Live",complete:"Finished"};class cn extends C{static styles=`
        .landing {
            ...
            & .recent-row {
                ...
                & .recent-row__who { font-weight: 600; font-size: 0.9rem; color: ${o("text")}; }
                & .recent-row__what {
                ...
                & .recent-row__when {
                    flex-shrink: 0; color: ${o("text-muted")}; font-size: 0.8rem;
                }
            }
...
`),rc=b('<button bind="btn" class="se-seg" type="button"></button>'),ac=b(`
    <div class="se-stats__group">
        <span bind="glabel" class="se-stats__group-label"></span>
...
`),oc=b('<div bind="rule" class="se-stats__rule"></div>');class lc extends C{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
        }
        ...
        .se-row {
            ...
            & .se-row__hcpinfo {
                ...
                border: 1px solid ${o("border")}; border-radius: 999px;
                color: ${o("text-muted")};
            ...
            & .se-row__slot { width: ${ye}px; display: flex; align-items: center; justify-content: center; }
            & .se-row__prev {
...
</div>`}const zc={ranked:Rc,match_summary:(s,e)=>Oc(s,e)},Lc={"default-score-grid":Ic,"compact-match-grid":Pc,"category-matrix-grid":Cc};function Ac(s){return`<div class="lb-diag">Unrenderable result section <code>${A(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Mc(s){return`<div class="lb-diag">Unsupported score-grid component <code>${A(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Hc(s,e,t,n=null){const i=zc[s.kind];return i?i(s,e,t,n):Ac(s.kind)}function Qi(s,e,t,n){const i=Ki(s),r=Lc[i];return r?r(s,e,t,n):Mc(i)}function Xi(s,e,t=Yi,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${A(s.formatLabel)}.</div>`:s.leaderboard.map(r=>Hc(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${A(s.formatLabel)}.</div>`}function Ji(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>Qi(r,e,t,{mode:i})).join(`
`)}const Fc=b(`
...
`);class ou extends C{static styles=`
        .hcp-checkin {
            margin-bottom: ${a("lg")};
            padding: ${a("md")} ${a("lg")};
            ${P()}
    ...
    `;svc=this.inject(ae);auth=this.inject(j);profile=this.inject(Fe);settled=new p(!1);editing=new p(!1);text=new p("");busy=new p(!1);error=new p("");state(){const e=this.profile.player.get();return ru({playerId:this.auth.currentUser.get()?.id??null,balls:this.svc.balls.get(),firstOpen:this.svc.firstOpen.get(),handicapConfirmedAt:e?.handicapConfirmedAt??null,handicapIndex:e?.handicapIndex??null,profileLoaded:e!==null,settled:this.settled.get(),now:Date.now()})}ensureProfileLoaded(){this.settled.get()||this.svc.firstOpen.get()&&this.auth.currentUser.get()&&this.svc.balls.get().length&&this.profile.load()}async confirm(){if(this.busy.get())return;this.error.set(""),this.busy.set(!0);const e=await this.profile.confirmHandicap();this.busy.set(!1),e?this.settled.set(!0):this.error.set("Could not save that right now. Try again.")}startEdit(){const e=this.profile.player.get()?.handicapIndex??null;this.text.set(e===null?"":St(e)),this.error.set(""),this.editing.set(!0)}async save(){if(this.busy.get())return;const e=de(this.text.get());if(e===null){this.error.set("Enter a handicap index, e.g. 18.4 or +2.4.");return}this.error.set(""),this.busy.set(!0);const t=await this.profile.saveIndex(e);this.busy.set(!1),t?(this.editing.set(!1),this.settled.set(!0)):this.error.set("Could not save that right now. Try again.")}render(){return this.track(I(()=>this.ensureProfileLoaded())),this.wire(au,{root:{className:()=>this.state().visible?"hcp-checkin":"hcp-checkin hidden"},ask:{className:()=>this.editing.get()?"hcp-checkin__ask hidden":"hcp-checkin__ask"},question:{textContent:()=>{const e=this.state().index;return e===null?"No handicap set — add one?":`Handicap ${St(e)} — still right?`}},confirm:{textContent:()=>this.state().index===null?"Not now":"Yes",disabled:()=>this.busy.get(),onclick:()=>{this.confirm()}},edit:{textContent:()=>this.state().index===null?"Add":"Update",disabled:()=>this.busy.get(),onclick:()=>this.startEdit()},editor:{className:()=>this.editing.get()?"hcp-checkin__editor":"hcp-checkin__editor hidden"},field:{value:()=>this.text.get(),oninput:e=>this.text.set(e.target.value)},save:{disabled:()=>this.busy.get(),onclick:()=>{this.save()}},cancel:{disabled:()=>this.busy.get(),onclick:()=>this.editing.set(!1)},err:{textContent:()=>this.error.get()}})}}function lu(s,e){if(!e)return!1;for(const t of s)for(const n of t.players)if(n.playerId===e)return!0;return!1}const du=b(`
...
`);class Bh extends C{static styles=`
        .setup {
            ...
            & .setup__friends {
                margin-top: ${a("sm")}; padding: ${a("sm")}; ${P()}
                &.hidden { display: none; }
...
`);function dr(s,e){return{row:{onclick:()=>e(s.roundId)},title:()=>yo(s),subtitle:()=>vo(s,ge),progress:()=>wo(s)}}function cr(){return`
        & .fr-row {
            display: flex; align-items: center; gap: ${a("md")};
            width: 100%;
            padding: ${a("md")} ${a("lg")};
...
`);class Sf extends C{static styles=`
        .cd {
            ...
            & .cd__official-banner {
                ${P()} padding: ${a("sm")} ${a("lg")}; margin-bottom: ${a("sm")};
                background: ${o("accent-soft")}; color: ${o("accent")};
