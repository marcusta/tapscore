...
</div>`}function In(r,e){const t=r.matches.map(s=>{const n=S(s.sideA.ballIds.map(e).join(" & ")),i=S(s.sideB.ballIds.map(e).join(" & ")),l=s.magnitude===0?"AS":`${s.magnitude} UP`,d=s.finished?"Final":`thru ${s.thru}`,u=s.leader==="a"?" lb-mp__team--lead":"",c=s.leader==="b"?" lb-mp__team--lead":"";return`<div class="lb-mp">
...
</div>`}const Tn={ranked:Sn,match_summary:(r,e)=>In(r,e)},Cn={"default-score-grid":yn,"compact-match-grid":vn,"category-matrix-grid":wn};function En(r){return r.componentId??"default-score-grid"}function Nn(r){return`<div class="lb-diag">Unrenderable result section <code>${S(r)}</code> — no generic view yet. Results are not hidden.</div>`}function Pn(r){return`<div class="lb-diag">Unsupported score-grid component <code>${S(r)}</code> — no generic view yet. Results are not hidden.</div>`}function zn(r,e,t){const s=Tn[r.kind];return s?s(r,e,t):Nn(r.kind)}function Rn(r,e,t,s){const n=En(r),i=Cn[n];return i?i(r,e,t,s):Pn(n)}function jn(r,e,t=St){return r.leaderboard.length===0&&r.cards.length===0?`<div class="lb-empty">No scores entered yet for ${S(r.formatLabel)}.</div>`:r.leaderboard.map(n=>zn(n,e,t)).join("")||`<div class="lb-empty">No leaderboard metric for ${S(r.formatLabel)}.</div>`}function On(r,e,t,s={}){if(r.cards.length===0)return"";const n=s.mode??"product";return r.cards.map(i=>Rn(i,e,t,{mode:n})).join(`
`)}const Ln=b(`
    <div bind="root" class="lb">
        <div bind="status" class="lb__status hidden"></div>
        <div bind="body" class="lb__body"></div>
    </div>
`);class Dn extends I{static styles=`
        .lb {
            ...
            & .lb-rank__pace--over { color: ${o("under-par")}; }
            & .lb-rank__pace--under { color: ${o("over-par")}; }
    ...
    `;svc=this.inject(Z);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(Ln,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=u=>{const c=this.svc.nameOf(u);return this.svc.isPending(u)?`${c} (open seat)`:c},i=jn(t,s,u=>this.svc.groupLabelOf(u)),l=On(t,e.routeSections,s),d=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return i+d}}function Mn(r,e){if(!e)return[];const t=[],s=new Set;for(const n of r)for(const i of n.players){if(i.playerId===e)return[];i.guestPlayerId===null||s.has(i.guestPlayerId)||(s.add(i.guestPlayerId),t.push({guestPlayerId:i.guestPlayerId,displayName:i.displayName}))}return t}const Hn=b(`
...
`);class Fn extends I{static styles=`
    ...
    `;let J=De;function Bn(r){if(!r)return{visible:!1,selfAllowed:!1,guestAllowed:!1,blockedMessage:null};const e=r.seats.length>0,t=r.claimedSeats.some(i=>i.viewerMayRelease),s=r.viewer.claimSeat.allowed,n=r.viewer.claimSeatAsGuest.allowed;return{visible:e||t,selfAllowed:e&&s,guestAllowed:e&&n,blockedMessage:e&&!s&&!n?r.viewer.claimSeat.message??r.viewer.claimSeatAsGuest.message??"Claiming seats is not available on this round.":null}}function Gn(r,e){const t=[];if(r.groupId!==null&&e.length>0){const s=e.findIndex(n=>n.id===r.groupId);if(s>=0){t.push(`Group ${s+1}`);const n=e[s].startTime;n.includes(":")&&t.push(n)}}return r.category!==null&&t.push(r.category),t.join(" · ")}function qn(r){return(r?.claimedSeats??[]).filter(e=>e.viewerMayRelease)}const Kn=b(`
...
`);class Zn extends I{static styles=`
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");joining=new h(!1);error=new h("");diagnostics=new h([]);teeId=new h("");tees=new h([]);loadedForCourseId=null;groupChoice=new h("");policyState(){return Yn(this.svc.startList.get())}eligible(){return this.policyState().visible&&Qn(this.svc.balls.get(),this.auth.currentUser.get()?.id??null,this.svc.round.get()?.status??null)}ensureTeesLoaded(){if(!this.eligible())return;const e=this.svc.round.get()?.courseId;!e||e===this.loadedForCourseId||(this.loadedForCourseId=e,_.setup.teesByCourse({courseId:e}).then(t=>{this.tees.set(t),!this.teeId.get()&&t[0]&&this.teeId.set(t[0].id)}).catch(()=>{this.loadedForCourseId=null}))}needsProfileUpdate(){return this.diagnostics.get().some(e=>e.code==="missing_gender"||e.code==="missing_handicap_index")}async join(){const e=this.tokenQ.get(),t=this.teeId.get();if(!(!e||!t||this.joining.get())){this.error.set(""),this.diagnostics.set([]),this.joining.set(!0);try{const s=this.groupChoice.get(),n=await _.friendlyRounds.join({token:e,teeId:t,...s?{groupChoice:s}:{}});n.ok?await this.svc.loadByToken(e):this.diagnostics.set(n.diagnostics)}catch(s){this.error.set(s instanceof A&&s.status===409?s.message??"You already play in this round, or it has already started.":"Could not join right now. Try again.")}finally{this.joining.set(!1)}}}render(){this.track(k(()=>this.ensureTeesLoaded()));const e=new y(()=>Xn(this.svc.groups(),this.svc.startList.get()?.viewer.createGroup.allowed??!0));this.track(k(()=>{const i=e.get(),l=this.groupChoice.get();(!l||!i.options.some(d=>d.value===l&&!d.disabled))&&this.groupChoice.set(i.defaultValue)}));const t=this.wire(Jn,{root:{className:()=>this.eligible()?"join-card":"join-card hidden"},blocked:{className:()=>this.policyState().blockedMessage!==null?"join-card__blocked":"join-card__blocked hidden",textContent:()=>this.policyState().blockedMessage??""},groupRow:{className:()=>this.svc.groups().length>0&&this.policyState().blockedMessage===null?"join-card__group":"join-card__group hidden"},row:{className:()=>this.policyState().blockedMessage===null?"join-card__row":"join-card__row hidden"},join:{disabled:()=>this.joining.get()||!this.teeId.get(),onclick:()=>{this.join()}},diag:{className:()=>this.diagnostics.get().length>0?"join-card__diag":"join-card__diag hidden"},diagText:{textContent:()=>this.diagnostics.get().map(i=>i.message).join(" · ")},profileLink:{className:()=>this.needsProfileUpdate()?"join-card__profile-link":"join-card__profile-link hidden",onclick:()=>this.router.navigate("/profile")},err:{textContent:()=>this.error.get()}}),s=new J({value:this.teeId,options:{get:()=>this.tees.get().map(i=>({value:i.id,label:i.name}))},placeholder:"Tee"});s.mount(this.ref(t,"teeHost")),this.track(()=>s.destroy());const n=new J({value:this.groupChoice,options:{get:()=>e.get().options},placeholder:"Group"});return n.mount(this.ref(t,"groupHost")),this.track(()=>n.destroy()),t}}const ei=b(`
...
`);class ti extends I{static styles=`
    ...
    `;router=this.inject(N);tokenQ=this.router.query("token");editable=new h(!1);render(){const e=this.tokenQ.get();return e&&_.friendlyRounds.setup({token:e}).then(t=>this.editable.set(t.editable===!0)).catch(()=>this.editable.set(!1)),this.wire(ei,{root:{className:()=>this.editable.get()?"edit-card":"edit-card hidden"},edit:{onclick:()=>{const t=this.tokenQ.get();t&&this.router.navigate("/create",{query:{token:t}})}}})}}function si(r,e){if(!e)return!1;for(const t of r)for(const s of t.players)if(s.playerId===e)return!0;return!1}const ni=b(`
...
`);class ii extends I{static styles=`
    ...
    `;svc=this.inject(Z);auth=this.inject(L);router=this.inject(N);tokenQ=this.router.query("token");open=new h(!1);leaving=new h(!1);error=new h("");diagnostics=new h([]);eligible(){return si(this.svc.balls.get(),this.auth.currentUser.get()?.id??null)}async leave(){const e=this.tokenQ.get();if(!(!e||this.leaving.get())){this.error.set(""),this.diagnostics.set([]),this.leaving.set(!0);try{const t=await _.friendlyRounds.leave({token:e});t.ok?await this.svc.loadByToken(e):this.diagnostics.set(t.diagnostics)}catch{this.error.set("Could not remove you right now. Try again.")}finally{this.leaving.set(!1)}}}render(){const e=this.wire(ni,{root:{className:()=>this.eligible()?"leave-card":"leave-card hidden"},leaveBtn:{onclick:()=>this.open.set(!0),disabled:()=>this.leaving.get()},diag:{textContent:()=>this.diagnostics.get().map(t=>t.message).join(" · ")},err:{textContent:()=>this.error.get()}});return this.spawn(B,this.ref(e,"confirmHost"),{open:this.open,title:"Remove yourself from this round?",message:"Your scores here will be deleted. Everyone else's stay, and the round keeps going without you.",confirmLabel:"Remove me",cancelLabel:"Cancel",danger:!0,onconfirm:()=>{this.leave()}}),e}}function ri(r){return!(r.tab!=="leaderboard"||!r.pageVisible||r.status==="complete")}const oi=2e4;function ai(r){if(!(r===null||r===""))return/^\d+$/.test(r)?Number(r):r}const li=b(`
...
`),di=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),ci=b('<button bind="pill" class="round-view__grp" type="button"></button>');class ui extends I{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);function lt(r){return r.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e[0].toUpperCase()).join("")}class Vi extends I{static styles=`
        .friends {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),gr=b('<option bind="option"></option>');class fr extends I{competitions=this.inject(ee);state=this.inject(de);router=this.inject(N);render(){const e=new y(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(pr,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Xi(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:s=>{s.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:s=>this.state.roundCourseDraft.set(s.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:s=>this.state.roundDateDraft.set(s.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(s,n,i)=>this.wireEl(gr,{option:{value:()=>s.id,textContent:()=>s.name}},i),s=>s.id),this.$each(this.ref(t,"rounds"),e,(s,n,i)=>this.wireEl(mr,{row:{disabled:()=>!s.shareToken,onclick:()=>{s.shareToken&&this.router.navigate("/round",{query:{token:s.shareToken}})}},number:()=>`Round ${s.roundNumber}`,meta:()=>[s.courseNameSnapshot,s.date].filter(Boolean).join(" · ")||(s.shareToken?"Open":"View-only"),status:{textContent:()=>hr[s.status]??s.status,className:()=>`cd__rstatus s-${s.status}`}},i),s=>JSON.stringify({id:s.id,status:s.status,shareToken:s.shareToken,courseName:s.courseNameSnapshot,date:s.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function br(r,e,t){return JSON.stringify({entry:r,points:e,columns:t})}function _r(r){return r.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const yr=b(`
    <div>
...
`),vr=b('<button bind="button" type="button"></button>'),wr=b('<th bind="cell"></th>'),xr=b('<tr bind="row"></tr>'),$r=b('<td bind="cell"><span bind="value"></span></td>'),kr=b(`
...
`);class Cr extends I{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class Nr extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Er,{});return this.spawn(js,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Qe,"/history":Ws,"/round":ui,"/create":Ai,"/login":Bi,"/friends":Vi,"/profile":Qi,...vt.competitions?{"/competitions":er,"/competition":Cr}:{}},Qe),e}}P.get(Qt);const dt=P.get(N),ct=P.get(L);await Xt(Nr,"#app",{hot:void 0,onInit:async()=>{await ct.load(),ct.currentUser.get()&&dt.route.get()==="/login"&&dt.navigate("/",!0)}});export{I as C,N as R,h as S,Qt as T,g as a,ae as b,y as c,k as e,C as r,b as t};
