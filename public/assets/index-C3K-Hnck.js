...
`);class Dh extends N{static styles=`
        .friends {
...
`);class Uh extends N{static styles=`
        .frounds {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class Yh extends N{static styles=`
    ...
    `;svc=this.inject(Ct);auth=this.inject(D);router=this.inject(B);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(I(()=>{const c=e.get();K(()=>{!c||!n()||(this.svc.setPlayer(c),this.svc.loadCourses())})}));const i=()=>this.svc.unavailable.get(),r=()=>i()===null?this.svc.courses.get():[],l=()=>(t.get()??"").trim(),d=this.wire(Kh,{back:{onclick:()=>this.router.navigate("/friend",{query:{id:e.get()??"",name:l()}})},subtitle:{textContent:()=>{const c=l();return c?`Where ${c} has played the rounds they share.`:"Where the rounds they share were played."}},refusal:{className:()=>i()!==null?"fcourses__refusal":"fcourses__refusal hidden"},refusalTitle:{textContent:()=>{const c=i();return c?ze[c].title:""}},refusalMsg:{textContent:()=>{const c=i();return c?ze[c].message:""}},anon:{className:()=>n()?"fcourses__anon hidden":"fcourses__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/friend-courses?id=${e.get()??""}&name=${encodeURIComponent(l())}`}})},state:{textContent:()=>i()!==null||!n()?"":this.svc.coursesLoading.get()?"Loading…":this.svc.coursesError.get()??""},retry:{className:()=>i()===null&&this.svc.coursesError.get()!==null&&!this.svc.coursesLoading.get()?"fcourses__retry":"fcourses__retry hidden",onclick:()=>{this.svc.loadCourses(!0)}},empty:{className:()=>i()===null&&this.svc.coursesLoaded.get()&&r().length===0&&this.svc.coursesError.get()===null?"fcourses__state":"fcourses__state hidden"},listCard:{className:()=>r().length>0?"fcourses__listcard":"fcourses__listcard hidden"},truncated:{className:()=>i()===null&&this.svc.coursesHasMore.get()&&r().length>0?"fcourses__state":"fcourses__state hidden"}});return this.$each(this.ref(d,"list"),r,(c,u,f)=>this.wireEl(Wh,{name:()=>c.courseName??"Course",facts:()=>_o(c,ge)},f),c=>c.courseId),d}}function Qh(s,e,t){const n=it(e);if(n){const d=jn(s);return d?`Watching · ${d} ${n}`:`Watching · ${n}`}const i=jn(s)??"this",r=it(t),l=r?` at ${r}`:"";return`Watching · ${i} round${l}`}function Xh(s,e,t,n){const i=it(s)===null?null:it(e),r=n!==null?`${n} holes`:null;return[i,r,t==="complete"?"Finished":t==="not_started"?"Not started":null].filter(c=>c!==null).join(" · ")||null}const Jh="You're watching this round. Only its players can enter scores.",Bn={forbidden:{title:"Round not available",message:"This round is no longer shared with you."},not_found:{title:"Round not found",message:"This round doesn't exist anymore."}};function jn(s){const e=it(s);return e?e.toLowerCase().endsWith("s")?`${e}'`:`${e}'s`:null}function it(s){return(s??"").trim()||null}const Zh=b(`
    <div class="spectate">
        ...
        <div bind="body" class="spectate__body">
            ...
            <p class="spectate__note">${Jh}</p>
        </div>
    </div>
`);class ep extends N{static styles=`
        .spectate {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
        ...
        ${Xi.styles}
    `;svc=this.inject(vi);auth=this.inject(D);router=this.inject(B);render(){const e=this.router.query("id"),t=this.router.query("name"),n=()=>this.auth.currentUser.get()!==null;this.track(I(()=>{const c=e.get();K(()=>{!c||!n()||(this.svc.setRound(c),this.svc.load())})}));const i=()=>this.svc.view.get(),r=()=>this.svc.unavailable.get(),l=()=>(t.get()??"").trim()||null;return this.wire(Zh,{back:{onclick:()=>{window.history.length>1?window.history.back():this.router.navigate("/")}},anon:{className:()=>n()?"spectate__anon hidden":"spectate__anon"},toLogin:{onclick:()=>this.router.navigate("/login",{query:{next:`/spectate?id=${e.get()??""}&name=${encodeURIComponent(l()??"")}`}})},refusal:{className:()=>n()&&r()!==null?"spectate__refusal":"spectate__refusal hidden"},refusalTitle:{textContent:()=>{const c=r();return c?Bn[c].title:""}},refusalMsg:{textContent:()=>{const c=r();return c?Bn[c].message:""}},state:{textContent:()=>r()!==null||!n()?"":this.svc.loading.get()&&i()===null?"Loading…":i()===null?this.svc.error.get()??"":""},retry:{className:()=>n()&&r()===null&&this.svc.error.get()!==null&&i()===null&&!this.svc.loading.get()?"spectate__retry":"spectate__retry hidden",onclick:()=>{this.svc.load(!0)}},body:{className:()=>n()&&r()===null&&i()!==null?"spectate__body":"spectate__body hidden"},title:{textContent:()=>{const c=i();return c?Qh(l(),c.round.name,c.round.courseNameSnapshot):""}},status:{textContent:()=>{const c=i();return c&&c.status==="active"?"Live":""},className:()=>`spectate__status s-${i()?.status??""}`},subtitle:{textContent:()=>{const c=i();return c?Xh(c.round.name,c.round.courseNameSnapshot,c.status,c.round.playHoles.length||null)??"":""}},date:{textContent:()=>ge(i()?.round.date??null)},board:{innerHTML:()=>this.renderBoards()}})}renderBoards(){const e=this.svc.view.get();if(!e)return"";const t=r=>this.svc.nameOf(r),n=r=>this.svc.groupLabelOf(r),i=e.result.slots;return i.length===0?'<div class="lb-empty">No formats in this round.</div>':i.map(r=>{const l=i.length>1?`<h2 class="spectate__slot-head">${tp(r.formatLabel)}</h2>`:"",d=Yi(r,t,n),c=Qi(r.cards,e.result.routeSections,t),u=c?`<h3 class="lb-cards__head">Scorecard</h3>${c}`:"";return l+d+u}).join("")}}function tp(s){return s.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}const sp=b(`
    <div class="profile">
...
`);class rp extends N{static styles=`
        .profile {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),op=b('<h3 bind="text" class="block__subhead"></h3>'),lp=b('<p bind="text" class="block__note"></p>'),dp=b(`
...
`),pp=b('<div bind="panels" class="statspanels"></div>');class or extends N{static styles=`
        .statspanels {
            display: flex; flex-direction: column; gap: ${a("sm")};
...
`);class yp extends N{static styles=`
        .stats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),Tp=b('<li bind="text" class="roundstats__legenditem"></li>');function Ip(){const s=[];for(const[e,t]of Object.entries(Ns))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
            `)}class Pp extends N{static styles=`
        .roundstats {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),Op={not_started:"Not started",active:"Playing",complete:"Done"};function zp(s){const e=[`${s.participants.length} players`,`${s.scoreEventCount} scores`];return s.lastEventAt?e.push(`last ${s.lastEventAt.replace("T"," ").slice(0,16)}`):e.push("never played"),e.join(" · ")}function Lp(s){const e=[`@${s.username}`,`${s.roundCount} rounds`];return s.lastRoundDate&&e.push(`last ${s.lastRoundDate}`),s.handicapIndex!==null&&e.push(`hcp ${s.handicapIndex}`),s.deletedAt&&e.push("DELETED"),e.join(" · ")}class Ap extends N{static styles=`
        .admin {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`);class jp extends N{static styles=`
        .comps {
            padding: ${a("xl")} ${a("lg")} ${a("2xl")};
...
`),sf=b('<option bind="option"></option>');class nf extends N{competitions=this.inject(Ce);state=this.inject(at);router=this.inject(B);render(){const e=new $(()=>this.competitions.detail.get()?.rounds??[]),t=this.wire(ef,{empty:{className:()=>e.get().length===0?"cd__empty":"cd__empty hidden"},form:{className:()=>this.state.admin.get()&&Mp(this.state.lifecycle.get())?"cd__addround":"cd__addround hidden",onsubmit:n=>{n.preventDefault(),this.createRound()}},course:{value:()=>this.state.roundCourseDraft.get(),onchange:n=>this.state.roundCourseDraft.set(n.target.value)},date:{value:()=>this.state.roundDateDraft.get(),oninput:n=>this.state.roundDateDraft.set(n.target.value)},add:{disabled:()=>this.competitions.mutating.get()}});return this.$each(this.ref(t,"course"),this.state.courses,(n,i,r)=>this.wireEl(sf,{option:{value:()=>n.id,textContent:()=>n.name}},r),n=>n.id),this.$each(this.ref(t,"rounds"),e,(n,i,r)=>this.wireEl(tf,{row:{disabled:()=>!n.shareToken,onclick:()=>{n.shareToken&&this.router.navigate("/round",{query:{token:n.shareToken}})}},number:()=>`Round ${n.roundNumber}`,meta:()=>[n.courseNameSnapshot,n.date].filter(Boolean).join(" · ")||(n.shareToken?"Open":"View-only"),status:{textContent:()=>Zp[n.status]??n.status,className:()=>`cd__rstatus s-${n.status}`}},r),n=>JSON.stringify({id:n.id,status:n.status,shareToken:n.shareToken,courseName:n.courseNameSnapshot,date:n.date})),t}async createRound(){const e=await this.state.createRound();e&&this.router.navigate("/round",{query:{token:e}})}}function rf(s,e,t){return JSON.stringify({entry:s,points:e,columns:t})}function af(s){return s.rounds.filter(e=>e.value!==null).map(e=>({text:String(e.value),dropped:e.status==="dropped"}))}const of=b(`
    <div>
...
`),lf=b('<button bind="button" type="button"></button>'),df=b('<th bind="cell"></th>'),cf=b('<tr bind="row"></tr>'),uf=b('<td bind="cell"><span bind="value"></span></td>'),hf=b(`
...
`);class gf extends N{static styles=`
        .cd {
            padding: ${a("lg")} ${a("lg")} ${a("2xl")};
...
`);class _f extends N{static styles=`
    ...
    `;router=this.inject(B);render(){const e=this.wire(bf,{header:{className:()=>gs(this.router.route.get())?"app-shell__header":"app-shell__header hidden"}});return this.spawn(wd,this.ref(e,"account")),this.spawn(za,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":cn,"/history":Fd,"/round":ih,"/create":Rh,"/login":Mh,"/friends":Dh,"/friend":qh,"/friend-rounds":Uh,"/friend-courses":Yh,"/spectate":ep,"/profile":rp,"/stats":yp,"/round-stats":Pp,"/admin":Ap,...ai.competitions?{"/competitions":jp,"/competition":gf}:{}},cn),e}}class yf extends D{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await nr(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Oh()),this.error.set(null)}catch(e){e instanceof V&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await zh(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof V&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logoutEverywhere(){this.loading.set(!0);try{const e=await Lh();return this.currentUser.set(null),this.error.set(null),e.revoked}catch(e){return e instanceof V&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"}),null}finally{this.loading.set(!1)}}}q.get(Ir);const Gn=q.get(B);q.set(D,new yf);const qn=q.get(D);await Nr(_f,"#app",{hot:void 0,onInit:async()=>{await qn.load(),qn.currentUser.get()&&Gn.route.get()==="/login"&&Gn.navigate("/",!0)}});export{ts as A,N as C,B as R,p as S,Ir as T,y as a,Ze as b,$ as c,_r as d,I as e,br as n,M as r,b as t};
