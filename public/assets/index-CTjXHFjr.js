...
`);class hl extends A{static styles=`
    ...
    `}const _h="/tapscore/";function vh(s=_h){const e=s.trim().replace(/\/+$/,"");return`${e===""?"":e}/manage/`}function wh(s){if(!s.signedIn)return[];const e=[{kind:"identity",displayName:(s.displayName??"").trim()||(s.username??"").trim()||"Signed in",username:(s.username??"").trim()},{kind:"profile",label:"Profile"}];return s.canManageCourses&&e.push({kind:"course-setup",label:"Course setup"}),s.isSuperAdmin&&e.push({kind:"admin",label:"Admin"}),e.push({kind:"signout",label:"Sign out"}),e.push({kind:"signout-all",label:"Sign out everywhere"}),e}function xh(s){return wh(s).map(e=>e.kind)}function li(s){return s.signedIn?"avatar":"signin"}const kh=b(`
...
`),Lh='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>',Fh=b(`
    <div class="round-row">
        <button bind="row" type="button" class="round-summary round-row__main">${$n}</button>
        <div bind="actions" class="round-row__actions">
...
`;function ra(s,e,t=""){const n=t?`info-dot ${t}`:"info-dot";return`<button bind="${s}" class="${n}" type="button" aria-label="${e}"><span aria-hidden="true">i</span></button>`}const aa=`
        .info-dot {
            ...
            &:hover { color: ${l("text")}; }
            &.hidden { display: none; }
        }`,Oe=60,hi=8,Zs=4,Wh=Array.from({length:Zs*2+1},(s,e)=>e-Zs),Yh="transform 320ms cubic-bezier(0.22, 1, 0.36, 1)",Xh=b(`
...
`),np=b('<button bind="btn" class="se-seg" type="button"></button>'),ip=b(`
...
`),rp=b('<div bind="rule" class="se-stats__rule"></div>'),Os=["short_game_difficulty","short_game_strokes"],ap={first_putt_m:{aria:"First putt, exact distance",title:"Exact distance"}};function op(s){const e=s.control;return e.kind!=="segments"?`stat:${s.key}`:`stat:${s.key}|${e.options.map(t=>t.value).join(",")}`}function gi(s,e){return ap[s]??{aria:e,title:e}}class lp extends A{static styles=`
        .se {
            margin-top: ${a("xl")};
            &.hidden { display: none; }
...
<tr class="lb-rank__panel${M?" lb-rank__panel--open":""}" data-panel-key="${D(V)}">
  ...
  <h4 class="lb-section__title">${D(i.metricLabel)}</h4>
  <table class="lb-rank">
    <colgroup>
    ...
    <div class="lb-mp__team lb-mp__team--a${r}">${D(i.sideAName)}</div>
    <div class="lb-mp__center"><span class="lb-mp__standing">${D(i.standing)}</span><span class="lb-mp__status">${D(i.status)}</span></div>
    <div class="lb-mp__team lb-mp__team--b${o}">${D(i.sideBName)}</div>
  </div>`}).join("");return`<div class="lb-section">
  <h4 class="lb-section__title">${D(t.title)}</h4>${n}
</div>`}const Mp={ranked:Np,match_summary:(s,e)=>Op(s,e)},Hp={"default-score-grid":Pp,"compact-match-grid":Cp,"category-matrix-grid":Ip};function Ap(s){return`<div class="lb-diag">Unrenderable result section <code>${D(s)}</code> — no generic view yet. Results are not hidden.</div>`}function Bp(s){return`<div class="lb-diag">Unsupported score-grid component <code>${D(s)}</code> — no generic view yet. Results are not hidden.</div>`}function zp(s,e,t,n=null){const i=Mp[s.kind];return i?i(s,e,t,n):Ap(s.kind)}function ua(s,e,t,n){const i=la(s),r=Hp[i];return r?r(s,e,t,n):Bp(i)}function ha(s,e,t=ca,n=null){return s.leaderboard.length===0&&s.cards.length===0?`<div class="lb-empty">No scores entered yet for ${D(s.formatLabel)}.</div>`:s.leaderboard.map(r=>zp(r,e,t,n&&r===n.plan.rankedSection?n:null)).join("")||`<div class="lb-empty">No leaderboard metric for ${D(s.formatLabel)}.</div>`}function pa(s,e,t,n={}){if(s.length===0)return"";const i=n.mode??"product";return s.map(r=>ua(r,e,t,{mode:i})).join(`
`)}const Lp=b(`
...
`);class Dp extends A{static styles=`
        .claim-card {
            margin-top: ${a("lg")};
            padding: ${a("lg")};
            ${O()}
...
`),Cm=b('<li bind="text" class="story__line"></li>'),Im=b(`
...
`),Fm=b('<button bind="pill" class="round-view__fmt" type="button"></button>'),Gm=b('<button bind="pill" class="round-view__grp" type="button"></button>');class jm extends A{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`),Cg='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8.5" cy="7.5" r="3.25"/><path d="M2.5 20c.5-3.5 2.7-5.5 6-5.5 2.7 0 5.1 1.8 5.8 4.5"/><circle cx="17.5" cy="16.5" r="4.25"/><path d="M15.5 16.5h4"/></svg>',Ig=b(`
...
`);function Ia(s,e){return{row:{onclick:()=>e(s.roundId)},title:()=>Kl(s),subtitle:()=>Wl(s,je),progress:()=>Yl(s)}}function Ea(){return`
        & .fr-row {
            display: flex; align-items: center; gap: ${a("md")};
            width: 100%;
            padding: ${a("md")} ${a("lg")};
...
`),Jg=b('<h3 bind="text" class="block__subhead"></h3>'),Zg=b(`
...
`),kb=b('<li bind="text" class="roundstats__legenditem"></li>');function $b(){const s=[];for(const[e,t]of Object.entries(Tn))t.fill!==void 0&&s.push(`& .cell--${e} { background: ${t.fill}; color: #fff; border-color: transparent;`+(t.boxy?" border-radius: 3px;":"")+" }");return s.join(`
...
`),ry=b('<button bind="button" type="button"></button>'),ay=b('<th bind="cell"></th>'),oy=b('<tr bind="row"></tr>'),ly=b('<td bind="cell"><span bind="value"></span></td>'),dy=b(`
