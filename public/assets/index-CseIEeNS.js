...
`);class Ds extends I{static styles=`
    ...
    `;let B=De;function Hs(i){const e=typeof navigator<"u"?navigator.language:void 0;return typeof e=="string"&&e.toLowerCase().startsWith("sv")?"sv":"en"}function $t(){return Hs()}class le{loading=new h(!1);error=new h(null);descriptors=new h([]);started=!1;async load(){if(this.started)return;this.started=!0;const e=await C(this.loading,this.error,()=>b.setup.formats());e?this.descriptors.set(e):this.started=!1}byId(e){return this.descriptors.get().find(t=>t.id===e)??null}labelOf(e,t=$t()){const s=typeof e=="string"?this.byId(e):e;return s?s.labels?.[t]??s.labels?.en??s.label:null}classify(e){const t=e.requirements.balls;if(t.ballMode==="team")return{kind:"team_ball",teamSize:{...t.producerCount}};if(t.requiresSlotTeamGrouping){const s=t.slotTeamGrouping??{};return{kind:"team_grouping",teamSize:{min:s.teamSize?.min??2,max:s.teamSize?.max??2},...s.teamCount?{teamCount:s.teamCount}:{}}}return{kind:"individual",teamSize:{min:1,max:1}}}classifyId(e){const t=this.byId(e);return t?this.classify(t):null}needsTeams(e){const t=this.classifyId(e);return!!t&&t.kind!=="individual"}isSideFormat(e){return this.classifyId(e)?.kind==="team_grouping"}acceptsSideSubjects(e){const t=this.byId(e);return!t||this.isSideFormat(e)?!1:(t.requirements.scoreEntry?.metadata?.length??0)===0}}function kt(i){const e=z.get(le);return e.load(),e.labelOf(i.formatId)??`${i.scoringMode} · ${i.teamShape}`}function Ms(i){return i.map(e=>({key:e.round.id,token:e.token,roundId:e.round.id,courseName:e.round.courseNameSnapshot??"",status:e.round.status,completedAt:e.round.completedAt,lastActivityAt:e.round.date,roleLabel:Cs(e)||null,date:e.round.date,formats:e.round.formatSlots.map(kt).join(" · ")}))}function As(i){return i.map(e=>({key:e.token,token:e.token,roundId:null,courseName:e.courseName,status:e.status,completedAt:e.completedAt??null,lastActivityAt:e.lastSeenAt,roleLabel:null,date:null,formats:null}))}const oe={fromMyRounds:Ms,fromDeviceRounds:As},Fs=14,Bs=1440*60*1e3;function se(i,e){return e(i)}function Gs(i,e,t,s=Fs){const n=e-s*Bs,r=[],l=[];for(const d of i){const u=se(d,t);if(u.status==="complete"){const c=u.completedAt?Date.parse(u.completedAt):NaN;(Number.isNaN(c)||c>=n)&&l.push(d)}else r.push(d)}return r.sort((d,u)=>Qe(se(d,t).lastActivityAt,se(u,t).lastActivityAt)),l.sort((d,u)=>Qe(se(d,t).completedAt,se(u,t).completedAt)),{ongoing:r,finished:l}}function Qe(i,e){const t=i?Date.parse(i):NaN,s=e?Date.parse(e):NaN,n=Number.isNaN(t)?Number.NEGATIVE_INFINITY:t,r=Number.isNaN(s)?Number.NEGATIVE_INFINITY:s;return n===r?0:r-n}const qs=_(`
    <div class="landing">
...
`);class Fn extends I{static styles=`
    ...
    `;svc=this.inject(Z);slots=()=>this.svc.result.get()?.slots??[];currentSlot=()=>{const e=this.slots(),t=this.svc.selectedSlotDefId();return e.find(s=>s.slotDefId===t)??e[0]??null};render(){return this.wire(An,{status:{className:()=>{const t=this.svc.resultLoading.get(),s=this.svc.result.get()===null;return t||s?"lb__status":"lb__status hidden"},textContent:()=>this.svc.resultLoading.get()?"Loading results…":"No results yet."},body:{innerHTML:()=>this.renderBody()}})}renderBody(){const e=this.svc.result.get();if(!e)return"";const t=this.currentSlot();if(!t)return'<div class="lb-empty">No formats in this round.</div>';const s=u=>{const c=this.svc.nameOf(u);return this.svc.isPending(u)?`${c} (open seat)`:c},r=Hn(t,s,u=>this.svc.groupLabelOf(u)),l=Mn(t,e.routeSections,s),d=l?`<h3 class="lb-cards__head">Scorecard</h3>${l}`:"";return r+d}}function Bn(i,e){if(!e)return[];const t=[],s=new Set;for(const n of i)for(const r of n.players){if(r.playerId===e)return[];r.guestPlayerId===null||s.has(r.guestPlayerId)||(s.add(r.guestPlayerId),t.push({guestPlayerId:r.guestPlayerId,displayName:r.displayName}))}return t}const Gn=_(`
...
`),pi=_('<button bind="pill" class="round-view__fmt" type="button"></button>'),mi=_('<button bind="pill" class="round-view__grp" type="button"></button>');class fi extends I{static styles=`
        .round-view {
            height: 100%;
            display: flex;
            flex-direction: column;
...
`);class qi extends I{static styles=`
        .setup {
            ...
            & .player {
                ...
                & .player__name { flex: 1; padding: ${a("md")}; font-size: 1rem; ${M()} }
                & .player__remove {
                ...
                & .player__index { flex: 1; min-width: 0; padding: ${a("md")}; font-size: 1rem; ${M()} }
            ...
            & .fslot {
                ...
                & .mrow {
                    ...
                    & .mrow__pct {
                        ...
                        &[hidden] { display: none; }
                        & input { width: 56px; padding: ${a("xs")} ${a("sm")}; ${M()} font-size: 0.95rem; }
                    }
                }
                ...
                & .fslot__flat {
                    ...
                    color: ${o("text-muted")};
                    &[hidden] { display: none; }
                    & .fslot__pct { width: 70px; padding: ${a("sm")}; font-size: 1rem; ${M()} }
                }
                & .fslot__bands {
                    display: flex; flex-direction: column; gap: ${a("xs")};
                    &[hidden] { display: none; }
                ...
                & .brow {
                    ...
                    & .brow__pct, & .brow__upto { width: 56px; padding: ${a("sm")}; font-size: 0.95rem; ${M()} }
                ...
                & .grp__start {
                    ...
                    & .grp__time { flex: 1; min-width: 0; padding: ${a("sm")} ${a("md")}; font-size: 1rem; font-family: inherit; ${M()} }
        ...
        .profile {
            ...
            & .profile__card {
                ...
                & .profile__edit {
                    ...
                    & input { width: 90px; padding: ${a("md")}; font-size: 1rem; text-align: center; ${M()} }
                    & button {
...
`);class ar extends I{static styles=`
        .comps {
            ...
            & .comps__create {
                ...
                gap: ${a("sm")};
                margin-bottom: ${a("md")};
                & input { flex: 1; padding: ${a("md")}; font-size: 1rem; ${M()} }
                & button {
                    padding: ${a("md")} ${a("lg")};
        ...
        .cd {
            ...
            & .cd__form {
                ...
                & .cd__field { display: flex; flex-direction: column; gap: ${a("xs")};
                    ...
                    & input, & select { padding: ${a("sm")} ${a("md")}; font-size: 0.95rem; ${M()} }
                }
            ...
            & .cd__guestrow input, & .cd__addroundrow input, & .cd__addroundrow select {
                padding: ${a("sm")} ${a("md")}; font-size: 0.9rem; ${M()} min-width: 0; }
...
`);class Dr extends I{static styles=`
    ...
    `;router=this.inject(N);render(){const e=this.wire(Lr,{});return this.spawn(Ds,this.ref(e,"nav")),this.$swap(this.ref(e,"content"),this.router.route,{"/":Ye,"/history":Xs,"/round":fi,"/create":qi,"/login":Qi,"/friends":Zi,"/profile":sr,...xt.competitions?{"/competitions":ar,"/competition":Or}:{}},Ye),e}}class Hr extends R{async login(e,t){this.loading.set(!0);try{return this.currentUser.set(await jt(e,t)),this.error.set(null),!0}catch{return this.error.set({message:"Sign-in failed.",code:"auth"}),!1}finally{this.loading.set(!1)}}async load(){this.loading.set(!0);try{this.currentUser.set(await Ki()),this.error.set(null)}catch(e){e instanceof A&&e.status===401?this.error.set(null):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}async logout(){this.loading.set(!0);try{await Vi(),this.currentUser.set(null),this.error.set(null)}catch(e){e instanceof A&&e.status===401?(this.currentUser.set(null),this.error.set(null)):this.error.set({message:"Cannot reach the server.",code:"network"})}finally{this.loading.set(!1)}}}z.get(Jt);const ut=z.get(N);z.set(R,new Hr);const ht=z.get(R);await es(Dr,"#app",{hot:void 0,onInit:async()=>{await ht.load(),ht.currentUser.get()&&ut.route.get()==="/login"&&ut.navigate("/",!0)}});export{I as C,N as R,h as S,Jt as T,f as a,ae as b,y as c,$ as e,C as r,_ as t};
