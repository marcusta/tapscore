...
/** Friends are kept displayName-sorted (locale-aware) so local add/remove
 * lands rows exactly where a fresh server load would. */
export function sortProfiles(list: PlayerProfile[]): PlayerProfile[] {
 ...
 * mirrors the server's idempotent POST /friends. */
export function upsertFriend(friends: PlayerProfile[], p: PlayerProfile): PlayerProfile[] {
    return sortProfiles([...friends.filter((f) => f.id !== p.id), p]);
}

/** Remove a friend by id. A miss is a no-op (idempotent, like the DELETE). */
export function dropFriend(friends: PlayerProfile[], id: string): PlayerProfile[] {
