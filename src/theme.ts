...
// imports `resolvedLight`/`resolvedDark` below and emits the iOS design
// system's `ThemeTokens.swift` from them, which is what keeps the native client
// on exactly these values instead of a hand-copied second palette. Purely a
// refactor: the objects, the bridge calls and `t` are unchanged.
...
/**
 * The complete token maps — the app palette above merged over the framework's
 * neutral base and expanded with the derived `--field-*` / `--btn-*` control
 * tokens. Exported because this, not the raw palette, is the full contract the
 * Swift generator mirrors.
 */
export const resolvedLight = bridgeLegacyControls(lightTokens, neutralLight);
export const resolvedDark = bridgeLegacyControls(darkTokens, neutralDark);
