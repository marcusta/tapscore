...
export interface CorrectionsApi {
    ...
    allowanceOverride(input: { roundId: string; slotDefId: string; newConfig: { type: 'flat'; pct: number } | { type: 'split'; bands: ({ upToCh: null | number; pct: number })[] }; reason: string; clientEventId: string }): Promise<{ ok: true; eventId: string; version: number } | { ok: false; diagnostics: CompilerDiagnostic[] }>;
    ruling(input: { roundId: string; target: 'ball_hole' | 'ball_total' | 'slot_ball_result'; targetId: string; rulingKind: 'dq' | 'penalty_strokes' | 'hole_adjudication' | 'wd'; value: unknown; reason: string; clientEventId: string }): Promise<{ ok: true; id: string } | { ok: false; diagnostics: CompilerDiagnostic[] }>;
}
