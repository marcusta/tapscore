...
export interface TeesApi {
    ...
    create(input: { courseId: string; name: string; colour?: null | string; holeLengths: ({ holeNumber: number; lengthM: number; strokeIndexOverride: null | number })[]; ratings: ({ gender: 'M' | 'F'; courseRating: number; slope: number; par: number; totalLengthM: number })[] }): Promise<Tee>;
    update(input: { id: string; name?: string; colour?: null | string; holeLengths?: ({ holeNumber: number; lengthM: number; strokeIndexOverride: null | number })[]; ratings?: ({ gender: 'M' | 'F'; courseRating: number; slope: number; par: number; totalLengthM: number })[] }): Promise<Tee>;
